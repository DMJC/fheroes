...
fheroes::GameMode Game::LoadGame()
{
    ...
    fheroes::ButtonGroup gameModeButtons;
    ...
    const fheroes::ButtonBase & buttonStandardGame = gameModeButtons.button( 0 );
    fheroes::ButtonBase & buttonCampaignGame = gameModeButtons.button( 1 );
    const fheroes::ButtonBase & buttonMultiplayerGame = gameModeButtons.button( 2 );

    const int32_t spaceBetweenButtons = 10;

    fheroes::StandardWindow background( gameModeButtons, true, buttonStandardGame.area().height * 3 + spaceBetweenButtons * 3 );
        ...
        if ( buttonStandardGame.isEnabled() ) {
            ...
            if ( le.MouseClickLeft( buttonStandardGame.area() ) || HotKeyPressEvent( HotKeyEvent::MAIN_MENU_STANDARD ) ) {
                if ( ListFiles::IsEmpty( GetSaveDir(), GetSaveFileExtension( Game::TYPE_STANDARD ) ) ) {
                    fheroes::showStandardTextMessage( _( "Load Game" ), _( "No save files to load." ), Dialog::OK );
                }
                else {
                    return fheroes::GameMode::LOAD_STANDARD;
                }
            }
                ...
                if ( ListFiles::IsEmpty( GetSaveDir(), GetSaveFileExtension( Game::TYPE_CAMPAIGN ) ) ) {
                    ...
                    return fheroes::GameMode::LOAD_CAMPAIGN;
                }
            }
        ...
        if ( buttonHotSeat.isEnabled() ) {
            ...
            if ( le.MouseClickLeft( buttonHotSeat.area() ) || HotKeyPressEvent( HotKeyEvent::MAIN_MENU_HOTSEAT ) ) {
                if ( ListFiles::IsEmpty( GetSaveDir(), GetSaveFileExtension( Game::TYPE_HOTSEAT ) ) ) {
                    ...
                    return fheroes::GameMode::LOAD_HOT_SEAT;
                }
            }
...
fheroes::GameMode Game::LoadStandard()
{
    Settings::Get().SetGameType( Game::TYPE_STANDARD );
    return DisplayLoadGameDialog();
}

fheroes::GameMode Game::DisplayLoadGameDialog()
{
    // Stop all sounds, but not the music
    AudioManager::stopSounds();
    ...
    const fheroes::GameMode returnValue = Game::Load( file );
    if ( returnValue == fheroes::GameMode::CANCEL ) {
        return fheroes::GameMode::LOAD_GAME;
    }
