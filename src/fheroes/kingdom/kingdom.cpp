...
const Recruits & Kingdom::GetRecruits()
{
    ...
    // (this mechanic is an extension of fheroes and is missing from the original game), it is necessary to offer one native hero
...
std::set<Heroes *> Kingdoms::resetRecruits()
{
    ...
    for ( Kingdom & kingdom : _kingdoms ) {
        ...
        // this kingdom is still playable. This mechanic is an extension of fheroes and is missing from the original game.
...
bool Kingdom::IsTileVisibleFromCrystalBall( const int32_t dest ) const
{
    for ( const Heroes * hero : heroes ) {
        ...
        if ( hero->GetBagArtifacts().isArtifactBonusPresent( fheroes::ArtifactBonusType::VIEW_MONSTER_INFORMATION ) ) {
