...
namespace
{
    int selectAdventureOption( const bool enableDig )
    {
        ...
        fheroes::ButtonGroup optionButtons( imageIcn );
        ...
        fheroes::ButtonBase & buttonWorld = optionButtons.button( 0 );
        fheroes::ButtonBase & buttonPuzzle = optionButtons.button( 1 );
        fheroes::ButtonBase & buttonInfo = optionButtons.button( 2 );
        fheroes::ButtonBase & buttonDig = optionButtons.button( 3 );
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonWorld.area() ) )
                fheroes::showStandardTextMessage( _( "View World" ), _( "View the entire world." ), Dialog::ZERO );
            else if ( le.isMouseRightButtonPressedInArea( buttonPuzzle.area() ) )
                fheroes::showStandardTextMessage( _( "Puzzle" ), _( "View the obelisk puzzle." ), Dialog::ZERO );
            else if ( le.isMouseRightButtonPressedInArea( buttonInfo.area() ) )
                fheroes::showStandardTextMessage( _( "Scenario Information" ), _( "View information on the scenario you are currently playing." ), Dialog::ZERO );
            else if ( le.isMouseRightButtonPressedInArea( buttonDig.area() ) )
                fheroes::showStandardTextMessage( _( "Digging" ), _( "Dig for the Ultimate Artifact." ), Dialog::ZERO );
