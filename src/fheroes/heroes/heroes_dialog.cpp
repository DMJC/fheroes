...
int Heroes::OpenDialog( const bool readonly, const bool fade, const bool disableDismiss, const bool disableSwitch, const bool renderBackgroundDialog, const bool isEditor,
                        const fheroes::SupportedLanguage language )
{
    ...
    fheroes::Rect dialogRoi;
    fheroes::Rect dialogWithShadowRoi;
    std::unique_ptr<fheroes::StandardWindow> background;
    std::unique_ptr<fheroes::ImageRestorer> restorer;

    if ( renderBackgroundDialog ) {
        background = std::make_unique<fheroes::StandardWindow>( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT, false );
        dialogRoi = background->activeArea();
        dialogWithShadowRoi = background->totalArea();
    }
    else {
        dialogRoi = { ( display.width() - fheroes::Display::DEFAULT_WIDTH ) / 2, ( display.height() - fheroes::Display::DEFAULT_HEIGHT ) / 2,
                      fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT };
        dialogWithShadowRoi = { dialogRoi.x - 2 * fheroes::borderWidthPx, dialogRoi.y - fheroes::borderWidthPx, dialogRoi.width + 3 * fheroes::borderWidthPx,
                                dialogRoi.height + 3 * fheroes::borderWidthPx };
        restorer = std::make_unique<fheroes::ImageRestorer>( display, dialogRoi.x, dialogRoi.y, dialogRoi.width, dialogRoi.height );
    }
    ...
    if ( fade && ( isDefaultScreenSize || !renderBackgroundDialog ) ) {
        fheroes::fadeOutDisplay( dialogRoi, !isDefaultScreenSize );
    }

    const fheroes::Sprite & backgroundImage = fheroes::AGG::GetICN( ICN::HEROBKG, 0 );
    fheroes::Blit( backgroundImage, display, dialogRoi.x, dialogRoi.y );
    fheroes::Blit( fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::HEROEXTE : ICN::HEROEXTG, 0 ), display, dialogRoi.x, dialogRoi.y );

    // Hero portrait.
    const fheroes::Rect portPos( dialogRoi.x + 49, dialogRoi.y + 31, 101, 93 );
    if ( isEditor && !isValidId( _portrait ) ) {
        fheroes::renderHeroRacePortrait( _race, portPos, display );
    }
    else {
    ...
    // Dialog title.
    const fheroes::Rect titleRoi( dialogRoi.x + 60, dialogRoi.y + 1, 519, 17 );
    ...
    auto drawTitleText
        = [&display, &titleRoi, &dialogRoi, &backgroundImage, language, this]( const std::string & heroName, const int heroRace, const bool restoreBackground ) {
              if ( restoreBackground ) {
                  fheroes::Copy( backgroundImage, titleRoi.x - dialogRoi.x, titleRoi.y - dialogRoi.y, display, titleRoi );
              }
              ...
              if ( !heroName.empty() ) {
                  ...
                  auto title = fheroes::getLocalizedText( fheroes::getLocalizedStrings( std::move( titleText ), gameLanguage, "%{name}", heroName, language ),
              ...
              const fheroes::Text title( std::move( titleText ), fheroes::FontType::normalWhite() );
              title.drawInRoi( titleRoi.x + ( titleRoi.width - title.width() ) / 2, titleRoi.y + 2, display, titleRoi );
          };

    drawTitleText( _name, _race, false );

    fheroes::Point dst_pt( dialogRoi.x + 156, dialogRoi.y + 31 );
    ...
    const fheroes::Sprite & sprite1 = fheroes::AGG::GetICN( ICN::HSICONS, 9 );
    const fheroes::Rect rectSpreadArmyFormat( dst_pt.x, dst_pt.y, sprite1.width(), sprite1.height() );
    const fheroes::Point army1_pt( dst_pt.x - 1, dst_pt.y - 1 );

    // Army format grouped icon.
    dst_pt.x = dialogRoi.x + 552;
    const fheroes::Sprite & sprite2 = fheroes::AGG::GetICN( ICN::HSICONS, 10 );
    const fheroes::Rect rectGroupedArmyFormat( dst_pt.x, dst_pt.y, sprite2.width(), sprite2.height() );
    const fheroes::Point army2_pt( dst_pt.x - 1, dst_pt.y - 1 );

    // Army format cursor.
    fheroes::MovableSprite cursorFormat( fheroes::AGG::GetICN( ICN::HSICONS, 11 ) );
    const fheroes::Point cursorFormatPos = _army.isSpreadFormation() ? army1_pt : army2_pt;

    // Do not show Army format in Editor.
    if ( !isEditor ) {
        fheroes::Copy( sprite1, 0, 0, display, rectSpreadArmyFormat );
        fheroes::Copy( sprite2, 0, 0, display, rectGroupedArmyFormat );
    ...
    const fheroes::Rect crestRect( portPos.x, portPos.y + 99, portPos.width, portPos.height );
    fheroes::Rect raceRect;

    auto redrawRace = [&raceRect, &crestRect, &display]( const int race ) {
        const fheroes::Sprite & raceSprite = fheroes::AGG::GetICN( ICN::NGEXTRA, Race::getRaceIcnIndex( race, true ) );
        fheroes::Copy( raceSprite, 0, 0, display, raceRect );
        ...
        fheroes::ApplyPalette( fheroes::AGG::GetICN( ICN::STRIP, 3 ), 0, offsetY, display, crestRect.x, posY, crestRect.width, sizeY,
                                PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        const fheroes::Text raceText( Race::String( race ), fheroes::FontType::smallWhite() );
    ...
    if ( isEditor && Modes( JAIL ) ) {
        ...
        fheroes::ApplyPalette( fheroes::AGG::GetICN( ICN::STRIP, 3 ), 0, 0, display, crestRect.x, crestRect.y, crestRect.width, crestRect.height,
                                PAL::GetPalette( PAL::PaletteType::DARKENING ) );

        const fheroes::Text raceText( _( "Hero class:" ), fheroes::FontType::normalWhite() );
        raceText.drawInRoi( crestRect.x, crestRect.y + 6, crestRect.width, display, crestRect );

        const fheroes::Sprite & raceSprite = fheroes::AGG::GetICN( ICN::NGEXTRA, Race::getRaceIcnIndex( _race, true ) );
        ...
        fheroes::Blit( fheroes::AGG::GetICN( ICN::NGEXTRA, 61U ), display, raceRect.x - 5, raceRect.y + 3 );
        ...
        fheroes::Copy( fheroes::AGG::GetICN( ICN::CREST, icnIndex ), 0, 0, display, crestRect );
    }
    ...
    const fheroes::Sprite & bar = fheroes::AGG::GetICN( ICN::HSBTNS, 8 );
    fheroes::Copy( bar, 0, 0, display, dst_pt.x, dst_pt.y, bar.width(), bar.height() );
    ...
    dst_pt.y = dialogRoi.y + fheroes::Display::DEFAULT_HEIGHT - 20;
    fheroes::Button buttonPrevHero( dst_pt.x, dst_pt.y, ICN::HSBTNS, 4, 5 );
    fheroes::TimedEventValidator timedButtonPrevHero( [&buttonPrevHero]() { return buttonPrevHero.isPressed(); } );
    buttonPrevHero.subscribe( &timedButtonPrevHero );

    // Next hero button.
    dst_pt.x += fheroes::Display::DEFAULT_WIDTH - 22;
    fheroes::Button buttonNextHero( dst_pt.x, dst_pt.y, ICN::HSBTNS, 6, 7 );
    fheroes::TimedEventValidator timedButtonNextHero( [&buttonNextHero]() { return buttonNextHero.isPressed(); } );
    ...
    std::unique_ptr<fheroes::ButtonSprite> buttonDismiss;

    if ( !isEditor && !readonly && !disableDismiss ) {
        const fheroes::Sprite & dismissReleased = fheroes::AGG::GetICN( ICN::BUTTON_VERTICAL_DISMISS, 0 );
        buttonDismiss = std::make_unique<fheroes::ButtonSprite>( dst_pt.x, dst_pt.y - dismissReleased.height() / 2, dismissReleased,
                                                                  fheroes::AGG::GetICN( ICN::BUTTON_VERTICAL_DISMISS, 1 ),
                                                                  fheroes::AGG::GetICN( ICN::DISMISS_HERO_DISABLED_BUTTON, 0 ) );

        if ( inCastle() || readonly || disableDismiss || Modes( NOTDISMISS ) ) {
            buttonDismiss->disable();
        }

        fheroes::addGradientShadow( dismissReleased, display, { dst_pt.x, dst_pt.y - dismissReleased.height() / 2 }, { -3, 5 } );

        buttonDismiss->draw();
    }

    // Hero Patrol mode button (used in Editor).
    std::unique_ptr<fheroes::ButtonSprite> buttonPatrol;

    if ( isEditor ) {
        const fheroes::Sprite & patrolReleased = fheroes::AGG::GetICN( ICN::BUTTON_VERTICAL_PATROL, 0 );
        buttonPatrol = std::make_unique<fheroes::ButtonSprite>( dst_pt.x, dst_pt.y - patrolReleased.height() / 2, patrolReleased,
                                                                 fheroes::AGG::GetICN( ICN::BUTTON_VERTICAL_PATROL, 1 ) );

        fheroes::addGradientShadow( patrolReleased, display, { dialogRoi.x + 9, dst_pt.y - patrolReleased.height() / 2 }, { -3, 5 } );
    ...
    const fheroes::Sprite & exitReleased = fheroes::AGG::GetICN( ICN::BUTTON_VERTICAL_EXIT, 0 );
    fheroes::ButtonSprite buttonExit( dst_pt.x, dst_pt.y - exitReleased.height() / 2, exitReleased, fheroes::AGG::GetICN( ICN::BUTTON_VERTICAL_EXIT, 1 ) );
    ...
    if ( fade ) {
        ...
        fheroes::fadeInDisplay( dialogRoi, !isDefaultScreenSize );
    }
    else {
    ...
    while ( le.HandleEvents() ) {
        ...
        // Dismiss hero.
            ...
            if ( ( le.MouseClickLeft( buttonDismiss->area() ) || Game::HotKeyPressEvent( Game::HotKeyEvent::ARMY_DISMISS ) )
                 && Dialog::YES == fheroes::showStandardTextMessage( GetName(), _( "Are you sure you want to dismiss this Hero?" ), Dialog::YES | Dialog::NO ) ) {
        ...
        if ( le.isMouseCursorPosInArea( moraleIndicator.GetArea() ) ) {
            ...
            if ( le.MouseClickLeft( titleRoi ) ) {
                ...
                const fheroes::Text body{ _( "Enter hero's name" ), fheroes::FontType::normalWhite() };
            ...
            if ( le.isMouseRightButtonPressedInArea( rectSpreadArmyFormat ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Grouped Formation" ),
