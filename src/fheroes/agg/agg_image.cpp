...
namespace
{
    ...
    std::vector<std::vector<fheroes::Sprite>> _icnVsSprite( ICN::LASTICN );
    std::array<std::vector<std::vector<fheroes::Image>>, TIL::LASTTIL> _tilVsImage;
    const fheroes::Sprite errorImage;

    const uint32_t headerSize = 6;

    std::map<int, std::vector<fheroes::Sprite>> _icnVsScaledSprite;
    ...
    {
        const fheroes::SupportedLanguage currentLanguage = fheroes::getCurrentLanguage();
        const fheroes::SupportedLanguage resourceLanguage = fheroes::getResourceLanguage();
        ...
        case fheroes::SupportedLanguage::Czech:
        case fheroes::SupportedLanguage::English:
        case fheroes::SupportedLanguage::French:
        case fheroes::SupportedLanguage::German:
        case fheroes::SupportedLanguage::Polish:
            return true;
        default:
            return false;
        }
    ...
    fheroes::Image createDigit( const int32_t width, const int32_t height, const std::vector<fheroes::Point> & points, const uint8_t pixelColor )
    {
        fheroes::Image digit( width, height );
        digit.reset();

        fheroes::SetPixel( digit, points, pixelColor );
        fheroes::Blit( fheroes::CreateContour( digit, 35 ), digit );

        return digit;
    }

    fheroes::Image addDigit( const fheroes::Image & original, const fheroes::Image & digit, const fheroes::Point & offset )
    {
        fheroes::Image combined( original.width() + digit.width() + offset.x, original.height() + ( offset.y < 0 ? 0 : offset.y ) );
        combined.reset();

        fheroes::Copy( original, 0, 0, combined, 0, 0, original.width(), original.height() );
        fheroes::Blit( digit, 0, 0, combined, original.width() + offset.x, combined.height() - digit.height() + ( offset.y >= 0 ? 0 : offset.y ), digit.width(),
                        digit.height() );

        return combined;
    }

    void populateCursorIcons( std::vector<fheroes::Sprite> & output, const fheroes::Sprite & origin, const std::vector<fheroes::Image> & digits,
                              const fheroes::Point & offset )
    {
        output.emplace_back( origin );
        for ( const fheroes::Image & digit : digits ) {
    ...
    void fillRandomPixelsFromImage( const fheroes::Image & original, const fheroes::Rect & originalRoi, fheroes::Image & output, const fheroes::Rect & outputRoi,
                                    Rand::PCG32 & seededGen )
    {
        for ( int x = outputRoi.x; x < outputRoi.x + outputRoi.width; ++x ) {
            for ( int y = outputRoi.y; y < outputRoi.y + outputRoi.height; ++y ) {
                const fheroes::Point pixelLocation{ static_cast<int32_t>( Rand::GetWithGen( originalRoi.x, originalRoi.x + originalRoi.width - 1, seededGen ) ),
                                                     static_cast<int32_t>( Rand::GetWithGen( originalRoi.y, originalRoi.y + originalRoi.height - 1, seededGen ) ) };

                fheroes::Copy( original, pixelLocation.x, pixelLocation.y, output, x, y, 1, 1 );
            }
        }
    }
    ...
    public:
        void preserve()
        {
            ...
            fheroes::AGG::GetICN( ICN::FONT, 0 );
            fheroes::AGG::GetICN( ICN::SMALFONT, 0 );
            fheroes::AGG::GetICN( ICN::BUTTON_GOOD_FONT_RELEASED, 0 );
            fheroes::AGG::GetICN( ICN::BUTTON_GOOD_FONT_PRESSED, 0 );
            fheroes::AGG::GetICN( ICN::BUTTON_EVIL_FONT_RELEASED, 0 );
            fheroes::AGG::GetICN( ICN::BUTTON_EVIL_FONT_PRESSED, 0 );
    ...
    private:
        ...
        std::vector<fheroes::Sprite> _normalFont;
        std::vector<fheroes::Sprite> _smallFont;
        std::vector<fheroes::Sprite> _buttonGoodReleasedFont;
        std::vector<fheroes::Sprite> _buttonGoodPressedFont;
        std::vector<fheroes::Sprite> _buttonEvilReleasedFont;
        std::vector<fheroes::Sprite> _buttonEvilPressedFont;
    };
    ...
    {
    public:
        ButtonFontOffsetRestorer( std::vector<fheroes::Sprite> & font, const int32_t offsetX )
            : _font( font )
        {
            _originalXOffsets.reserve( _font.size() );

            for ( fheroes::Sprite & characterSprite : _font ) {
    ...
    private:
        std::vector<fheroes::Sprite> & _font;
    ...
    void addTransparency( fheroes::Image & image, const uint8_t valueToReplace )
    {
        fheroes::ReplaceColorIdByTransformId( image, valueToReplace, 1 );
    }

    void invertTransparency( fheroes::Image & image )
    {
    ...
    {
        if ( isGoodInterface ) {
            return isReleasedState ? fheroes::GetColorId( 216, 184, 152 ) : fheroes::GetColorId( 184, 136, 96 );
        }
        return isReleasedState ? fheroes::GetColorId( 180, 180, 180 ) : fheroes::GetColorId( 144, 144, 144 );
    }

    // NOTE: Do not call this with an evil style button's ICN ID.
    void convertToEvilButtonBackground( fheroes::Sprite & released, fheroes::Sprite & pressed, const int goodButtonIcnId, const uint32_t index )
    {
        released = fheroes::AGG::GetICN( goodButtonIcnId, index );
        pressed = fheroes::AGG::GetICN( goodButtonIcnId, index + 1 );

        const std::vector<uint8_t> & goodToEvilPalette = PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_BUTTON );
        fheroes::ApplyPalette( released, goodToEvilPalette );
        fheroes::ApplyPalette( pressed, goodToEvilPalette );
    }
    ...
    void createCampaignButtonSet( const int campaignSetIcnId, const std::array<const char *, 5> & untranslatedTexts )
    {
        ...
        for ( size_t i = 0; i < untranslatedTexts.size(); ++i ) {
            ...
            const char * translatedText = fheroes::getSupportedText( untranslatedTexts[i], fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[campaignSetIcnId][icnIndex], _icnVsSprite[campaignSetIcnId][icnIndex + 1], translatedText, emptyButtonIcnID,
                                            buttonBackgroundICN );
        }
    }

    void convertToEvilInterface( fheroes::Sprite & image, const fheroes::Rect & roi )
    {
        fheroes::ApplyPalette( image, roi.x, roi.y, image, roi.x, roi.y, roi.width, roi.height, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );
    }

    // Sets the upper left (offset 3 pixels), upper right (offset 2 pixels), lower right (offset 3 pixels) corners transparent.
    void setButtonCornersTransparent( fheroes::Sprite & buttonSprite )
    {
    ...
    void fillTransparentButtonText( fheroes::Sprite & released )
    {
        fheroes::Sprite background( released.width(), released.height(), released.x(), released.y() );
        background.fill( 10 );
    ...
    void setTransformLayerTransparent( fheroes::Image & image )
    {
    ...
    void drawImageOnButton( const fheroes::Image & image, const int32_t maxImageWidth, const int32_t maxImageHeight, fheroes::Image & releasedSprite,
                            fheroes::Image & pressedSprite )
    {
        assert( !image.empty() && !releasedSprite.empty() && !pressedSprite.empty() );

        fheroes::Image newImage( std::min( maxImageWidth, image.width() + 4 ), std::min( maxImageHeight, image.height() + 4 ) );
        newImage.reset();
        fheroes::Blit( image, 0, 0, newImage, 2, 2, 35, 25 );
        ...
        fheroes::updateShadow( newImage, { 1, -1 }, 2U, false );
        fheroes::updateShadow( newImage, { 2, -2 }, 5U, false );
        fheroes::updateShadow( newImage, { -1, 1 }, 6U, false );
        fheroes::updateShadow( newImage, { -2, 2 }, 9U, false );
        ...
        fheroes::Blit( newImage, pressedSprite, offsetX + 1, offsetY );
        fheroes::ReplaceTransformIdByColorId( newImage, 6U, 10U );
        fheroes::Blit( newImage, releasedSprite, offsetX + 2, offsetY - 1 );
    }
    ...
    void replacePOLAssetWithSW( const int id, const int assetIndex )
    {
        ...
        fheroes::ICNHeader header1;
        imageStream >> header1;

        fheroes::ICNHeader header2;
        imageStream >> header2;
        ...
        _icnVsSprite[id][assetIndex] = fheroes::decodeICNSprite( data, dataEnd, header1 );
    }
    ...
    bool readIcnFromAgg( const int id )
    {
        ...
        for ( uint32_t i = 0; i < count; ++i ) {
            ...
            if ( headerSize + header1.offsetData + dataSize > body.size() ) {
                ...
                throw fheroes::InvalidDataResources( "ICN Id " + std::to_string( id ) + ", index " + std::to_string( i )
                                                      + " is being corrupted. "
            ...
            _icnVsSprite[id][i] = fheroes::decodeICNSprite( data, dataEnd, header1 );
        }
    ...
    void CopyICNWithPalette( const int icnId, const int originalIcnId, const PAL::PaletteType paletteType )
    {
        ...
        fheroes::AGG::GetICN( originalIcnId, 0 ); // always avoid calling `readIcnFromAgg()` directly

        _icnVsSprite[icnId] = _icnVsSprite[originalIcnId];
        const std::vector<uint8_t> & palette = PAL::GetPalette( paletteType );
        for ( fheroes::Sprite & sprite : _icnVsSprite[icnId] ) {
    ...
    void generateDefaultImages( const int id )
    {
        ...
        case ICN::BUTTON_MAP_SELECT_GOOD: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::NGEXTRA, 64 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::NGEXTRA, 65 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "SELECT" ), fheroes::FontType::buttonReleasedWhite() );
        ...
        case ICN::BUTTON_MAPSIZE_SMALL: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::REQUESTS, 9 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::REQUESTS, 10 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "S" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_MAPSIZE_MEDIUM: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::REQUESTS, 11 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::REQUESTS, 12 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "M" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_MAPSIZE_LARGE: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::REQUESTS, 13 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::REQUESTS, 14 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "L" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_MAPSIZE_XLARGE: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::REQUESTS, 15 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::REQUESTS, 16 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "X-L" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_MAPSIZE_ALL: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::REQUESTS, 17 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::REQUESTS, 18 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "ALL" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 58, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_1_EVIL: {
            ...
            const char * text = fheroes::getSupportedText( "1", fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, isEvilInterface,
        ...
        case ICN::BUTTON_2_EVIL: {
            ...
            const char * text = fheroes::getSupportedText( "2", fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_3_EVIL: {
            ...
            const char * text = fheroes::getSupportedText( "3", fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_4_EVIL: {
            ...
            const char * text = fheroes::getSupportedText( "4", fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_5_EVIL: {
            ...
            const char * text = fheroes::getSupportedText( "5", fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 46, 25 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_GUILDWELL_EXIT: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::WELLXTRA, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::WELLXTRA, 1 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "EXIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, ICN::EMPTY_GUILDWELL_BUTTON, ICN::UNKNOWN );

            break;
        }
        case ICN::BUTTON_EXIT_TOWN: {
            std::vector<fheroes::Sprite> & buttonStates = _icnVsSprite[id];
            buttonStates.resize( 2 );

            if ( useOriginalResources() ) {
                buttonStates[0] = fheroes::AGG::GetICN( ICN::TREASURY, 1 );
                buttonStates[1] = fheroes::AGG::GetICN( ICN::TREASURY, 2 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "smallerButton|EXIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 70, 25 }, false, ICN::BLACKBAK );
        ...
        case ICN::BUTTON_OKAY_TOWN: {
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "OKAY" ), fheroes::FontType::buttonReleasedWhite() );
        ...
        case ICN::BUTTON_EXIT_PUZZLE_DIM_DOOR_EVIL: {
            ...
            if ( useOriginalResources() ) {
                ...
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( originalButtonICN, 4 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( originalButtonICN, 5 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "smallerButton|EXIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 70, 25 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_VIEWWORLD_EXIT_EVIL: {
            ...
            if ( useOriginalResources() ) {
                ...
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( originalIcnId, 2 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( originalIcnId, 3 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "smallerButton|EXIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 70, 35 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_KINGDOM_EXIT: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::OVERVIEW, 4 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::OVERVIEW, 5 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "smallerButton|EXIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 89, 25 }, false, ICN::BROWNBAK );
        ...
        case ICN::BUTTON_KINGDOM_HEROES: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::OVERVIEW, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::OVERVIEW, 1 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "HEROES" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 89, 42 }, false, ICN::BROWNBAK );
        ...
        case ICN::BUTTON_KINGDOM_TOWNS: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::OVERVIEW, 2 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::OVERVIEW, 3 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "TOWNS/\nCASTLES" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 90, 42 }, false, ICN::BROWNBAK );
        ...
        case ICN::BUTTON_SMALL_CANCEL_EVIL: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( isEvilInterface ? ICN::CPANELE : ICN::CPANEL, 8 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( isEvilInterface ? ICN::CPANELE : ICN::CPANEL, 9 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "CANCEL" ), fheroes::FontType::buttonReleasedWhite() );
        ...
        case ICN::BUTTON_SMALL_OKAY_EVIL: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( isEvilInterface ? ICN::SPANBTNE : ICN::SPANBTN, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( isEvilInterface ? ICN::SPANBTNE : ICN::SPANBTN, 1 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "OKAY" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                            isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_SMALL_ACCEPT_GOOD: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::SURRENDR, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::SURRENDR, 1 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "ACCEPT" ), fheroes::FontType::buttonReleasedWhite() );
        ...
        case ICN::BUTTON_SMALL_DECLINE_GOOD: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::SURRENDR, 2 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::SURRENDR, 3 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "DECLINE" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, ICN::EMPTY_GOOD_BUTTON, ICN::STONEBAK );
        ...
        case ICN::BUTTON_SMALL_LEARN_EVIL: {
            ...
            if ( useOriginalResources() ) {
                ...
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( baseIcnID, 9 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( baseIcnID, 10 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "LEARN" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
        ...
        case ICN::BUTTON_SMALL_TRADE_EVIL: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( isEvilInterface ? ICN::TRADPOSE : ICN::TRADPOST, 15 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( isEvilInterface ? ICN::TRADPOSE : ICN::TRADPOST, 16 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "TRADE" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                  isEvilInterface ? ICN::UNIFORMBAK_EVIL : ICN::UNIFORMBAK_GOOD );
            break;
        }
        ...
        case ICN::BUTTON_SMALL_YES_EVIL: {
            ...
            if ( useOriginalResources() ) {
                const int baseIcnID = isEvilInterface ? ICN::SYSTEME : ICN::SYSTEM;
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( baseIcnID, 5 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( baseIcnID, 6 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "YES" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                            isEvilInterface ? ICN::UNIFORMBAK_EVIL : ICN::UNIFORMBAK_GOOD );
            break;
        }
        ...
        case ICN::BUTTON_SMALL_NO_EVIL: {
            ...
            if ( useOriginalResources() ) {
                const int baseIcnID = isEvilInterface ? ICN::SYSTEME : ICN::SYSTEM;
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( baseIcnID, 7 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( baseIcnID, 8 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "NO" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                            isEvilInterface ? ICN::UNIFORMBAK_EVIL : ICN::UNIFORMBAK_GOOD );
            break;
        }
        ...
        case ICN::BUTTON_SMALL_UPGRADE_EVIL: {
            ...
            if ( useOriginalResources() ) {
                ...
                fheroes::Sprite & released = _icnVsSprite[id][0];
                fheroes::Sprite & pressed = _icnVsSprite[id][1];
                ...
                const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( buttonIcnID, buttonIcnIndex );
                const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( buttonIcnID, buttonIcnIndex + 1 );
                ...
                for ( fheroes::Sprite & image : _icnVsSprite[id] ) {
            ...
            text = fheroes::getSupportedText( text, fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                  isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_GIFT_GOOD: {
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "GIFT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                            isEvilInterface ? ICN::UNIFORMBAK_EVIL : ICN::UNIFORMBAK_GOOD );
        ...
        case ICN::BUTTON_SMALL_RESTART_GOOD: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::NON_UNIFORM_GOOD_RESTART_BUTTON, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::NON_UNIFORM_GOOD_RESTART_BUTTON, 1 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "RESTART" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, ICN::EMPTY_GOOD_BUTTON, ICN::STONEBAK );
        ...
        case ICN::BUTTON_SMALL_MAX_GOOD: {
            ...
            if ( useOriginalResources() ) {
                ...
                _icnVsSprite[id][0] = fheroes::Crop( fheroes::AGG::GetICN( ICN::RECRUIT, 4 ), 5, 0, 60, 25 );
                _icnVsSprite[id][1] = fheroes::Crop( fheroes::AGG::GetICN( ICN::RECRUIT, 5 ), 5, 0, 60, 25 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "MAX" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 61, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_SMALL_MIN_GOOD: {
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "MIN" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { fheroes::AGG::GetICN( ICN::BUTTON_SMALL_MAX_GOOD, 0 ).width() - 10, 25 },
        ...
        case ICN::UNIFORM_GOOD_MAX_BUTTON: {
            ...
            released = fheroes::AGG::GetICN( buttonIcnID, 0 );
            pressed = fheroes::AGG::GetICN( buttonIcnID, 1 );
            ...
            fheroes::makeTransparentBackground( released, pressed, ICN::UNIFORMBAK_GOOD );
        ...
        case ICN::UNIFORM_EVIL_OKAY_BUTTON: {
            ...
            if ( useOriginalResources() ) {
                const int baseIcnId = isEvilInterface ? ICN::SYSTEME : ICN::SYSTEM;
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( baseIcnId, 1 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( baseIcnId, 2 );
                break;
            }
        ...
        case ICN::UNIFORM_EVIL_CANCEL_BUTTON: {
            ...
            if ( useOriginalResources() ) {
                const int baseIcnId = isEvilInterface ? ICN::SYSTEME : ICN::SYSTEM;
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( baseIcnId, 3 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( baseIcnId, 4 );
                break;
            }
        ...
        case ICN::UNIFORM_EVIL_EXIT_BUTTON: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( baseIcnId, 17 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( baseIcnId, 18 );
                break;
            }
        ...
        case ICN::BUTTON_VERTICAL_DISMISS: {
            ...
            if ( useOriginalResources() ) {
                ...
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::EMPTY_VERTICAL_GOOD_BUTTON, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::EMPTY_VERTICAL_GOOD_BUTTON, 1 );

                // Copy the DISMISS text back from the original button
                const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( ICN::HSBTNS, 0 );
                const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( ICN::HSBTNS, 1 );

                Copy( originalReleased, 9, 2, _icnVsSprite[id][0], 5, 2, 21, 112 );
                Copy( originalPressed, 9, 5, _icnVsSprite[id][1], 4, 5, 19, 111 );

                fheroes::makeTransparentBackground( _icnVsSprite[id][0], _icnVsSprite[id][1], ICN::REDBAK_SMALL_VERTICAL );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "D\nI\nS\nM\nI\nS\nS" ), fheroes::FontType::buttonReleasedWhite() );
        ...
        case ICN::BUTTON_VERTICAL_EXIT: {
            ...
            if ( useOriginalResources() ) {
                ...
                const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( ICN::HSBTNS, 2 );
                const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( ICN::HSBTNS, 3 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "E\nX\nI\nT" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, ICN::EMPTY_VERTICAL_GOOD_BUTTON, ICN::REDBAK_SMALL_VERTICAL );
        ...
        case ICN::BUTTON_HSCORES_VERTICAL_STANDARD: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( originalID, originalICNIndex );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( originalID, originalICNIndex + 1 );
                break;
            }

            for ( size_t i = 0; i < _icnVsSprite[id].size(); ++i ) {
                const fheroes::Sprite & originalButton = fheroes::AGG::GetICN( originalID, originalICNIndex + static_cast<uint32_t>( i ) );
                fheroes::Sprite & out = _icnVsSprite[id][i];
            ...
            const char * translatedText = fheroes::getSupportedText( buttonText, fheroes::FontType{ fheroes::FontSize::BUTTON_RELEASED, fheroes::FontColor::WHITE } );
            fheroes::renderTextOnButton( _icnVsSprite[id][0], _icnVsSprite[id][1], translatedText, { 3, 4 }, { 2, 5 }, { 23, 133 }, fheroes::FontColor::WHITE );
        ...
        case ICN::UNIFORM_EVIL_MIN_BUTTON: {
            ...
            _icnVsSprite[id].resize( fheroes::AGG::GetICNCount( goodButtonIcnID ) );
        ...
        case ICN::BUTTONS_FILE_DIALOG_GOOD: {
            ...
            const fheroes::FontType buttonFontType = fheroes::FontType::buttonReleasedWhite();
            fheroes::makeSymmetricBackgroundSprites( _icnVsSprite[id],
                                                      { fheroes::getSupportedText( gettext_noop( "NEW\nGAME" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "LOAD\nGAME" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "RESTART\nGAME" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "SAVE\nGAME" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "QUICK\nSAVE" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "QUIT" ), buttonFontType ) },
                                                      isEvilInterface, 80 );
            break;
        }
        case ICN::BUTTONS_EDITOR_FILE_DIALOG_GOOD: {
            ...
            if ( useOriginalResources() ) {
                ...
                for ( uint32_t i = 0; i < 4; ++i ) {
                    _icnVsSprite[id][i] = fheroes::AGG::GetICN( buttonIcnID, i );
                }
                // Save Map
                _icnVsSprite[id][6] = fheroes::AGG::GetICN( buttonIcnID, 4U );
                _icnVsSprite[id][7] = fheroes::AGG::GetICN( buttonIcnID, 5U );
                // Add generated buttons.
                const fheroes::FontType buttonFontType = fheroes::FontType::buttonReleasedWhite();
                const fheroes::Size buttonSize{ _icnVsSprite[id][0].width() - 10, _icnVsSprite[id][0].height() };
                fheroes::makeButtonSprites( _icnVsSprite[id][4], _icnVsSprite[id][5], fheroes::getSupportedText( gettext_noop( "START\nMAP" ), buttonFontType ),
                                             buttonSize, false, ICN::STONEBAK );
                fheroes::makeButtonSprites( _icnVsSprite[id][8], _icnVsSprite[id][9], fheroes::getSupportedText( gettext_noop( "MAIN\nMENU" ), buttonFontType ),
                                             buttonSize, false, ICN::STONEBAK );

                // Quit
                _icnVsSprite[id][10] = fheroes::AGG::GetICN( buttonIcnID, 6U );
                _icnVsSprite[id][11] = fheroes::AGG::GetICN( buttonIcnID, 7U );
            ...
            fheroes::makeSymmetricBackgroundSprites( _icnVsSprite[id],
                                                      { fheroes::getSupportedText( gettext_noop( "NEW\nMAP" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "LOAD\nMAP" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "START\nMAP" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "SAVE\nMAP" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "MAIN\nMENU" ), buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "QUIT" ), buttonFontType ) },
                                                      false, 86 );
            break;
        }
        ...
        case ICN::BUTTONS_ADVENTURE_OPTIONS_DIALOG_EVIL: {
            ...
            if ( useOriginalResources() ) {
                ...
                for ( size_t i = 0; i < _icnVsSprite[id].size(); ++i ) {
                    _icnVsSprite[id][i] = fheroes::AGG::GetICN( originalIcnID, static_cast<uint32_t>( i ) );
                }
                break;
            }
            ...
            fheroes::makeSymmetricBackgroundSprites( _icnVsSprite[id],
                                                      { fheroes::getSupportedText( "\n", buttonFontType ), fheroes::getSupportedText( "\n", buttonFontType ),
                                                        fheroes::getSupportedText( gettext_noop( "INFO" ), buttonFontType ),
                                                        fheroes::getSupportedText( "\n", buttonFontType ) },
                                                      isEvilInterface, 80 );

            // Copy images from the original buttons.
            // Let's hope that game resources remain the same for other versions of the game.
            const std::array<fheroes::Rect, imageCount> imageRoi = { fheroes::Rect{ 20, 15, 59, 27 },
                                                                      fheroes::Rect{ 19, 16, 59, 27 },
                                                                      fheroes::Rect{ 28, 15, 42, 27 },
                                                                      fheroes::Rect{ 27, 16, 42, 27 },
                                                                      fheroes::Rect{},
                                                                      fheroes::Rect{},
                                                                      fheroes::Rect{ 21, 17, 59, 25 },
                                                                      fheroes::Rect{ 20, 18, 59, 25 } };
            for ( size_t i = 0; i < imageRoi.size(); ++i ) {
                const auto & originalImage = fheroes::AGG::GetICN( originalIcnID, static_cast<uint32_t>( i ) );
                fheroes::Copy( originalImage, imageRoi[i].x, imageRoi[i].y, _icnVsSprite[id][i],
        ...
        case ICN::BUTTON_QUIT_GOOD: {
            ...
            if ( useOriginalResources() ) {
                ...
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( buttonIcnID, 6 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( buttonIcnID, 7 );
                break;
            }

            const char * text = fheroes::getSupportedText( gettext_noop( "QUIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 86, 56 }, isEvilInterface,
                                         isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTONS_NEW_GAME_MENU_GOOD: {
            ...
            if ( useOriginalResources() ) {
                for ( size_t i = 0; i < 3; ++i ) {
                    _icnVsSprite[id][i * 2] = fheroes::AGG::GetICN( ICN::BTNNEWGM, static_cast<uint32_t>( i * 2 ) );
                    _icnVsSprite[id][i * 2 + 1] = fheroes::AGG::GetICN( ICN::BTNNEWGM, static_cast<uint32_t>( i * 2 + 1 ) );
                }
                // Add generated buttons.
                const fheroes::FontType buttonFontType = fheroes::FontType::buttonReleasedWhite();
                const fheroes::Size buttonSize{ _icnVsSprite[id][0].width() - 10, _icnVsSprite[id][0].height() };
                fheroes::makeButtonSprites( _icnVsSprite[id][6], _icnVsSprite[id][7], fheroes::getSupportedText( gettext_noop( "BATTLE\nONLY" ), buttonFontType ),
                                             buttonSize, false, ICN::STONEBAK );
                fheroes::makeButtonSprites( _icnVsSprite[id][8], _icnVsSprite[id][9], fheroes::getSupportedText( gettext_noop( "SETTINGS" ), buttonFontType ),
                                             buttonSize, false, ICN::STONEBAK );
                // Add cancel button.
                _icnVsSprite[id][10] = fheroes::AGG::GetICN( ICN::BTNNEWGM, 6 );
                _icnVsSprite[id][11] = fheroes::AGG::GetICN( ICN::BTNNEWGM, 7 );
                // Add hot seat button.
                _icnVsSprite[id][12] = fheroes::AGG::GetICN( ICN::BTNMP, 0 );
                _icnVsSprite[id][13] = fheroes::AGG::GetICN( ICN::BTNMP, 1 );
                // Add player count buttons.
                for ( size_t i = 0; i < 5; ++i ) {
                    _icnVsSprite[id][( i + 7 ) * 2] = fheroes::AGG::GetICN( ICN::BTNHOTST, static_cast<uint32_t>( i * 2 ) );
                    _icnVsSprite[id][( i + 7 ) * 2 + 1] = fheroes::AGG::GetICN( ICN::BTNHOTST, static_cast<uint32_t>( i * 2 + 1 ) );
                }

                if ( isPoLPresent ) {
                    _icnVsSprite[id][24] = fheroes::AGG::GetICN( ICN::X_LOADCM, 0 );
                    _icnVsSprite[id][25] = fheroes::AGG::GetICN( ICN::X_LOADCM, 1 );
                    _icnVsSprite[id][26] = fheroes::AGG::GetICN( ICN::X_LOADCM, 2 );
                    _icnVsSprite[id][27] = fheroes::AGG::GetICN( ICN::X_LOADCM, 3 );
                }
                break;
            }
            const fheroes::FontType buttonFontType = fheroes::FontType::buttonReleasedWhite();
            std::vector<const char *> texts = { fheroes::getSupportedText( gettext_noop( "STANDARD\nGAME" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "CAMPAIGN\nGAME" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "MULTI-\nPLAYER\nGAME" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "BATTLE\nONLY" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "SETTINGS" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "CANCEL" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "HOT SEAT" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "2 PLAYERS" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "3 PLAYERS" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "4 PLAYERS" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "5 PLAYERS" ), buttonFontType ),
                                                fheroes::getSupportedText( gettext_noop( "6 PLAYERS" ), buttonFontType ) };
            if ( isPoLPresent ) {
                texts.emplace_back( fheroes::getSupportedText( gettext_noop( "ORIGINAL\nCAMPAIGN" ), buttonFontType ) );
                texts.emplace_back( fheroes::getSupportedText( gettext_noop( "EXPANSION\nCAMPAIGN" ), buttonFontType ) );
            }

            fheroes::makeSymmetricBackgroundSprites( _icnVsSprite[id], texts, false, 117 );
            break;
        }
        case ICN::BUTTONS_EDITOR_MENU_GOOD: {
            ...
            if ( useOriginalResources() ) {
                for ( size_t i = 0; i < 2; ++i ) {
                    _icnVsSprite[id][i * 2] = fheroes::AGG::GetICN( ICN::BTNEMAIN, static_cast<uint32_t>( i * 2 ) );
                    _icnVsSprite[id][i * 2 + 1] = fheroes::AGG::GetICN( ICN::BTNEMAIN, static_cast<uint32_t>( i * 2 + 1 ) );
                }
                // Add generated Main Menu button.
                const fheroes::FontType buttonFontType = fheroes::FontType::buttonReleasedWhite();
                const fheroes::Size buttonSize{ _icnVsSprite[id][0].width() - 10, _icnVsSprite[id][0].height() };
                fheroes::makeButtonSprites( _icnVsSprite[id][4], _icnVsSprite[id][5], fheroes::getSupportedText( gettext_noop( "MAIN\nMENU" ), buttonFontType ),
                                             buttonSize, false, ICN::STONEBAK );
                // Add generated Back button.
                fheroes::makeButtonSprites( _icnVsSprite[id][6], _icnVsSprite[id][7],
                                             fheroes::getSupportedText( gettext_noop( "editorMainMenu|BACK" ), buttonFontType ), buttonSize, false, ICN::STONEBAK );

                // Add From Scratch and Random buttons.
                for ( size_t i = 0; i < 2; ++i ) {
                    _icnVsSprite[id][( i + 4 ) * 2] = fheroes::AGG::GetICN( ICN::BTNENEW, static_cast<uint32_t>( i * 2 ) );
                    _icnVsSprite[id][( i + 4 ) * 2 + 1] = fheroes::AGG::GetICN( ICN::BTNENEW, static_cast<uint32_t>( i * 2 + 1 ) );
                }
                // Add map size buttons.
                for ( size_t i = 0; i < 4; ++i ) {
                    _icnVsSprite[id][( i + 6 ) * 2] = fheroes::AGG::GetICN( ICN::BTNESIZE, static_cast<uint32_t>( i * 2 ) );
                    _icnVsSprite[id][( i + 6 ) * 2 + 1] = fheroes::AGG::GetICN( ICN::BTNESIZE, static_cast<uint32_t>( i * 2 + 1 ) );
                }

                break;
            }
            const fheroes::FontType buttonFontType = fheroes::FontType::buttonReleasedWhite();
            const std::vector<const char *> texts = { fheroes::getSupportedText( gettext_noop( "NEW\nMAP" ), buttonFontType ),
                                                      fheroes::getSupportedText( gettext_noop( "LOAD\nMAP" ), buttonFontType ),
                                                      fheroes::getSupportedText( gettext_noop( "MAIN\nMENU" ), buttonFontType ),
                                                      fheroes::getSupportedText( gettext_noop( "editorMainMenu|BACK" ), buttonFontType ),
                                                      fheroes::getSupportedText( gettext_noop( "newMap|FROM\nSCRATCH" ), buttonFontType ),
                                                      fheroes::getSupportedText( gettext_noop( "newMap|RANDOM" ), buttonFontType ),
                                                      "36 X 36",
                                                      "72 X 72",
                                                      "108 X 108",
                                                      "144 X 144" };
        ...
        case ICN::EVIL_CAMPAIGN_BUTTONS: {
            ...
            if ( useOriginalResources() ) {
                ...
                for ( uint32_t i = 0; i < 8; i += 2 ) {
                    // released
                    const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( originalIcnId, i );

                    fheroes::Sprite & released = _icnVsSprite[id][i];
                    ...
                    // pressed
                    const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( originalIcnId, i + 1 );

                    fheroes::Sprite & pressed = _icnVsSprite[id][i + 1];
                    ...
                    setButtonCornersTransparent( released );
                    fheroes::makeTransparentBackground( released, pressed, buttonBackground );
                }

                // Generate the DIFFICULTY button because it is not present in the original resources
                const char * text = fheroes::getSupportedText( gettext_noop( "DIFFICULTY" ), fheroes::FontType::buttonReleasedWhite() );
                fheroes::getTextAdaptedSprite( _icnVsSprite[id][8], _icnVsSprite[id][9], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                                buttonBackground );
                break;
            }
        ...
        case ICN::POL_CAMPAIGN_BUTTONS: {
            ...
            if ( useOriginalResources() ) {
                for ( uint32_t i = 0; i < 8; i += 2 ) {
                    // released
                    const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( baseIcnId, i );
                    ...
                    const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( baseIcnId, i + 1 );
                    ...
                    fheroes::makeTransparentBackground( released, pressed, ICN::STONEBAK_SMALL_POL );
                }

                // generate the DIFFICULTY button as it is not present in the original resources
                const char * text = fheroes::getSupportedText( gettext_noop( "DIFFICULTY" ), fheroes::FontType::buttonReleasedWhite() );
                fheroes::getTextAdaptedSprite( _icnVsSprite[id][8], _icnVsSprite[id][9], text, ICN::EMPTY_POL_BUTTON, ICN::STONEBAK_SMALL_POL );
                break;
            }
            createCampaignButtonSet( id, { gettext_noop( "VIEW INTRO" ), gettext_noop( "RESTART" ), gettext_noop( "OKAY" ), gettext_noop( "CANCEL" ),
                                           gettext_noop( "DIFFICULTY" ) } );
        ...
        case ICN::BUTTON_EXIT_HEROES_MEETING: {
            ...
            if ( useOriginalResources() ) {
                _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::SWAPBTN, 0 );
                _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::SWAPBTN, 1 );
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "smallerButton|EXIT" ), fheroes::FontType::buttonReleasedWhite() );
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], text, { 70, 25 }, false, ICN::STONEBAK );
        ...
        case ICN::BUTTON_RESET_EVIL: {
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "RESET" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, isEvilInterface ? ICN::EMPTY_EVIL_BUTTON : ICN::EMPTY_GOOD_BUTTON,
                                  isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK );
        ...
        case ICN::BUTTON_START_GOOD: {
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "START" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, ICN::EMPTY_GOOD_BUTTON, ICN::STONEBAK );
        ...
        case ICN::BUTTON_VERTICAL_PATROL: {
            ...
            const char * text = fheroes::getSupportedText( gettext_noop( "P\nA\nT\nR\nO\nL" ), fheroes::FontType::buttonReleasedWhite() );
            getTextAdaptedSprite( _icnVsSprite[id][0], _icnVsSprite[id][1], text, ICN::EMPTY_VERTICAL_GOOD_BUTTON, ICN::REDBAK_SMALL_VERTICAL );
        ...
        case ICN::DISMISS_HERO_DISABLED_BUTTON: {
            ...
            const fheroes::Sprite & released = fheroes::AGG::GetICN( buttonIcnId, 0 );
            const fheroes::Sprite & pressed = fheroes::AGG::GetICN( buttonIcnId, 1 );

            fheroes::Sprite & output = _icnVsSprite[id][0];
            output = released;

            fheroes::ApplyPalette( output, PAL::GetPalette( PAL::PaletteType::DARKENING ) );

            fheroes::Image common = fheroes::ExtractCommonPattern( { &released, &pressed } );
            common = fheroes::FilterOnePixelNoise( common );
            common = fheroes::FilterOnePixelNoise( common );
            common = fheroes::FilterOnePixelNoise( common );
            FillTransform( common, 1, common.height() - 1, 23, 1, 1 );

            fheroes::Blit( common, output );
            break;
        }
        ...
        case ICN::BUTTON_TOGGLE_ALL_ON_EVIL: {
            ...
            fheroes::makeButtonSprites( _icnVsSprite[id][0], _icnVsSprite[id][1], "", { 86, 25 }, isEvilInterface, isEvilInterface ? ICN::STONBAKE : ICN::STONEBAK );

            fheroes::Sprite icon;
            fheroes::h2d::readImage( "toggle-all-on-released.image", icon );
            if ( isEvilInterface ) {
                fheroes::ApplyPalette( icon, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_BUTTON ) );
            }

            fheroes::Blit( icon, 0, 0, _icnVsSprite[id][0], ( _icnVsSprite[id][0].width() - icon.width() ) / 2, ( _icnVsSprite[id][0].height() - icon.height() ) / 2,
                            icon.width(), icon.height() );

            fheroes::h2d::readImage( "toggle-all-on-pressed.image", icon );
            if ( isEvilInterface ) {
                fheroes::ApplyPalette( icon, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_BUTTON ) );
            }

            fheroes::Blit( icon, 0, 0, _icnVsSprite[id][1], ( _icnVsSprite[id][1].width() - icon.width() ) / 2 - 1,
        ...
        case ICN::BUTTON_TOGGLE_ALL_OFF_EVIL: {
            ...
            fheroes::Sprite icon;
            fheroes::h2d::readImage( "toggle-all-off-released.image", icon );
            if ( isEvilInterface ) {
                fheroes::ApplyPalette( icon, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_BUTTON ) );
            }
            ...
            fheroes::h2d::readImage( "toggle-all-off-pressed.image", icon );
            if ( isEvilInterface ) {
                fheroes::ApplyPalette( icon, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_BUTTON ) );
            }
    ...
    bool generateGermanSpecificImages( const int id )
    {
        ...
        case ICN::BUTTON_SMALL_ACCEPT_GOOD: {
            ...
            fheroes::Sprite & released = _icnVsSprite[id][0];
            fheroes::Sprite & pressed = _icnVsSprite[id][1];
            released = fheroes::AGG::GetICN( ICN::SURRENDR, buttonIcnIndex );
            pressed = fheroes::AGG::GetICN( ICN::SURRENDR, buttonIcnIndex + 1 );
        ...
        case ICN::BUTTON_HSCORES_VERTICAL_STANDARD: {
            ...
            fheroes::Sprite & released = _icnVsSprite[id][0];
            fheroes::Sprite & pressed = _icnVsSprite[id][1];
            released = fheroes::AGG::GetICN( originalID, originalICNIndex );
            pressed = fheroes::AGG::GetICN( originalID, originalICNIndex + 1 );
            // Fill wrong transparent text pixels by using single-colored background.
            fillTransparentButtonText( released );
            return true;
        }
    ...
    bool generateFrenchSpecificImages( const int id )
    {
        ...
        case ICN::BUTTONS_NEW_GAME_MENU_GOOD: {
            ...
            for ( int32_t i = 0; i < 2; ++i ) {
                fheroes::Sprite & out = _icnVsSprite[id][6 + i];
                out = fheroes::AGG::GetICN( ICN::BTNNEWGM, 6 + i );
                // Clean the button
                Fill( out, 27 - i, 21 + i, 77, 14, getButtonFillingColor( i == 0 ) );
                const int32_t secondLine = 28;
                // Add 'MODE'
                Copy( fheroes::AGG::GetICN( ICN::BTNNEWGM, 4 + i ), 35 - i, 13, out, 40 - i, 13, 50, 15 );
                // Clean up 'MODE'
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 109 - i, 18, out, 89 - i, 18, 1, 10 );
                // Add 'BA'
                Copy( fheroes::AGG::GetICN( ICN::BTNBAUD, 2 + i ), 42 - i, 28, out, 23 - i, secondLine, 22, 15 );
                // Clean up 'BA'
                Copy( fheroes::AGG::GetICN( ICN::BTNBAUD, 2 + i ), 42 - i, 31, out, 34 - i, secondLine, 1, 1 );
                Copy( fheroes::AGG::GetICN( ICN::BTNBAUD, 2 + i ), 39 - i, 31, out, 44 - i, secondLine + 4, 1, 2 );
                // Add 'T'
                Copy( fheroes::AGG::GetICN( ICN::BTNDC, 2 + i ), 89 - i, 21, out, 44 - i, secondLine, 12, 15 );
                // Clean up 'AT'
                Copy( fheroes::AGG::GetICN( ICN::BTNDC, 2 + i ), 89 - i, 18, out, 45 - i, secondLine, 1, 1 );
                Copy( fheroes::AGG::GetICN( ICN::BTNDC, 2 + i ), 92 - ( 5 * i ), 27 - i, out, 44 - i, secondLine + 4 + i, 1, 3 );
                // Add 'AI'.
                Copy( fheroes::AGG::GetICN( ICN::BTNMP, 6 + i ), 51 - i, 13, out, 57 - i, secondLine, 18, 15 );
                // Clean up 'TA'
                Copy( fheroes::AGG::GetICN( ICN::BTNBAUD, 2 + i ), 51 - i, 40, out, 55 - i, secondLine + 12, 3, 3 );
                // Add 'LLE'
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 80 - i, 13, out, 76 - i, secondLine, 31, 15 );
                // Clean up "IL"
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 80 - i, 18, out, 76 - i, secondLine + 7, 1, 1 );
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 89 - i, 17, out, 75 - i, secondLine + 4, 2, 2 );
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 88 - i, 25, out, 74 - i, secondLine + 12, 3, 3 );
                Copy( fheroes::AGG::GetICN( ICN::BTNDC, 4 + i ), 23 - i, 8, out, 74 - i, secondLine + 5, 1, 10 );
                Copy( fheroes::AGG::GetICN( ICN::BTNMP, 6 + i ), 68 - i, 16, out, 74 - i, secondLine + 9, 1, 1 );
            }
            // Add config button.
            _icnVsSprite[id][8] = fheroes::AGG::GetICN( ICN::BTNDCCFG, 4 );
            _icnVsSprite[id][9] = fheroes::AGG::GetICN( ICN::BTNDCCFG, 5 );
            // Add cancel button.
            _icnVsSprite[id][10] = fheroes::AGG::GetICN( ICN::BTNNEWGM, 6 );
            _icnVsSprite[id][11] = fheroes::AGG::GetICN( ICN::BTNNEWGM, 7 );
            // Add hot seat button.
        ...
        case ICN::BUTTON_GIFT_GOOD: {
            ...
            for ( int32_t i = 0; i < static_cast<int32_t>( _icnVsSprite[id].size() ); ++i ) {
                fheroes::Sprite & out = _icnVsSprite[id][i];
                const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i );
                ...
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 48 - i, 28 + i, out, offsetXD - i, offsetY + i, 10, 15 );
                // Clean up 'D' and restore button ornament
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 48 - i, 36, out, offsetXD - 1 - i, offsetY + 4 + i, 1, 1 );
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 48 - i, 35, out, offsetXD - i, offsetY + 9 + i, 1, 2 );
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 48 - i, 35, out, offsetXD - 1 - i, offsetY + 13 + i, 1, 1 );
                Fill( out, offsetXD + 9 - i, offsetY + 13 + i, 1, 1, getButtonFillingColor( i == 0 ) );
                Copy( fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i ), offsetXD, offsetY, out, offsetXD, offsetY, 1, 1 );
                // Add 'O'
                const int32_t offsetXO = 10;
                Copy( fheroes::AGG::GetICN( ICN::CAMPXTRG, i ), 40 - ( 7 * i ), 5 + i, out, offsetXD + offsetXO + 1 - i, offsetY + i, 13 - i, 15 );
                // Clean up 'DO'
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 51 - i, 34, out, offsetXD + offsetXO - i, offsetY + 5, 2, 2 );
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 51 - i, 34, out, offsetXD + offsetXO - i, offsetY + 7, 1, 1 + i );
                Copy( fheroes::AGG::GetICN( ICN::CPANEL, 4 + i ), 55 - i, 28 + i, out, offsetXD + 9 - i, offsetY + 2 + i, 3, 3 );
                Fill( out, offsetXD + 11 - i, offsetY + i, 2, 2, getButtonFillingColor( i == 0 ) );
                // Add 'N'
                const int32_t offsetXN = 13;
                Copy( fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i ), 50 - i, 5, out, offsetXD + offsetXO + offsetXN - i, offsetY, 14, 15 );
                // Clean up 'ON'
                ...
                // Add 'N'
                Copy( fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i ), 50 - i, 5, out, offsetXD + 10 + offsetXN + offsetXN - i, offsetY, 14, 15 );
                // Clean up 'NN'
                Fill( out, offsetXD + offsetXO + offsetXN + offsetXN - i, offsetY + 9, 1, 1, getButtonFillingColor( i == 0 ) );
                // Add 'ER'
                Copy( fheroes::AGG::GetICN( ICN::CAMPXTRG, 2 + i ), 75 - ( 8 * i ), 5, out, offsetXD + offsetXO + offsetXN + offsetXN + offsetXN - ( 2 * i ), offsetY,
                      23, 15 );
                // Restore button ornament
                Copy( fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i ), offsetXD + offsetXO + offsetXN + offsetXN + offsetXN + 20, offsetY, out,
                      offsetXD + offsetXO + offsetXN + offsetXN + offsetXN + 20, offsetY, 1, 1 );
                Copy( fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i ), offsetXD + offsetXO + offsetXN + offsetXN + offsetXN + 21, offsetY + 1, out,
        ...
        case ICN::BUTTON_SMALL_MAX_GOOD: {
            ...
            fheroes::Sprite & released = _icnVsSprite[id][0];
            fheroes::Sprite & pressed = _icnVsSprite[id][1];
            const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( ICN::RECRUIT, 4 );
            const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( ICN::RECRUIT, 5 );
        ...
        case ICN::BUTTON_SMALL_MIN_GOOD: {
            ...
            for ( int32_t i = 0; i < static_cast<int32_t>( _icnVsSprite[id].size() ); ++i ) {
                fheroes::Sprite & out = _icnVsSprite[id][i];
                out = fheroes::AGG::GetICN( ICN::BUTTON_SMALL_MAX_GOOD, i );
                ...
                // Add 'I'
                Copy( fheroes::AGG::GetICN( ICN::APANEL, 4 + i ), 25 - i, 19 + i, out, 27 - i, 4 + i, 7 - i, 15 );
                Copy( fheroes::AGG::GetICN( ICN::RECRUIT, 4 + i ), 28 - i, 7 + i, out, 31 - i, 7 + i, 3, 9 );
                Fill( out, 32 - i, 16 + i, 2, 3, getButtonFillingColor( i == 0 ) );
                // Add 'N'
                Copy( fheroes::AGG::GetICN( ICN::TRADPOST, 17 + i ), 50 - i, 5, out, 36 - i, 5, 14, 15 );
        ...
        case ICN::BUTTON_SMALL_ACCEPT_GOOD: {
            ...
            fheroes::ReplaceColorId( released, 109, 39 );
            fheroes::ReplaceColorId( pressed, 178, 39 );
            return true;
        }
        case ICN::BUTTON_HSCORES_VERTICAL_CAMPAIGN:
        case ICN::BUTTON_HSCORES_VERTICAL_EXIT:
    ...
    bool generatePolishSpecificImages( const int id )
    {
        ...
        case ICN::BUTTONS_NEW_GAME_MENU_GOOD: {
            ...
            for ( int32_t i = 0; i < 2; ++i ) {
                ...
                // Add 'BI'
                Copy( fheroes::AGG::GetICN( ICN::BTNMCFG, 2 + i ), 58 - i, 29, out, offsetX - i, offsetY, 14, 11 );
                // Add 'T'
                Copy( fheroes::AGG::GetICN( ICN::BTNNEWGM, 0 + i ), 19 - i, 29, out, offsetX + 14 - i, offsetY, 9, 11 );
                // Add 'WA'
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 40 - i, 23, out, offsetX + 23 - i, offsetY, 24, 11 );
                // Add pixel to 'W'
                Copy( fheroes::AGG::GetICN( ICN::BTNEMAIN, 0 + i ), 42 - i, 23 + i, out, offsetX + 38 - i, offsetY + i, 1, 1 );
            }
            // Add config button.
            _icnVsSprite[id][8] = fheroes::AGG::GetICN( ICN::BTNDCCFG, 4 );
            _icnVsSprite[id][9] = fheroes::AGG::GetICN( ICN::BTNDCCFG, 5 );
        ...
        case ICN::BUTTON_SMALL_MIN_GOOD: {
            ...
            for ( int32_t i = 0; i < static_cast<int32_t>( _icnVsSprite[id].size() ); ++i ) {
                fheroes::Sprite & out = _icnVsSprite[id][i];
                const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::BUTTON_SMALL_MAX_GOOD, i );
                out = original;
                // Wipe the button
                ...
                // 'I'
                Copy( fheroes::AGG::GetICN( ICN::BTNMCFG, 4 + i ), 53 - i, 23 + i, out, 28 - i, 7 + i, 5, 10 );
                // 'N'
                Copy( fheroes::AGG::GetICN( ICN::BTNMCFG, 0 + i ), 83 - i, 29 + i, out, 34 - i, 7 + i, 11, 10 );
                // '.'
                Copy( original, 52 - i, 14 + i, out, 47 - i, 14 + i, 3, 3 );
                fheroes::Fill( out, 44 - i, 8 + i, 1, 9, getButtonFillingColor( i == 0 ) );
                if ( i == 0 ) {
                    fheroes::ReplaceColorId( out, 56, 55 );
                }
    ...
    void generateLanguageSpecificImages( int id )
    {
        ...
        if ( fheroes::getCurrentLanguage() == resourceLanguage ) {
                               ...
                               const uint8_t contourInnerColor, const uint8_t contourOuterColor )
    {
        ...
        fheroes::AGG::GetICN( originalFontId, 0 );
        const std::vector<fheroes::Sprite> & original = _icnVsSprite[originalFontId];

        _icnVsSprite[fontId].resize( original.size() );

        for ( size_t i = 0; i < original.size(); ++i ) {
            const fheroes::Sprite & in = original[i];
            fheroes::Sprite & out = _icnVsSprite[fontId][i];
    ...
    //  This function modifies (fixes) the original ICNs and generate new fheroes-related ICNs.
    ...
    void processICN( const int id )
    {
        ...
        case ICN::SMALFONT: {
            ...
            const std::vector<uint8_t> & body = ::AGG::getDataFromAggFile( ICN::getIcnFileName( id ), false );
            const uint32_t crc32 = fheroes::calculateCRC32( body.data(), body.size() );

            if ( id == ICN::SMALFONT ) {
                // Small font in official Polish GoG version has all letters shifted 1 pixel down.
                if ( crc32 == 0xE9EC7A63 ) {
                    for ( fheroes::Sprite & letter : imageArray ) {
                ...
                for ( fheroes::Sprite & fontImage : imageArray ) {
            ...
            if ( imageArray.size() == 162 && imageArray[121].width() == verifiedFontWidth ) {
                ...
                const fheroes::Sprite firstSprite{ imageArray[0] };
        ...
        case ICN::SPELLS:
            ...
            for ( uint32_t i = 60; i < 66; ++i ) {
                ...
                const fheroes::Sprite & originalImage = _icnVsSprite[id][originalIndex];
                fheroes::Sprite & image = _icnVsSprite[id][i];
            ...
            fheroes::h2d::readImage( "petrification_spell_icon.image", _icnVsSprite[id][66] );

            // Generate random spell image for Editor.
            {
                const fheroes::Sprite & randomSpellImage = _icnVsSprite[id][2];
                ...
                for ( uint32_t i = 1; i < 6; ++i ) {
                    fheroes::Sprite & originalImage = _icnVsSprite[id][i + 67];
                    Copy( randomSpellImage, originalImage );

                    const fheroes::Text text( std::to_string( i ), fheroes::FontType::normalWhite() );
            ...
            // Mass Dispel icon.
            {
                const auto & spellAnimation = fheroes::AGG::GetICN( ICN::MAGIC07, 2 );
                _icnVsSprite[id][73] = fheroes::Crop( spellAnimation, 28, 27, 39, 36 );

                fheroes::Sprite contour;
                fheroes::h2d::readImage( "mass-dispel-contour.image", contour );

                fheroes::Blit( contour, _icnVsSprite[id][73] );
            }
        ...
        case ICN::CSLMARKER:
            ...
            for ( uint32_t i = 0; i < 3; ++i ) {
                _icnVsSprite[id][i] = fheroes::AGG::GetICN( ICN::LOCATORS, 24 );
                if ( i == 1 ) {
        ...
        case ICN::PHOENIX:
            ...
            if ( _icnVsSprite[id].size() >= 32 ) {
                const fheroes::Sprite & in = _icnVsSprite[id][32];
        ...
        case ICN::MONH0028: // phoenix
            if ( _icnVsSprite[id].size() == 1 ) {
                const fheroes::Sprite & correctFrame = fheroes::AGG::GetICN( ICN::PHOENIX, 32 );
                Copy( correctFrame, 60, 73, _icnVsSprite[id][0], 58, 70, 14, 13 );
            }
            break;
        case ICN::CAVALRYR:
            // fheroes::Sprite 23 has incorrect colors, we need to replace them
            if ( _icnVsSprite[id].size() >= 23 ) {
                fheroes::Sprite & out = _icnVsSprite[id][23];
        ...
        case ICN::TROLLMSL:
            if ( _icnVsSprite[id].size() == 1 ) {
                fheroes::Sprite & out = _icnVsSprite[id][0];
        ...
        case ICN::X_LOADCM: {
            ...
            for ( size_t i = 0; i < _icnVsSprite[id].size() / 2; ++i ) {
                fheroes::Sprite & released = _icnVsSprite[id][i * 2];
                fheroes::Sprite & pressed = _icnVsSprite[id][i * 2 + 1];

                fheroes::Sprite tempReleased;
                fheroes::Sprite tempPressed;
                ...
                fheroes::Copy( released, removedShadowOffsetWidth, 0, tempReleased, 0, 0, released.width() - removedShadowOffsetWidth,
                                released.height() - removedShadowOffsetHeight );
                fheroes::Copy( pressed, removedShadowOffsetWidth, 0, tempPressed, 0, 0, pressed.width() - removedShadowOffsetWidth,
            ...
            if ( id == ICN::BTNNEWGM ) {
                ...
                fheroes::Image common = fheroes::ExtractCommonPattern( { &_icnVsSprite[id][2], &_icnVsSprite[id][3] } );
                common = fheroes::FilterOnePixelNoise( common );
                common = fheroes::FilterOnePixelNoise( common );
                common = fheroes::FilterOnePixelNoise( common );
                fheroes::Blit( common, _icnVsSprite[id][2] );
            }
        ...
        case ICN::GOLEM2:
            ...
            if ( _icnVsSprite[id].size() == 40 ) {
                ...
                for ( int32_t i = 0; i < copyFramesNum; ++i, ++copyFrame ) {
                    ...
                    fheroes::Sprite originalFrame = _icnVsSprite[id][copyFrame];
        ...
        case ICN::LOCATORS:
            if ( _icnVsSprite[id].size() > 15 ) {
                if ( _icnVsSprite[id][12].width() == 47 ) {
                    fheroes::Sprite & out = _icnVsSprite[id][12];
                    out = Crop( out, 0, 0, out.width() - 1, out.height() );
                }
                if ( _icnVsSprite[id][15].width() == 47 ) {
                    fheroes::Sprite & out = _icnVsSprite[id][15];
                    out = Crop( out, 0, 0, out.width() - 1, out.height() );
                }
            }
            ...
            if ( _icnVsSprite[id].size() == 25 ) {
                ...
                const fheroes::Text text( "? ? ?", fheroes::FontType::normalWhite() );
        ...
        case ICN::TOWNBKG2:
            if ( _icnVsSprite[id].size() == 1 ) {
                fheroes::Sprite fix;
                fheroes::h2d::readImage( "townbkg2_fix.image", fix );
        ...
        case ICN::TWNSSPEC:
            if ( _icnVsSprite[id].size() == 1 ) {
                fheroes::Sprite fix;
                fheroes::h2d::readImage( "twnsspec_fix.image", fix );
        ...
        case ICN::HSICONS:
            if ( _icnVsSprite[id].size() > 7 ) {
                fheroes::Sprite & out = _icnVsSprite[id][7];
        ...
        case ICN::EVIL_DIALOG_PLAIN_CORNERS: {
            ...
            fheroes::Sprite & cornerSprite = _icnVsSprite[id][0];
            const int32_t cornerSideLength = 43;
            cornerSprite.resize( cornerSideLength * 2, cornerSideLength * 2 );
            const fheroes::Sprite & originalGoodDialog = fheroes::AGG::GetICN( ICN::WINLOSE, 0 );
            ...
            fheroes::ApplyPalette( cornerSprite, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );

            break;
        }
        case ICN::MONS32:
            if ( _icnVsSprite[id].size() > 4 ) { // Veteran Pikeman
                fheroes::Sprite & modified = _icnVsSprite[id][4];

                fheroes::Sprite temp( modified.width(), modified.height() + 1 );
                temp.reset();
            ...
            if ( _icnVsSprite[id].size() > 6 ) { // Master Swordsman
                fheroes::Sprite & modified = _icnVsSprite[id][6];
            ...
            if ( _icnVsSprite[id].size() > 8 ) { // Champion
                fheroes::Sprite & modified = _icnVsSprite[id][8];
            ...
            if ( _icnVsSprite[id].size() > 33 ) {
                ...
                fheroes::Sprite & modified = _icnVsSprite[id][33];
            ...
            if ( _icnVsSprite[id].size() > 62 ) {
                for ( size_t i = 0; i < 62; ++i ) {
                    fheroes::Sprite & modified = _icnVsSprite[id][i];
                    const fheroes::Point originalOffset( modified.x(), modified.y() );
                    fheroes::Sprite temp = addShadow( modified, { -1, 2 }, 2 );
                    temp.setPosition( originalOffset.x - 1, originalOffset.y + 2 );

                    const fheroes::Rect area = GetActiveROI( temp, 2 );
                    if ( area.x > 0 || area.height != temp.height() ) {
                        const fheroes::Point offset( temp.x() - area.x, temp.y() - temp.height() + area.y + area.height );
            ...
            if ( _icnVsSprite[id].size() > 63 && _icnVsSprite[id][63].width() == 19 && _icnVsSprite[id][63].height() == 37 ) { // Air Elemental
                fheroes::Sprite & modified = _icnVsSprite[id][63];
        ...
        case ICN::MONSTER_SWITCH_LEFT_ARROW:
            ...
            for ( uint32_t i = 0; i < 2; ++i ) {
                const fheroes::Sprite & source = fheroes::AGG::GetICN( ICN::RECRUIT, i );
                fheroes::Sprite & out = _icnVsSprite[id][i];
        ...
        case ICN::MONSTER_SWITCH_RIGHT_ARROW:
            _icnVsSprite[id].resize( 2 );
            for ( uint32_t i = 0; i < 2; ++i ) {
                const fheroes::Sprite & source = fheroes::AGG::GetICN( ICN::RECRUIT, i + 2 );
                fheroes::Sprite & out = _icnVsSprite[id][i];
                out.resize( source.height(), source.width() );
                Transpose( source, out );
                out = Flip( out, false, true );
        ...
        case ICN::SURRENDR:
            ...
            if ( _icnVsSprite[id].size() >= 4 ) {
                for ( const uint32_t i : { 0, 2 } ) {
                    fheroes::Sprite & out = _icnVsSprite[id][i + 1];
                    fheroes::Sprite tmp( out.width(), out.height() );
                    tmp.reset();
                    ...
                    fheroes::makeTransparentBackground( _icnVsSprite[id][i], _icnVsSprite[id][i + 1], ICN::STONEBAK );
                }
            }
            break;
        case ICN::NON_UNIFORM_GOOD_RESTART_BUTTON:
            _icnVsSprite[id].resize( 2 );
            _icnVsSprite[id][0] = Crop( fheroes::AGG::GetICN( ICN::CAMPXTRG, 2 ), 6, 0, 108, 25 );
            _icnVsSprite[id][0].setPosition( 0, 0 );

            _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::CAMPXTRG, 3 );
            _icnVsSprite[id][1].setPosition( 0, 0 );
        ...
        case ICN::WHITE_LARGE_FONT: {
            fheroes::AGG::GetICN( ICN::FONT, 0 );
            const std::vector<fheroes::Sprite> & original = _icnVsSprite[ICN::FONT];
        ...
        case ICN::SWAP_ARROW_RIGHT_TO_LEFT: {
            ...
            const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );
            std::array<fheroes::Image, 4> input;

            const int32_t width = 45;
            const int32_t height = 20;

            for ( fheroes::Image & image : input ) {
            ...
            fheroes::Image out = fheroes::ExtractCommonPattern( { &input[0], &input[1], &input[2], &input[3] } );
        ...
        case ICN::SWAP_ARROWS_CIRCULAR: {
            const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::SWAP_ARROW_LEFT_TO_RIGHT, 0 );
            ...
            fheroes::Copy( Flip( rotatedArrow, true, true ), 0, 0, out, 0, 5, arrowHeadWidth, arrowTipToShaftLength );
            fheroes::Copy( Flip( rotatedArrow, true, false ), 0, 0, out, 27, 19, arrowHeadWidth, arrowTipToShaftLength );
            ...
            // Clean black corner.
            fheroes::Copy( original, 0, 0, rotatedArrow, 0, rotatedArrow.height() - 1, 1, 1 );

            fheroes::Copy( rotatedArrow, 0, 0, out, 32, 6, arrowEndWidth, arrowEndHeight );
            fheroes::Copy( Flip( rotatedArrow, false, true ), 0, 0, out, 5, 25, arrowEndWidth, arrowEndHeight );
            ...
            // Lower arrow.
            // Fix overlaps.
            fheroes::SetPixel( out, out.width() - 9, out.height() - 5, 119 );
            fheroes::DrawLine( out, { out.width() - 13, out.height() - 6 }, { out.width() - 12, out.height() - 6 }, 109 );
            fheroes::SetPixel( out, 12, out.height() - 10, 119 );
            fheroes::SetPixel( out, out.width() - 14, out.height() - 10, 119 );

            // Lower right corner.
            Copy( original, 5, 10, out, out.width() - 13, out.height() - 5, 4, 5 );
            fheroes::DrawLine( out, { out.width() - 6, out.height() - 4 }, { out.width() - 9, out.height() - 1 }, 59 );
            fheroes::DrawLine( out, { out.width() - 6, out.height() - 5 }, { out.width() - 9, out.height() - 2 }, 59 );
            fheroes::DrawLine( out, { out.width() - 7, out.height() - 5 }, { out.width() - 9, out.height() - 3 }, 129 );
            fheroes::DrawLine( out, { out.width() - 8, out.height() - 5 }, { out.width() - 9, out.height() - 4 }, 123 );
            fheroes::SetPixel( out, out.width() - 9, out.height() - 5, 119 );
            fheroes::SetPixel( out, out.width() - 11, out.height() - 6, 112 );

            // Lower left corner.
            Copy( original, 5, 9, out, 9, out.height() - 6, 3, 6 );
            Copy( rotatedArrow, 0, 0, out, 5, out.height() - 7, 4, 2 );
            fheroes::DrawLine( out, { 5, out.height() - 5 }, { 8, out.height() - 2 }, 129 );
            fheroes::DrawLine( out, { 6, out.height() - 5 }, { 8, out.height() - 3 }, 123 );
            fheroes::DrawLine( out, { 7, out.height() - 5 }, { 8, out.height() - 4 }, 119 );
            fheroes::SetPixel( out, 8, out.height() - 5, 116 );
            fheroes::SetPixel( out, 9, out.height() - 6, 112 );

            // Fix shading.
            fheroes::DrawLine( out, { 14, out.height() - 15 }, { 14, out.height() - 11 }, 59 );

            // Upper arrow.
            // Upper left corner.
            fheroes::Copy( out, 15, 0, out, 9, 0, 4, 5 );
            fheroes::Copy( out, 21, 5, out, 11, 5, 3, 2 );
            fheroes::DrawLine( out, { 8, 1 }, { 5, 4 }, 129 );
            fheroes::DrawLine( out, { 8, 2 }, { 6, 4 }, 119 );
            fheroes::DrawLine( out, { 8, 3 }, { 7, 4 }, 114 );
            fheroes::SetPixel( out, 8, 4, 112 );
            fheroes::SetPixel( out, 9, 4, 112 );

            // Upper right corner.
            fheroes::Copy( out, 21, 0, out, 33, 0, 3, 6 );
            fheroes::Copy( out, 36, 7, out, 36, 5, 5, 1 );
            fheroes::Copy( out, 37, 6, out, 37, 4, 3, 1 );
            fheroes::Copy( out, 37, 6, out, 37, 4, 3, 1 );
            fheroes::Copy( out, 34, 1, out, 36, 1, 1, 3 );
            fheroes::DrawLine( out, { 36, 0 }, { 40, 4 }, 129 );
            fheroes::DrawLine( out, { 37, 2 }, { 38, 3 }, 119 );
            fheroes::DrawLine( out, { 36, 4 }, { 37, 3 }, 113 );

            // Fix overlap.
            fheroes::DrawLine( out, { 33, 8 }, { 33, 9 }, 123 );
            fheroes::SetPixel( out, 32, 9, 129 );
            fheroes::SetPixel( out, 13, 9, 129 );

            // Fix shading.
            fheroes::DrawLine( out, { 10, 22 }, { 18, 14 }, 59 );
            fheroes::DrawLine( out, { 11, 22 }, { 19, 14 }, 59 );
            fheroes::DrawLine( out, { 34, 17 }, { 40, 17 }, 59 );
            fheroes::DrawLine( out, { 41, 5 }, { 41, 16 }, 59 );
            fheroes::SetPixel( out, 40, 16, 59 );
            fheroes::Copy( original, 0, 0, out, 1, 12, 4, 1 );
            fheroes::Copy( original, 0, 0, out, 15, 12, 5, 1 );
        ...
        case ICN::EDITBTNS:
            if ( _icnVsSprite[id].size() == 35 ) {
                ...
                fheroes::Sprite released = fheroes::AGG::GetICN( ICN::EDITBTNS, 4 );
                fheroes::Sprite pressed = fheroes::AGG::GetICN( ICN::EDITBTNS, 5 );
                ...
                drawImageOnButton( fheroes::AGG::GetICN( ICN::X_LOC1, 0 ), 39, 29, _icnVsSprite[id][35], _icnVsSprite[id][36] );

                // Kingdom objects button.
                drawImageOnButton( fheroes::AGG::GetICN( ICN::OBJNARTI, 13 ), 39, 29, _icnVsSprite[id][37], _icnVsSprite[id][38] );

                // Monsters objects button.
                drawImageOnButton( fheroes::AGG::GetICN( ICN::MONS32, 11 ), 39, 29, _icnVsSprite[id][39], _icnVsSprite[id][40] );

                // Undo and Redo buttons.
                fheroes::Image undoImage( 19, 13 );
                undoImage.reset();
                const int mainColor = 56;
                fheroes::SetPixel( undoImage, 0, 6, mainColor );
                for ( int x = 1; x < 7; ++x ) {
                    fheroes::DrawLine( undoImage, { x, 6 - x }, { x, 6 + x }, mainColor );
                }
                fheroes::Fill( undoImage, 7, 4, 9, 5, mainColor );
                fheroes::SetPixel( undoImage, 16, 5, mainColor );
                fheroes::Fill( undoImage, 16, 6, 2, 4, mainColor );
                fheroes::Fill( undoImage, 17, 8, 2, 4, mainColor );
                fheroes::SetPixel( undoImage, 18, 12, mainColor );
                drawImageOnButton( undoImage, 39, 29, _icnVsSprite[id][41], _icnVsSprite[id][42] );
                drawImageOnButton( fheroes::Flip( undoImage, true, false ), 39, 29, _icnVsSprite[id][43], _icnVsSprite[id][44] );
                // Fix shadow pixels
                fheroes::SetPixel( _icnVsSprite[id][43], 27, 11, 41 );
                fheroes::SetPixel( _icnVsSprite[id][44], 26, 12, 46 );
                fheroes::SetPixel( _icnVsSprite[id][43], 16, 17, 47 );
                fheroes::SetPixel( _icnVsSprite[id][44], 15, 18, 52 );
                // Add aliasing
                fheroes::SetPixel( _icnVsSprite[id][41], 32, 19, 52 );
                fheroes::DrawLine( _icnVsSprite[id][41], { 33, 20 }, { 33, 21 }, 52 );
                fheroes::SetPixel( _icnVsSprite[id][42], 31, 20, 53 );
                fheroes::DrawLine( _icnVsSprite[id][42], { 32, 21 }, { 32, 22 }, 53 );
            }
            break;
        ...
        case ICN::DROPLISL_EVIL: {
            ...
            for ( auto & image : _icnVsSprite[id] ) {
                fheroes::ApplyPalette( image, 0, 0, image, 0, 0, image.width(), image.height(), palette );
            }
            break;
        }
        case ICN::CELLWIN_EVIL: {
            ...
            if ( _icnVsSprite[ICN::CELLWIN].size() > 18 ) {
                ...
                const std::vector<uint8_t> palette
                    = PAL::CombinePalettes( PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ), PAL::GetPalette( PAL::PaletteType::GRAY ) );
                for ( fheroes::Sprite & image : _icnVsSprite[ICN::CELLWIN_EVIL] ) {
                    fheroes::ApplyPalette( image, 0, 0, image, 0, 0, image.width(), image.height(), palette );
                }
            }
            break;
        ...
        case ICN::EDITPANL:
            if ( _icnVsSprite[id].size() == 6 ) {
                ...
                Blit( fheroes::AGG::GetICN( ICN::MTNCRCK, 3 ), 4, 0, _icnVsSprite[id][6], 1, 1, 24, 24 );

                // Make Rocks objects button.
                Blit( fheroes::AGG::GetICN( ICN::OBJNGRAS, 41 ), 1, 0, _icnVsSprite[id][7], 1, 9, 23, 12 );

                // Make Trees object button. Also used as erase terrain objects button image.
                Copy( fheroes::AGG::GetICN( ICN::EDITBTNS, 2 ), 13, 4, _icnVsSprite[id][8], 1, 1, 24, 24 );
                ...
                Blit( fheroes::AGG::GetICN( ICN::OBJNWAT2, 0 ), 0, 3, _icnVsSprite[id][9], 5, 1, 13, 3 );
                Blit( fheroes::AGG::GetICN( ICN::OBJNWAT2, 2 ), 5, 0, _icnVsSprite[id][9], 1, 4, 24, 21 );

                // Make Landscape Miscellaneous objects button.
                Blit( fheroes::AGG::GetICN( ICN::OBJNDIRT, 73 ), 8, 0, _icnVsSprite[id][10], 1, 1, 24, 24 );

                // Make Dwellings button image.
                Blit( fheroes::AGG::GetICN( ICN::OBJNMULT, 114 ), 7, 0, _icnVsSprite[id][11], 1, 1, 24, 24 );

                // Make Mines button image.
                Blit( fheroes::AGG::GetICN( ICN::MTNMULT, 82 ), 8, 4, _icnVsSprite[id][12], 1, 1, 24, 24 );

                // Make Power-ups button image.
                Blit( fheroes::AGG::GetICN( ICN::OBJNMULT, 72 ), 0, 6, _icnVsSprite[id][13], 1, 1, 24, 24 );

                // Make Adventure Water objects button.
                Blit( fheroes::AGG::GetICN( ICN::OBJNWATR, 24 ), 3, 0, _icnVsSprite[id][14], 1, 1, 24, 24 );

                // Make Adventure Miscellaneous objects button.
                Blit( fheroes::AGG::GetICN( ICN::OBJNMUL2, 198 ), 2, 0, _icnVsSprite[id][15], 1, 1, 24, 24 );

                // Make erase Roads button image.
                Blit( fheroes::AGG::GetICN( ICN::ROAD, 2 ), 0, 0, _icnVsSprite[id][16], 1, 8, 24, 5 );
                Blit( fheroes::AGG::GetICN( ICN::ROAD, 1 ), 1, 0, _icnVsSprite[id][16], 1, 13, 24, 5 );

                // Make erase Streams button image.
                Blit( fheroes::AGG::GetICN( ICN::STREAM, 2 ), 0, 0, _icnVsSprite[id][17], 1, 8, 24, 11 );
            }
            break;
        case ICN::TEXTBAR:
        ...
        case ICN::MINIHERO:
            if ( _icnVsSprite[id].size() == 42 ) {
                ...
                for ( size_t i = 0; i < 6; ++i ) {
                    fheroes::Sprite & kinght = _icnVsSprite[id][i * 7];
                    const fheroes::Sprite & barbarian = _icnVsSprite[id][i * 7 + 1];
                    const int32_t width = kinght.width();
                    const int32_t height = kinght.height();
                    fheroes::Sprite fixed( width + 2, height );
        ...
        case ICN::HEROES:
            if ( !_icnVsSprite[id].empty() ) {
                fheroes::Sprite & original = _icnVsSprite[id][0];
                ...
                if ( !::AGG::getDataFromAggFile( ICN::getIcnFileName( ICN::X_TRACK1 ), false ).empty() ) {
                    fheroes::Sprite editorIcon;
                    fheroes::h2d::readImage( "main_menu_editor_icon.image", editorIcon );
        ...
        case ICN::BTNSHNGL:
            if ( _icnVsSprite[id].size() == 20 ) {
                ...
                fheroes::h2d::readImage( "main_menu_editor_released_button.image", _icnVsSprite[id][20] );
                fheroes::h2d::readImage( "main_menu_editor_highlighted_button.image", _icnVsSprite[id][21] );
                fheroes::h2d::readImage( "main_menu_editor_pressed_button.image", _icnVsSprite[id][22] );
            }
            break;
        case ICN::TOWNBKG3:
        ...
        case ICN::MINIPORT:
            ...
            if ( _icnVsSprite[id].size() > 60 ) {
                fheroes::Sprite & original = _icnVsSprite[id][60];
            ...
            if ( _icnVsSprite[id].size() > 61 ) {
                fheroes::Sprite & original = _icnVsSprite[id][61];
                if ( original.width() == 30 && original.height() == 22 ) {
                    original._disableTransformLayer();
                    uint8_t * imageData = original.image();
                    imageData[51] = 30;
            ...
            if ( _icnVsSprite[id].size() > 65 ) {
                fheroes::Sprite & original = _icnVsSprite[id][65];
                if ( original.width() == 30 && original.height() == 22 ) {
                    original._disableTransformLayer();
                    uint8_t * imageData = original.image();
                    imageData[499] = 60;
            ...
            if ( _icnVsSprite[id].size() > 67 ) {
                fheroes::Sprite & original = _icnVsSprite[id][67];
                if ( original.width() == 30 && original.height() == 22 ) {
                    original._disableTransformLayer();
        ...
        case ICN::MINICAPT:
            ...
            if ( _icnVsSprite[id].size() > 1 ) {
                fheroes::Sprite & original = _icnVsSprite[id][1];
                if ( original.width() == 30 && original.height() == 22 ) {
                    original._disableTransformLayer();
        ...
        case ICN::CSTLWZRD:
            if ( _icnVsSprite[id].size() >= 8 ) {
                ...
                fheroes::Sprite & original = _icnVsSprite[id][7];
            ...
            if ( _icnVsSprite[id].size() > 28 ) {
                ...
                for ( const uint32_t index : { 23, 28 } ) {
                    fheroes::Sprite & original = _icnVsSprite[id][index];
                    if ( original.width() == 135 && original.height() == 57 ) {
                        original._disableTransformLayer();
        ...
        case ICN::CSTLSORC:
            if ( _icnVsSprite[id].size() >= 14 ) {
                ...
                fheroes::Sprite & original = _icnVsSprite[id][13];
                if ( original.width() == 135 && original.height() == 57 ) {
                    original._disableTransformLayer();
                    uint8_t * imageData = original.image();
                    imageData[2047] = 160;
            ...
            if ( _icnVsSprite[id].size() >= 25 ) {
                ...
                fheroes::Sprite & original = _icnVsSprite[id][24];
                if ( original.width() == 135 && original.height() == 57 ) {
                    original._disableTransformLayer();
                    uint8_t * imageData = original.image();
                    imageData[2830] = 165;
        ...
        case ICN::MONO_CURSOR_ADVENTURE_MAP: {
            ...
            const std::vector<fheroes::Point> twoPoints = { { 2, 1 }, { 3, 1 }, { 1, 2 }, { 4, 2 }, { 3, 3 }, { 2, 4 }, { 1, 5 }, { 2, 5 }, { 3, 5 }, { 4, 5 } };
            const std::vector<fheroes::Point> threePoints
                = { { 1, 1 }, { 2, 1 }, { 3, 1 }, { 4, 2 }, { 1, 3 }, { 2, 3 }, { 3, 3 }, { 4, 4 }, { 1, 5 }, { 2, 5 }, { 3, 5 } };
            const std::vector<fheroes::Point> fourPoints = { { 1, 1 }, { 3, 1 }, { 1, 2 }, { 3, 2 }, { 1, 3 }, { 2, 3 }, { 3, 3 }, { 4, 3 }, { 3, 4 }, { 3, 5 } };
            const std::vector<fheroes::Point> fivePoints
                = { { 1, 1 }, { 2, 1 }, { 3, 1 }, { 4, 1 }, { 1, 2 }, { 1, 3 }, { 2, 3 }, { 3, 3 }, { 4, 4 }, { 1, 5 }, { 2, 5 }, { 3, 5 } };
            const std::vector<fheroes::Point> sixPoints = { { 2, 1 }, { 3, 1 }, { 1, 2 }, { 1, 3 }, { 2, 3 }, { 3, 3 }, { 1, 4 }, { 4, 4 }, { 2, 5 }, { 3, 5 } };
            const std::vector<fheroes::Point> sevenPoints = { { 1, 1 }, { 2, 1 }, { 3, 1 }, { 4, 1 }, { 4, 2 }, { 3, 3 }, { 2, 4 }, { 2, 5 } };
            const std::vector<fheroes::Point> plusPoints = { { 2, 1 }, { 1, 2 }, { 2, 2 }, { 3, 2 }, { 2, 3 } };

            const bool isColorCursor = ( id == ICN::COLOR_CURSOR_ADVENTURE_MAP );
            const uint8_t digitColor = isColorCursor ? 115 : 11;

            std::vector<fheroes::Image> digits( 7 );
            ...
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 4 ), digits,
                                 isColorCursor ? fheroes::Point( -2, 1 ) : fheroes::Point( -4, -6 ) );
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 5 ), digits,
                                 isColorCursor ? fheroes::Point( 1, 1 ) : fheroes::Point( -6, -6 ) );
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 6 ), digits,
                                 isColorCursor ? fheroes::Point( 0, 1 ) : fheroes::Point( -8, -7 ) );
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 7 ), digits,
                                 isColorCursor ? fheroes::Point( -2, 1 ) : fheroes::Point( -15, -8 ) );
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 8 ), digits,
                                 isColorCursor ? fheroes::Point( 1, 1 ) : fheroes::Point( -16, -11 ) );
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 9 ), digits,
                                 isColorCursor ? fheroes::Point( -6, 1 ) : fheroes::Point( -8, -1 ) );
            populateCursorIcons( _icnVsSprite[id], fheroes::AGG::GetICN( originalCursorId, 28 ), digits,
                                 isColorCursor ? fheroes::Point( 0, 1 ) : fheroes::Point( -8, -7 ) );
        ...
        case ICN::KNIGHT_CASTLE_RIGHT_FARM: {
            _icnVsSprite[id].resize( 1 );
            fheroes::Sprite & output = _icnVsSprite[id][0];
            output = fheroes::AGG::GetICN( ICN::TWNKWEL2, 0 );
        ...
        case ICN::KNIGHT_CASTLE_LEFT_FARM:
            _icnVsSprite[id].resize( 1 );
            fheroes::h2d::readImage( "knight_castle_left_farm.image", _icnVsSprite[id][0] );
            break;
        case ICN::BARBARIAN_CASTLE_CAPTAIN_QUARTERS_LEFT_SIDE:
            _icnVsSprite[id].resize( 1 );
            fheroes::h2d::readImage( "barbarian_castle_captain_quarter_left_side.image", _icnVsSprite[id][0] );
            break;
        case ICN::SORCERESS_CASTLE_CAPTAIN_QUARTERS_LEFT_SIDE:
            _icnVsSprite[id].resize( 1 );
            fheroes::h2d::readImage( "sorceress_castle_captain_quarter_left_side.image", _icnVsSprite[id][0] );
            break;
        case ICN::NECROMANCER_CASTLE_STANDALONE_CAPTAIN_QUARTERS: {
            _icnVsSprite[id].resize( 1 );
            fheroes::Sprite & output = _icnVsSprite[id][0];
            const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::TWNNCAPT, 0 );
            ...
            const fheroes::Sprite & castle = fheroes::AGG::GetICN( ICN::TWNNCSTL, 0 );
        ...
        case ICN::ESCROLL:
            if ( _icnVsSprite[id].size() > 4 ) {
                ...
                fheroes::Sprite & out = _icnVsSprite[id][4];
        ...
        case ICN::MAP_TYPE_ICON: {
            _icnVsSprite[id].resize( 3 );
            for ( fheroes::Sprite & icon : _icnVsSprite[id] ) {
                icon._disableTransformLayer();
                icon.resize( 17, 17 );
                icon.fill( 0 );
            }

            const fheroes::Sprite & successionWarsIcon = fheroes::AGG::GetICN( ICN::ARTFX, 6 );
            const fheroes::Sprite & priceOfLoyaltyIcon = fheroes::AGG::GetICN( ICN::ARTFX, 90 );
            const fheroes::Sprite & resurrectionIcon = fheroes::AGG::GetICN( ICN::ARTFX, 101 );
        ...
        case ICN::TWNWWEL2: {
            if ( _icnVsSprite[id].size() == 7 ) {
                ...
                for ( size_t i = 1; i < 7; ++i ) {
                    fheroes::Sprite & original = _icnVsSprite[id][i];
        ...
        case ICN::GOOD_MARKET_BUTTON: {
            ...
            Copy( fheroes::AGG::GetICN( ICN::ADVBTNS, releasedIndex ), _icnVsSprite[id][0] );
            Copy( fheroes::AGG::GetICN( ICN::ADVBTNS, releasedIndex + 1 ), _icnVsSprite[id][1] );
        ...
        case ICN::EVIL_MARKET_BUTTON: {
            ...
            Copy( fheroes::AGG::GetICN( ICN::ADVEBTNS, releasedIndex ), _icnVsSprite[id][0] );
            Copy( fheroes::AGG::GetICN( ICN::ADVEBTNS, releasedIndex + 1 ), _icnVsSprite[id][1] );
        ...
        case ICN::CSPANBTE: {
            if ( !_icnVsSprite[id].empty() ) {
                ...
                fheroes::Sprite released( out.width() + 1, out.height() );
                released.reset();
        ...
        case ICN::TRADPOSE: {
            if ( _icnVsSprite[id].size() >= 19 ) {
                ...
                for ( const uint32_t i : { 16, 18 } ) {
                    ...
                    _icnVsSprite[id][i] = fheroes::AGG::GetICN( ICN::SYSTEME, 12 );
        ...
        case ICN::NGEXTRA: {
            std::vector<fheroes::Sprite> & images = _icnVsSprite[id];
            ...
            for ( size_t i = 0; i < 6; ++i ) {
                ...
                fheroes::Sprite & humonOrAiImage = images[i + 82];
        ...
        case ICN::DIFFICULTY_ICON_IMPOSSIBLE: {
            ...
            const fheroes::Sprite & originalBackground = fheroes::AGG::GetICN( originalIcnId, 0 );
            ...
            if ( !originalBackground.empty() ) {
                ...
                fheroes::ApplyPalette( _icnVsSprite[id][0], _icnVsSprite[id][1], goodToEvilPalette );
            }
        ...
        case ICN::METALLIC_BORDERED_TEXTBOX_GOOD: {
            ...
            if ( !originalBackground.empty() ) {
                ...
                const fheroes::Image & redPart = fheroes::Flip( Crop( originalBackground, 80, 45, 81, 19 ), true, false );
        ...
        case ICN::METALLIC_BORDERED_TEXTBOX_EVIL: {
            const fheroes::Sprite & originalEvilBackground = fheroes::AGG::GetICN( ICN::CAMPBKGE, 0 );
            ...
            if ( !originalEvilBackground.empty() ) {
                ...
                const fheroes::Sprite goodBox = fheroes::AGG::GetICN( ICN::METALLIC_BORDERED_TEXTBOX_GOOD, 0 );
        ...
        case ICN::MONO_CURSOR_ADVMBW: {
            ...
            for ( size_t i = 0; i < _icnVsSprite[id].size(); ++i ) {
                ...
                _icnVsSprite[id][i] = fheroes::decodeBMPFile( AGG::getDataFromAggFile( std::string( "ADVMBW" ) + digit + ".BMP", false ) );
            }
            break;
        }
        case ICN::MONO_CURSOR_SPELBW: {
            ...
            for ( size_t i = 0; i < _icnVsSprite[id].size(); ++i ) {
                ...
                _icnVsSprite[id][i] = fheroes::decodeBMPFile( AGG::getDataFromAggFile( std::string( "SPELBW" ) + digit + ".BMP", false ) );
            }
            break;
        }
        case ICN::MONO_CURSOR_CMSSBW: {
            ...
            for ( size_t i = 0; i < _icnVsSprite[id].size(); ++i ) {
                ...
                _icnVsSprite[id][i] = fheroes::decodeBMPFile( AGG::getDataFromAggFile( std::string( "CMSEBW" ) + digit + ".BMP", false ) );
            }
            break;
        }
        case ICN::ADVBTNS:
        case ICN::ADVEBTNS:
            if ( _icnVsSprite[id].size() == 16 && _icnVsSprite[id][2].width() == 36 && _icnVsSprite[id][2].height() == 36 && _icnVsSprite[id][3].width() == 36
                ...
                const fheroes::Sprite & originalActionCursor = fheroes::AGG::GetICN( ICN::ADVMCO, 9 );
                ...
                fheroes::Image buttonImage( actionCursorWidth, actionCursorHeight );
                buttonImage.reset();
                ...
                const fheroes::Sprite & emptyButtonPressed = fheroes::AGG::GetICN( emptyButtonId, 1 );
                ...
                const fheroes::Sprite & emptyButtonReleased = fheroes::AGG::GetICN( emptyButtonId, 0 );
                Copy( emptyButtonReleased, _icnVsSprite[id][16] );
                Blit( buttonImage, _icnVsSprite[id][16], 5, 3 );

                // Generate inactive horse icon.
                fheroes::Sprite & releasedInactiveHorse = _icnVsSprite[id][18];
                ...
                fheroes::SetPixel( releasedInactiveHorse, 31, 13, secondShadingColor );
                ...
                fheroes::SetPixel( releasedInactiveHorse, 21, 12, mainReleasedColor );

                // Copy one leg 4 times.
                Copy( _icnVsSprite[id][2], 13, 22, releasedInactiveHorse, 11, 22, 4, 6 );
                fheroes::SetPixel( releasedInactiveHorse, 11, 24, 10 );
                fheroes::SetPixel( releasedInactiveHorse, 14, 24, secondShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 14, 27, secondShadingColor );
                ...
                fheroes::SetPixel( releasedInactiveHorse, 24, 27, fourthShadingColor );

                fheroes::SetPixel( releasedInactiveHorse, 17, 18, mainReleasedColor );
                fheroes::SetPixel( releasedInactiveHorse, 21, 18, mainReleasedColor );
                fheroes::DrawLine( releasedInactiveHorse, { 22, 20 }, { 23, 20 }, mainReleasedColor );

                // Tail.
                fheroes::DrawLine( releasedInactiveHorse, { 11, 14 }, { 10, 15 }, mainReleasedColor );
                fheroes::SetPixel( releasedInactiveHorse, 11, 15, mainReleasedColor );
                Fill( releasedInactiveHorse, 9, 16, 2, 4, mainReleasedColor );
                fheroes::DrawLine( releasedInactiveHorse, { 7, 19 }, { 8, 19 }, mainReleasedColor );
                fheroes::DrawLine( releasedInactiveHorse, { 8, 20 }, { 9, 20 }, mainReleasedColor );

                // Shading.
                fheroes::DrawLine( releasedInactiveHorse, { 27, 13 }, { 27, 20 }, mainShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 21, 11 }, { 20, 12 }, mainShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 19, 12 }, { 17, 12 }, secondShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 20, 11, secondShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 16, 12 }, { 12, 12 }, mainShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 11, 12, secondShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 11, 13 }, { 9, 15 }, mainShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 10, 13, fifthShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 9, 14, 10 );
                fheroes::DrawLine( releasedInactiveHorse, { 8, 15 }, { 8, 18 }, 10 );
                fheroes::SetPixel( releasedInactiveHorse, 7, 18, secondShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 6, 19 }, { 8, 21 }, 10 );
                fheroes::DrawLine( releasedInactiveHorse, { 6, 20 }, { 7, 21 }, thirdShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 9, 21, 10 );
                fheroes::SetPixel( releasedInactiveHorse, 10, 20, thirdShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 11, 16, mainShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 11, 17, secondShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 11, 21, 10 );
                fheroes::DrawLine( releasedInactiveHorse, { 14, 20 }, { 14, 21 }, thirdShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 17, 19 }, { 17, 20 }, mainShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 18, 18 }, { 20, 18 }, mainShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 21, 19, mainShadingColor );
                fheroes::SetPixel( releasedInactiveHorse, 18, 19, thirdShadingColor );
                fheroes::DrawLine( releasedInactiveHorse, { 19, 19 }, { 20, 19 }, 10 );
                fheroes::DrawLine( releasedInactiveHorse, { 21, 20 }, { 21, 21 }, 10 );
                fheroes::SetPixel( releasedInactiveHorse, 24, 20, thirdShadingColor );

                // Clean up button border.
                fheroes::DrawLine( releasedInactiveHorse, { 23, 4 }, { 22, 5 }, 10 );
                const uint8_t backgroundReleasedColor = isGoodButton ? 41 : 16;
                fheroes::SetPixel( releasedInactiveHorse, 22, 4, backgroundReleasedColor );
                ...
                fheroes::ReplaceColorId( buttonImage, mainReleasedColor, mainPressedColor );
                const uint8_t pressedColorOffset = isGoodButton ? 4 : 6;
                fheroes::ReplaceColorId( buttonImage, backgroundReleasedColor, backgroundPressedColor );
                fheroes::ReplaceColorId( buttonImage, mainShadingColor, mainShadingColor + pressedColorOffset );
                fheroes::ReplaceColorId( buttonImage, secondShadingColor, secondShadingColor + pressedColorOffset );
                fheroes::ReplaceColorId( buttonImage, thirdShadingColor, thirdShadingColor + pressedColorOffset );
                fheroes::ReplaceColorId( buttonImage, fourthShadingColor, fourthShadingColor + pressedColorOffset );
                fheroes::ReplaceColorId( buttonImage, fifthShadingColor, fifthShadingColor + pressedColorOffset );
                fheroes::ReplaceColorId( buttonImage, 10, isGoodButton ? 40 : 14 );
                ...
                // Fix left border.
                if ( isGoodButton ) {
                    fheroes::DrawLine( _icnVsSprite[id][19], { 5, 5 }, { 5, 19 }, 44 );
                    fheroes::DrawLine( _icnVsSprite[id][19], { 5, 21 }, { 5, 28 }, 44 );
                }
            }
            break;
        case ICN::ARTFX:
            if ( _icnVsSprite[id].size() > 82 ) {
                ...
                const fheroes::Sprite & originalImage = fheroes::AGG::GetICN( ICN::ARTIFACT, 83 );
        ...
        case ICN::ARTIFACT:
            if ( _icnVsSprite[id].size() > 99 ) {
                ...
                for ( const int32_t index : { 88, 99 } ) {
                    fheroes::Sprite & originalImage = _icnVsSprite[id][index];
                    fheroes::Sprite temp( originalImage.width(), originalImage.height() );
                ...
                fheroes::Sprite & targetImage = _icnVsSprite[id][83];
        ...
        case ICN::OBJNARTI:
            if ( _icnVsSprite[id].size() == 206 ) {
                ...
                fheroes::Sprite shadow = _icnVsSprite[id][162];
                ...
                fheroes::Sprite body( 21, 32 );
                Copy( fheroes::AGG::GetICN( ICN::ARTFX, 81 ), 6, 0, body, 0, 0, 21, 32 );
        ...
        case ICN::TWNSDW_5:
            if ( !_icnVsSprite[id].empty() && _icnVsSprite[id][0].width() == 140 && _icnVsSprite[id][0].height() == 165 ) {
                fheroes::Sprite & image = _icnVsSprite[id][0];
        ...
        case ICN::SCENIBKG:
            if ( !_icnVsSprite[id].empty() && _icnVsSprite[id][0].width() == 436 && _icnVsSprite[id][0].height() == 476 ) {
                const fheroes::Sprite & helper = fheroes::AGG::GetICN( ICN::CSPANBKE, 1 );
                if ( !helper.empty() ) {
                    fheroes::Sprite & original = _icnVsSprite[id][0];
                    fheroes::Sprite temp( original.width(), original.height() + 12 );
                    temp.reset();
        ...
        case ICN::CSTLCAPS:
            if ( !_icnVsSprite[id].empty() && _icnVsSprite[id][0].width() == 84 && _icnVsSprite[id][0].height() == 81 ) {
                const fheroes::Sprite & castle = fheroes::AGG::GetICN( ICN::TWNSCSTL, 0 );
                if ( !castle.empty() ) {
        ...
        case ICN::LGNDXTRE:
            ...
            break;
        case ICN::OVERBACK: {
            fheroes::Sprite & background = _icnVsSprite[id][0];
            ...
            fheroes::Copy( fheroes::AGG::GetICN( ICN::OVERBACK, 0 ), 540, 444, background, 540, 402, 99, 5 );
            break;
        }
        case ICN::ESPANBKG_EVIL: {
            _icnVsSprite[id].resize( 2 );

            const fheroes::Rect roi{ 28, 28, 265, 206 };

            fheroes::Sprite & output = _icnVsSprite[id][0];
            _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::CSPANBKE, 0 );
            Copy( fheroes::AGG::GetICN( ICN::ESPANBKG, 0 ), roi.x, roi.y, output, roi.x, roi.y, roi.width, roi.height );

            convertToEvilInterface( output, roi );

            _icnVsSprite[id][1] = fheroes::AGG::GetICN( ICN::ESPANBKG, 1 );

            break;
        }
        case ICN::STONEBAK_EVIL: {
            fheroes::AGG::GetICN( ICN::STONEBAK, 0 );
            _icnVsSprite[id] = _icnVsSprite[ICN::STONEBAK];
            if ( !_icnVsSprite[id].empty() ) {
                const fheroes::Rect roi( 0, 0, _icnVsSprite[id][0].width(), _icnVsSprite[id][0].height() );
        ...
        case ICN::STONEBAK_SMALL_POL: {
            _icnVsSprite[id].resize( 1 );
            const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::X_CMPBKG, 0 );
            if ( !original.empty() ) {
        ...
        case ICN::REDBAK_SMALL_VERTICAL: {
            _icnVsSprite[id].resize( 1 );
            const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::HEROBKG, 0 );
            if ( !original.empty() ) {
        ...
        case ICN::BLACKBAK: {
            _icnVsSprite[id].resize( 1 );
            fheroes::Image & background = _icnVsSprite[id][0];
            // This is enough to cover the largest buttons.
            background.resize( 200, 200 );
            fheroes::Fill( background, 0, 0, background.width(), background.height(), 9 );
            break;
        }
        case ICN::BROWNBAK: {
            _icnVsSprite[id].resize( 1 );
            ...
            // This is enough to cover the largest buttons.
            background.resize( 200, 200 );
            fheroes::Fill( background, 0, 0, background.width(), background.height(), 57 );
            break;
        }
        ...
        case ICN::UNIFORMBAK_EVIL: {
            ...
            const fheroes::Sprite & original = fheroes::AGG::GetICN( isEvilInterface ? ICN::BUYBUILE : ICN::BUYBUILD, 1 );
            if ( !original.empty() ) {
        ...
        case ICN::WELLBKG_EVIL: {
            fheroes::AGG::GetICN( ICN::WELLBKG, 0 );
            _icnVsSprite[id] = _icnVsSprite[ICN::WELLBKG];
            if ( !_icnVsSprite[id].empty() ) {
                const fheroes::Rect roi( 0, 0, _icnVsSprite[id][0].width(), _icnVsSprite[id][0].height() - 19 );
                convertToEvilInterface( _icnVsSprite[id][0], roi );
            }
        ...
        case ICN::CASLWIND_EVIL: {
            fheroes::AGG::GetICN( ICN::CASLWIND, 0 );
        ...
        case ICN::CASLXTRA_EVIL: {
            fheroes::AGG::GetICN( ICN::CASLXTRA, 0 );
        ...
        case ICN::STRIP_BACKGROUND_EVIL: {
            _icnVsSprite[id].resize( 1 );
            _icnVsSprite[id][0] = fheroes::AGG::GetICN( ICN::STRIP, 11 );

            const fheroes::Rect roi( 0, 0, _icnVsSprite[id][0].width(), _icnVsSprite[id][0].height() - 7 );
            convertToEvilInterface( _icnVsSprite[id][0], roi );
        ...
        case ICN::P_BFLG32:
            if ( _icnVsSprite[id].size() > 31 && _icnVsSprite[id][31].height() == 248 ) {
                fheroes::Sprite & original = _icnVsSprite[id][31];
                fheroes::Sprite temp = Crop( original, 0, 0, original.width(), 4 );
        ...
        case ICN::FLAG32: {
            ...
            if ( flagImages.size() == 49 ) {
                ...
                for ( size_t i = 42; i < 42 + 7; ++i ) {
                    const fheroes::Sprite & original = flagImages[i];
        ...
        case ICN::MINI_MONSTER_SHADOW: {
            ...
            for ( fheroes::Sprite & image : _icnVsSprite[ICN::MINI_MONSTER_IMAGE] ) {
                uint8_t * transform = image.transform();
                const uint8_t * transformEnd = transform + image.width() * image.height();
                for ( ; transform != transformEnd; ++transform ) {
                    if ( *transform > 1 ) {
            ...
            for ( fheroes::Sprite & image : _icnVsSprite[ICN::MINI_MONSTER_SHADOW] ) {
                uint8_t * transform = image.transform();
                const uint8_t * transformEnd = transform + image.width() * image.height();
                for ( ; transform != transformEnd; ++transform ) {
                    if ( *transform == 0 ) {
        ...
        case ICN::HISCORE: {
            if ( _icnVsSprite[id].size() == 9 ) {
                ...
                const fheroes::Sprite temp = Crop( _icnVsSprite[id][7], 215, 0, 300, imageHeight );
        ...
        case ICN::SPELLINL: {
            if ( _icnVsSprite[id].size() > 11 ) {
                ...
                fheroes::h2d::readImage( "petrification_spell_icon_mini.image", _icnVsSprite[id][11] );
            }
        ...
        case ICN::EMPTY_EVIL_BUTTON: {
            ...
            const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( originalId, 12 );
        ...
        case ICN::EMPTY_POL_BUTTON: {
            ...
            const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( originalID, 4 );
            const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( originalID, 5 );
            if ( originalReleased.width() != 94 && originalPressed.width() != 94 && originalReleased.height() < 5 && originalPressed.height() < 5 ) {
                break;
            }
            fheroes::Sprite & releasedWithDarkBorder = _icnVsSprite[id][0];
        ...
        case ICN::EMPTY_GUILDWELL_BUTTON: {
            ...
            for ( int32_t i = 0; i < static_cast<int32_t>( _icnVsSprite[id].size() ); ++i ) {
                const fheroes::Sprite & original = fheroes::AGG::GetICN( originalID, 0 + i );
                ...
                fheroes::DrawLine( out, { 3 - i, 3 + i }, { 4 - i, 2 + i }, borderColor );
                fheroes::SetPixel( out, 5 - 3 * i, 2 + i, borderColor );
                fheroes::SetPixel( out, 57 - i, 2 + i, borderColor );
                fheroes::DrawLine( out, { 26, 1 + i }, { 42, 1 + i }, borderColor );
                const uint8_t secondBorderColor = i == 0 ? 42 : 46;
                fheroes::DrawLine( out, { 26, 0 + i }, { 42, 0 + i }, secondBorderColor );
            }
        ...
        case ICN::EMPTY_VERTICAL_GOOD_BUTTON: {
            ...
            _icnVsSprite[id].resize( 2 );
            const fheroes::Sprite & originalReleased = fheroes::AGG::GetICN( originalId, 2 );
            const fheroes::Sprite & originalPressed = fheroes::AGG::GetICN( originalId, 3 );
            ...
            if ( originalReleased.width() > 2 && originalReleased.height() > 2 && originalPressed.width() > 2 && originalPressed.height() > 2 ) {
                ...
                fheroes::Image exitCommonMask = fheroes::ExtractCommonPattern( { &released, &pressed } );
                ...
                const fheroes::Sprite & originalDismiss = fheroes::AGG::GetICN( ICN::HSBTNS, 0 );
        ...
        case ICN::EMPTY_MAP_SELECT_BUTTON: {
            ...
            for ( int32_t i = 0; i < static_cast<int32_t>( _icnVsSprite[id].size() ); ++i ) {
                const fheroes::Sprite & original = fheroes::AGG::GetICN( originalId, 64 + i );
        ...
        case ICN::BRCREST: {
            ...
            if ( _icnVsSprite[id].size() == 7 ) {
                fheroes::Sprite neutralShield( fheroes::AGG::GetICN( ICN::SPELLS, 15 ) );
                ...
                fheroes::Sprite neutralColorSprite( _icnVsSprite[id][0].width(), _icnVsSprite[id][0].height() );
        ...
        case ICN::GAME_OPTION_ICON: {
            ...
            fheroes::h2d::readImage( "hotkeys_icon.image", _icnVsSprite[id][0] );
            fheroes::h2d::readImage( "graphics_icon.image", _icnVsSprite[id][1] );
        ...
        case ICN::RESOLUTION_ICON: {
            ...
            fheroes::h2d::readImage( "resolution_icon.image", _icnVsSprite[id][0] );
        ...
        case ICN::COVR0012: {
            ...
            if ( !_icnVsSprite[id].empty() ) {
                fheroes::Sprite & sprite = _icnVsSprite[id][0];
        ...
        case ICN::OBJNDSRT: {
            if ( _icnVsSprite[id].size() == 131 ) {
                ...
                fheroes::h2d::readImage( "missing_sphinx_part.image", _icnVsSprite[id][131] );
            }
        ...
        case ICN::OBJNGRAS: {
            if ( _icnVsSprite[id].size() == 151 ) {
                ...
                if ( _icnVsSprite[ICN::OBJNSNOW].size() > 210 ) {
                    ...
                    fheroes::h2d::readImage( "adventure-map-grass-cave-diff-01.image", temp );
                    ...
                    fheroes::h2d::readImage( "adventure-map-grass-cave-diff-02.image", temp );
                    ...
                    fheroes::h2d::readImage( "lean-to-diff-part1.image", temp );
                    ...
                    fheroes::h2d::readImage( "lean-to-diff-part2.image", temp );
        ...
        case ICN::OBJNMUL2: {
            ...
            if ( images.size() == 218 ) {
                ...
                for ( size_t i = 0; i < 14; ++i ) {
                    ...
                    fheroes::Transpose( images[i], images[218 + i] );
                    images[218 + i].setPosition( images[i].y(), images[i].x() );
                }

                // 1 Stone Liths.
                fheroes::Sprite temp;
                fheroes::h2d::readImage( "circular_stone_liths_center.image", temp );
                ...
                fheroes::h2d::readImage( "circular_stone_liths_left.image", images[233] );
                fheroes::h2d::readImage( "circular_stone_liths_top.image", images[234] );

                // Generic Observation Tower.
                images[235] = images[201];
                fheroes::h2d::readImage( "observation_tower_generic_bottom_part.image", temp );
                Blit( temp, 0, 0, images[235], 0, temp.y() - images[235].y(), temp.width(), temp.height() );

                // Desert Observation Tower.
                images[236] = images[201];
                fheroes::h2d::readImage( "observation_tower_desert_bottom_part.image", temp );
                Blit( temp, 0, 0, images[236], 0, temp.y() - images[236].y(), temp.width(), temp.height() );

                fheroes::h2d::readImage( "observation_tower_desert_right_part.image", images[237] );

                // Snow Observation Tower.
                images[238] = images[201];
                fheroes::h2d::readImage( "observation_tower_snow_bottom_part.image", temp );
                Blit( temp, 0, 0, images[238], 0, temp.y() - images[238].y(), temp.width(), temp.height() );

                fheroes::h2d::readImage( "observation_tower_snow_right_part.image", images[239] );

                images[240] = images[198];
                fheroes::h2d::readImage( "observation_tower_snow_top_part.image", temp );
                Blit( temp, 0, 0, images[240], 0, 0, temp.width(), temp.height() );

                // Black Cat. Main object image.
                fheroes::h2d::readImage( "black_cat.image", images[241] );

                // Black Cat. Tail and eyes animation.
                for ( size_t i = 0; i < 6; ++i ) {
                    fheroes::h2d::readImage( "black_cat_animation_" + std::to_string( i ) + ".image", images[242 + i] );
                }
                ...
                for ( size_t i = 0; i < 6; ++i ) {
                    fheroes::h2d::readImage( "barrel_animation_" + std::to_string( i ) + ".image", images[249 + i] );
                }
            }
        ...
        case ICN::SCENIBKG_EVIL: {
            ...
            fheroes::Copy( evilTextBox, 0, 0, outputImage, 46, 23, textWidth / 2, evilTextBox.height() );
            fheroes::Copy( evilTextBox, evilTextBox.width() - ( textWidth - textWidth / 2 ), 0, outputImage, 46 + textWidth / 2, 23, ( textWidth - textWidth / 2 ),
                            evilTextBox.height() );
        ...
        case ICN::TWNZDOCK: {
            if ( _icnVsSprite[id].size() == 6 && _icnVsSprite[id][4].width() == 285 ) {
                ...
                fheroes::Sprite & original = _icnVsSprite[id][4];

                fheroes::Sprite fixed( newWidth, original.height(), original.x(), original.y() );
                fheroes::Copy( original, 0, 0, fixed, 0, 0, newWidth, original.height() );
        ...
        case ICN::WIZARD_CASTLE_BAY: {
            ...
            if ( _icnVsSprite[docksIcnId].size() == 6 ) {
                ...
                fheroes::Sprite temp;
                fheroes::h2d::readImage( "wizard_bay_diff_to_twnzdock.image", temp );
                fheroes::Blit( temp, 0, 0, baySprites[0], temp.x() - baySprites[0].x(), temp.y() - baySprites[0].y(), baySprites[0].width(), baySprites[0].height() );

                for ( size_t i = 1; i < 6; ++i ) {
                    fheroes::h2d::readImage( "wizard_bay_diff_to_twnzdock_animation_" + std::to_string( i - 1 ) + ".image", temp );
                    fheroes::Blit( temp, 0, 0, baySprites[i], temp.x() - baySprites[i].x(), temp.y() - baySprites[i].y(), baySprites[i].width(),
                                    baySprites[i].height() );
                }
            }
            break;
        ...
        case ICN::BOOK: {
            if ( _icnVsSprite[id].size() == 7 ) {
                ...
                fheroes::Sprite & corner = _icnVsSprite[id].emplace_back();
                fheroes::h2d::readImage( "book_corner.image", corner );
            }
            break;
        }
        ...
        case ICN::BUTTON_VIRTUAL_KEYBOARD_EVIL: {
            ...
            if ( _icnVsSprite[emptyButtonIcn].size() == 2 ) {
                ...
                fheroes::h2d::readImage( releasedImageName, temp );

                // Generate empty buttons.
                fheroes::Sprite & released = _icnVsSprite[id].emplace_back();
                fheroes::Sprite & pressed = _icnVsSprite[id].emplace_back();

                fheroes::Point pressedOffset;
                fheroes::Point releasedOffset;

                constexpr int32_t extraWidth = 19;

                fheroes::getCustomNormalButton( released, pressed, isEvil, { temp.width() + extraWidth, temp.height() }, releasedOffset, pressedOffset, ICN::UNKNOWN );

                // We subtract 1 from `releasedOffset.y` argument because the  loaded released image from `h2d` file is shifted 1 pixel down.
                fheroes::Blit( temp, 0, 0, released, extraWidth / 2 + releasedOffset.x, releasedOffset.y - 1, temp.width(), temp.height() );

                const char * const pressedImageName = ( isEvil ? "keyboard_button_pressed_evil.image" : "keyboard_button_pressed_good.image" );
                fheroes::h2d::readImage( pressedImageName, temp );
                fheroes::Blit( temp, 0, 0, pressed, extraWidth / 2 + pressedOffset.x, pressedOffset.y, temp.width(), temp.height() );

                // Make a button pressed sprite for the "uniform" dialog background.
                fheroes::Sprite & pressedUniform = _icnVsSprite[id].emplace_back( pressed );
                const int uniformBackgroundIcn = isEvil ? ICN::UNIFORMBAK_EVIL : ICN::UNIFORMBAK_GOOD;
                loadICN( uniformBackgroundIcn );
                fheroes::makeTransparentBackground( released, pressedUniform, uniformBackgroundIcn );
                ...
                fheroes::makeTransparentBackground( released, pressed, standardDialogBackgroundIcn );
            }
            break;
        }
        case ICN::SWORDSM2:
        case ICN::SWORDSMN: {
            if ( _icnVsSprite[id].size() == 45 && _icnVsSprite[id][3].width() == 38 && _icnVsSprite[id][3].height() > 69 ) {
                ...
                fheroes::Sprite & newFrame = _icnVsSprite[id].emplace_back( _icnVsSprite[id][3] );

                fheroes::Sprite temp;
                fheroes::h2d::readImage( "swordsman_walking_frame_extra_part_mask.image", temp );
                fheroes::copyTransformLayer( temp, 0, 0, newFrame, temp.x(), newFrame.height() - temp.height(), temp.width(), temp.height() );

                fheroes::h2d::readImage( "swordsman_walking_frame_extra_part.image", temp );
                fheroes::Blit( temp, 0, 0, newFrame, temp.x(), newFrame.height() - temp.height(), temp.width(), temp.height() );
        ...
        case ICN::EVIWPUZL: {
            if ( _icnVsSprite[id].size() == 1 ) {
                ...
                const auto & streamsImage = fheroes::AGG::GetICN( ICN::EDITPANL, 3 );
                fheroes::Sprite croppedImage = fheroes::Crop( streamsImage, 0, streamsImage.height() - 48, streamsImage.width(), 48 );
                if ( id == ICN::EVIWPUZL ) {
                    fheroes::ApplyPalette( croppedImage, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );
                }

                fheroes::Copy( croppedImage, 0, 8, original, 0, original.height() - 40, original.width(), 40 );

                fheroes::CreateDitheringTransition( croppedImage, 0, 0, original, 0, original.height() - 48, original.width(), 8, false, false );
            }
            break;
        }
    ...
    size_t GetMaximumTILIndex( const int id )
    {
        ...
        if ( tilImages.empty() ) {
            ...
            std::vector<fheroes::Image> & originalTIL = tilImages[0];
            ...
            for ( size_t i = 0; i < count; ++i ) {
                for ( uint32_t shapeId = 1; shapeId < 4; ++shapeId ) {
                    fheroes::Image & image = tilImages[shapeId][i];
    ...
    const fheroes::Sprite & GetScaledICN( const int icnId, const uint32_t index )
    {
        const fheroes::Sprite & originalIcn = _icnVsSprite[icnId][index];
        const fheroes::Display & display = fheroes::Display::instance();

        if ( display.width() == fheroes::Display::DEFAULT_WIDTH && display.height() == fheroes::Display::DEFAULT_HEIGHT ) {
            return originalIcn;
        }
        ...
        fheroes::Sprite & resizedIcn = _icnVsScaledSprite[icnId][index];

        if ( originalIcn.singleLayer() && !resizedIcn.singleLayer() ) {
            resizedIcn._disableTransformLayer();
        }

        const double scaleFactorX = static_cast<double>( display.width() ) / fheroes::Display::DEFAULT_WIDTH;
        const double scaleFactorY = static_cast<double>( display.height() ) / fheroes::Display::DEFAULT_HEIGHT;
        ...
        const int32_t offsetX = static_cast<int32_t>( std::lround( display.width() - fheroes::Display::DEFAULT_WIDTH * scaleFactor ) ) / 2;
        const int32_t offsetY = static_cast<int32_t>( std::lround( display.height() - fheroes::Display::DEFAULT_HEIGHT * scaleFactor ) ) / 2;
