...
namespace
{
    ...
    int32_t saveHighscoreEntry( fheroes::HighscoreData && data, std::vector<fheroes::HighscoreData> & entries, const bool isCampaign )
    {
        ...
        std::sort( entries.begin(), entries.end(), [isCampaign]( const fheroes::HighscoreData & first, const fheroes::HighscoreData & second ) {
            if ( isCampaign ) {
...
namespace fheroes
{
    ...
    {
        languageAbbreviation = fheroes::getLanguageAbbreviation( fheroes::SupportedLanguage::English );
    ...
    bool HighScoreDataContainer::load( const std::string & fileName )
    {
        ...
        for ( fheroes::HighscoreData & data : _highScoresCampaign ) {
            if ( data.rating == 0 ) {
    ...
    void HighScoreDataContainer::populateStandardDefaultHighScores()
    {
        ...
        const std::string lang = fheroes::getLanguageAbbreviation( fheroes::getCurrentLanguage() );
