...
namespace
{
    ...
    void fadeDisplay( const uint8_t startAlpha, const uint8_t endAlpha, const fheroes::Rect & roi, const uint32_t fadeTimeMs, const uint32_t frameCount )
    {
        ...
        const fheroes::Rect fadeRoi( roi ^ fheroes::Rect( 0, 0, display.width(), display.height() ) );

        fheroes::Image temp{ fadeRoi.width, fadeRoi.height };
...
namespace fheroes
{
    ...
    SystemInfoRenderer::SystemInfoRenderer()
        ...
        , _text( fheroes::Display::instance() )
    {}

    void SystemInfoRenderer::preRender()
    {
        const int32_t offsetX = 26;
        const int32_t offsetY = fheroes::Display::instance().height() - 30;
        ...
        _text.update( std::make_unique<fheroes::Text>( std::move( info ), fheroes::FontType::normalWhite() ) );
    ...
    void colorFade( const std::vector<uint8_t> & palette, const fheroes::Rect & frameRoi, const uint32_t durationMs, const double fps )
    {
        assert( fps > 0 );
        ...
        const uint8_t * originalPalette = fheroes::getGamePalette();
        ...
        fheroes::ApplyPalette( display, frameRoi.x, frameRoi.y, display, frameRoi.x, frameRoi.y, frameRoi.width, frameRoi.height, assignedValue );
    ...
    void FadeDisplayWithPalette( const Image & top, const Point & pos, const uint8_t paletteId, const int32_t fadeTimeMs, const int32_t frameCount )
    {
        ...
        Image shadow = top;
        const fheroes::Rect roi{ pos.x, pos.y, shadow.width(), shadow.height() };
    ...
    std::optional<int32_t> processIntegerValueTyping( const int32_t min, const int32_t max, std::string & valueBuf )
    {
        ...
        if ( le.isKeyPressed( fheroes::Key::KEY_BACKSPACE ) || le.isKeyPressed( fheroes::Key::KEY_DELETE ) ) {
            valueBuf.clear();

            return zeroBufValue;
        }

        if ( le.isKeyPressed( fheroes::Key::KEY_MINUS ) || le.isKeyPressed( fheroes::Key::KEY_KP_MINUS ) ) {
            if ( min >= 0 ) {
                return {};
            }
        ...
        if ( const std::optional<char> newDigit = [&le]() -> std::optional<char> {
                 using KeyUnderlyingType = std::underlying_type_t<fheroes::Key>;

                 const fheroes::Key keyValue = le.getPressedKeyValue();

                 if ( keyValue >= fheroes::Key::KEY_0 && keyValue <= fheroes::Key::KEY_9 ) {
                     return fheroes::checkedCast<char>( static_cast<KeyUnderlyingType>( keyValue ) - static_cast<KeyUnderlyingType>( fheroes::Key::KEY_0 ) + '0' );
                 }

                 if ( keyValue >= fheroes::Key::KEY_KP_0 && keyValue <= fheroes::Key::KEY_KP_9 ) {
                     return fheroes::checkedCast<char>( static_cast<KeyUnderlyingType>( keyValue ) - static_cast<KeyUnderlyingType>( fheroes::Key::KEY_KP_0 ) + '0' );
                 }
    ...
    void renderHeroRacePortrait( const int race, const fheroes::Rect & portPos, fheroes::Image & output )
    {
        fheroes::Image racePortrait( portPos.width, portPos.height );

        auto preparePortrait = [&racePortrait, &portPos]( const int icnId, const int bkgIndex, const bool applyRandomPalette ) {
            fheroes::SubpixelResize( fheroes::AGG::GetICN( ICN::STRIP, bkgIndex ), racePortrait );
            const fheroes::Sprite & heroSprite = fheroes::AGG::GetICN( icnId, 1 );
            if ( applyRandomPalette ) {
                fheroes::Sprite tmp = heroSprite;
                fheroes::ApplyPalette( tmp, PAL::GetPalette( PAL::PaletteType::PURPLE ) );
                fheroes::Blit( tmp, 0, std::max( 0, tmp.height() - portPos.height ), racePortrait, ( portPos.width - tmp.width() ) / 2,
                                std::max( 0, portPos.height - tmp.height() ), tmp.width(), portPos.height );
            }
            else {
                fheroes::Blit( heroSprite, 0, std::max( 0, heroSprite.height() - portPos.height ), racePortrait, ( portPos.width - heroSprite.width() ) / 2,
        ...
        fheroes::Copy( racePortrait, 0, 0, output, portPos );
    }
