...
namespace Video
{
    ...
    bool ShowVideo( const std::vector<VideoInfo> & infos, const std::vector<Subtitle> & subtitles /* = {} */, const bool fadeColorsOnEnd /* = false */ )
    {
        ...
        for ( const auto & info : infos ) {
            ...
            const fheroes::Rect frameRoi{ info.offset.x, info.offset.y, video->width(), video->height() };
            const VideoState state{ info.control, frameRoi, delay, delay };

            if ( videoRoi == fheroes::Rect{} ) {
                videoRoi = frameRoi;
            }
            else {
                videoRoi = fheroes::getBoundaryRect( videoRoi, frameRoi );
            }
        ...
        const fheroes::Point videoOffset{ ( display.width() - videoRoi.width ) / 2, ( display.height() - videoRoi.height ) / 2 };
        ...
        if ( fadeColorsOnEnd ) {
            ...
            fheroes::colorFade( currPalette, videoRoi, 1000, 15.0 );
        }
        else {
            display.fill( 0 );
        }
    ...
    Subtitle::Subtitle( const fheroes::TextBase & subtitleText, const uint32_t startTimeMS, const uint32_t durationMS,
                        const fheroes::Point & position /* = { -1, -1 } */, const int32_t maxWidth /* = fheroes::Display::DEFAULT_WIDTH */ )
        : _position( position )
        , _startTimeMS( startTimeMS )
    {
        ...
        fheroes::ReplaceColorIdByTransformId( _subtitleImage, blackColor, 1 );
        // Add black contour to the text.
        fheroes::Blit( fheroes::CreateContour( _subtitleImage, blackColor ), _subtitleImage );
        ...
        if ( ( _position.x < 0 ) || ( _position.y < 0 ) ) {
            _position.x = ( fheroes::Display::DEFAULT_WIDTH - _subtitleImage.width() ) / 2;
            _position.y = fheroes::Display::DEFAULT_HEIGHT - _subtitleImage.height();
        }
        else {
