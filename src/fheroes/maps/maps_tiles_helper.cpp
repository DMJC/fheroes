...
namespace
{
    ...
    bool placeObjectOnTile( const Maps::Tile & tile, const Maps::ObjectInfo & info )
    {
        ...
        const fheroes::Point mainTilePos = tile.GetCenter();
        ...
        for ( const auto & partInfo : info.groundLevelParts ) {
            ...
            const fheroes::Point pos = mainTilePos + partInfo.tileOffset;
...
namespace Maps
{
    ...
    void updateObjectInfoTile( Tile & tile, const bool isFirstLoad )
    {
        ...
        case MP2::OBJ_ALCHEMIST_LAB: {
            ...
            const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::MERCURY ).mercury );
        ...
        case MP2::OBJ_SAWMILL: {
            ...
            const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::WOOD ).wood );
            assert( resourceCount.has_value() && resourceCount > 0U );
        ...
        case MP2::OBJ_MINE: {
            ...
            case 0: {
                const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::ORE ).ore );
                assert( resourceCount.has_value() && resourceCount > 0U );
            ...
            case 1: {
                const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::SULFUR ).sulfur );
                assert( resourceCount.has_value() && resourceCount > 0U );
            ...
            case 2: {
                const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::CRYSTAL ).crystal );
                assert( resourceCount.has_value() && resourceCount > 0U );
            ...
            case 3: {
                const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::GEMS ).gems );
                assert( resourceCount.has_value() && resourceCount > 0U );
            ...
            case 4: {
                const auto resourceCount = fheroes::checkedCast<uint32_t>( ProfitConditions::FromMine( Resource::GOLD ).gold );
                assert( resourceCount.has_value() && resourceCount > 0U );
    ...
    void restoreAbandonedMine( Tile & tile, const int resource )
    {
        ...
        switch ( resource ) {
        case Resource::ORE:
            resourceCount = fheroes::checkedCast<uint32_t>( info.ore );
            break;
        case Resource::SULFUR:
            resourceCount = fheroes::checkedCast<uint32_t>( info.sulfur );
            break;
        case Resource::CRYSTAL:
            resourceCount = fheroes::checkedCast<uint32_t>( info.crystal );
            break;
        case Resource::GEMS:
            resourceCount = fheroes::checkedCast<uint32_t>( info.gems );
            break;
        case Resource::GOLD:
            resourceCount = fheroes::checkedCast<uint32_t>( info.gold );
            break;
        default:
            assert( 0 );
            break;
    ...
    void updateFogDirectionsInArea( const fheroes::Point & minPos, const fheroes::Point & maxPos, const PlayerColorsSet colors )
    {
    ...
    std::set<uint32_t> getObjectUidsInArea( const int32_t startTileId, const int32_t endTileId )
    {
        ...
        const fheroes::Point startTileOffset = GetPoint( startTileId );
        const fheroes::Point endTileOffset = GetPoint( endTileId );
