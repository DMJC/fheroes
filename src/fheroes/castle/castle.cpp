...
bool Castle::isPosition( const fheroes::Point & pt ) const
{
    const fheroes::Point & mp = GetCenter();
...
{
    return fheroes::getBuildingName( race, static_cast<BuildingType>( buildingType ) );
}
...
bool Castle::RecruitMonster( const Troop & troop, bool showDialog )
{
    ...
    if ( !GetArmy().JoinTroop( troop ) ) {
        ...
        if ( hero == nullptr || !hero->GetArmy().JoinTroop( troop ) ) {
            if ( showDialog ) {
                fheroes::showStandardTextMessage( "", _( "There is no room in the garrison for this army." ), Dialog::OK );
            }
            return false;
        }
    }
...
BuildingStatus Castle::CheckBuyBuilding( const uint32_t build ) const
{
    ...
    const uint32_t requirement = fheroes::getBuildingRequirement( _race, static_cast<BuildingType>( build ) );
...
void Castle::DrawImageCastle( const fheroes::Point & pt ) const
{
    ...
    for ( uint32_t ii = 0; ii < 5; ++ii ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::OBJNTWBA, index + ii );
    ...
    for ( uint32_t ii = 0; ii < 5; ++ii ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::OBJNTWBA, index + 5 + ii );
        dst_pt.x = pt.x + ii * 32 + sprite.x();
    ...
    if ( !( BUILD_CASTLE & _constructedBuildings ) )
        index += 16;
    const fheroes::Sprite & sprite2 = fheroes::AGG::GetICN( ICN::OBJNTOWN, index );
    dst_pt.x = pt.x + 2 * 32 + sprite2.x();
    dst_pt.y = pt.y + sprite2.y();
    fheroes::Blit( sprite2, display, dst_pt.x, dst_pt.y );
    for ( uint32_t ii = 0; ii < 5; ++ii ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::OBJNTOWN, index + 1 + ii );
        dst_pt.x = pt.x + ii * 32 + sprite.x();
        dst_pt.y = pt.y + 32 + sprite.y();
        fheroes::Blit( sprite, display, dst_pt.x, dst_pt.y );
    }
    for ( uint32_t ii = 0; ii < 5; ++ii ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::OBJNTOWN, index + 6 + ii );
        dst_pt.x = pt.x + ii * 32 + sprite.x();
        dst_pt.y = pt.y + 2 * 32 + sprite.y();
        fheroes::Blit( sprite, display, dst_pt.x, dst_pt.y );
    }
    for ( uint32_t ii = 0; ii < 5; ++ii ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::OBJNTOWN, index + 11 + ii );
        dst_pt.x = pt.x + ii * 32 + sprite.x();
        dst_pt.y = pt.y + 3 * 32 + sprite.y();
        fheroes::Blit( sprite, display, dst_pt.x, dst_pt.y );
    }
...
{
    const fheroes::Point possibleSeaTile{ center.x, center.y + 2 };
...
uint32_t Castle::GetUpgradeBuilding( const uint32_t buildingId ) const
{
    if ( _race == Race::WRLK && buildingId == DWELLING_MONSTER6 && isBuild( DWELLING_UPGRADE6 ) ) {
        ...
        return fheroes::getUpgradeForBuilding( _race, DWELLING_UPGRADE6 );
    }

    return fheroes::getUpgradeForBuilding( _race, static_cast<BuildingType>( buildingId ) );
}
...
int Castle::GetPowerModificator( std::string * strs ) const
{
    ...
    if ( Race::NECR == _race && isBuild( BUILD_SPEC ) ) {
        ...
        if ( strs ) {
            ...
            fheroes::appendModifierToString( *strs, mod );
        }
    }
...
void AllCastles::AddCastle( std::unique_ptr<Castle> && castle )
{
    ...
    const fheroes::Point & center = castle->GetCenter();
    ...
    */
    ...
    for ( int32_t y = -2; y <= 1; ++y ) {
        for ( int32_t x = -2; x <= 2; ++x ) {
            ...
            if ( const auto [dummy, inserted] = _castleTiles.try_emplace( center + fheroes::Point( x, y ), id ); !inserted ) {
                DEBUG_LOG( DBG_GAME, DBG_INFO, "Tile [" << center.x + x << ", " << center.y + y << "] is occupied by another castle" )
            }
        }
    }

    if ( const auto [dummy, inserted] = _castleTiles.try_emplace( center + fheroes::Point( 0, -3 ), id ); !inserted ) {
        DEBUG_LOG( DBG_GAME, DBG_INFO, "Tile [" << center.x << ", " << center.y - 3 << "] is occupied by another castle" )
    }
}

Castle * AllCastles::Get( const fheroes::Point & position ) const
{
...
void AllCastles::removeCastle( const fheroes::Point & position )
{
    auto iter = _castleTiles.find( position );
    if ( iter == _castleTiles.end() ) {
        return;
...
{
    std::string res = fheroes::getBuildingDescription( GetRace(), static_cast<BuildingType>( buildingType ) );
