...
{
    fheroes::drawCastleIcon( castle, fheroes::Display::instance(), { posX, posY } );
}

void Interface::redrawHeroesIcon( const Heroes & hero, const int32_t posX, const int32_t posY )
{
    hero.PortraitRedraw( posX, posY, PORT_SMALL, fheroes::Display::instance() );
}

void Interface::IconsBar::redrawBackground( fheroes::Image & output, const fheroes::Point & offset, const int32_t validItemCount ) const
{
    const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();

    const fheroes::Sprite & icnadv = fheroes::AGG::GetICN( isEvilInterface ? ICN::ADVBORDE : ICN::ADVBORD, 0 );
    fheroes::Rect srcrt( icnadv.width() - fheroes::radarWidthPx - fheroes::borderWidthPx, fheroes::radarWidthPx + 2 * fheroes::borderWidthPx,
                          fheroes::radarWidthPx / 2, 32 );

    fheroes::Blit( icnadv, srcrt.x, srcrt.y, output, offset.x, offset.y, srcrt.width, srcrt.height );
    ...
    if ( _iconsCount > 2 ) {
        for ( int32_t i = 0; i < _iconsCount - 2; ++i ) {
            fheroes::Blit( icnadv, srcrt.x, srcrt.y, output, offset.x, internalOffsetY, srcrt.width, srcrt.height );
    ...
    for ( int32_t i = validItemCount; i < _iconsCount; ++i ) {
        const fheroes::Sprite & background = fheroes::AGG::GetICN( isEvilInterface ? ICN::LOCATORE : ICN::LOCATORS, 1 + i % 8 );
        fheroes::Copy( background, 0, 0, output, offset.x + 5, offset.y + 5 + i * ( IconsBar::getItemHeight() + 10 ), background.width(), background.height() );
    }
}
...
void Interface::CastleIcons::RedrawItem( const CASTLE & item, int32_t ox, int32_t oy, bool current )
{
    if ( item && _show ) {
        ...
        if ( current ) {
            fheroes::Blit( _marker, fheroes::Display::instance(), ox, oy );
        }
    }
}

void Interface::CastleIcons::RedrawBackground( const fheroes::Point & pos )
{
    redrawBackground( fheroes::Display::instance(), pos, _show ? _size() : 0 );
}
...
void Interface::CastleIcons::setPos( const int32_t px, const int32_t py )
{
    ...
    _topLeftCorner = fheroes::Point( px, py );
    ...
    const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( icnscroll, 4 );
    const fheroes::Image scrollbarSlider
        = fheroes::generateScrollbarSlider( originalSlider, false, iconsCursorHeight * _iconsCount - 38, _iconsCount, static_cast<int32_t>( castles.size() ),
...
void Interface::HeroesIcons::RedrawBackground( const fheroes::Point & pos )
{
    redrawBackground( fheroes::Display::instance(), pos, _show ? _size() : 0 );
}
...
void Interface::HeroesIcons::setPos( const int32_t px, const int32_t py )
{
    ...
    const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( icnscroll, 4 );
    const fheroes::Image scrollbarSlider
        = fheroes::generateScrollbarSlider( originalSlider, false, iconsCursorHeight * _iconsCount - 38, _iconsCount, static_cast<int32_t>( heroes.size() ),
                                             { 0, 0, originalSlider.width(), 8 }, { 0, 7, originalSlider.width(), 8 } );
    ...
    , _interface( interface )
{
    ...
    fheroes::DrawBorder( _sfMarker, fheroes::GetColorId( 0xA0, 0xE0, 0xE0 ) );
}
...
void Interface::IconsPanel::SetPos( int32_t x, int32_t y )
{
    ...
    if ( Settings::Get().isHideInterfaceEnabled() ) {
        ...
        const int32_t count_h = ( fheroes::Display::instance().height() - fheroes::Display::DEFAULT_HEIGHT ) / fheroes::tileWidthPx;
...
void Interface::IconsPanel::resetIcons( const HeroesCastlesIcons type )
{
    ...
    if ( !kingdom.isControlAI() ) {
        ...
        if ( type & ICON_HEROES ) {
            const fheroes::Image scrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, false, iconsCursorHeight * _heroesIcons.getIconsCount() - 38,
        ...
        if ( type & ICON_CASTLES ) {
            const fheroes::Image scrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, false, iconsCursorHeight * _castleIcons.getIconsCount() - 38,
