...
namespace Interface
{
    ...
    public:
        ...
        static bool isScrollLeft( const fheroes::Point & cursorPos )
        {
            return cursorPos.x < fheroes::borderWidthPx;
        }

        static bool isScrollRight( const fheroes::Point & cursorPos )
        {
            const fheroes::Display & display = fheroes::Display::instance();

            return cursorPos.x >= display.width() - fheroes::borderWidthPx;
        }

        static bool isScrollTop( const fheroes::Point & cursorPos )
        {
            return cursorPos.y < fheroes::borderWidthPx;
        }

        static bool isScrollBottom( const fheroes::Point & cursorPos )
        {
            const fheroes::Display & display = fheroes::Display::instance();

            return cursorPos.y >= display.height() - fheroes::borderWidthPx;
        }
        ...
        static fheroes::GameMode EventExit();
