...
namespace
{
    std::vector<fheroes::TimeDelay> delays( Game::LAST_DELAY, fheroes::TimeDelay( 0 ) );
    ...
    void SetupHeroMovement( const int speed, fheroes::TimeDelay & delay, int & multiplier )
    {
        switch ( speed ) {
        case 1:
            delay.setDelay( 18 );
...
void Game::AnimateDelaysInitialize()
{
    ...
    for ( fheroes::TimeDelay & delay : delays ) {
        delay.reset();
    }
}
