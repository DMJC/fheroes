...
void Dialog::GameInfo()
{
    ...
    const fheroes::Sprite & window = fheroes::AGG::GetICN( isEvilInterface ? ICN::SCENIBKG_EVIL : ICN::SCENIBKG, 0 );

    const fheroes::Point dialogOffset( ( display.width() - window.width() - DIALOG_SHADOW_OFFSET_X ) / 2, ( display.height() - window.height() ) / 2 );
    const fheroes::Point shadowOffset( dialogOffset.x + DIALOG_SHADOW_OFFSET_X, dialogOffset.y + DIALOG_SHADOW_OFFSET_Y / 2 );

    fheroes::ImageRestorer restorer( display, dialogOffset.x, shadowOffset.y, window.width(), window.height() );

    fheroes::Blit( window, display, dialogOffset.x, shadowOffset.y );

    const fheroes::Rect scenarioNameRoi{ 37 + shadowOffset.x, 29 + shadowOffset.y, 349, 19 };

    fheroes::Text text( mapInfo.name, fheroes::FontType::normalWhite(), mapLanguage );
    text.fitToOneRow( scenarioNameRoi.width );
    text.draw( scenarioNameRoi.x, shadowOffset.y + 32, scenarioNameRoi.width, display );

    text.set( _( "Map\nDifficulty" ), fheroes::FontType::smallWhite() );
    ...
    text.set( _( "Rating" ), fheroes::FontType::smallWhite() );
    ...
    text.set( _( "Map Size" ), fheroes::FontType::smallWhite() );
    ...
    conditionsText = fheroes::getLocalizedText( GameOver::GetActualDescription( mapInfo.ConditionLoss(), mapLanguage ), fheroes::FontType::smallWhite() );
    conditionsText->setUniformVerticalAlignment( false );
    ...
    fheroes::Button buttonOk( shadowOffset.x + OK_BUTTON_OFFSET - fheroes::AGG::GetICN( buttonOkIcnId, 0 ).width() / 2, shadowOffset.y + 426, buttonOkIcnId, 0, 1 );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( scenarioNameRoi ) ) {
            text.set( mapInfo.name, fheroes::FontType::normalYellow(), mapLanguage );
            fheroes::showMessage( text, fheroes::Text{}, Dialog::ZERO );
        }
