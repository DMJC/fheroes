...
public:
    ...
    {
        return !fheroes::getArtifactData( id ).curses.empty();
    }
    ...
    {
        return fheroes::getArtifactData( id ).getDescription( ext );
    }
...
public:
    ...
    bool isArtifactBonusPresent( const fheroes::ArtifactBonusType type ) const;
    bool isArtifactCursePresent( const fheroes::ArtifactCurseType type ) const;

    // These methods must be called only for bonuses with cumulative effect.
    int32_t getTotalArtifactEffectValue( const fheroes::ArtifactBonusType bonus ) const;
    int32_t getTotalArtifactEffectValue( const fheroes::ArtifactBonusType bonus, std::string & description ) const;

    int32_t getTotalArtifactEffectValue( const fheroes::ArtifactCurseType curse ) const;
    int32_t getTotalArtifactEffectValue( const fheroes::ArtifactCurseType curse, std::string & description ) const;

    // These methods must be called only for bonuses with multiplication effect.
    std::vector<int32_t> getTotalArtifactMultipliedPercent( const fheroes::ArtifactBonusType bonus ) const;
    std::vector<int32_t> getTotalArtifactMultipliedPercent( const fheroes::ArtifactCurseType curse ) const;

    // Ideally, these methods should be called only for unique bonuses. However, it can be called for other bonus types.
    Artifact getFirstArtifactWithBonus( const fheroes::ArtifactBonusType bonus ) const;
    Artifact getFirstArtifactWithCurse( const fheroes::ArtifactCurseType curse ) const;
...
public:
    ...
    void RedrawItem( Artifact &, const fheroes::Rect &, bool, fheroes::Image & ) override;

    void ResetSelected();
    void Redraw( fheroes::Image & dstsf );
...
protected:
    fheroes::MovableSprite spcursor;
