...
void Interface::StatusPanel::SetPos( int32_t x, int32_t y )
{
    ...
    if ( !Settings::Get().isHideInterfaceEnabled() ) {
        height = fheroes::Display::instance().height() - y - fheroes::borderWidthPx;
    }
...
void Interface::StatusPanel::_redraw() const
{
    ...
    const fheroes::Rect & pos = GetArea();

    if ( conf.isHideInterfaceEnabled() ) {
        fheroes::Fill( fheroes::Display::instance(), pos.x, pos.y, pos.width, pos.height, fheroes::GetColorId( 0x51, 0x31, 0x18 ) );
        BorderWindow::Redraw();
    }
    else {
        _drawBackground();
    ...
    const fheroes::Sprite & ston = fheroes::AGG::GetICN( conf.isEvilInterfaceEnabled() ? ICN::STONBAKE : ICN::STONBACK, 0 );
...
void Interface::StatusPanel::NextState()
{
    ...
    const fheroes::Sprite & ston = fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::STONBAKE : ICN::STONBACK, 0 );
    const int32_t stonHeight = ston.height();
...
{
    fheroes::Point pos = GetArea().getPosition();
    ...
    fheroes::Blit( fheroes::AGG::GetICN( ICN::RESSMALL, 0 ), display, pos.x + 6, pos.y );

    pos.y += 27;

    // count castle
    fheroes::Text text( std::to_string( myKingdom.GetCountCastle() ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 26 - text.width() / 2, pos.y, display );
    // count town
    text.set( std::to_string( myKingdom.GetCountTown() ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 78 - text.width() / 2, pos.y, display );
    // count gold
    text.set( std::to_string( myKingdom.GetFunds().Get( Resource::GOLD ) ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 122 - text.width() / 2, pos.y, display );

    pos.y += 30;

    // count wood
    text.set( fheroes::abbreviateNumber( myKingdom.GetFunds().Get( Resource::WOOD ) ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 15 - text.width() / 2, pos.y, display );
    // count mercury
    text.set( fheroes::abbreviateNumber( myKingdom.GetFunds().Get( Resource::MERCURY ) ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 37 - text.width() / 2, pos.y, display );
    // count ore
    text.set( fheroes::abbreviateNumber( myKingdom.GetFunds().Get( Resource::ORE ) ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 60 - text.width() / 2, pos.y, display );
    // count sulfur
    text.set( fheroes::abbreviateNumber( myKingdom.GetFunds().Get( Resource::SULFUR ) ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 84 - text.width() / 2, pos.y, display );
    // count crystal
    text.set( fheroes::abbreviateNumber( myKingdom.GetFunds().Get( Resource::CRYSTAL ) ), fheroes::FontType::smallWhite() );
    text.draw( pos.x + 108 - text.width() / 2, pos.y, display );
    // count gems
    text.set( fheroes::abbreviateNumber( myKingdom.GetFunds().Get( Resource::GEMS ) ), fheroes::FontType::smallWhite() );
...
void Interface::StatusPanel::_drawResourceInfo( const int32_t offsetY ) const
{
    ...
    fheroes::Text text{ std::move( message ), fheroes::FontType::smallWhite() };
    text.draw( pos.x, pos.y + 6 + offsetY, pos.width, display );

    const fheroes::Sprite & spr = fheroes::AGG::GetICN( ICN::RESOURCE, Resource::getIconIcnIndex( _lastResource ) );
    fheroes::Blit( spr, display, pos.x + ( pos.width - spr.width() ) / 2, pos.y + 6 + offsetY + text.height( pos.width ) );

    text.set( std::to_string( _lastResourceCount ), fheroes::FontType::smallWhite() );
...
void Interface::StatusPanel::_drawAITurns() const
{
    ...
    const fheroes::Sprite & glass = fheroes::AGG::GetICN( ICN::HOURGLAS, 0 );
    const fheroes::Rect & statusRoi = GetArea();

    int32_t posX = statusRoi.x + ( statusRoi.width - glass.width() ) / 2;
    int32_t posY = statusRoi.y + ( statusRoi.height - glass.height() ) / 2;

    fheroes::Blit( glass, display, posX, posY );
    ...
    const fheroes::Sprite & crest = fheroes::AGG::GetICN( ICN::BRCREST, colorIndex );

    posX += 2;
    posY += 2;

    fheroes::Blit( crest, display, posX, posY );
    ...
    const fheroes::Sprite & sandGains = fheroes::AGG::GetICN( ICN::HOURGLAS, animationIndex );
    fheroes::Blit( sandGains, display, posX - sandGains.width() - sandGains.x(), posY + sandGains.y() );

    const fheroes::Sprite & sand = fheroes::AGG::GetICN( ICN::HOURGLAS, 1 + ( _aiTurnProgress % 10 ) );
    fheroes::Blit( sand, display, posX - sand.width() - sand.x(), posY + sand.y() );
}

void Interface::StatusPanel::_drawBackground() const
{
    fheroes::Display & display = fheroes::Display::instance();
    const fheroes::Sprite & icnston = fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::STONBAKE : ICN::STONBACK, 0 );
    const fheroes::Rect & pos = GetArea();

    if ( !Settings::Get().isHideInterfaceEnabled() && display.height() - fheroes::borderWidthPx - icnston.height() > pos.y ) {
        // top
        const int32_t startY = 11;
        const int32_t copyHeight = 46;
        fheroes::Rect srcrt( 0, 0, icnston.width(), startY );
        fheroes::Point dstpt( pos.x, pos.y );
        fheroes::Copy( icnston, srcrt.x, srcrt.y, display, dstpt.x, dstpt.y, srcrt.width, srcrt.height );
        ...
        for ( int32_t i = 0; i < count; ++i ) {
            dstpt = fheroes::Point( pos.x, pos.y + copyHeight * i + startY );
            fheroes::Copy( icnston, srcrt.x, srcrt.y, display, dstpt.x, dstpt.y, srcrt.width, srcrt.height );
        }

        // bottom
        srcrt.height = icnston.height() - startY;
        dstpt = fheroes::Point( pos.x, pos.y + pos.height - srcrt.height );
        fheroes::Copy( icnston, srcrt.x, srcrt.y, display, dstpt.x, dstpt.y, srcrt.width, srcrt.height );
    }
    else {
        fheroes::Copy( icnston, 0, 0, display, pos.x, pos.y, icnston.width(), icnston.height() );
    }
}
...
void Interface::StatusPanel::QueueEventProcessing()
{
    ...
    if ( le.MouseClickLeft( pos ) ) {
        ...
        if ( isFullInfo ) {
            fheroes::showStandardTextMessage( _( "Status Window" ), _( "This window provides information on the status of your hero or kingdom, and shows the date." ),
                                               Dialog::ZERO );
        }
        else {
