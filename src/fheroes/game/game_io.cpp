...
fheroes::GameMode Game::Load( const std::string & filePath )
{
    DEBUG_LOG( DBG_GAME, DBG_INFO, filePath )

    const auto showGenericErrorMessage = []() { fheroes::showStandardTextMessage( _( "Error" ), _( "The save file is corrupted." ), Dialog::OK ); };
    ...
    if ( saveFileVersion > CURRENT_FORMAT_VERSION || saveFileVersion < LAST_SUPPORTED_FORMAT_VERSION ) {
        ...
        fheroes::showStandardTextMessage( _( "Error" ), errorMessage, Dialog::OK );
    ...
    if ( ( conf.GameType() & header.gameType ) == 0 ) {
        fheroes::showStandardTextMessage( _( "Error" ), _( "This file contains a save with an invalid game type." ), Dialog::OK );
    ...
    if ( ( header.requirements & HeaderSAV::REQUIRES_POL_RESOURCES ) && !conf.isPriceOfLoyaltySupported() ) {
        fheroes::showStandardTextMessage( _( "Error" ),
    ...
    fheroes::GameMode returnValue = fheroes::GameMode::START_GAME;
    ...
    if ( conf.isCampaignGameType() ) {
        ...
        if ( !saveData.isStarting() && saveData.getCurrentScenarioInfoId() == saveData.getLastCompletedScenarioInfoID() ) {
            ...
            returnValue = fheroes::GameMode::COMPLETE_CAMPAIGN_SCENARIO_FROM_LOAD_FILE;
        }
    }
    ...
    if ( !conf.loadedFileLanguage().empty() && conf.loadedFileLanguage() != "en" && conf.loadedFileLanguage() != conf.getGameLanguage() ) {
        ...
        warningMessage.append( fheroes::getLanguageName( fheroes::getLanguageFromAbbreviation( conf.loadedFileLanguage() ) ) );
        warningMessage.append( _( "' language, but the current language of the game is '" ) );
        warningMessage.append( fheroes::getLanguageName( fheroes::getLanguageFromAbbreviation( conf.getGameLanguage() ) ) );
        warningMessage += "'.";

        fheroes::showStandardTextMessage( _( "Warning" ), warningMessage, Dialog::OK );
    }
    ...
    if ( Game::GetVersionOfCurrentSaveFile() < FORMAT_VERSION_1109_RELEASE && header.info.version != GameVersion::RESURRECTION
         && fheroes::getCurrentLanguage() == fheroes::SupportedLanguage::French && fheroes::getResourceLanguage() == fheroes::SupportedLanguage::French ) {
        ...
        fheroes::fixFrenchCharactersForMP2Map( header.info.name );
        fheroes::fixFrenchCharactersForMP2Map( header.info.description );
