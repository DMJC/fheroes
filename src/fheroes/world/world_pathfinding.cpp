...
void AIWorldPathfinder::reEvaluateIfNeeded( const Heroes & hero )
{
    const bool isSummonBoatSpellAvailable = [this, &hero]() {
        ...
        return ( fheroes::getSummonableBoat( hero ) != -1 );
    }();
...
std::list<Route::Step> AIWorldPathfinder::buildDimensionDoorPath( const int targetIndex )
{
    ...
    const fheroes::Point targetPoint = Maps::GetPoint( targetIndex );

    fheroes::Point current = Maps::GetPoint( _pathStart );
    fheroes::Point difference = targetPoint - current;
    ...
    for ( uint32_t spellsUsed = 0; spellsUsed < maxCasts; ++spellsUsed ) {
        ...
        if ( Maps::isValidForDimensionDoor( anotherNodeIdx, isHeroOnWater ) ) {
            ...
            int bestNextIdx = -1;
            fheroes::Point bestDirectionDiff;
            ...
            for ( size_t i = 0; i < directions.size(); ++i ) {
                ...
                const fheroes::Point newPoint = Maps::GetPoint( newIndex );
                const fheroes::Point directionDiff{ std::abs( current.x - newPoint.x ), std::abs( current.y - newPoint.y ) };
