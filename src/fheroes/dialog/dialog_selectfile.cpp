...
namespace
{
    ...
    void redrawDateTime( fheroes::Image & output, const time_t timestamp, const int32_t dstx, const int32_t dsty, const fheroes::FontType font )
    {
        ...
        fheroes::Text text( shortMonth, font );
    ...
    void redrawTextInputField( const std::string & filename, const fheroes::Rect & field, fheroes::Image & output )
    {
        if ( filename.empty() ) {
            return;
        }

        fheroes::Text textInput( filename, fheroes::FontType::normalYellow() );
    ...
    public:
        ...
        {
            const fheroes::Text header( System::GetStem( info.filename ), fheroes::FontType::normalYellow() );

            fheroes::MultiFontText body;

            body.add( { _( "Map: " ), fheroes::FontType::normalYellow() } );
            body.add( { info.name, fheroes::FontType::normalWhite(), info.getSupportedLanguage() } );

            if ( info.worldDay > 0 || info.worldWeek > 0 || info.worldMonth > 0 ) {
                body.add( { _( "\n\nMonth: " ), fheroes::FontType::normalYellow() } );
                body.add( { std::to_string( info.worldMonth ), fheroes::FontType::normalWhite() } );
                body.add( { _( ", Week: " ), fheroes::FontType::normalYellow() } );
                body.add( { std::to_string( info.worldWeek ), fheroes::FontType::normalWhite() } );
                body.add( { _( ", Day: " ), fheroes::FontType::normalYellow() } );
                body.add( { std::to_string( info.worldDay ), fheroes::FontType::normalWhite() } );
            }
    ...
    void FileInfoListBox::RedrawItem( const Maps::FileInfo & info, int32_t dstx, int32_t dsty, bool current )
    {
        ...
        fheroes::Text text{ std::move( savname ), font };
    ...
    std::string selectFileListSimple( const std::string & header, const std::string & lastfile, const bool isEditing )
    {
        ...
        const int32_t maxDialogHeight = fheroes::getFontHeight( fheroes::FontSize::NORMAL ) * std::clamp( static_cast<int32_t>( lists.size() ), 11, 22 )
                                        + listAreaOffsetY + listAreaHeightDeduction + listHeightDeduction;

        fheroes::Display & display = fheroes::Display::instance();

        // Dialog height is also capped with the current screen height.
        fheroes::StandardWindow background( maxFileNameWidth + 204, std::min( display.height() - 100, maxDialogHeight ), true, display );

        const fheroes::Rect dialogArea( background.activeArea() );
        const fheroes::Rect listRoi( dialogArea.x + 24, dialogArea.y + 57, dialogArea.width - 75, dialogArea.height - listHeightDeduction );
        const fheroes::Rect nameHeaderRoi( listRoi.x, listRoi.y - 28, maxFileNameWidth + 8, 28 );
        const fheroes::Rect textInputRoi( listRoi.x, listRoi.y + listRoi.height + 3, maxFileNameWidth + 8, 23 );
        ...
        const fheroes::Rect dateHeaderRoi( dateTimeOffsetX, nameHeaderRoi.y, dateTimeWidth, nameHeaderRoi.height );
        ...
        fheroes::ImageRestorer textInputBackground( display, textInputRoi.x, textInputRoi.y, textInputRoi.width, textInputRoi.height );
        fheroes::ImageRestorer dateBackground( display, dateTimeOffsetX, textInputRoi.y, dateTimeWidth, textInputRoi.height );
        const fheroes::Rect textInputAndDateROI( textInputRoi.x, textInputRoi.y, listRoi.width, textInputRoi.height );
        ...
        std::unique_ptr<fheroes::Button> buttonVirtualKB;
        std::unique_ptr<fheroes::TextInputField> textInput;

        const fheroes::Text title( header, fheroes::FontType::normalYellow() );
        title.drawInRoi( dialogArea.x + ( dialogArea.width - title.width() ) / 2, dialogArea.y + 9, display, dialogArea );

        const fheroes::Text nameHeader( _( "saveLoadDialog|Name" ), fheroes::FontType::normalYellow() );
        ...
        const fheroes::Text dateHeader( _( "saveLoadDialog|Date" ), fheroes::FontType::normalYellow() );
        ...
        const fheroes::Sprite & markBackground = fheroes::AGG::GetICN( sortMarkIcnID, 4 );

        const fheroes::Rect markBackgroundNameRoi( nameHeaderRoi.x + 5, nameHeaderRoi.y + 6, markBackground.width(), markBackground.height() );
        const fheroes::Rect markBackgroundDateRoi( dateTimeOffsetX + 5, nameHeaderRoi.y + 6, markBackground.width(), markBackground.height() );
        fheroes::Blit( markBackground, display, markBackgroundNameRoi );
        fheroes::Blit( markBackground, display, markBackgroundDateRoi );

        const fheroes::Sprite & mark = fheroes::AGG::GetICN( sortMarkIcnID, 5 );

        if ( fileSortingMethod == SaveFileSortingMethod::FILENAME ) {
            fheroes::Blit( mark, display, { markBackgroundNameRoi.x + 3, markBackgroundNameRoi.y + 3, mark.width(), mark.height() } );
        }
        else {
            fheroes::Blit( mark, display, { markBackgroundDateRoi.x + 3, markBackgroundDateRoi.y + 3, mark.width(), mark.height() } );
        }
        ...
        auto switchFileSorting
            = [&markBackgroundNameRoi, &markBackgroundDateRoi, &markBackground, &mark, &display, &listbox, &lists, &settings]( const bool doSortByDate ) {
                  const fheroes::Rect roiMarkBackground = doSortByDate ? markBackgroundNameRoi : markBackgroundDateRoi;
                  const fheroes::Rect roiMark = doSortByDate ? markBackgroundDateRoi : markBackgroundNameRoi;
                  fheroes::Blit( markBackground, display, roiMarkBackground );
                  fheroes::Blit( mark, display, { roiMark.x + 3, roiMark.y + 3, roiMark.width, roiMark.height } );
        ...
        if ( isEditing ) {
            ...
            buttonVirtualKB = std::make_unique<fheroes::Button>();
            ...
            textInput = std::make_unique<fheroes::TextInputField>( fheroes::Rect{ textInputRoi.x + 4, textInputRoi.y + 2, maxFileNameWidth, textInputRoi.height - 2 },
                                                                    false, true, display );

            if ( !listbox.isSelected() ) {
                textInput->draw( filename, static_cast<int32_t>( charInsertPos ) );
                redrawDateTime( display, std::time( nullptr ), dateTimeOffsetX, textInputRoi.y + 4, fheroes::FontType::normalWhite() );
            }
        }
        ...
        if ( listbox.isSelected() ) {
            ...
            redrawDateTime( display, listbox.GetCurrent().timestamp, dateTimeOffsetX, textInputRoi.y + 4, fheroes::FontType::normalYellow() );
        }
        ...
        while ( le.HandleEvents() && result.empty() ) {
            ...
            if ( le.isKeyPressed( fheroes::Key::KEY_DELETE ) && isListboxSelected ) {
                ...
                if ( le.MouseClickLeft( buttonVirtualKB->area() ) || ( isInGameKeyboardRequired && le.MouseClickLeft( textInputRoi ) ) ) {
                    fheroes::openVirtualKeyboard( filename, lengthLimit );
                    ...
                    if ( isListboxSelected ) {
                        ...
                        // the behavior of `fitToOneRow` of `fheroes::Text` before calling `fheroes::getTextInputCursorPosition()`.
                    ...
                    if ( ( filename.size() < lengthLimit || pressedKey == fheroes::Key::KEY_BACKSPACE || pressedKey == fheroes::Key::KEY_DELETE )
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
                ...
                if ( isEditing ) {
                    fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to save the current game." ), Dialog::ZERO );
                }
                else {
                    fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to load a previously saved game." ), Dialog::ZERO );
                }
            }
                ...
                fheroes::showStandardTextMessage( _( "Sort by Name" ), _( "Click here if you wish to sort save files by their name." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( dateHeaderRoi ) ) {
                fheroes::showStandardTextMessage( _( "Sort by Date" ), _( "Click here if you you wish to sort save files by their last modified date." ), Dialog::ZERO );
            }
