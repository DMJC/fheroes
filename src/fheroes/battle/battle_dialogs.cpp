...
namespace
{
    ...
    void setupAnimation( fheroes::Display & display, const fheroes::Rect & animationRoi, LoopedAnimationSequence & sequence )
    {
        sequence.push( ICN::WINCMBT, true ); // needs specific for battle summary
        const fheroes::Sprite & sequenceBase = fheroes::AGG::GetICN( sequence.id(), 0 );
        const fheroes::Sprite & sequenceStart = fheroes::AGG::GetICN( sequence.id(), 1 );
        Copy( sequenceBase, 0, 0, display, animationRoi.x, animationRoi.y, sequenceBase.width(), sequenceBase.height() );
        fheroes::Blit( sequenceStart, display, animationRoi.x + sequenceStart.x(), animationRoi.y + sequenceStart.y() );
    }

    void updateAnimation( fheroes::Display & display, int & lastSequence, LoopedAnimationSequence & sequence, const fheroes::Rect & animationRoi )
    {
        if ( Game::validateAnimationDelay( Game::BATTLE_DIALOG_DELAY ) && !sequence.nextFrame() ) {
            if ( lastSequence != sequence.id() ) {
                ...
                const fheroes::Sprite & base = fheroes::AGG::GetICN( lastSequence, 0 );

                Copy( base, 0, 0, display, animationRoi.x + base.x(), animationRoi.y + base.y(), base.width(), base.height() );
            }
            const fheroes::Sprite & sequenceCurrent = fheroes::AGG::GetICN( sequence.id(), sequence.frameId() );

            fheroes::Blit( sequenceCurrent, display, animationRoi.x + sequenceCurrent.x(), animationRoi.y + sequenceCurrent.y() );
    ...
    void RedrawBattleSettings( const std::vector<fheroes::Rect> & areas )
    {
        assert( areas.size() == 9 );
        ...
        const fheroes::Sprite & speedIcon = fheroes::AGG::GetICN( ICN::CSPANEL, speedIcnIndex );
        fheroes::drawOption( areas[0], speedIcon, _( "Speed" ), str, fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const bool isShowTurnOrderEnabled = conf.BattleShowTurnOrder();
        const fheroes::Sprite & turnOrderIcon = fheroes::AGG::GetICN( ICN::CSPANEL, isShowTurnOrderEnabled ? 4 : 3 );
        fheroes::drawOption( areas[1], turnOrderIcon, _( "Turn Order" ), isShowTurnOrderEnabled ? _( "On" ) : _( "Off" ),
                              fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const bool isBattleAudoSpellCastEnabled = conf.BattleAutoSpellcast();
        const fheroes::Sprite & battleAutoSpellCastIcon = fheroes::AGG::GetICN( ICN::CSPANEL, isBattleAudoSpellCastEnabled ? 7 : 6 );
        fheroes::drawOption( areas[2], battleAutoSpellCastIcon, _( "Auto Spell Casting" ), isBattleAudoSpellCastEnabled ? _( "On" ) : _( "Off" ),
                              fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const bool isShowBattleGridEnabled = conf.BattleShowGrid();
        const fheroes::Sprite & battleGridIcon = fheroes::AGG::GetICN( ICN::CSPANEL, isShowBattleGridEnabled ? 9 : 8 );
        fheroes::drawOption( areas[3], battleGridIcon, _( "Grid" ), isShowBattleGridEnabled ? _( "On" ) : _( "Off" ), fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const bool isShowMoveShadowEnabled = conf.BattleShowMoveShadow();
        const fheroes::Sprite & moveShadowIcon = fheroes::AGG::GetICN( ICN::CSPANEL, isShowMoveShadowEnabled ? 11 : 10 );
        fheroes::drawOption( areas[4], moveShadowIcon, _( "Shadow Movement" ), isShowMoveShadowEnabled ? _( "On" ) : _( "Off" ),
                              fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const bool isShowMouseShadowEnabled = conf.BattleShowMouseShadow();
        const fheroes::Sprite & mouseShadowIcon = fheroes::AGG::GetICN( ICN::CSPANEL, isShowMouseShadowEnabled ? 13 : 12 );
        fheroes::drawOption( areas[5], mouseShadowIcon, _( "Shadow Cursor" ), isShowMouseShadowEnabled ? _( "On" ) : _( "Off" ),
                              fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const fheroes::Sprite & audioSettingsIcon = fheroes::AGG::GetICN( ICN::SPANEL, 1 );
        fheroes::drawOption( areas[6], audioSettingsIcon, _( "Audio" ), _( "Settings" ), fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const fheroes::Sprite & hotkeysIcon = fheroes::AGG::GetICN( ICN::GAME_OPTION_ICON, 0 );
        fheroes::drawOption( areas[7], hotkeysIcon, _( "Hot Keys" ), _( "Configure" ), fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );

        const bool isShowBattleDamageInfoEnabled = conf.isBattleShowDamageInfoEnabled();
        const fheroes::Sprite & damageInfoIcon = fheroes::AGG::GetICN( ICN::CSPANEL, isShowBattleDamageInfoEnabled ? 4 : 3 );
        fheroes::drawOption( areas[8], damageInfoIcon, _( "Damage Info" ), isShowBattleDamageInfoEnabled ? _( "On" ) : _( "Off" ),
                              fheroes::UiOptionTextWidth::THREE_ELEMENTS_ROW );
    }
    ...
    DialogAction openBattleOptionDialog( bool & saveConfiguration )
    {
        ...
        background.renderButton( buttonOk, buttonOkIcnId, 0, 1, { 0, 5 }, fheroes::StandardWindow::Padding::BOTTOM_CENTER );

        fheroes::ImageRestorer emptyDialogRestorer( display, windowRoi.x, windowRoi.y, windowRoi.width, windowRoi.height );

        const fheroes::Sprite & panelSprite = fheroes::AGG::GetICN( ICN::CSPANEL, 0 );
        const int32_t panelWidth = panelSprite.width();
        const int32_t panelHeight = panelSprite.height();

        const fheroes::Point optionOffset( windowRoi.x + 20, windowRoi.y + 31 );
        const fheroes::Point optionStep( 92, 110 );

        std::vector<fheroes::Rect> optionAreas;
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.MouseClickLeft( optionAreas[8] ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Speed" ), _( "Set the speed of combat actions and animations." ), 0 );
            }
            else if ( le.isMouseRightButtonPressedInArea( optionAreas[1] ) ) {
                fheroes::showStandardTextMessage( _( "Turn Order" ), _( "Toggle to display the turn order during the battle." ), 0 );
            }
                ...
                fheroes::showStandardTextMessage( _( "Shadow Movement" ), _( "Toggle on or off shadows showing where your creatures can move and attack." ), 0 );
            }
            else if ( le.isMouseRightButtonPressedInArea( optionAreas[5] ) ) {
                fheroes::showStandardTextMessage( _( "Shadow Cursor" ), _( "Toggle on or off a shadow showing the current hex location of the mouse cursor." ), 0 );
            }
                ...
                fheroes::showStandardTextMessage( _( "Damage Info" ), _( "Toggle to display damage information during the battle." ), 0 );
            }
...
bool Battle::Arena::DialogBattleSummary( const Result & res, const std::vector<Artifact> & artifacts, const bool allowToRestart ) const
{
    ...
    fheroes::StandardWindow background( bsTextWidth + 32, 424, true, display );

    const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();

    const fheroes::Sprite & originalBorderImage = fheroes::AGG::GetICN( isEvilInterface ? ICN::WINLOSEE : ICN::WINLOSE, 0 );
    const fheroes::Rect animationBorderRoi{ 43, 32, 231, 133 };

    const fheroes::Rect & roi( background.activeArea() );
    const fheroes::Rect animationRoi( roi.x + ( ( roi.width - animationBorderRoi.width ) / 2 ) + 4, roi.y + 21, animationBorderRoi.width, animationBorderRoi.height );
    ...
    fheroes::FontType summaryTitleFont = fheroes::FontType::normalWhite();
    if ( ( res.attacker & RESULT_WINS ) && attackerIsHuman ) {
        ...
        summaryTitleFont = fheroes::FontType::normalYellow();
    ...
    const fheroes::Rect summaryRoi( roi.x + 11, roi.y + bsTextYOffset, roi.width - 22, roi.height - bsTextYOffset );
    fheroes::ImageRestorer summaryBackground( display, summaryRoi.x, summaryRoi.y, roi.width, summaryRoi.height );
    ...
    // Draw texts
    if ( !title.empty() ) {
        fheroes::Text box( title, summaryTitleFont );
    ...
    const fheroes::FontType bodyFont = fheroes::FontType::normalWhite();
    if ( !outcomeText.empty() ) {
        if ( !surrenderText.empty() ) {
            ...
            fheroes::Text upperText( surrenderText, bodyFont );
            fheroes::Text lowerText( outcomeText, bodyFont );
            ...
            fheroes::Text upperText( outcomeText, bodyFont );
            upperText.setUniformVerticalAlignment( false );
        ...
        const fheroes::Text upperText( surrenderText, bodyFont );
        upperText.draw( summaryRoi.x, summaryBodyOffset + remainingSummaryBodyHeight / 2 - ( upperText.height( summaryRoi.width ) / 2 ), summaryRoi.width, display );
    }

    // Battlefield casualties
    const fheroes::FontType casualtiesFont = fheroes::FontType::smallWhite();
    fheroes::Text text( _( "Battlefield Casualties" ), casualtiesFont );
    ...
    std::unique_ptr<fheroes::Button> buttonRestart;
    if ( allowToRestart ) {
        buttonRestart = std::make_unique<fheroes::Button>();
        ...
        background.renderButton( *buttonRestart, isEvilInterface ? ICN::BUTTON_SMALL_RESTART_EVIL : ICN::BUTTON_SMALL_RESTART_GOOD, 0, 1,
                                 { buttonHorizontalMargin, buttonVerticalMargin }, fheroes::StandardWindow::Padding::BOTTOM_RIGHT );
    }

    const int buttonOkICN = isEvilInterface ? ICN::BUTTON_SMALL_OKAY_EVIL : ICN::BUTTON_SMALL_OKAY_GOOD;

    fheroes::Button buttonOk;
    const fheroes::StandardWindow::Padding buttonOkPadding
        = allowToRestart ? fheroes::StandardWindow::Padding::BOTTOM_LEFT : fheroes::StandardWindow::Padding::BOTTOM_CENTER;
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonOk.area() ) ) {
            fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to leave the battle results." ), Dialog::ZERO );
        }
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonRestart->area() ) ) {
                fheroes::showStandardTextMessage( _( "Restart" ), _( "Click to restart the battle in manual mode." ), Dialog::ZERO );
            }
        }
    ...
    if ( !artifacts.empty() ) {
        ...
        background.renderButton( buttonOk, buttonOkICN, 0, 1, { 0, buttonVerticalMargin }, fheroes::StandardWindow::Padding::BOTTOM_CENTER );

        const fheroes::Sprite & border = fheroes::AGG::GetICN( ICN::WINLOSEB, 0 );
        const fheroes::Rect artifactArea( summaryRoi.x + ( summaryRoi.width - border.width() ) / 2, casualtiesOffsetY + 38, border.width(), border.height() );
        Copy( border, 0, 0, display, artifactArea.x, artifactArea.y, artifactArea.width, artifactArea.height );

        fheroes::ImageRestorer artifactHeader( display, summaryRoi.x, summaryRoi.y, summaryRoi.width, 66 );
        fheroes::ImageRestorer artifactName( display, summaryRoi.x, artifactArea.y + artifactArea.height, summaryRoi.width, 18 );
        std::string artMsg;
        ...
        for ( const Artifact & art : artifacts ) {
            ...
            if ( prevArtifactId == art.GetID() ) {
                ...
                if ( !art.isUltimate() ) {
                    ...
                    if ( !needHeaderRedraw ) {
                        ...
                        const fheroes::Text box( artMsg, fheroes::FontType::normalYellow() );
                ...
                const fheroes::Sprite & artifact = fheroes::AGG::GetICN( ICN::ARTIFACT, art.IndexSprite64() );
                Copy( artifact, 0, 0, display, artifactArea.x + 8, artifactArea.y + 8, artifact.width(), artifact.height() );

                artifactName.restore();

                const fheroes::Text artNameText( artName, fheroes::FontType::smallWhite() );
            ...
            while ( le.HandleEvents() ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( artifactArea ) ) {
                    fheroes::ArtifactDialogElement( art ).showPopup( Dialog::ZERO );
                }
...
void Battle::Arena::DialogBattleNecromancy( const uint32_t raiseCount )
{
    ...
    fheroes::Display & display = fheroes::Display::instance();
    fheroes::StandardWindow background( 287, 424, true, display );
    ...
    const fheroes::Rect & roi( background.activeArea() );
    const fheroes::Rect animationRoi( roi.x + ( ( roi.width - animationBorderRoi.width ) / 2 ) + 4, roi.y + 20, animationBorderRoi.width, animationBorderRoi.height );
    Copy( originalBorderImage, animationBorderRoi.x, animationBorderRoi.y, display, animationRoi.x - 4, animationRoi.y - 4, animationRoi.width, animationRoi.height );
    ...
    const fheroes::Text titleBox( _( "Necromancy!" ), fheroes::FontType::normalYellow() );
    ...
    const fheroes::Text messageBox( msg, fheroes::FontType::normalWhite() );
    ...
    fheroes::Blit( monsterSprite, display, ( display.width() - monsterSprite.width() ) / 2, yOffset );

    fheroes::Text raiseCountText( std::to_string( raiseCount ), fheroes::FontType::smallWhite() );
    ...
    fheroes::Button buttonOk;
    background.renderButton( buttonOk, buttonICN, 0, 1, { 0, 5 }, fheroes::StandardWindow::Padding::BOTTOM_CENTER );
...
int Battle::Arena::DialogBattleHero( HeroBase & hero, const bool buttons, Status & status ) const
{
    ...
    const fheroes::Sprite & dialog = fheroes::AGG::GetICN( ( conf.isEvilInterfaceEnabled() ? ICN::VGENBKGE : ICN::VGENBKG ), 0 );

    const fheroes::Point dialogShadow( 15, 15 );

    fheroes::Display & display = fheroes::Display::instance();
    fheroes::Rect pos_rt( ( display.width() - dialog.width() - dialogShadow.x ) / 2, ( display.height() - dialog.height() - dialogShadow.y ) / 2, dialog.width(),
                           dialog.height() );

    fheroes::ImageRestorer back( display, pos_rt.x, pos_rt.y, pos_rt.width, pos_rt.height );

    fheroes::Blit( dialog, display, pos_rt.x, pos_rt.y );

    // first 15 pixels in the dialog is left shadow, skip
    pos_rt.x += dialogShadow.x;
    pos_rt.width -= dialogShadow.x;

    const fheroes::Rect portraitArea( pos_rt.x + 7, pos_rt.y + 35, 113, 108 );
    ...
    fheroes::Blit( fheroes::AGG::GetICN( ICN::VIEWGEN, col ), display, pos_rt.x + 133, pos_rt.y + 36 );
    ...
    fheroes::Text text( str, fheroes::FontType::smallWhite() );
    fheroes::Point tp{ pos_rt.x + ( pos_rt.width - text.width() ) / 2, pos_rt.y + 11 };
    text.draw( tp.x, tp.y + 2, display );

    const int32_t letterShadowCompensation = 1;
    const fheroes::Point statsTextOffset{ pos_rt.x + 133 + letterShadowCompensation, pos_rt.y + 40 };
    ...
    text.set( str, fheroes::FontType::smallWhite() );
    ...
    fheroes::Button btnCast( pos_rt.x + 15, pos_rt.y + 148, ICN::VIEWGEN, 9, 10 );
    fheroes::Button btnRetreat( pos_rt.x + 74, pos_rt.y + 148, ICN::VIEWGEN, 11, 12 );
    fheroes::Button btnSurrender( pos_rt.x + 133, pos_rt.y + 148, ICN::VIEWGEN, 13, 14 );
    fheroes::Button btnClose( pos_rt.x + 192, pos_rt.y + 148, ICN::VIEWGEN, 15, 16 );
    ...
    while ( le.HandleEvents() && !result ) {
        ...
        if ( le.MouseClickLeft( portraitArea ) && heroForHeroScreen != nullptr ) {
            ...
            heroForHeroScreen->OpenDialog( true, true, true, true, false, false, fheroes::getLanguageFromAbbreviation( conf.getGameLanguage() ) );

            // Fade-in to restore the screen after closing the hero dialog.
            fheroes::fadeInDisplay( _interface->GetInterfaceRoi(), !display.isDefaultSize() );
        }
        ...
        if ( le.isMouseRightButtonPressedInArea( btnCast.area() ) && currentColor == hero.GetColor() ) {
            ...
            fheroes::showStandardTextMessage( _( "Hero Screen" ), _( "Open Hero Screen to view full information about the hero." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( btnClose.area() ) ) {
            fheroes::showStandardTextMessage( _( "Cancel" ), _( "Return to the battle." ), Dialog::ZERO );
        }
...
bool Battle::DialogBattleSurrender( const HeroBase & hero, uint32_t cost, Kingdom & kingdom )
{
    ...
    const fheroes::Sprite & dialog = fheroes::AGG::GetICN( isEvilInterface ? ICN::SURDRBKE : ICN::SURDRBKG, 0 );

    fheroes::Rect pos_rt( ( display.width() - dialog.width() + 16 ) / 2, ( display.height() - dialog.height() + 16 ) / 2, dialog.width(), dialog.height() );
    ...
    fheroes::ButtonSprite btnAccept
        = fheroes::makeButtonWithShadow( pos_rt.x + 91, pos_rt.y + 152, fheroes::AGG::GetICN( icnAccept, 0 ), fheroes::AGG::GetICN( icnAccept, 1 ), display );

    fheroes::ButtonSprite btnDecline
        = fheroes::makeButtonWithShadow( pos_rt.x + 295, pos_rt.y + 152, fheroes::AGG::GetICN( icnDecline, 0 ), fheroes::AGG::GetICN( icnDecline, 1 ), display );

    fheroes::ButtonSprite btnMarket = fheroes::makeButtonWithShadow( pos_rt.x + ( pos_rt.width - 16 ) / 2, pos_rt.y + 145, fheroes::AGG::GetICN( icnMarket, 0 ),
                                                                       fheroes::AGG::GetICN( icnMarket, 1 ), display );
    ...
    const auto drawGoldMsg = [cost, &kingdom, &display]( const fheroes::Rect & btnAcceptArea ) {
        std::string str = _( "Not enough gold (%{gold})" );

        StringReplace( str, "%{gold}", cost - kingdom.GetFunds().gold );

        const fheroes::Text text( str, fheroes::FontType::smallWhite() );
    ...
    const fheroes::Sprite & window = fheroes::AGG::GetICN( icn, 4 );
    fheroes::Blit( window, display, pos_rt.x + 55, pos_rt.y + 32 );
    ...
    fheroes::Text box( str, fheroes::FontType::normalWhite() );
