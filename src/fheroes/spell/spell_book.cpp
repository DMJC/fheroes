...
namespace
{
    ...
    const fheroes::Point bookmarkInfoOffset( 123, 273 );
    const fheroes::Point bookmarkAdvOffset( 266, 269 );
    const fheroes::Point bookmarkCombatoOffset( 299, 276 );
    const fheroes::Point bookmarkCloseOffset( 416, 280 );

    const fheroes::Point spellBookShadow( -16, 16 );

    fheroes::Size getSpellBookSize( const SpellBook::Filter displayableSpells )
    {
        const fheroes::Sprite & bookPage = fheroes::AGG::GetICN( ICN::BOOK, 0 );
        const fheroes::Sprite & bookmark_info = fheroes::AGG::GetICN( ICN::BOOK, 6 );
        const fheroes::Sprite & bookmark_advn = fheroes::AGG::GetICN( ICN::BOOK, 3 );
        const fheroes::Sprite & bookmark_cmbt = fheroes::AGG::GetICN( ICN::BOOK, 4 );
        const fheroes::Sprite & bookmark_clos = fheroes::AGG::GetICN( ICN::BOOK, 5 );
    ...
    void SpellBookRedrawSpells( const SpellStorage & spells, std::vector<fheroes::Rect> & coords, const size_t index, const int32_t px, const int32_t py,
                                const HeroBase & hero, const bool canCastSpell, const bool isRight, fheroes::Image & output, const fheroes::Point & outputOffset )
    {
        for ( size_t i = 0; i < spellsPerPage; ++i ) {
            ...
            const fheroes::Sprite & icon = fheroes::AGG::GetICN( ICN::SPELLS, spell.IndexSprite() );
            const int32_t vertOffset = std::min( 6, 49 - icon.height() );
            fheroes::Rect rect( px + ox - ( icon.width() + icon.width() % 2 ) / 2, py + oy - icon.height() - vertOffset + 2, icon.width(), icon.height() + 10 );
            fheroes::Blit( icon, output, rect.x, rect.y );

            const int32_t maxTextWidth = 80;
            const int32_t rowCount = fheroes::Text{ spellName, fheroes::FontType::smallWhite() }.rows( maxTextWidth );

            const fheroes::Text text( spellName + ( rowCount == 1 ? '\n' : ' ' ) + '[' + std::to_string( spellCost ) + ']',
                                       isAvailable ? fheroes::FontType::smallWhite() : fheroes::FontType{ fheroes::FontSize::SMALL, fheroes::FontColor::GRAY } );
    ...
    void SpellBookRedrawManaPoints( const fheroes::Point & dst, uint32_t manaPoints, fheroes::Image & output )
    {
        fheroes::Point tp( dst.x + 11, dst.y + 11 );
        if ( manaPoints > 999 ) {
            manaPoints = 999; // just in case of a broken code
        }

        fheroes::Text text( manaPoints >= 100 ? std::to_string( manaPoints / 100 ) : " ", fheroes::FontType::smallWhite() );
        text.draw( tp.x - text.width() / 2, tp.y, output );
        tp.y += text.height();

        text.set( manaPoints >= 10 ? std::to_string( ( manaPoints % 100 ) / 10 ) : " ", fheroes::FontType::smallWhite() );
        text.draw( tp.x - text.width() / 2, tp.y, output );
        tp.y += text.height();

        text.set( manaPoints > 0 ? std::to_string( manaPoints % 10 ) : "0", fheroes::FontType::smallWhite() );
        text.draw( tp.x - text.width() / 2, tp.y, output );
    }

    void spellBookRedrawLists( const SpellStorage & spells, std::vector<fheroes::Rect> & coords, const size_t index, const fheroes::Point & pt,
                               const uint32_t spellPoints, const SpellBook::Filter displayableSpells, const HeroBase & hero, const bool canCastSpell )
    {
        const fheroes::Size spellBookSize = getSpellBookSize( displayableSpells );

        fheroes::Sprite output( spellBookSize.width, spellBookSize.height );
        output.reset();

        const fheroes::Sprite & bookPage = fheroes::AGG::GetICN( ICN::BOOK, 0 );

        // Left page.
        fheroes::Blit( bookPage, 0, 0, output, 0, 0, bookPage.width(), bookPage.height(), true );
        if ( index == 0 ) {
            const fheroes::Sprite & straightCorner = fheroes::AGG::GetICN( ICN::BOOK, 7 );
            fheroes::Blit( straightCorner, 0, 0, output, bookPage.width() - straightCorner.x() - straightCorner.width(), straightCorner.y(), straightCorner.width(),
                            straightCorner.height(), true );
        }

        // Right page.
        fheroes::Blit( bookPage, 0, 0, output, bookPage.width(), 0, bookPage.width(), bookPage.height() );
        if ( spells.size() <= ( index + ( spellsPerPage * 2 ) ) ) {
            const fheroes::Sprite & straightCorner = fheroes::AGG::GetICN( ICN::BOOK, 7 );
            fheroes::Blit( straightCorner, 0, 0, output, bookPage.width() + straightCorner.x(), straightCorner.y(), straightCorner.width(), straightCorner.height() );
        }

        const fheroes::Sprite & bookmarkSpellPoints = fheroes::AGG::GetICN( ICN::BOOK, 6 );
        fheroes::Blit( bookmarkSpellPoints, output, bookmarkInfoOffset.x, bookmarkInfoOffset.y );

        if ( displayableSpells != SpellBook::Filter::CMBT ) {
            const fheroes::Sprite & bookmarkAdvantureSpells = fheroes::AGG::GetICN( ICN::BOOK, 3 );
            fheroes::Blit( bookmarkAdvantureSpells, output, bookmarkAdvOffset.x, bookmarkAdvOffset.y );
        }
        if ( displayableSpells != SpellBook::Filter::ADVN ) {
            const fheroes::Sprite & bookmarkCombatSpells = fheroes::AGG::GetICN( ICN::BOOK, 4 );
            fheroes::Blit( bookmarkCombatSpells, output, bookmarkCombatoOffset.x, bookmarkCombatoOffset.y );
        }

        const fheroes::Sprite & bookmarkClose = fheroes::AGG::GetICN( ICN::BOOK, 5 );
        fheroes::Blit( bookmarkClose, output, bookmarkCloseOffset.x, bookmarkCloseOffset.y );
        ...
        output = fheroes::addShadow( output, spellBookShadow, 3 );

        fheroes::Blit( output, 0, 0, fheroes::Display::instance(), pt.x + spellBookShadow.x, pt.y, output.width(), output.height() );
    }
}
                       ...
                       const std::function<void( const std::string & )> & statusCallback ) const
{
    if ( !hero.HaveSpellBook() ) {
        fheroes::showStandardTextMessage( "", _( "You have no Magic Book, so you cannot cast a spell." ), Dialog::OK );
        return Spell::NONE;
    }
    ...
    if ( canCastSpell && displayedSpells.empty() ) {
        fheroes::showStandardTextMessage( "", _( "No spell to cast." ), Dialog::OK );
        return Spell::NONE;
    }
    ...
    const fheroes::Rect pos( ( display.width() - ( bookPage.width() * 2 ) ) / 2, ( display.height() - bookPage.height() ) / 2, spellBookSize.width,
                              spellBookSize.height );

    fheroes::ImageRestorer restorer( display, pos.x + spellBookShadow.x, pos.y, pos.width - spellBookShadow.x, pos.height + spellBookShadow.y );

    const fheroes::Rect previousPageRoi( pos.x + 30, pos.y + 8, 30, 25 );
    const fheroes::Rect nextPageRoi( pos.x + 410, pos.y + 8, 30, 25 );

    const fheroes::Rect spellPointsRoi( pos.x + bookmarkInfoOffset.x, pos.y + bookmarkInfoOffset.y, bookmark_info.width(), bookmark_info.height() );
    const fheroes::Rect adventureSpellsRoi( pos.x + bookmarkAdvOffset.x, pos.y + bookmarkAdvOffset.y, bookmark_advn.width(), bookmark_advn.height() );
    const fheroes::Rect combatSpellsRoi( pos.x + bookmarkCombatoOffset.x, pos.y + bookmarkCombatoOffset.y, bookmark_cmbt.width(), bookmark_cmbt.height() );
    const fheroes::Rect closeRoi( pos.x + bookmarkCloseOffset.x, pos.y + bookmarkCloseOffset.y, bookmark_clos.width(), bookmark_clos.height() );

    Spell curspell = Spell::NONE;

    std::vector<fheroes::Rect> coords;
    ...
    while ( le.HandleEvents() ) {
        if ( !_isFirstPage() && ( le.MouseClickLeft( previousPageRoi ) || HotKeyPressEvent( Game::HotKeyEvent::DEFAULT_LEFT ) ) ) {
            ...
            fheroes::showStandardTextMessage( "", getHeroSpellPointsInfo(), Dialog::OK );
        }
            ...
            if ( 0 <= index ) {
                ...
                if ( spell < displayedSpells.end() ) {
                    if ( canCastSpell ) {
                        ...
                        fheroes::showStandardTextMessage( spell->GetName(), str, Dialog::OK );
                        display.render();
                    }
                    else {
                        fheroes::SpellDialogElement( *spell, &hero ).showPopup( Dialog::OK );
                        display.render();
                    }
                }
            }
        ...
        if ( le.isMouseRightButtonPressedInArea( pos ) ) {
            ...
            if ( 0 <= index ) {
                const SpellStorage::const_iterator spell = displayedSpells.begin() + ( index + _startSpellIndex );
                if ( spell < displayedSpells.end() ) {
                    fheroes::SpellDialogElement( *spell, &hero ).showPopup( Dialog::ZERO );
                    display.render();
                }
            }
        }
...
void SpellBook::Edit( const HeroBase & hero )
{
    ...
    const fheroes::Size spellBookSize = getSpellBookSize( Filter::ALL );
    ...
    const fheroes::Rect previousPageRoi( pos.x + 30, pos.y + 8, 30, 25 );
    const fheroes::Rect nextPageRoi( pos.x + 410, pos.y + 8, 30, 25 );
    const fheroes::Rect closeRoi( pos.x + 420, pos.y + 284, bookmark_clos.width(), bookmark_clos.height() );
