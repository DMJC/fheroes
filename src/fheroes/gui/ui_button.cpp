...
namespace
{
    fheroes::Image resizeButton( const fheroes::Image & original, const fheroes::Size & buttonSize )
    {
        ...
        fheroes::Image output( buttonSize.width, buttonSize.height );
        output.reset();
        ...
        if ( buttonSize.width > originalWidth && buttonSize.height == originalHeight ) {
            ...
            fheroes::Copy( original, 0, 0, output, 0, 0, middleWidth, originalHeight );

            int32_t offsetX = middleWidth;
            for ( int32_t i = 0; i < middleWidthCount; ++i ) {
                fheroes::Copy( original, middleWidth, 0, output, offsetX, 0, middleWidth, originalHeight );
                offsetX += middleWidth;
            }

            if ( middleWidthLeftOver > 0 ) {
                fheroes::Copy( original, middleWidth, 0, output, offsetX, 0, middleWidthLeftOver, originalHeight );
                offsetX += middleWidthLeftOver;
            }
            assert( offsetX + originalWidth - middleWidth * 2 == buttonSize.width );

            fheroes::Copy( original, originalWidth - middleWidth, 0, output, offsetX, 0, middleWidth, originalHeight );
        }
        // Buttons that only are taller.
            ...
            fheroes::Copy( original, 0, 0, output, 0, 0, originalWidth, middleHeight );

            int32_t offsetY = middleHeight;
            for ( int32_t i = 0; i < middleHeightCount; ++i ) {
                fheroes::Copy( original, 0, middleHeight, output, 0, offsetY, originalWidth, middleHeight );
                offsetY += middleHeight;
            }

            if ( middleHeightLeftOver > 0 ) {
                fheroes::Copy( original, 0, middleHeight, output, 0, offsetY, originalWidth, middleHeightLeftOver );
                offsetY += middleHeightLeftOver;
            }
            assert( offsetY + originalHeight - middleHeight * 4 == buttonSize.height );

            fheroes::Copy( original, 0, originalHeight - middleHeight, output, 0, offsetY, originalWidth, middleHeight );
        }
        // Buttons that have shrunk in any direction.
        else if ( buttonSize.height <= originalHeight && buttonSize.width <= originalWidth ) {
            fheroes::Copy( original, 0, 0, output, 0, 0, buttonSize.width / 2, buttonSize.height / 2 );
            ...
            fheroes::Copy( original, 0, originalHeight - secondHalfHeight, output, 0, buttonSize.height - secondHalfHeight, secondHalfWidth, secondHalfHeight );
            fheroes::Copy( original, originalWidth - secondHalfWidth, 0, output, buttonSize.width - secondHalfWidth, 0, secondHalfWidth, secondHalfHeight );
            fheroes::Copy( original, originalWidth - secondHalfWidth, originalHeight - secondHalfHeight, output, buttonSize.width - secondHalfWidth,
        ...
        // Buttons that are taller but less wide.
            ...
            fheroes::Copy( original, 0, 0, output, 0, 0, middleWidth, middleHeight );
            fheroes::Copy( original, originalWidth - rightSideWidth, 0, output, middleWidth, 0, rightSideWidth, middleHeight );

            int32_t offsetY = middleHeight;
            for ( int32_t i = 0; i < middleHeightCount; ++i ) {
                fheroes::Copy( original, 0, middleHeight, output, 0, offsetY, middleWidth, middleHeight );
                fheroes::Copy( original, originalWidth - rightSideWidth, middleHeight, output, middleWidth, offsetY, rightSideWidth, middleHeight );
                offsetY += middleHeight;
            }

            if ( middleHeightLeftOver > 0 ) {
                fheroes::Copy( original, 0, middleHeight, output, 0, offsetY, middleWidth, middleHeightLeftOver );
                fheroes::Copy( original, originalWidth - rightSideWidth, middleHeight, output, middleWidth, offsetY, rightSideWidth, middleHeightLeftOver );
                offsetY += middleHeightLeftOver;
            }
            assert( offsetY + originalHeight - middleHeight * 4 == buttonSize.height );

            fheroes::Copy( original, 0, originalHeight - middleHeight, output, 0, offsetY, middleWidth, middleHeight );
            fheroes::Copy( original, originalWidth - rightSideWidth, originalHeight - middleHeight, output, middleWidth, offsetY, rightSideWidth, middleHeight );
        }
        // Buttons that have increased width and height.
            ...
            fheroes::Copy( original, originalWidth - rightPartWidth, 0, output, buttonSize.width - rightPartWidth, 0, middleWidth, middleHeight );

            int32_t offsetY = middleHeight;
            for ( int32_t i = 0; i < middleHeightCount; ++i ) {
                fheroes::Copy( original, 0, middleHeight, output, 0, offsetY, middleWidth, middleHeight );
                fheroes::Copy( original, originalWidth - rightPartWidth, middleHeight, output, buttonSize.width - rightPartWidth, offsetY, middleWidth, middleHeight );
                offsetY += middleHeight;
            }

            if ( middleHeightLeftOver > 0 ) {
                fheroes::Copy( original, 0, middleHeight, output, 0, offsetY, middleWidth, middleHeightLeftOver );
                fheroes::Copy( original, originalWidth - rightPartWidth, middleHeight, output, buttonSize.width - rightPartWidth, offsetY, middleWidth,
            ...
            fheroes::Copy( original, 0, originalHeight - bottomPartHeight, output, 0, offsetY, middleWidth, bottomPartHeight );
            fheroes::Copy( original, originalWidth - rightPartWidth, originalHeight - bottomPartHeight, output, buttonSize.width - rightPartWidth, offsetY, middleWidth,
                            bottomPartHeight );

            int32_t offsetX = middleWidth;
            for ( int32_t i = 0; i < middleWidthCount; ++i ) {
                fheroes::Copy( original, middleWidth, 0, output, offsetX, 0, middleWidth, middleHeight );
                fheroes::Copy( original, middleWidth, originalHeight - bottomPartHeight, output, offsetX, offsetY, middleWidth, middleHeight );
                offsetX += middleWidth;
            }

            if ( middleWidthLeftOver > 0 ) {
                fheroes::Copy( original, middleWidth, 0, output, offsetX, 0, middleWidthLeftOver, middleHeight );
                fheroes::Copy( original, middleWidth, originalHeight - bottomPartHeight, output, offsetX, offsetY, middleWidthLeftOver, middleHeight );
                offsetX += middleWidthLeftOver;
            }
            ...
            fheroes::Fill( output, middleWidth, middleHeight, offsetX - middleWidth, offsetY - middleHeight, original.image()[centralPosition] );
        }
        else {
    ...
    void addButtonShine( fheroes::Sprite & buttonImage, const int emptyButtonIcnID )
    {
        ...
        if ( isGoodButton || emptyButtonIcnID == ICN::EMPTY_EVIL_BUTTON ) {
            ...
            // Left-side shine
            fheroes::SetPixel( buttonImage, 11, 4, firstColor );
            fheroes::SetPixel( buttonImage, 13, 4, firstColor );
            fheroes::SetPixel( buttonImage, 9, 6, firstColor );
            fheroes::SetPixel( buttonImage, 10, 5, secondColor );
            fheroes::SetPixel( buttonImage, 12, 5, secondColor );
            fheroes::SetPixel( buttonImage, 8, 7, lastColor );
            fheroes::SetPixel( buttonImage, 15, 4, lastColor );
            // Right-side shine
            const int32_t buttonWidth = buttonImage.width();
            fheroes::SetPixel( buttonImage, buttonWidth - 9, 4, firstColor );
            fheroes::SetPixel( buttonImage, buttonWidth - 7, 4, firstColor );
            fheroes::DrawLine( buttonImage, { buttonWidth - 10, 5 }, { buttonWidth - 11, 6 }, secondColor );
            fheroes::SetPixel( buttonImage, buttonWidth - 8, 5, secondColor );
            ...
            if ( buttonWidth > 50 ) {
                ...
                fheroes::DrawLine( buttonImage, { 11, 4 }, { 9, 6 }, firstColor );
                fheroes::SetPixel( buttonImage, 8, 7, lastColor );
                fheroes::SetPixel( buttonImage, 13, 4, secondColor );
                fheroes::DrawLine( buttonImage, { 12, 5 }, { 9, 8 }, lastColor );
                fheroes::DrawLine( buttonImage, { 8, 9 }, { 7, 10 }, firstColor );
                fheroes::SetPixel( buttonImage, 6, 11, lastColor );
                fheroes::DrawLine( buttonImage, { 15, 4 }, { 13, 6 }, firstColor );
                fheroes::SetPixel( buttonImage, 12, 7, lastColor );
                // Longer right-side shine
                fheroes::SetPixel( buttonImage, buttonWidth - 10, 2, firstColor );
                fheroes::SetPixel( buttonImage, buttonWidth - 8, 3, secondColor );
                fheroes::SetPixel( buttonImage, buttonWidth - 11, 3, secondColor );
                fheroes::SetPixel( buttonImage, buttonWidth - 12, 4, firstColor );
                fheroes::DrawLine( buttonImage, { buttonWidth - 13, 5 }, { buttonWidth - 14, 6 }, lastColor );
                fheroes::DrawLine( buttonImage, { buttonWidth - 10, 5 }, { buttonWidth - 12, 7 }, firstColor );
                fheroes::DrawLine( buttonImage, { buttonWidth - 13, 8 }, { buttonWidth - 14, 9 }, secondColor );
                fheroes::DrawLine( buttonImage, { buttonWidth - 8, 5 }, { buttonWidth - 10, 7 }, firstColor );
                fheroes::SetPixel( buttonImage, buttonWidth - 11, 8, lastColor );
                // Bottom left-side shine
                fheroes::SetPixel( buttonImage, 11, buttonHeight - 6, lastColor );
                fheroes::DrawLine( buttonImage, { 12, buttonHeight - 7 }, { 13, buttonHeight - 8 }, firstColor );
                fheroes::SetPixel( buttonImage, 14, buttonHeight - 9, lastColor );
                // This is mainly to decorate the space bar button
                if ( buttonWidth > 96 || buttonHeight > 25 ) {
                    // Longer bottom left shine
                    fheroes::SetPixel( buttonImage, 10, buttonHeight - 5, lastColor );
                    // Center part shine
                    fheroes::SetPixel( buttonImage, 30, buttonHeight - 5, secondColor );
                    fheroes::SetPixel( buttonImage, 31, buttonHeight - 6, lastColor );
                    fheroes::DrawLine( buttonImage, { 32, buttonHeight - 5 }, { 34, buttonHeight - 7 }, firstColor );
                    fheroes::SetPixel( buttonImage, 35, buttonHeight - 8, secondColor );
                    fheroes::SetPixel( buttonImage, 36, buttonHeight - 9, lastColor );
                    fheroes::SetPixel( buttonImage, 34, buttonHeight - 5, secondColor );
                    fheroes::SetPixel( buttonImage, 35, buttonHeight - 6, lastColor );
                }
                if ( buttonHeight > 25 ) {
                    // Bottom right-side shine
                    fheroes::DrawLine( buttonImage, { buttonWidth - 20, buttonHeight - 5 }, { buttonWidth - 16, buttonHeight - 9 }, secondColor );
                    fheroes::SetPixel( buttonImage, buttonWidth - 15, buttonHeight - 10, lastColor );
                    fheroes::DrawLine( buttonImage, { buttonWidth - 18, buttonHeight - 5 }, { buttonWidth - 14, buttonHeight - 9 }, firstColor );
                    fheroes::DrawLine( buttonImage, { buttonWidth - 13, buttonHeight - 10 }, { buttonWidth - 12, buttonHeight - 11 }, secondColor );
                    fheroes::DrawLine( buttonImage, { buttonWidth - 11, buttonHeight - 12 }, { buttonWidth - 10, buttonHeight - 13 }, lastColor );
                    fheroes::DrawLine( buttonImage, { buttonWidth - 16, buttonHeight - 5 }, { buttonWidth - 12, buttonHeight - 9 }, secondColor );
                    fheroes::SetPixel( buttonImage, buttonWidth - 11, buttonHeight - 10, lastColor );
                }
            }
        }
    }

    void getButtonSpecificValues( const int emptyButtonIcnID, fheroes::FontColor & font, fheroes::Point & textAreaBorders, fheroes::Size & minimumTextArea,
                                  fheroes::Size & maximumTextArea, fheroes::Size & backgroundBorders, fheroes::Point & releasedOffset,
                                  fheroes::Point & pressedOffset )
    {
        switch ( emptyButtonIcnID ) {
        case ICN::EMPTY_GOOD_BUTTON:
            font = fheroes::FontColor::WHITE;
...
namespace fheroes
{
    ...
    void ButtonBase::drawShadow( Image & output ) const
    {
        ...
        fheroes::addGradientShadow( _getReleased(), output, buttonPoint, { -5, 5 } );
    }
    ...
    ButtonGroup::ButtonGroup( const int icnID )
    {
        ...
        const int32_t buttonCount = static_cast<int32_t>( fheroes::AGG::GetICNCount( icnID ) ) / 2;
    ...
    void makeButtonSprites( Sprite & released, Sprite & pressed, const std::string & text, const Size buttonSize, const bool isEvilInterface, const int backgroundIcnId )
    {
        ...
        const fheroes::FontColor buttonFontColor = isEvilInterface ? fheroes::FontColor::GRAY : fheroes::FontColor::WHITE;
                                         ...
                                         const int32_t minWidth )
    {
        ...
        const FontType buttonFontType = { FontSize::BUTTON_RELEASED, ( isEvilInterface ? fheroes::FontColor::GRAY : fheroes::FontColor::WHITE ) };
