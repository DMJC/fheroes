...
void Cursor::SetThemes( const int theme, const bool force /* = false */ )
{
    ...
    const fheroes::Sprite & spr = fheroes::AGG::GetICN( icnID, 0xFF & theme );
    SetOffset( theme, { ( spr.width() - spr.x() ) / 2, ( spr.height() - spr.y() ) / 2 } );

    fheroes::Cursor & cursor = fheroes::cursor();
    ...
    // Apply new offset.
    const fheroes::Point & currentPos = LocalEvent::Get().getMouseCursorPos();
    Move( currentPos.x, currentPos.y );
}

void Cursor::setCustomImage( const fheroes::Image & image, const fheroes::Point & offset )
{
    _theme = NONE;
...
fheroes::Rect Cursor::updateCursorPosition( const int32_t x, const int32_t y )
{
    if ( fheroes::cursor().isSoftwareEmulation() ) {
        Cursor::Get().Move( x, y );
        if ( fheroes::cursor().isVisible() ) {
            return { x, y, 1, 1 };
        }
    }
...
{
    fheroes::cursor().setPosition( x + _offset.x, y + _offset.y );
}

void Cursor::SetOffset( int name, const fheroes::Point & defaultOffset )
{
    switch ( name ) {
    case Cursor::POINTER:
...
{
    fheroes::Cursor & cursorRenderer = fheroes::cursor();
    ...
    if ( isShown && noThemeChange && cursorRenderer.isSoftwareEmulation() ) {
        const fheroes::Point & pos = LocalEvent::Get().getMouseCursorPos();
        fheroes::Display::instance().render( { pos.x, pos.y, 1, 1 } );
    }
}
