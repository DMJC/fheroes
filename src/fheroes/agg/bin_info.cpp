...
namespace
{
    ...
    {
        return fheroes::getLEValue<T>( reinterpret_cast<const char *>( data ), base, offset );
    }

    const char * GetFilename( const int monsterId )
    {
        return fheroes::getMonsterData( monsterId ).binFileName;
    }
...
namespace Bin_Info
{
    ...
    MonsterAnimInfo::MonsterAnimInfo( const int monsterID /* = 0 */, const std::vector<uint8_t> & bytes /* = std::vector<uint8_t>() */ )
    {
        ...
        if ( monsterID == Monster::ORC || monsterID == Monster::ORC_CHIEF ) {
            for ( fheroes::Point & offset : projectileOffset ) {
                offset.x -= 20;
                offset.y = -36;
            }
        }
