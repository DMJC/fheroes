    ...
    , _randomGenerator( randomGenerator )
{
    ...
    if ( _interface ) {
        ...
        while ( Mixer::isPlaying( -1 ) && le.HandleEvents() ) {
            if ( le.isKeyPressed( fheroes::Key::KEY_ESCAPE ) || le.MouseClickMiddle() || le.MouseClickRight() ) {
...
bool Battle::Arena::isSpellcastDisabled() const
{
    ...
    if ( attackingHero != nullptr && attackingHero->GetBagArtifacts().isArtifactBonusPresent( fheroes::ArtifactBonusType::DISABLE_ALL_SPELL_COMBAT_CASTING ) ) {
        return true;
    }

    const HeroBase * defendingHero = _defendingArmy->GetCommander();
    if ( defendingHero != nullptr && defendingHero->GetBagArtifacts().isArtifactBonusPresent( fheroes::ArtifactBonusType::DISABLE_ALL_SPELL_COMBAT_CASTING ) ) {
        return true;
    }
...
Battle::Unit * Battle::Arena::CreateElemental( const Spell & spell )
{
    ...
    const uint32_t count = fheroes::getSummonMonsterCount( spell, hero->GetPower(), hero );
...
bool Battle::Arena::IsShootingPenalty( const Unit & attacker, const Unit & defender ) const
{
    ...
    if ( hero ) {
        // golden bow artifact
        if ( hero->GetBagArtifacts().isArtifactBonusPresent( fheroes::ArtifactBonusType::NO_SHOOTING_PENALTY ) )
            return false;
    ...
    const std::vector<fheroes::Point> points = getLinePoints( attacker.GetBackPoint(), defender.GetBackPoint(), Cell::widthPx / 3 );

    for ( std::vector<fheroes::Point>::const_iterator it = points.begin(); it != points.end(); ++it ) {
