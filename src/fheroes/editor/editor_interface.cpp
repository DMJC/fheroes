...
namespace
{
    ...
    class MonsterMultiSelection final : public fheroes::DialogElement
    {
    public:
        MonsterMultiSelection( std::vector<int> allowed, std::vector<int> selected, std::string text, const bool isEvilInterface )
            ...
            , _text( std::move( text ), fheroes::FontType::normalWhite() )
            , _buttonSelection( 0, 0, ( isEvilInterface ? ICN::BUTTON_SELECT_EVIL : ICN::BUTTON_SELECT_GOOD ), 0, 1 )
        {
            const int32_t offset{ 5 };

            const fheroes::Size textArea{ _text.width(), _text.height() };
            const fheroes::Size buttonArea{ _buttonSelection.area().width, _buttonSelection.area().height };
        ...
        void processEvents( const fheroes::Point & offset ) const override
        {
            ...
            const fheroes::Rect buttonRect{ offset.x + _buttonArea.x, offset.y + _buttonArea.y, _buttonArea.width, _buttonArea.height };

            if ( le.isMouseRightButtonPressedInArea( buttonRect ) ) {
                fheroes::showStandardTextMessage( _( "SELECT" ), _( "Click to make selection." ), 0 );
            }
    ...
    private:
        ...
        fheroes::Rect _textArea;
        fheroes::Rect _buttonArea;

        fheroes::Text _text;
        mutable fheroes::Button _buttonSelection;
    };
    ...
    fheroes::Point getBrushAreaIndicies( const fheroes::Rect & brushSize, const int32_t startIndex )
    {
        ...
        fheroes::Point startPos{ ( startIndex % worldWidth ) + brushSize.x, ( startIndex / worldWidth ) + brushSize.y };
        fheroes::Point endPos{ startPos.x + brushSize.width - 1, startPos.y + brushSize.height - 1 };
    ...
    bool isObjectPlacementAllowed( const Maps::ObjectInfo & info, const fheroes::Point & mainTilePos )
    {
    ...
    bool isActionObjectAllowed( const Maps::ObjectInfo & info, const fheroes::Point & mainTilePos )
    {
        ...
        for ( const auto & objectPart : info.groundLevelParts ) {
            ...
            const fheroes::Point pos{ mainTilePos.x + objectPart.tileOffset.x, mainTilePos.y + objectPart.tileOffset.y };
    ...
    bool isConditionValid( const std::vector<fheroes::Point> & offsets, const fheroes::Point & mainTilePos,
                           const std::function<bool( const Maps::Tile & tile )> & condition )
    {
        ...
        for ( const auto & offset : offsets ) {
            const fheroes::Point temp{ mainTilePos.x + offset.x, mainTilePos.y + offset.y };
    ...
    bool checkConditionForUsedTiles( const Maps::ObjectInfo & info, const fheroes::Point & mainTilePos,
                                     const std::function<bool( const Maps::Tile & tile )> & condition )
    {
    ...
    bool verifyTerrainPlacement( const fheroes::Point & tilePos, const Maps::ObjectGroup groupType, const int32_t objectType, std::string & errorMessage )
    {
        switch ( groupType ) {
        ...
        case Maps::ObjectGroup::KINGDOM_TOWNS: {
            ...
            const int32_t basementId = fheroes::getTownBasementId( groundType );
    ...
    bool verifyObjectPlacement( const fheroes::Point & tilePos, const Maps::ObjectGroup groupType, const int32_t objectType, std::string & errorMessage )
    {
        switch ( groupType ) {
        case Maps::ObjectGroup::ADVENTURE_ARTIFACTS:
        case Maps::ObjectGroup::ADVENTURE_DWELLINGS:
...
namespace Interface
{
    void EditorInterface::reset()
    {
        ...
        const int32_t xOffset = display.width() - fheroes::borderWidthPx - fheroes::radarWidthPx;
        _radar.SetPos( xOffset, fheroes::borderWidthPx );

        _editorPanel.setPos( xOffset, _radar.GetArea().y + _radar.GetArea().height
                                          + ( ( display.height() > fheroes::Display::DEFAULT_HEIGHT + fheroes::borderWidthPx ) ? fheroes::borderWidthPx : 0 ) );

        const fheroes::Point prevCenter = _gameArea.getCurrentCenterInPixels();
        const fheroes::Rect prevRoi = _gameArea.GetROI();

        _gameArea.SetAreaPosition( fheroes::borderWidthPx, fheroes::borderWidthPx, display.width() - fheroes::radarWidthPx - 3 * fheroes::borderWidthPx,
                                   display.height() - 2 * fheroes::borderWidthPx );

        const fheroes::Rect newRoi = _gameArea.GetROI();

        if ( prevRoi == fheroes::Rect{} ) {
        ...
        _gameArea.SetCenterInPixels( prevCenter + fheroes::Point( newRoi.x + newRoi.width / 2, newRoi.y + newRoi.height / 2 )
                                     - fheroes::Point( prevRoi.x + prevRoi.width / 2, prevRoi.y + prevRoi.height / 2 ) );
    }
    ...
    void EditorInterface::redraw( const uint32_t force )
    {
        ...
        if ( combinedRedraw & REDRAW_GAMEAREA ) {
            ...
            if ( _warningMessage.isValid() ) {
                const fheroes::Rect & roi = _gameArea.GetROI();

                fheroes::Text text{ _warningMessage.message(), fheroes::FontType::normalWhite() };
            ...
            if ( _editorPanel.showAreaSelectRect() && ( _tileUnderCursor > -1 ) ) {
                const fheroes::Rect brushSize = _editorPanel.getBrushArea();

                if ( brushSize.width > 0 && brushSize.height > 0 ) {
                    const fheroes::Point indices = getBrushAreaIndicies( brushSize, _tileUnderCursor );
    ...
    fheroes::GameMode EditorInterface::startEdit()
    {
        ...
        const fheroes::ScreenPaletteRestorer restorer;
        ...
        while ( res == fheroes::GameMode::CANCEL ) {
            if ( !le.HandleEvents( Game::isDelayNeeded( delayTypes ), true ) ) {
                if ( EventExit() == fheroes::GameMode::QUIT_GAME ) {
                    res = fheroes::GameMode::QUIT_GAME;
            ...
            if ( le.isAnyKeyPressed() ) {
                if ( HotKeyPressEvent( Game::HotKeyEvent::MAIN_MENU_QUIT ) || HotKeyPressEvent( Game::HotKeyEvent::DEFAULT_CANCEL ) ) {
                    ...
                    const int returnValue
                        = fheroes::showStandardTextMessage( _( "Editor" ), _( "Do you wish to return to the game's Main Menu? All unsaved changes will be lost." ),
                                                             Dialog::YES | Dialog::NO );

                    if ( returnValue == Dialog::YES ) {
                        res = fheroes::GameMode::MAIN_MENU;
                    }
                }
            ...
            if ( res != fheroes::GameMode::CANCEL ) {
                break;
            }
            ...
            // Mouse is captured by the game area for scrolling by dragging
                ...
                if ( fheroes::cursor().isFocusActive() && conf.ScrollSpeed() != SCROLL_SPEED_NONE ) {
            ...
            if ( isCursorOverGameArea ) {
                ...
                const fheroes::Point posInGameArea = _gameArea.getInternalPosition( le.getMouseCursorPos() );
                const fheroes::Point tilePos{ posInGameArea.x / fheroes::tileWidthPx, posInGameArea.y / fheroes::tileWidthPx };
                const bool isValidTile = ( tilePos.x >= 0 && tilePos.y >= 0 && tilePos.x < world.w() && tilePos.y < world.h() );
                const bool isBrushEmpty = ( _editorPanel.getBrushArea() == fheroes::Rect() );
                ...
                if ( isValidTile ) {
                    ...
                    const fheroes::Point clampedPoint{ std::clamp( tilePos.x, 0, world.w() - 1 ), std::clamp( tilePos.y, 0, world.h() - 1 ) };
            ...
            if ( _areaSelectionStartTileId > -1 && le.isMouseLeftButtonReleased() ) {
                if ( isCursorOverGameArea && _tileUnderCursor > -1 && _editorPanel.getBrushArea().width == 0 ) {
                    if ( _editorPanel.isTerrainEdit() ) {
                        ...
                        fheroes::ActionCreator action( _historyManager, _mapFormat );
            ...
            assert( res == fheroes::GameMode::CANCEL );
    ...
    fheroes::GameMode EditorInterface::eventLoadMap()
    {
        return Dialog::YES
                       == fheroes::showStandardTextMessage( "", _( "Are you sure you want to load a new map? (Any unsaved changes to the current map will be lost.)" ),
                                                             Dialog::YES | Dialog::NO )
                   ? fheroes::GameMode::EDITOR_LOAD_MAP
                   : fheroes::GameMode::CANCEL;
    }

    fheroes::GameMode EditorInterface::eventNewMap()
    {
        return Dialog::YES
                       == fheroes::showStandardTextMessage( "", _( "Are you sure you want to create a new map? (Any unsaved changes to the current map will be lost.)" ),
                                                             Dialog::YES | Dialog::NO )
                   ? fheroes::GameMode::EDITOR_NEW_MAP
                   : fheroes::GameMode::CANCEL;
    }

    fheroes::GameMode EditorInterface::eventFileDialog()
    {
        ...
        const fheroes::ButtonBase & buttonNewMap = optionButtons.button( 0 );
        const fheroes::ButtonBase & buttonLoadMap = optionButtons.button( 1 );
        const fheroes::ButtonBase & buttonStartMap = optionButtons.button( 2 );
        const fheroes::ButtonBase & buttonSaveMap = optionButtons.button( 3 );
        const fheroes::ButtonBase & buttonMainMenu = optionButtons.button( 4 );
        const fheroes::ButtonBase & buttonQuit = optionButtons.button( 5 );
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.MouseClickLeft( buttonNewMap.area() ) || Game::HotKeyPressEvent( Game::HotKeyEvent::EDITOR_NEW_MAP_MENU ) ) {
                if ( eventNewMap() == fheroes::GameMode::EDITOR_NEW_MAP ) {
                    return fheroes::GameMode::EDITOR_NEW_MAP;
                }
            }
            if ( le.MouseClickLeft( buttonLoadMap.area() ) || Game::HotKeyPressEvent( Game::HotKeyEvent::MAIN_MENU_LOAD_GAME ) ) {
                if ( eventLoadMap() == fheroes::GameMode::EDITOR_LOAD_MAP ) {
                    return fheroes::GameMode::EDITOR_LOAD_MAP;
                }
            }
            ...
            if ( le.MouseClickLeft( buttonMainMenu.area() ) || Game::HotKeyPressEvent( Game::HotKeyEvent::EDITOR_TO_GAME_MAIN_MENU ) ) {
                ...
                if ( conf.getCurrentMapInfo().colorsAvailableForHumans == 0 ) {
                    fheroes::showStandardTextMessage( _( "Unplayable Map" ),
                    ...
                    if ( fheroes::
                             showStandardTextMessage( _( "Start Map" ),
                                                      ...
                                                      Dialog::YES | Dialog::NO )
                         == Dialog::YES ) {
                        return fheroes::GameMode::NEW_STANDARD;
                    }
                }
            }
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonNewMap.area() ) ) {
                ...
                fheroes::showStandardTextMessage( _( "New Map" ), _( "Create a new map from scratch." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonLoadMap.area() ) ) {
                fheroes::showStandardTextMessage( _( "Load Map" ), _( "Load an existing map." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonSaveMap.area() ) ) {
                fheroes::showStandardTextMessage( _( "Save Map" ), _( "Save the current map." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonQuit.area() ) ) {
                fheroes::showStandardTextMessage( _( "Quit" ), _( "Quit out of the map editor." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonMainMenu.area() ) ) {
                fheroes::showStandardTextMessage( _( "Main Menu" ), _( "Return to the game's Main Menu." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonStartMap.area() ) ) {
                fheroes::showStandardTextMessage( _( "Start Map" ), _( "Leave the Editor and play the map in the Standard Game mode." ), Dialog::ZERO );
            }
        ...
        if ( _editorPanel.isDetailEdit() ) {
            ...
            for ( const auto & object : _mapFormat.tiles[tileIndex].objects ) {
                ...
                if ( objectType == MP2::OBJ_HERO || objectType == MP2::OBJ_JAIL ) {
                    ...
                    const fheroes::Text body{ std::move( header ), fheroes::FontType::normalWhite() };
                    if ( Dialog::inputString( fheroes::Text{}, body, signText, 0, true, _mapFormat.mainLanguage ) && originalMessage != signText ) {
                        fheroes::ActionCreator action( _historyManager, _mapFormat );
                    ...
                    std::unique_ptr<const fheroes::MonsterDialogElement> monsterUi = nullptr;

                    if ( tempMonster.isValid() ) {
                        monsterUi = std::make_unique<const fheroes::MonsterDialogElement>( tempMonster );
                    }
                    ...
                    if ( objectInfo.objectType == MP2::OBJ_RANDOM_ULTIMATE_ARTIFACT ) {
                        ...
                        fheroes::ArtifactDialogElement( static_cast<int>( objectInfo.metadata[0] ) ).showPopup( Dialog::OK );
                    }
                    else {
                    ...
                    fheroes::showStandardTextMessage( MP2::StringObject( objectType ), std::move( str ), Dialog::OK );
                }
            ...
            if ( brushSize.width > 0 ) {
                const fheroes::Point indices = getBrushAreaIndicies( brushSize, tileIndex );
    ...
    void EditorInterface::_handleObjectMouseLeftClick( Maps::Tile & tile )
    {
        ...
        const fheroes::Point tilePos = tile.GetCenter();
    ...
    bool EditorInterface::loadMap( const std::string & filePath )
    {
        if ( !Maps::Map_Format::loadMap( filePath, _mapFormat ) ) {
            fheroes::showStandardTextMessage( _( "Error" ), "Failed to load the map.", Dialog::OK );
            return false;
        }

        if ( !Maps::readMapInEditor( _mapFormat ) ) {
            fheroes::showStandardTextMessage( _( "Error" ), "Failed to read the map.", Dialog::OK );
            return false;
        }
    ...
    {
        if ( !Maps::updateMapPlayers( _mapFormat ) ) {
            fheroes::showStandardTextMessage( _( "Error" ), _( "The map is corrupted." ), Dialog::OK );
            return;
        }

        const std::string dataPath = System::GetDataDirectory( "fheroes" );
        if ( dataPath.empty() ) {
            fheroes::showStandardTextMessage( _( "Error" ), _( "Unable to locate data directory to save the map." ), Dialog::OK );
            return;
        }
        ...
        if ( !System::IsDirectory( mapDirectory ) && !System::MakeDirectory( mapDirectory ) ) {
            fheroes::showStandardTextMessage( _( "Error" ), _( "Unable to create a directory to save the map." ), Dialog::OK );
            return;
        }
        ...
        // Since the name of the map directory can be in arbitrary case, we need to get its real case-sensitive name first
        {
            ...
            if ( !System::GetCaseInsensitivePath( mapDirectory, correctedMapDirectory ) ) {
                fheroes::showStandardTextMessage( _( "Error" ), _( "Unable to locate a directory to save the map." ), Dialog::OK );
                return;
            }
        ...
        while ( true ) {
            ...
            if ( !System::IsFile( fullPath )
                 || fheroes::showStandardTextMessage( "", _( "Are you sure you want to overwrite the existing map?" ), Dialog::YES | Dialog::NO ) == Dialog::YES ) {
                break;
            }
        }
        ...
        fheroes::showStandardTextMessage( _( "Error" ), _( "Failed to save the map." ), Dialog::OK );
    }
    ...
    void EditorInterface::_validateObjectsOnTerrainUpdate()
    {
        ...
        for ( size_t i = 0; i < _mapFormat.tiles.size(); ++i ) {
            const fheroes::Point pos{ static_cast<int32_t>( i ) % world.w(), static_cast<int32_t>( i ) / world.w() };
