...
namespace
{
    ...
    const fheroes::Size rumorArea{ 550, 300 };
    ...
    public:
        ...
        RumorListBox( const fheroes::Point & pt, const fheroes::SupportedLanguage language )
            : ListBox( pt )
            , _language( language )
        {
            // Do nothing.
        ...
        {
            fheroes::Text text{ rumor, ( current ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() ), _language };
...
namespace Editor
{
    bool openRumorWindow( std::vector<std::string> & rumors, const fheroes::SupportedLanguage language )
    {
        ...
        fheroes::StandardWindow background{ rumorArea.width + 50, rumorArea.height + 100, true, display };

        const fheroes::Rect windowArea{ background.activeArea() };

        int32_t offsetY = windowArea.y + elementOffset;

        const fheroes::Text title( _( "Rumors" ), fheroes::FontType::normalYellow() );
        title.draw( windowArea.x + ( windowArea.width - title.width() ) / 2, offsetY, display );

        offsetY += title.height() + elementOffset;

        const fheroes::Rect rumorsRoi{ windowArea.x + elementOffset, offsetY, rumorArea.width, rumorArea.height };
        ...
        rumorList.setScrollBarImage( fheroes::AGG::GetICN( listIcnId, 4 ) );
        ...
        fheroes::Button buttonAdd( rumorsRoi.x, rumorsRoi.y + rumorsRoi.height + 5, minibuttonIcnId, 13, 14 );
        buttonAdd.draw();

        fheroes::Button buttonEdit( rumorsRoi.x + buttonOffset, rumorsRoi.y + rumorsRoi.height + 5, minibuttonIcnId, 15, 16 );
        buttonEdit.draw();

        fheroes::Button buttonDelete( rumorsRoi.x + rumorArea.width - buttonWidth, rumorsRoi.y + rumorsRoi.height + 5, minibuttonIcnId, 17, 18 );
        buttonDelete.draw();
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.MouseClickLeft( buttonAdd.area() ) ) {
                ...
                const fheroes::Text body{ _( "Rumor:" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, newRumor, longestRumor, true, language ) ) {
                    if ( std::any_of( rumors.begin(), rumors.end(), [&newRumor]( const auto & rumor ) { return rumor == newRumor; } ) ) {
                        fheroes::showStandardTextMessage( _( "Rumor" ), _( "This rumor already exists in the list." ), Dialog::OK );
                        continue;
                    }
                ...
                const fheroes::Text body{ _( "Rumor:" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, temp, longestRumor, true, language ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to save the rumors." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonAdd.area() ) ) {
                fheroes::showStandardTextMessage( _( "Add Rumor" ), _( "Add an additional rumor." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonEdit.area() ) ) {
                fheroes::showStandardTextMessage( _( "Edit Rumor" ), _( "Edit an existing rumor." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonDelete.area() ) ) {
                fheroes::showStandardTextMessage( _( "Delete Rumor" ), _( "Delete an existing rumor." ), Dialog::ZERO );
            }
