...
namespace
{
    ...
    std::vector<fheroes::Point> getCampaignIconOffsets( const int campaignId, const bool archibaldLowerBetrayalBranch = false )
    {
        switch ( campaignId ) {
    ...
    void DrawCampaignScenarioIcon( const int icnId, const uint32_t iconIdx, const fheroes::Point & offset, const int posX, const int posY )
    {
        const fheroes::Sprite & icon = fheroes::AGG::GetICN( icnId, iconIdx );
        fheroes::Blit( icon, fheroes::Display::instance(), offset.x + posX, offset.y + posY );
    }

    void addScenarioButton( fheroes::ButtonGroup & buttonGroup, const int buttonId, const Campaign::ScenarioInfoId & scenarioInfo,
                            const std::vector<Campaign::ScenarioInfoId> & availableMaps, const std::vector<Campaign::ScenarioInfoId> & clearedMaps,
                            const fheroes::Point & trackOffset, const bool archibaldLowerBetrayalBranch )
    {
        const int deltaY = 42;
        const int deltaX = 37;

        const std::vector<fheroes::Point> & iconOffsets = getCampaignIconOffsets( scenarioInfo.campaignId, archibaldLowerBetrayalBranch );
        ...
        fheroes::Point offset = iconOffsets[scenarioInfo.scenarioId];
        offset.x *= deltaX;
        offset.y *= deltaY;
        ...
        if ( std::find( availableMaps.begin(), availableMaps.end(), scenarioInfo ) != availableMaps.end() ) {
            const fheroes::Sprite & availableIcon = fheroes::AGG::GetICN( iconsId, iconStatusOffset + SCENARIO_ICON_AVAILABLE );
            const fheroes::Sprite & selectedIcon = fheroes::AGG::GetICN( iconsId, selectedIconIdx );
    ...
    void DrawCampaignScenarioIcons( fheroes::ButtonGroup & buttonGroup, const Campaign::CampaignData & campaignData, const fheroes::Point & top,
                                    const int chosenScenarioId )
    {
        ...
        const fheroes::Sprite & track = fheroes::AGG::GetICN( campaignTrack, 0 );
        const fheroes::Point trackOffset( top.x + track.x(), top.y + track.y() );
        fheroes::Blit( track, fheroes::Display::instance(), trackOffset.x, trackOffset.y );
    ...
    void DrawCampaignScenarioDescription( const Campaign::ScenarioData & scenario, const fheroes::Point & top )
    {
        fheroes::Display & display = fheroes::Display::instance();

        const std::vector<Campaign::ScenarioBonusData> & bonuses = scenario.getBonuses();
        fheroes::Text mapName( scenario.getScenarioName(), fheroes::FontType::normalWhite() );
        ...
        const fheroes::Text campaignMapId( std::to_string( scenarioId ), fheroes::FontType::normalWhite() );
        campaignMapId.draw( top.x + 172 - campaignMapId.width() / 2, top.y + 99 - campaignMapId.height() / 2, display );

        fheroes::Text mapDescription( scenario.getDescription(), fheroes::FontType::normalWhite() );
        ...
        const fheroes::Point initialOffset{ top.x + 425, top.y + 211 };

        for ( size_t i = 0; i < bonuses.size(); ++i ) {
            fheroes::Text choice( bonuses[i].getName(), fheroes::FontType::normalWhite() );
    ...
    bool displayScenarioBonusPopupWindow( const Campaign::ScenarioData & scenario, const fheroes::Point & top )
    {
        const std::vector<Campaign::ScenarioBonusData> & bonuses = scenario.getBonuses();
        if ( bonuses.empty() ) {
            // Nothing to process.
        ...
        for ( size_t i = 0; i < bonuses.size(); ++i ) {
            if ( le.isMouseRightButtonPressedInArea( { top.x + 414, top.y + 198 + 22 * static_cast<int>( i ), 200, 22 } ) ) {
                fheroes::showScenarioBonusDataPopupWindow( bonuses[i] );
                return true;
            }
        }
    ...
    void drawObtainedCampaignAwards( const Campaign::CampaignSaveData & campaignSaveData, const fheroes::Point & top )
    {
        ...
        if ( awardCount == 0 ) {
            const fheroes::Text text( _( "None" ), fheroes::FontType::normalWhite() );
        ...
        for ( size_t i = 0; i < indexEnd; ++i ) {
            if ( i < 3 ) {
                award.set( obtainedAwards[i].getName(), fheroes::FontType::normalWhite() );
            }
            else {
                // if we have exactly 4 obtained awards, display the fourth award, otherwise show "and more..."
                award.set( awardCount == 4 ? obtainedAwards[i].getName() : std::string( _( "and more..." ) ), fheroes::FontType::normalWhite() );
            }
    ...
    bool displayScenarioAwardsPopupWindow( const Campaign::CampaignSaveData & campaignSaveData, const fheroes::Point & top )
    {
        if ( isBetrayalScenario( campaignSaveData.getCurrentScenarioInfoId() ) ) {
            return false;
        }
        ...
        for ( size_t i = 0; i < indexEnd; ++i ) {
            if ( le.isMouseRightButtonPressedInArea( { top.x + 414, top.y + 100 - yOffset / 2 + yOffset * static_cast<int>( i ), 200, yOffset } ) ) {
                fheroes::showAwardDataPopupWindow( obtainedAwards[i] );
                return true;
            }
        }
    ...
    void drawCampaignNameHeader( const int campaignId, fheroes::Image & output, const fheroes::Point & offset )
    {
        ...
        const fheroes::Sprite & header = fheroes::AGG::GetICN( ICN::X_CMPEXT, campaignNameHeader );
        fheroes::Blit( header, output, offset.x + 24, offset.y + 25 );
    }
    ...
    int32_t setCampaignDifficulty( int32_t currentDifficulty )
    {
        ...
        const fheroes::StandardWindow frameborder( 300, 284, true );
        const fheroes::Rect & windowRoi = frameborder.activeArea();

        const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();
        const int buttonIcnId = isEvilInterface ? ICN::BUTTON_SMALL_OKAY_EVIL : ICN::BUTTON_SMALL_OKAY_GOOD;
        const fheroes::Sprite & buttonSprite = fheroes::AGG::GetICN( buttonIcnId, 0 );

        fheroes::Display & display = fheroes::Display::instance();

        const fheroes::Rect buttonMaxRoi( windowRoi.x + 5, windowRoi.y, windowRoi.width - 10, windowRoi.height - 5 );
        fheroes::ButtonSprite buttonOk = fheroes::makeButtonWithShadow( buttonMaxRoi.x + ( buttonMaxRoi.width - buttonSprite.width() ) / 2,
                                                                          buttonMaxRoi.y + buttonMaxRoi.height - buttonSprite.height(),
                                                                          fheroes::AGG::GetICN( buttonIcnId, 0 ), fheroes::AGG::GetICN( buttonIcnId, 1 ), display );

        buttonOk.draw();

        const fheroes::Text caption( _( "Campaign Difficulty" ), fheroes::FontType::normalYellow() );
        ...
        const std::array<fheroes::Rect, 3> copyFromArea{ fheroes::Rect{ 20, 94, fullIconSize, fullIconSize }, fheroes::Rect{ 97, 94, fullIconSize, fullIconSize },
                                                          fheroes::Rect{ 173, 94, fullIconSize, fullIconSize } };

        const std::array<fheroes::Point, 3> copyToOffset{ fheroes::Point{ pawnIconOffsetX, windowRoi.y + 40 }, fheroes::Point{ horseIconOffsetX, windowRoi.y + 40 },
                                                           fheroes::Point{ rookIconOffsetX, windowRoi.y + 40 } };

        const fheroes::Sprite & chessIcon = fheroes::AGG::GetICN( ICN::NGHSBKG, 0 );

        for ( size_t i = 0; i < copyToOffset.size(); ++i ) {
            fheroes::Copy( chessIcon, copyFromArea[i].x, copyFromArea[i].y, display, copyToOffset[i].x, copyToOffset[i].y, copyFromArea[i].width,
        ...
        if ( isEvilInterface ) {
            ...
            for ( size_t i = 0; i < copyToOffset.size(); ++i ) {
                fheroes::ApplyPalette( display, copyToOffset[i].x, copyToOffset[i].y, display, copyToOffset[i].x, copyToOffset[i].y, copyFromArea[i].width,
                                        copyFromArea[i].height, goodToEvilPalette );
            }
        }

        const fheroes::Sprite & selectionImage = fheroes::AGG::GetICN( ICN::NGEXTRA, 62 );

        fheroes::MovableSprite selection( selectionImage );
        ...
        const std::array<fheroes::Rect, 3> difficultyArea{ fheroes::Rect( copyToOffset[0].x + 1, windowRoi.y + 37, selectionImage.width(), selectionImage.height() ),
                                                            fheroes::Rect( copyToOffset[1].x + 1, windowRoi.y + 37, selectionImage.width(), selectionImage.height() ),
                                                            fheroes::Rect( copyToOffset[2].x + 1, windowRoi.y + 37, selectionImage.width(), selectionImage.height() ) };
        ...
        const fheroes::Point textOffset{ windowRoi.x + 8, windowRoi.y + 140 };

        fheroes::Text description( currentDescription, fheroes::FontType::normalWhite() );
        fheroes::ImageRestorer restorer( display, textOffset.x, textOffset.y, textWidth, description.height( textWidth ) );
        description.setUniformVerticalAlignment( false );
        description.draw( textOffset.x, textOffset.y, textWidth, display );

        const fheroes::Text easyName( getCampaignDifficultyText( Campaign::CampaignDifficulty::Easy ), fheroes::FontType::normalWhite() );
        const fheroes::Text normalName( getCampaignDifficultyText( Campaign::CampaignDifficulty::Normal ), fheroes::FontType::normalWhite() );
        const fheroes::Text hardName( getCampaignDifficultyText( Campaign::CampaignDifficulty::Hard ), fheroes::FontType::normalWhite() );
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonOk.area() ) ) {
                ...
                fheroes::showStandardTextMessage( getCampaignDifficultyText( Campaign::CampaignDifficulty::Easy ), easyDescription, Dialog::ZERO );
                updateInfo = true;
            }
            else if ( le.isMouseRightButtonPressedInArea( difficultyArea[1] ) ) {
                fheroes::showStandardTextMessage( getCampaignDifficultyText( Campaign::CampaignDifficulty::Normal ), normalDescription, Dialog::ZERO );
                updateInfo = true;
            }
            else if ( le.isMouseRightButtonPressedInArea( difficultyArea[2] ) ) {
                fheroes::showStandardTextMessage( getCampaignDifficultyText( Campaign::CampaignDifficulty::Hard ), hardDescription, Dialog::ZERO );
                updateInfo = true;
            }
            ...
            if ( updateInfo ) {
                ...
                description.set( currentDescription, fheroes::FontType::normalWhite() );
                restorer.reset();
...
bool Game::isPriceOfLoyaltyCampaignPresent()
{
    ...
    if ( fheroes::AGG::GetICN( ICN::X_LOADCM, 0 ).empty() || fheroes::AGG::GetICN( ICN::X_IVY, 0 ).empty() ) {
        return false;
    }
...
fheroes::GameMode Game::CompleteCampaignScenario( const bool isLoadingSaveFile )
{
    ...
    if ( campaignData.isLastScenario( lastCompletedScenarioInfo ) ) {
        ...
        // fheroes::PlayMusic is run here in order to start playing before displaying the high score.
        AudioManager::PlayMusicAsync( MUS::VICTORY, Music::PlaybackMode::REWIND_AND_PLAY_INFINITE );
        return fheroes::GameMode::HIGHSCORES_CAMPAIGN;
    }
    ...
    return fheroes::GameMode::SELECT_CAMPAIGN_SCENARIO;
}

fheroes::GameMode Game::SelectCampaignScenario( const fheroes::GameMode prevMode, const bool allowToRestart )
{
    fheroes::Display & display = fheroes::Display::instance();
    display.fill( 0 );
    ...
    const fheroes::GameInterfaceTypeRestorer gameInterfaceRestorer( chosenCampaignID == Campaign::ROLAND_CAMPAIGN ? InterfaceType::GOOD : InterfaceType::EVIL );
    ...
    const fheroes::Sprite & backgroundImage = fheroes::AGG::GetICN( backgroundIconID, 0 );
    const int32_t backgroundImageWidth = backgroundImage.width();
    const fheroes::Point top( ( display.width() - backgroundImageWidth ) / 2, ( display.height() - backgroundImage.height() ) / 2 );

    fheroes::Blit( backgroundImage, display, top.x, top.y );
    ...
    const int32_t endOfViewIntroPlacement = viewIntroPlacement + fheroes::AGG::GetICN( buttonIconID, 0 ).width();
    const int32_t cancelPlacement = top.x + backgroundImageWidth - backgroundMargin - fheroes::AGG::GetICN( buttonIconID, 6 ).width();
    const int32_t spaceBetweenViewIntroAndCancel = cancelPlacement - endOfViewIntroPlacement;
    const int32_t difficultyButtonWidth = fheroes::AGG::GetICN( buttonIconID, 8 ).width();
    const uint32_t okayRestartIndex = allowToRestart ? 2 : 4;
    const int32_t middleButtonsMargin
        = ( spaceBetweenViewIntroAndCancel - difficultyButtonWidth - ( fheroes::AGG::GetICN( buttonIconID, okayRestartIndex ).width() ) ) / 3;
    ...
    fheroes::Button buttonViewIntro( viewIntroPlacement, buttonOffsetY, buttonIconID, 0, 1 );
    fheroes::Button buttonRestart( okRestartPlacement, buttonOffsetY, buttonIconID, 2, 3 );
    fheroes::Button buttonOk( okRestartPlacement, buttonOffsetY, buttonIconID, 4, 5 );
    fheroes::Button buttonCancel( cancelPlacement, buttonOffsetY, buttonIconID, 6, 7 );
    fheroes::Button buttonDifficulty( difficultyPlacement, buttonOffsetY, buttonIconID, 8, 9 );

    fheroes::addGradientShadow( fheroes::AGG::GetICN( buttonIconID, 0 ), display, { viewIntroPlacement, buttonOffsetY }, { -5, 5 } );
    fheroes::addGradientShadow( fheroes::AGG::GetICN( buttonIconID, okayRestartIndex ), display, { okRestartPlacement, buttonOffsetY }, { -5, 5 } );
    fheroes::addGradientShadow( fheroes::AGG::GetICN( buttonIconID, 6 ), display, { cancelPlacement, buttonOffsetY }, { -5, 5 } );
    fheroes::addGradientShadow( fheroes::AGG::GetICN( buttonIconID, 8 ), display, { difficultyPlacement, buttonOffsetY }, { -5, 5 } );

    // create scenario bonus choice buttons
    fheroes::ButtonGroup buttonChoices;
    fheroes::OptionButtonGroup optionButtonGroup;

    std::optional<int32_t> scenarioBonusId;
    const std::vector<Campaign::ScenarioBonusData> & bonusChoices = scenario.getBonuses();

    const fheroes::Point optionButtonOffset( 590, 199 );
    const int32_t optionButtonStep = 22;

    const fheroes::Sprite & pressedButton = fheroes::AGG::GetICN( ICN::CAMPXTRG, 8 );
    fheroes::Sprite releaseButton( pressedButton.width(), pressedButton.height(), pressedButton.x(), pressedButton.y() );
    fheroes::Copy( backgroundImage, optionButtonOffset.x + pressedButton.x(), optionButtonOffset.y + pressedButton.y(), releaseButton, 0, 0, releaseButton.width(),
    ...
    const fheroes::Text textDaysSpent( std::to_string( campaignSaveData.getDaysPassed() ), fheroes::FontType::normalWhite() );
    textDaysSpent.draw( top.x + 582 - textDaysSpent.width() / 2, top.y + 33, display );

    DrawCampaignScenarioDescription( scenario, top );

    fheroes::Rect scenarioTitleArea;
    switch ( chosenCampaignID ) {
    case Campaign::ARCHIBALD_CAMPAIGN:
    case Campaign::ROLAND_CAMPAIGN:
    ...
    fheroes::ButtonGroup selectableScenarioButtons;
    ...
    if ( validateDisplayFadeIn() ) {
        fheroes::fadeInDisplay( { top.x, top.y, backgroundImage.width(), backgroundImage.height() }, false );
    }

    std::vector<fheroes::Rect> choiceArea( bonusChoiceCount );
    for ( uint32_t i = 0; i < bonusChoiceCount; ++i ) {
    ...
    const fheroes::Rect areaDaysSpent{ top.x + 413, top.y + 27, 201, 25 };
    ...
    while ( le.HandleEvents() ) {
        ...
        for ( uint32_t i = 0; i < bonusChoiceCount; ++i ) {
            if ( le.isMouseLeftButtonPressedInArea( choiceArea[i] ) || ( i < hotKeyBonusChoice.size() && HotKeyPressEvent( hotKeyBonusChoice[i] ) ) ) {
                scenarioBonusId = fheroes::checkedCast<int32_t>( i );
                buttonChoices.button( i ).press();
                optionButtonGroup.draw();
                display.render();
        ...
        if ( le.MouseClickLeft( buttonCancel.area() ) || HotKeyPressEvent( HotKeyEvent::DEFAULT_CANCEL ) ) {
            ...
            fheroes::fadeOutDisplay( { top.x, top.y, backgroundImage.width(), backgroundImage.height() }, false );

            if ( prevMode == fheroes::GameMode::LOAD_CAMPAIGN || prevMode == fheroes::GameMode::MAIN_MENU ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Okay" ), _( "Start the selected scenario." ), Dialog::ZERO );
            updateDisplay = true;
        }
        else if ( le.isMouseRightButtonPressedInArea( buttonViewIntro.area() ) ) {
            fheroes::showStandardTextMessage( _( "View Intro" ), _( "View the intro video for the current state of the campaign." ), Dialog::ZERO );
            updateDisplay = true;
        }
        else if ( le.isMouseRightButtonPressedInArea( buttonDifficulty.area() ) ) {
            fheroes::showStandardTextMessage( _( "Campaign Difficulty" ),
            ...
            fheroes::showStandardTextMessage( _( "Restart" ), _( "Restart the current scenario." ), Dialog::ZERO );
            updateDisplay = true;
        }
            ...
            if ( ( !campaignSaveData.isStarting() || allowToRestart ) && currentDifficulty != campaignSaveData.getDifficulty()
                 && fheroes::showStandardTextMessage( _( "Difficulty" ),
            ...
            if ( restartButtonClicked
                 && fheroes::showStandardTextMessage( _( "Restart" ), _( "Are you sure you want to restart this scenario?" ), Dialog::YES | Dialog::NO )
                        == Dialog::NO ) {
                continue;
            }
            ...
            if ( mapInfo.version != GameVersion::RESURRECTION && fheroes::getCurrentLanguage() == fheroes::SupportedLanguage::French
                 && fheroes::getResourceLanguage() == fheroes::SupportedLanguage::French ) {
                fheroes::fixFrenchCharactersForMP2Map( mapInfo.name );
                fheroes::fixFrenchCharactersForMP2Map( mapInfo.description );
            }
            ...
            if ( !world.LoadMapMP2( mapInfo.filename, isSWCampaign ) ) {
                fheroes::showStandardTextMessage( _( "Campaign Scenario loading failure" ), _( "Please make sure that campaign files are correct and present." ),
                                                   Dialog::OK );
            ...
            return fheroes::GameMode::START_GAME;
        }
        else if ( le.MouseClickLeft( buttonViewIntro.area() ) || HotKeyPressEvent( HotKeyEvent::CAMPAIGN_VIEW_INTRO ) ) {
            AudioManager::ResetAudio();
            fheroes::ImageRestorer restorer( display, top.x, top.y, backgroundImage.width(), backgroundImage.height() );

            fheroes::fadeOutDisplay( restorer.rect(), false );
            ...
            fheroes::fadeInDisplay( restorer.rect(), false );
        }
        else if ( le.isMouseRightButtonPressedInArea( areaDaysSpent ) ) {
            fheroes::showStandardTextMessage( _( "Days spent" ), _( "The number of days spent on this campaign." ), Dialog::ZERO );
            updateDisplay = true;
        }
        else if ( le.isMouseRightButtonPressedInArea( scenarioTitleArea ) ) {
            fheroes::showStandardTextMessage( _( "Scenario Title" ), scenario.getScenarioName(), Dialog::ZERO );
            updateDisplay = true;
        }
