...
namespace
{
    fheroes::ButtonSprite createMoveButton( const int32_t icnId, const int32_t offsetX, const int32_t offsetY, const fheroes::Image & display )
    {
        const fheroes::Sprite & originalReleasedImage = fheroes::AGG::GetICN( icnId, 0 );
        const fheroes::Sprite & originalPressedImage = fheroes::AGG::GetICN( icnId, 1 );
        ...
        return fheroes::makeButtonWithShadow( offsetX + minX, offsetY + minY, originalReleasedImage, originalPressedImage, display, fheroes::Point( -3, 3 ) );
    }
...
public:
    ...
    void RedrawBackground( const fheroes::Rect & roi, fheroes::Image & image ) override
    {
        if ( _cachedBackground.empty() ) {
            _cachedBackground.resize( roi.width, roi.height );
            fheroes::Copy( image, roi.x, roi.y, _cachedBackground, 0, 0, roi.width, roi.height );
        }

        fheroes::Blit( _cachedBackground, 0, 0, image, roi.x, roi.y, roi.width, roi.height );
    }

    void RedrawItem( ArmyTroop & troop, const fheroes::Rect & roi, bool isSelected, fheroes::Image & image ) override
    {
        if ( !troop.isValid() )
            return;

        const fheroes::Text text( std::to_string( troop.GetCount() ), fheroes::FontType::smallWhite() );

        const fheroes::Sprite & mons32 = fheroes::AGG::GetICN( ICN::MONS32, troop.GetSpriteIndex() );
        fheroes::Rect srcrt( 0, 0, mons32.width(), mons32.height() );
        ...
        fheroes::Blit( mons32, srcrt.x, srcrt.y, image, roi.x + offsetX, roi.y + offsetY, srcrt.width, srcrt.height );
...
private:
    fheroes::Image _cachedBackground;
};
...
public:
    ...
    void RedrawItem( Artifact & arifact, const fheroes::Rect & roi, bool isSelected, fheroes::Image & image ) override
    {
        if ( !arifact.isValid() )
            return;

        const fheroes::Sprite & artifactSprite = fheroes::AGG::GetICN( ICN::ARTFX, arifact.IndexSprite32() );
        fheroes::Blit( artifactSprite, image, roi.x + 1, roi.y + 1 );
...
public:
    ...
    void RedrawItem( Skill::Secondary & skill, const fheroes::Rect & roi, fheroes::Image & image ) override
    {
        if ( !skill.isValid() )
            return;

        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::MINISS, skill.GetIndexSprite2() );
        fheroes::Blit( sprite, image, roi.x + ( roi.width - sprite.width() ) / 2, roi.y + ( roi.height - sprite.height() ) / 2 );

        const fheroes::Text text( std::to_string( skill.Level() ), fheroes::FontType::smallWhite() );
...
void Heroes::MeetingDialog( Heroes & otherHero )
{
    ...
    const fheroes::Sprite & backSprite = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );
    const fheroes::Point cur_pt( ( display.width() - backSprite.width() ) / 2, ( display.height() - backSprite.height() ) / 2 );
    fheroes::ImageRestorer restorer( display, cur_pt.x, cur_pt.y, backSprite.width(), backSprite.height() );

    const fheroes::Rect src_rt( 0, 0, fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT );

    // Fade-out game screen only for 640x480 resolution.
    const fheroes::Rect fadeRoi( src_rt + cur_pt );
    ...
    fheroes::Blit( backSprite, src_rt.x, src_rt.y, display, dst_pt.x, dst_pt.y, src_rt.width, src_rt.height );

    // shadow
    if ( !isDefaultScreenSize ) {
        fheroes::addGradientShadowForArea( display, dst_pt, backSprite.width(), backSprite.height(), fheroes::borderWidthPx );
    }
    ...
    fheroes::Text text( std::move( message ), fheroes::FontType::normalWhite() );
    ...
    text.set( std::move( message ), fheroes::FontType::smallWhite() );
    ...
    const fheroes::Sprite & portrait1 = GetPortrait( PORT_BIG );
    const fheroes::Rect hero1Area( dst_pt.x, dst_pt.y, portrait1.width(), portrait1.height() );
    PortraitRedraw( dst_pt.x, dst_pt.y, PORT_BIG, display );

    dst_pt.x = cur_pt.x + 445;
    dst_pt.y = cur_pt.y + portraitYOffset;
    const fheroes::Sprite & portrait2 = otherHero.GetPortrait( PORT_BIG );
    const fheroes::Rect hero2Area( dst_pt.x, dst_pt.y, portrait2.width(), portrait2.height() );
    ...
    // primary skill
    fheroes::ImageRestorer backPrimary( display, cur_pt.x + 255, cur_pt.y + 50, 130, 135 );
    ...
    fheroes::RedrawPrimarySkillInfo( cur_pt, &primskill_bar1, &primskill_bar2 );
    ...
    const fheroes::Sprite & moveButtonBackground = fheroes::AGG::GetICN( ICN::STONEBAK, 0 );
    fheroes::Blit( moveButtonBackground, 292, 270, display, cur_pt.x + 292, cur_pt.y + 270, 48, 44 );

    // The original resources do not have such animated buttons so we have to create those.
    fheroes::ButtonSprite moveArmyToHero2 = createMoveButton( ICN::SWAP_ARROW_LEFT_TO_RIGHT, cur_pt.x + 126, cur_pt.y + 319, display );
    fheroes::ButtonSprite moveArmyToHero1 = createMoveButton( ICN::SWAP_ARROW_RIGHT_TO_LEFT, cur_pt.x + 472, cur_pt.y + 319, display );
    fheroes::ButtonSprite swapArmies = createMoveButton( ICN::SWAP_ARROWS_CIRCULAR, cur_pt.x + 297, cur_pt.y + 268, display );

    fheroes::ImageRestorer armyCountBackgroundRestorerLeft( display, cur_pt.x + 36, cur_pt.y + 311, 223, 8 );
    fheroes::ImageRestorer armyCountBackgroundRestorerRight( display, cur_pt.x + 381, cur_pt.y + 311, 223, 8 );
    ...
    fheroes::Blit( moveButtonBackground, 292, 363, display, cur_pt.x + 292, cur_pt.y + 363, 48, 44 );
    fheroes::ButtonSprite moveArtifactsToHero2 = createMoveButton( ICN::SWAP_ARROW_LEFT_TO_RIGHT, cur_pt.x + 126, cur_pt.y + 426, display );
    fheroes::ButtonSprite moveArtifactsToHero1 = createMoveButton( ICN::SWAP_ARROW_RIGHT_TO_LEFT, cur_pt.x + 472, cur_pt.y + 426, display );
    fheroes::ButtonSprite swapArtifacts = createMoveButton( ICN::SWAP_ARROWS_CIRCULAR, cur_pt.x + 297, cur_pt.y + 361, display );

    // button exit
    dst_pt.x = cur_pt.x + 280;
    dst_pt.y = cur_pt.y + 428;
    fheroes::Button buttonExit( dst_pt.x, dst_pt.y, ICN::BUTTON_EXIT_HEROES_MEETING, 0, 1 );
    ...
    fheroes::fadeInDisplay( fadeRoi, !isDefaultScreenSize );

    const int32_t hero1ScoutAreaBonus = bag_artifacts.getTotalArtifactEffectValue( fheroes::ArtifactBonusType::AREA_REVEAL_DISTANCE );
    const int32_t hero2ScoutAreaBonus = otherHero.GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::AREA_REVEAL_DISTANCE );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( isHero1LeftClicked || le.MouseClickLeft( hero2Area ) ) {
            ...
            const fheroes::Rect restorerRoi( cur_pt.x - 2 * fheroes::borderWidthPx, cur_pt.y - fheroes::borderWidthPx, src_rt.width + 3 * fheroes::borderWidthPx,
                                              src_rt.height + 3 * fheroes::borderWidthPx );
            fheroes::ImageRestorer dialogRestorer( display, restorerRoi.x, restorerRoi.y, restorerRoi.width, restorerRoi.height );
            ...
            if ( !isDefaultScreenSize ) {
                fheroes::fadeOutDisplay( fadeRoi, true );
            }
        ...
        if ( le.isMouseRightButtonPressedInArea( hero1Area ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Artifact Transfer" ), std::move( message ), Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "Swap Artifacts" ), _( "Swap all artifacts between heroes." ), Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "Army Transfer" ), std::move( message ), Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "Swap Armies" ), _( "Swap armies between heroes." ), Dialog::ZERO );
        }
    }
    ...
    fheroes::fadeOutDisplay( fadeRoi, !isDefaultScreenSize );

    restorer.restore();

    // If the scout area bonus is increased with the new artifact we reveal the fog and update the radar.
    if ( hero1ScoutAreaBonus < bag_artifacts.getTotalArtifactEffectValue( fheroes::ArtifactBonusType::AREA_REVEAL_DISTANCE ) ) {
        Scout( GetIndex() );
        ScoutRadar();
    }
    if ( hero2ScoutAreaBonus < otherHero.GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::AREA_REVEAL_DISTANCE ) ) {
