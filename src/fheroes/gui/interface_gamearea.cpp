...
namespace
{
    ...
    static_assert( std::is_trivially_copyable<fheroes::ObjectRenderingInfo>::value, "This class is not trivially copyable anymore. Add std::move where required." );

    struct TileUnfitRenderObjectInfo
    {
        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> bottomImages;
        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> bottomBackgroundImages;
        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> topImages;

        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> lowPriorityBottomImages;
        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> highPriorityBottomImages;

        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> heroBackgroundImages;

        std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> shadowImages;
    };

    void populateStaticTileUnfitObjectInfo( TileUnfitRenderObjectInfo & tileUnfit, std::vector<fheroes::ObjectRenderingInfo> & imageInfo,
                                            std::vector<fheroes::ObjectRenderingInfo> & shadowInfo, const fheroes::Point & offset, const uint8_t alphaValue,
                                            const uint16_t fogDirection )
    {
        for ( auto & objectInfo : imageInfo ) {
            const fheroes::Point imagePos = objectInfo.tileOffset;
        ...
        for ( auto & objectInfo : shadowInfo ) {
            const fheroes::Point imagePos = objectInfo.tileOffset + offset;
    ...
    void populateStaticTileUnfitBackgroundObjectInfo( TileUnfitRenderObjectInfo & tileUnfit, std::vector<fheroes::ObjectRenderingInfo> & imageInfo,
                                                      const fheroes::Point & offset, const uint8_t alphaValue )
    {
        for ( auto & objectInfo : imageInfo ) {
            const fheroes::Point imagePos = objectInfo.tileOffset;
            objectInfo.alphaValue = alphaValue;
    ...
    void populateHeroObjectInfo( TileUnfitRenderObjectInfo & tileUnfit, const Heroes * hero, const uint16_t fogDirection )
    {
        ...
        const fheroes::Point & heroPos = hero->GetCenter();
        fheroes::Point nextHeroPos = heroPos;
        ...
        for ( auto & objectInfo : spriteShadowInfo ) {
            const fheroes::Point imagePos = objectInfo.tileOffset + heroPos;
    ...
    void renderImagesOnTiles( fheroes::Image & output, const std::map<fheroes::Point, std::deque<fheroes::ObjectRenderingInfo>> & images,
                              const Interface::GameArea & area )
    {
        for ( const auto & [offset, imgInfo] : images ) {
            for ( const auto & info : imgInfo ) {
                area.BlitOnTile( output, fheroes::AGG::GetICN( info.icnId, info.icnIndex ), info.area, info.imageOffset.x, info.imageOffset.y, offset, info.isFlipped,
                                 info.alphaValue );
            }
        }
    }
...
void Interface::GameArea::generate( const fheroes::Size & screenSize, const bool withoutBorders )
{
    if ( withoutBorders ) {
        SetAreaPosition( 0, 0, screenSize.width, screenSize.height );
    }
    else {
        SetAreaPosition( fheroes::borderWidthPx, fheroes::borderWidthPx, screenSize.width - fheroes::radarWidthPx - 3 * fheroes::borderWidthPx,
                         screenSize.height - 2 * fheroes::borderWidthPx );
    }
}
...
void Interface::GameArea::SetAreaPosition( int32_t x, int32_t y, int32_t w, int32_t h )
{
    ...
    if ( worldSize.width > w ) {
        _minLeftOffset = -( w / 2 ) - fheroes::tileWidthPx / 2;
    ...
    if ( worldSize.height > h ) {
        _minTopOffset = -( h / 2 ) - fheroes::tileWidthPx / 2;
    ...
    _visibleTileCount = { ( w + fheroes::tileWidthPx - 1 ) / fheroes::tileWidthPx + 1, ( h + fheroes::tileWidthPx - 1 ) / fheroes::tileWidthPx + 1 };

    _setCenterToTile( fheroes::Point( world.w() / 2, world.h() / 2 ) );
}

void Interface::GameArea::BlitOnTile( fheroes::Image & dst, const fheroes::Image & src, int32_t ox, int32_t oy, const fheroes::Point & mp, bool flip,
                                      uint8_t alpha ) const
{
    const fheroes::Point tileOffset = GetRelativeTilePosition( mp );

    const fheroes::Rect imageRoi{ tileOffset.x + ox, tileOffset.y + oy, src.width(), src.height() };
    const fheroes::Rect overlappedRoi = _windowROI ^ imageRoi;

    fheroes::AlphaBlit( src, overlappedRoi.x - imageRoi.x, overlappedRoi.y - imageRoi.y, dst, overlappedRoi.x, overlappedRoi.y, overlappedRoi.width,
                         overlappedRoi.height, alpha, flip );
}

void Interface::GameArea::BlitOnTile( fheroes::Image & dst, const fheroes::Image & src, const fheroes::Rect & srcRoi, int32_t ox, int32_t oy,
                                      const fheroes::Point & mp, bool flip, uint8_t alpha ) const
{
    const fheroes::Point tileOffset = GetRelativeTilePosition( mp );

    const fheroes::Rect imageRoi{ tileOffset.x + ox, tileOffset.y + oy, srcRoi.width, srcRoi.height };
    const fheroes::Rect overlappedRoi = _windowROI ^ imageRoi;

    fheroes::AlphaBlit( src, srcRoi.x + overlappedRoi.x - imageRoi.x, srcRoi.y + overlappedRoi.y - imageRoi.y, dst, overlappedRoi.x, overlappedRoi.y,
                         overlappedRoi.width, overlappedRoi.height, alpha, flip );
}

void Interface::GameArea::DrawTile( fheroes::Image & dst, const fheroes::Image & src, const fheroes::Point & mp ) const
{
    const fheroes::Point tileOffset = GetRelativeTilePosition( mp );

    const fheroes::Rect imageRoi{ tileOffset.x, tileOffset.y, src.width(), src.height() };
    const fheroes::Rect overlappedRoi = _windowROI ^ imageRoi;

    fheroes::Copy( src, overlappedRoi.x - imageRoi.x, overlappedRoi.y - imageRoi.y, dst, overlappedRoi.x, overlappedRoi.y, overlappedRoi.width, overlappedRoi.height );
}

void Interface::GameArea::Redraw( fheroes::Image & dst, int flag, bool isPuzzleDraw ) const
{
    const fheroes::Rect & tileROI = GetVisibleTileROI();
    ...
    for ( int32_t y = 0; y < tileROI.height; ++y ) {
        fheroes::Point offset( tileROI.x, tileROI.y + y );
    ...
    std::vector<fheroes::Point> ghostAnimationPos;
        ...
        for ( int32_t posX = roiToRenderMinX; posX < roiToRenderMaxX; ++posX ) {
            ...
            case MP2::OBJ_HERO: {
                if ( isEditor ) {
                    ...
                    std::vector<fheroes::ObjectRenderingInfo> temp;
    ...
    if ( drawRoutes && ( currentHero != nullptr ) && currentHero->GetPath().isShow() ) {
        ...
        const fheroes::Rect extendedVisibleRoi{ tileROI.x - 1, tileROI.y - 1, tileROI.width + 2, tileROI.height + 2 };

        for ( ; currentStep != path.end(); ++currentStep ) {
            const int32_t tileIndex = currentStep->GetIndex();
            const fheroes::Point & mp = Maps::GetPoint( tileIndex );
            ...
            const fheroes::Sprite & routeSprite = fheroes::AGG::GetICN( ( ( greenColorSteps < 0 ) ? ICN::ROUTERED : ICN::ROUTE ), routeSpriteIndex );
...
void Interface::GameArea::renderTileAreaSelect( fheroes::Image & dst, const int32_t startTile, const int32_t endTile, const bool isActionObject ) const
{
    if ( startTile < 0 || endTile < 0 ) {
        return;
    }

    const fheroes::Point startTileOffset = GetRelativeTilePosition( Maps::GetPoint( startTile ) );
    const fheroes::Point endTileOffset = GetRelativeTilePosition( Maps::GetPoint( endTile ) );
    ...
    const int32_t sizeX = fheroes::tileWidthPx + std::abs( startTileOffset.x - endTileOffset.x );
    const int32_t sizeY = fheroes::tileWidthPx + std::abs( startTileOffset.y - endTileOffset.y );

    const fheroes::Rect imageRoi{ startX, startY, sizeX, sizeY };
    const fheroes::Rect overlappedRoi = _windowROI ^ imageRoi;
    ...
    fheroes::Fill( dst, overlappedRoi.x, overlappedRoi.y, overlappedRoi.width, limitedLineHeight, color );
    fheroes::Fill( dst, overlappedRoi.x, overlappedRoi.y + 2, limitedLineWidth, overlappedRoi.height - 4, color );
    fheroes::Fill( dst, overlappedRoi.x, overlappedRoi.y + overlappedRoi.height - limitedLineHeight, overlappedRoi.width, limitedLineHeight, color );
    fheroes::Fill( dst, overlappedRoi.x + overlappedRoi.width - limitedLineWidth, overlappedRoi.y + 2, limitedLineWidth, overlappedRoi.height - 4, color );
}
...
fheroes::Image Interface::GameArea::GenerateUltimateArtifactAreaSurface( const int32_t index, const fheroes::Point & offset )
{
    if ( !Maps::isValidAbsIndex( index ) ) {
        DEBUG_LOG( DBG_GAME, DBG_WARN, "Ultimate artifact is not found on index " << index )
        return fheroes::Image();
    }

    fheroes::Image result;
    ...
    const fheroes::Point pt = Maps::GetPoint( index );
    gamearea.SetCenter( pt + offset );

    gamearea.Redraw( result, LEVEL_OBJECTS, true );

    const fheroes::Sprite & marker = fheroes::AGG::GetICN( ICN::ROUTE, 0 );
    const fheroes::Point markerPos( gamearea.GetRelativeTilePosition( pt ) - gamearea._middlePoint() - fheroes::Point( gamearea._windowROI.x, gamearea._windowROI.y )
                                     + fheroes::Point( result.width() / 2, result.height() / 2 ) );

    fheroes::Blit( marker, result, markerPos.x, markerPos.y + 8 );
    fheroes::ApplyPalette( result, PAL::GetPalette( PAL::PaletteType::TAN ) );
...
bool Interface::GameArea::mouseIndicatesFastScroll( const fheroes::Point & mousePosition )
{
...
void Interface::GameArea::QueueEventProcessing()
{
    ...
    const fheroes::Point & mousePosition = le.getMouseCursorPos();
    ...
    const fheroes::Point tileOffset = getInternalPosition( mousePosition );
    const fheroes::Point tilePos( ( tileOffset.x / fheroes::tileWidthPx ) * fheroes::tileWidthPx - _topLeftTileOffset.x + _windowROI.x,
                                   ( tileOffset.y / fheroes::tileWidthPx ) * fheroes::tileWidthPx - _topLeftTileOffset.y + _windowROI.x );

    const fheroes::Rect tileROI( tilePos.x, tilePos.y, fheroes::tileWidthPx, fheroes::tileWidthPx );
...
fheroes::Point Interface::GameArea::_getStartTileId() const
{
    const int32_t x
        = ( _topLeftTileOffset.x < 0 ? ( _topLeftTileOffset.x - fheroes::tileWidthPx - 1 ) / fheroes::tileWidthPx : _topLeftTileOffset.x / fheroes::tileWidthPx );
    const int32_t y
        = ( _topLeftTileOffset.y < 0 ? ( _topLeftTileOffset.y - fheroes::tileWidthPx - 1 ) / fheroes::tileWidthPx : _topLeftTileOffset.y / fheroes::tileWidthPx );

    return { x, y };
}

void Interface::GameArea::_setCenterToTile( const fheroes::Point & tile )
{
    SetCenterInPixels( { tile.x * fheroes::tileWidthPx + fheroes::tileWidthPx / 2, tile.y * fheroes::tileWidthPx + fheroes::tileWidthPx / 2 } );
}

void Interface::GameArea::SetCenterInPixels( const fheroes::Point & point )
{
    const fheroes::Point & middlePos = _middlePoint();
    ...
    _topLeftTileOffset = fheroes::Point( offsetX, offsetY );
}

int32_t Interface::GameArea::GetValidTileIdFromPoint( const fheroes::Point & point ) const
{
    const fheroes::Point offset = getInternalPosition( point );
    if ( offset.x < 0 || offset.y < 0 )
        return -1;

    const int32_t x = offset.x / fheroes::tileWidthPx;
    const int32_t y = offset.y / fheroes::tileWidthPx;
...
fheroes::Point Interface::GameArea::GetRelativeTilePosition( const fheroes::Point & tileId ) const
{
    return { tileId.x * fheroes::tileWidthPx - _topLeftTileOffset.x + _windowROI.x, tileId.y * fheroes::tileWidthPx - _topLeftTileOffset.y + _windowROI.y };
}
