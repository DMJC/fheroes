...
namespace Video
{
    ...
    public:
        ...
        Subtitle( const fheroes::TextBase & subtitleText, const uint32_t startTimeMS, const uint32_t durationMS, const fheroes::Point & position = { -1, -1 },
                  const int32_t maxWidth = fheroes::Display::DEFAULT_WIDTH );
        ...
        void render( fheroes::Image & output, const fheroes::Rect & frameRoi ) const
        {
            fheroes::Blit( _subtitleImage, 0, 0, output, frameRoi.x + _position.x, frameRoi.y + _position.y, _subtitleImage.width(), _subtitleImage.height() );
        }

    private:
        fheroes::Point _position{ -1, -1 };
        fheroes::Image _subtitleImage;
