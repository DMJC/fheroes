...
namespace
{
    ...
    fheroes::Sprite getHeroIcon( const int32_t heroPortait, const int32_t race, const PlayerColor color, const int townIcnId )
    {
        // To render hero icons we use castle flags and frame.
        const uint32_t flagIcnIndex = fheroes::getCastleLeftFlagIcnIndex( color );

        const fheroes::Sprite & castleLeftFlag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex );
        const fheroes::Sprite & castleFrame = fheroes::AGG::GetICN( townIcnId, 22 );
        const fheroes::Sprite & castleRightFlag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex + 1 );

        fheroes::Sprite castleIcon( castleFrame.width() + castleLeftFlag.width() + castleRightFlag.width() + 4, castleFrame.height() );
        castleIcon.reset();

        fheroes::Blit( castleLeftFlag, 0, 0, castleIcon, 0, 5, castleLeftFlag.width(), castleLeftFlag.height() );
        fheroes::Blit( castleFrame, 0, 0, castleIcon, castleLeftFlag.width() + 2, 0, castleFrame.width(), castleFrame.height() );
        fheroes::Blit( castleRightFlag, 0, 0, castleIcon, castleFrame.width() + castleLeftFlag.width() + 4, 5, castleRightFlag.width(), castleRightFlag.height() );

        if ( heroPortait > 0 ) {
            const fheroes::Sprite & heroPortrait = fheroes::AGG::GetICN( ICN::MINIPORT, heroPortait - 1 );
            fheroes::Copy( heroPortrait, 0, 0, castleIcon, castleLeftFlag.width() + 6, 4, heroPortrait.width(), heroPortrait.height() );
        }
        else {
            ...
            fheroes::Copy( fheroes::AGG::GetICN( ICN::NGEXTRA, portraitIndex ), 17, 10, castleIcon, castleLeftFlag.width() + 6, 4, 30, 22 );
        }

        return castleIcon;
    }

    fheroes::Sprite getTownIcon( const bool isTown, const int32_t race, const PlayerColor color, const int townIcnId )
    {
        const uint32_t flagIcnIndex = fheroes::getCastleLeftFlagIcnIndex( color );

        const fheroes::Sprite & castleLeftFlag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex );
        const fheroes::Sprite & castleFrame = fheroes::AGG::GetICN( townIcnId, 23 );
        const fheroes::Sprite & castleRightFlag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex + 1 );
        ...
        const uint32_t icnIndex = fheroes::getCastleIcnIndex( race, !isTown );

        const fheroes::Sprite & castleImage = fheroes::AGG::GetICN( townIcnId, icnIndex );
        fheroes::Copy( castleImage, 0, 0, castleIcon, castleLeftFlag.width() + 6, 4, castleImage.width(), castleImage.height() );
    ...
    std::vector<fheroes::LocalizedString> getHeroTitle( const std::string & name, const fheroes::SupportedLanguage language, const int race, const int32_t tileIndex,
                                                         const int32_t mapWidth )
    {
        const fheroes::SupportedLanguage gameLanguage = fheroes::getLanguageFromAbbreviation( Settings::Get().getGameLanguage() );
        ...
        return fheroes::getLocalizedStrings( std::move( text ), gameLanguage, "%{name}", name, language );
    }

    std::vector<fheroes::LocalizedString> getTownTitle( const std::string & name, const fheroes::SupportedLanguage language, const int race, const bool isTown,
    ...
    fheroes::Rect renderConditionsHeroIconAndName( const HeroInfo & heroInfo, const int32_t mapWidth, const fheroes::Rect & roi, fheroes::Image & output )
    {
        const fheroes::Sprite & heroFrame = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );

        const int32_t heroFrameWidth = 111;
        const int32_t heroFrameHeight = 105;

        fheroes::Rect selectConditionRoi = { roi.x + ( roi.width - heroFrameWidth ) / 2, roi.y + 4, heroFrameWidth, heroFrameHeight };

        fheroes::Blit( heroFrame, 88, 66, output, selectConditionRoi.x, selectConditionRoi.y, heroFrameWidth, heroFrameHeight );

        // To render hero icons we use castle flags and frame.
        const uint32_t flagIcnIndex = fheroes::getCastleLeftFlagIcnIndex( heroInfo.color );
        const fheroes::Sprite & castleLeftFlag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex );
        const fheroes::Sprite & castleRightFlag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex + 1 );
        ...
        if ( heroMetadata->customPortrait > 0 ) {
            const fheroes::Sprite & heroPortrait = fheroes::AGG::GetICN( ICN::getHeroPortraitIcnId( heroMetadata->customPortrait ), 0 );

            fheroes::Copy( heroPortrait, 0, 0, output, selectConditionRoi.x + 5, selectConditionRoi.y + 6, heroPortrait.width(), heroPortrait.height() );
        }
        else {
            fheroes::renderHeroRacePortrait( heroMetadata->race, { selectConditionRoi.x + 5, selectConditionRoi.y + 6, 101, 93 }, output );
        }
    ...
    fheroes::Rect renderConditionsTownIconAndName( const TownInfo & townInfo, const bool isEvilInterface, const int32_t mapWidth, const fheroes::Rect & roi,
                                                    fheroes::Image & output )
    {
        ...
        const fheroes::Sprite townIcon( getTownIcon( isTown, townInfo.race, townInfo.color, isEvilInterface ? ICN::LOCATORE : ICN::LOCATORS ) );

        fheroes::Blit( townIcon, output, roi.x, roi.y + 4 );
    ...
    {
    public:
        explicit SelectMapHero( const fheroes::Size & dialogSize, std::string title, std::string description, const int32_t mapWidth,
        ...
        void ActionListPressRight( int & index ) override
        {
            ...
            fheroes::showMessage( fheroes::Text{}, *text, Dialog::ZERO );
        }
    ...
    {
    public:
        explicit SelectMapCastle( const fheroes::Size & dialogSize, std::string title, std::string description, const int32_t mapWidth,
    ...
    void redrawVictoryCondition( const uint8_t condition, const fheroes::Rect & roi, const bool yellowFont, fheroes::Image & output )
    {
        const fheroes::Sprite & winIcon = fheroes::AGG::GetICN( ICN::REQUESTS, getVictoryIcnIndex( condition ) );
        fheroes::Copy( winIcon, 0, 0, output, roi.x + 1, roi.y, winIcon.width(), winIcon.height() );
        const fheroes::Text winText( getVictoryConditionText( condition ), yellowFont ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() );
        winText.drawInRoi( roi.x + 20, roi.y + 2, output, roi );
    }

    void redrawLossCondition( const uint8_t condition, const fheroes::Rect & roi, const bool yellowFont, fheroes::Image & output )
    {
        const fheroes::Sprite & icon = fheroes::AGG::GetICN( ICN::REQUESTS, getLossIcnIndex( condition ) );
        fheroes::Copy( icon, 0, 0, output, roi.x + 1, roi.y, icon.width(), icon.height() );
        const fheroes::Text winText( getLossConditionText( condition ), yellowFont ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() );
        winText.drawInRoi( roi.x + 20, roi.y + 2, output, roi );
    }
    ...
    public:
        ...
        explicit DropBoxList( const fheroes::Point & pt, const int32_t itemsCount, const bool isLossList, const int dropBoxIcn )
            ...
            , _isLossList( isLossList )
        {
            ...
            const fheroes::Sprite & image = fheroes::AGG::GetICN( dropBoxIcn, 0 );
            ...
            _restorer = std::make_unique<fheroes::ImageRestorer>( display, pt.x, pt.y, listWidth + 1, listHeight );

            // Top part of list background.
            fheroes::Copy( image, 0, 0, display, pt.x, pt.y, listWidth, topPartHeight );
            const int32_t lineFixOffsetX = pt.x + listWidth;
            fheroes::Copy( image, 0, 0, display, lineFixOffsetX, pt.y, 1, topPartHeight );
            ...
            for ( int32_t i = 0; i < middlePartCount; ++i ) {
                ...
                fheroes::Copy( image, 0, 2, display, pt.x, posY, listWidth, copyHeight );
                fheroes::Copy( image, 0, 2, display, lineFixOffsetX, posY, 1, copyHeight );
            ...
            fheroes::Copy( image, 0, 2, display, pt.x, offsetY, listWidth, bottomPartHeight );
            fheroes::Copy( image, 0, 2, display, lineFixOffsetX, offsetY, 1, bottomPartHeight );

            _background = std::make_unique<fheroes::ImageRestorer>( display, pt.x + 2, pt.y + 2, listWidth - 3, listHeight - 4 );
        ...
        {
            if ( _isLossList ) {
                redrawLossCondition( condition, { dstX, dstY, _itemWidth, _itemHeight }, current, fheroes::Display::instance() );
            }
            else {
                redrawVictoryCondition( condition, { dstX, dstY, _itemWidth, _itemHeight }, current, fheroes::Display::instance() );
            }
        }
        ...
        {
            if ( _isLossList ) {
                fheroes::showStandardTextMessage( _( "Special Loss Condition" ), getLossConditionText( condition ), Dialog::ZERO );
            }
            else {
                fheroes::showStandardTextMessage( _( "Special Victory Condition" ), getVictoryConditionText( condition ), Dialog::ZERO );
            }
        }

        fheroes::Rect getArea() const
        {
    ...
    private:
        ...
        int32_t _itemWidth;
        int32_t _itemHeight{ fheroes::getFontHeight( fheroes::FontSize::NORMAL ) + 1 };
        std::unique_ptr<fheroes::ImageRestorer> _restorer;
        std::unique_ptr<fheroes::ImageRestorer> _background;
    };

    class VictoryConditionUI final
    {
    public:
        VictoryConditionUI( fheroes::Image & output, const fheroes::Rect & roi, const Maps::Map_Format::MapFormat & mapFormat, const bool isEvilInterface )
        ...
        void render( fheroes::Image & output, const bool renderEverything )
        {
            if ( renderEverything ) {
            ...
            case Maps::FileInfo::VICTORY_CAPTURE_TOWN: {
                ...
                const fheroes::Rect roi{ _restorer.rect() };
            ...
            case Maps::FileInfo::VICTORY_OBTAIN_ARTIFACT: {
                if ( renderEverything ) {
                    ...
                    const fheroes::Sprite & artifactFrame = fheroes::AGG::GetICN( ICN::RESOURCE, 7 );
                    _selectConditionRoi = { roi.x + ( roi.width - artifactFrame.width() ) / 2, roi.y + 4, artifactFrame.width(), artifactFrame.height() };

                    fheroes::Blit( artifactFrame, output, _selectConditionRoi.x, _selectConditionRoi.y );
                ...
                const fheroes::Sprite & artifactImage = fheroes::AGG::GetICN( ICN::ARTIFACT, Artifact( static_cast<int>( _victoryArtifactId ) ).IndexSprite64() );

                fheroes::Copy( artifactImage, 0, 0, output, _selectConditionRoi.x + 6, _selectConditionRoi.y + 6, artifactImage.width(), artifactImage.height() );
            ...
            case Maps::FileInfo::VICTORY_DEFEAT_OTHER_SIDE: {
                if ( renderEverything ) {
                    ...
                    fheroes::Text text( _( "Set alliances:" ), fheroes::FontType::normalWhite() );
                    ...
                    for ( size_t i = 0; i < _alliances.size(); ++i ) {
                        ...
                        text.set( std::move( allianceText ), fheroes::FontType::normalWhite() );
            ...
            case Maps::FileInfo::VICTORY_COLLECT_ENOUGH_GOLD: {
                if ( renderEverything ) {
                    const fheroes::Size valueSectionUiSize = fheroes::ValueSelectionDialogElement::getArea();
                    const fheroes::Rect roi{ _restorer.rect() };
                    const fheroes::Point uiOffset{ roi.x + ( roi.width - valueSectionUiSize.width ) / 2, roi.y };

                    _goldAccumulationValue.setOffset( uiOffset );
                    _goldAccumulationValue.draw( output );

                    const fheroes::Text text( _( "Gold:" ), fheroes::FontType::normalWhite() );
        ...
        bool processEvents()
        {
            ...
            case Maps::FileInfo::VICTORY_CAPTURE_TOWN: {
                ...
                if ( le.MouseClickLeft( _selectConditionRoi ) ) {
                    ...
                    const int32_t maxHeight = std::min( 100 + SelectMapCastle::itemsOffsetY * 12, fheroes::Display::instance().height() - 100 );
                ...
                if ( le.isMouseRightButtonPressedInArea( _selectConditionRoi ) ) {
                    fheroes::showStandardTextMessage( _( "Special Victory Condition" ), _( "Click here to change the town needed to capture to achieve victory." ),
                                                       Dialog::ZERO );
                    return false;
                }
            ...
            case Maps::FileInfo::VICTORY_KILL_HERO: {
                ...
                if ( le.isMouseRightButtonPressedInArea( _selectConditionRoi ) ) {
                    fheroes::showStandardTextMessage( _( "Special Victory Condition" ), _( "Click here to change the hero needed to defeat to achieve victory." ),
                                                       Dialog::ZERO );
                    return false;
                }
            ...
            case Maps::FileInfo::VICTORY_OBTAIN_ARTIFACT: {
                ...
                if ( le.isMouseRightButtonPressedInArea( _selectConditionRoi ) ) {
                    fheroes::ArtifactDialogElement( Artifact( static_cast<int>( _victoryArtifactId ) ) ).showPopup( Dialog::ZERO );
            ...
            case Maps::FileInfo::VICTORY_DEFEAT_OTHER_SIDE: {
                ...
                for ( size_t allianceNumber = 0; allianceNumber < _alliancesCheckboxes.size(); ++allianceNumber ) {
                    for ( size_t playerNumber = 0; playerNumber < _alliancesCheckboxes[allianceNumber].size(); ++playerNumber ) {
                        const fheroes::Rect & checkboxRect = _alliancesCheckboxes[allianceNumber][playerNumber]->getRect();
    ...
    private:
        ...
        fheroes::ImageRestorer _restorer;

        fheroes::ValueSelectionDialogElement _goldAccumulationValue{ 10000, 1000000, 10000, 1000, {} };

        fheroes::MovableSprite _allowNormalVictory;
        fheroes::MovableSprite _allowVictoryConditionForAI;
        fheroes::Rect _allowNormalVictoryRoi;
        fheroes::Rect _allowVictoryConditionForAIRoi;
        fheroes::Rect _selectConditionRoi;
    };

    class LossConditionUI final
    {
    public:
        LossConditionUI( fheroes::Image & output, const fheroes::Rect & roi, const Maps::Map_Format::MapFormat & mapFormat, const bool isEvilInterface )
        ...
        void render( fheroes::Image & output, const bool renderEverything )
        {
            ...
            case Maps::FileInfo::LOSS_OUT_OF_TIME: {
                const fheroes::Rect roi = _restorer.rect();
                const fheroes::Point uiOffset{ roi.x + ( roi.width - fheroes::ValueSelectionDialogElement::getArea().width ) / 2, roi.y };

                _outOfTimeValue.setOffset( uiOffset );
                _outOfTimeValue.draw( output );

                fheroes::Text text( _( "Days:" ), fheroes::FontType::normalWhite() );
                text.draw( uiOffset.x - text.width() - 5, roi.y + ( fheroes::ValueSelectionDialogElement::getArea().height - text.height() ) / 2 + 2, output );

                const int32_t offsetY = roi.y + fheroes::ValueSelectionDialogElement::getArea().height + 14;

                text.set( Editor::getDateDescription( _outOfTimeValue.getValue() ), fheroes::FontType::normalWhite() );
        ...
        bool processEvents()
        {
            ...
            case Maps::FileInfo::LOSS_TOWN: {
                ...
                if ( le.isMouseRightButtonPressedInArea( _selectConditionRoi ) ) {
                    fheroes::showStandardTextMessage( _( "Special Loss Condition" ), _( "Click here to change the town whose loss would mean defeat." ), Dialog::ZERO );
                    return false;
                }
            ...
            case Maps::FileInfo::LOSS_HERO: {
                ...
                if ( le.isMouseRightButtonPressedInArea( _selectConditionRoi ) ) {
                    fheroes::showStandardTextMessage( _( "Special Loss Condition" ), _( "Click here to change the hero whose loss would mean defeat." ), Dialog::ZERO );
                    return false;
                }
    ...
    private:
        ...
        fheroes::ValueSelectionDialogElement _outOfTimeValue{ 1, 10 * daysInYear, daysInMonth, 1, {} };

        fheroes::Rect _selectConditionRoi;
    };

    uint8_t showWinLoseList( const Maps::Map_Format::MapFormat & mapFormat, const fheroes::Point & offset, const uint8_t selectedCondition, const bool isLossList,
                             const int dropBoxIcn )
    {
        ...
        const fheroes::Rect listArea( conditionList.getArea() );
...
namespace Editor
{
    bool mapSpecificationsDialog( Maps::Map_Format::MapFormat & mapFormat, const int32_t maxMapNameLength )
    {
        ...
        fheroes::StandardWindow background( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT, !isDefaultScreenSize );
        const fheroes::Rect activeArea( background.activeArea() );

        const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();

        if ( isDefaultScreenSize ) {
            const fheroes::Sprite & backgroundImage = fheroes::AGG::GetICN( isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK, 0 );
            fheroes::Copy( backgroundImage, 0, 0, display, activeArea );
        }
        ...
        // Map name.
        const fheroes::Sprite & scenarioBox = fheroes::AGG::GetICN( isEvilInterface ? ICN::METALLIC_BORDERED_TEXTBOX_EVIL : ICN::METALLIC_BORDERED_TEXTBOX_GOOD, 0 );
        const fheroes::Rect scenarioBoxRoi( activeArea.x + ( activeArea.width - scenarioBox.width() ) / 2, activeArea.y + 10, scenarioBox.width(),
                                             scenarioBox.height() );
        const fheroes::Rect mapNameRoi( scenarioBoxRoi.x + 6, scenarioBoxRoi.y + 5, scenarioBoxRoi.width - 12, scenarioBoxRoi.height - 11 );

        fheroes::Copy( scenarioBox, 0, 0, display, scenarioBoxRoi );
        fheroes::addGradientShadow( scenarioBox, display, scenarioBoxRoi.getPosition(), { -5, 5 } );

        fheroes::Text text( mapFormat.name, fheroes::FontType::normalWhite(), mapFormat.mainLanguage );
        ...
        std::vector<fheroes::Rect> playerRects( availablePlayersCount );
        const PlayerColorsVector availableColors( mapFormat.availablePlayerColors );

        const fheroes::Sprite & playerIconShadow = fheroes::AGG::GetICN( ICN::NGEXTRA, 61 );
        for ( int32_t i = 0; i < availablePlayersCount; ++i ) {
            playerRects[i].x = offsetX + i * playerStepX;
            playerRects[i].y = offsetY;

            fheroes::Blit( playerIconShadow, display, playerRects[i].x - 5, playerRects[i].y + 3 );

            const uint32_t icnIndex = Color::GetIndex( availableColors[i] ) + getPlayerIcnIndex( mapFormat, availableColors[i] );

            const fheroes::Sprite & playerIcon = fheroes::AGG::GetICN( ICN::NGEXTRA, icnIndex );
            playerRects[i].width = playerIcon.width();
            playerRects[i].height = playerIcon.height();
            fheroes::Copy( playerIcon, 0, 0, display, playerRects[i].x, playerRects[i].y, playerRects[i].width, playerRects[i].height );
        }
        ...
        text.set( _( "Map Difficulty" ), fheroes::FontType::normalWhite() );
        text.draw( offsetX + ( difficultyStepX * 4 - text.width() ) / 2, scenarioBoxRoi.y + scenarioBoxRoi.height + 65, display );

        std::array<fheroes::Rect, 4> difficultyRects;
        offsetY += 23;

        const fheroes::Sprite & difficultyCursorSprite = fheroes::AGG::GetICN( ICN::NGEXTRA, 62 );
        ...
        for ( int i = 0; i < 4; ++i ) {
            ...
            const fheroes::Sprite & icon = fheroes::AGG::GetICN( ICN::DIFFICULTY_ICON_EASY + i, difficultyIcnIndex );
            fheroes::Copy( icon, 0, 0, display, difficultyRects[i] );
            fheroes::addGradientShadow( icon, display, { difficultyRects[i].x, difficultyRects[i].y }, { -5, 5 } );

            difficultyRects[i].x -= 3;
            difficultyRects[i].y -= 3;

            text.set( Difficulty::String( setDifficultyByIndex( i ) ), fheroes::FontType::smallWhite() );
        ...
        fheroes::MovableSprite difficultyCursor( difficultyCursorSprite );
        ...
        text.set( _( "Map Description" ), fheroes::FontType::normalWhite() );
        text.draw( offsetX + ( descriptionBoxWidth - text.width() ) / 2, offsetY, display );

        offsetY += 25;
        const fheroes::Rect descriptionTextRoi( offsetX, offsetY, descriptionBoxWidth, descriptionBoxHeight );
        ...
        fheroes::ImageRestorer descriptionBackground( display, descriptionTextRoi.x, descriptionTextRoi.y, descriptionTextRoi.width, descriptionTextRoi.height );

        text.set( mapFormat.description, fheroes::FontType::normalWhite(), mapFormat.mainLanguage );
        ...
        text.set( _( "Special Victory Condition" ), fheroes::FontType::normalWhite() );
        ...
        const fheroes::Sprite & itemBackground = fheroes::AGG::GetICN( dropListIcn, 0 );
        ...
        fheroes::Copy( itemBackground, 0, 0, display, offsetX, offsetY, itemBackgroundWidth, itemBackgroundHeight );
        const fheroes::Rect victoryTextRoi( offsetX + 2, offsetY + 3, itemBackgroundWidth - 4, itemBackgroundHeight - 5 );

        redrawVictoryCondition( mapFormat.victoryConditionType, victoryTextRoi, false, display );

        const fheroes::Sprite & dropListButtonSprite = fheroes::AGG::GetICN( dropListIcn, 1 );
        const fheroes::Sprite & dropListButtonPressedSprite = fheroes::AGG::GetICN( dropListIcn, 2 );

        fheroes::ButtonSprite victoryDroplistButton( offsetX + itemBackgroundWidth, offsetY, dropListButtonSprite, dropListButtonPressedSprite );
        const fheroes::Rect victoryDroplistButtonRoi( fheroes::getBoundaryRect( victoryDroplistButton.area(), victoryTextRoi ) );
        victoryDroplistButton.draw();

        offsetY += 30;

        const fheroes::Rect victoryConditionUIRoi{ offsetX, offsetY, victoryDroplistButtonRoi.width, 150 };
        ...
        text.set( _( "Special Loss Condition" ), fheroes::FontType::normalWhite() );
        ...
        fheroes::Copy( itemBackground, 0, 0, display, offsetX, offsetY, itemBackgroundWidth, itemBackground.height() );
        const fheroes::Rect lossTextRoi( offsetX + 2, offsetY + 3, victoryTextRoi.width, victoryTextRoi.height );

        redrawLossCondition( mapFormat.lossConditionType, lossTextRoi, false, display );

        fheroes::ButtonSprite lossDroplistButton( offsetX + itemBackgroundWidth, offsetY, dropListButtonSprite, dropListButtonPressedSprite );
        const fheroes::Rect lossDroplistButtonRoi( fheroes::getBoundaryRect( lossDroplistButton.area(), lossTextRoi ) );
        lossDroplistButton.draw();

        offsetY += 30;

        const fheroes::Rect lossConditionUIRoi{ offsetX, offsetY, lossDroplistButtonRoi.width, 150 };
        ...
        background.renderButton( buttonCancel, buttonCancelIcn, 0, 1, { 20, 6 }, fheroes::StandardWindow::Padding::BOTTOM_RIGHT );

        fheroes::Button buttonOk;
        const int buttonOkIcn = isEvilInterface ? ICN::BUTTON_SMALL_OKAY_EVIL : ICN::BUTTON_SMALL_OKAY_GOOD;
        background.renderButton( buttonOk, buttonOkIcn, 0, 1, { 20 + buttonCancel.area().width + 10, 6 }, fheroes::StandardWindow::Padding::BOTTOM_RIGHT );

        fheroes::ButtonSprite buttonRumors;
        const char * translatedText = fheroes::getSupportedText( gettext_noop( "RUMORS" ), fheroes::FontType::buttonReleasedWhite() );
        background.renderTextAdaptedButtonSprite( buttonRumors, translatedText, { 20, 6 }, fheroes::StandardWindow::Padding::BOTTOM_LEFT );

        fheroes::ButtonSprite buttonEvents;
        translatedText = fheroes::getSupportedText( gettext_noop( "EVENTS" ), fheroes::FontType::buttonReleasedWhite() );
        background.renderTextAdaptedButtonSprite( buttonEvents, translatedText, { 20 + buttonRumors.area().width + 10, 6 },
                                                  fheroes::StandardWindow::Padding::BOTTOM_LEFT );

        fheroes::ButtonSprite buttonLanguage;
        translatedText = fheroes::getSupportedText( gettext_noop( "LANGUAGE" ), fheroes::FontType::buttonReleasedWhite() );
        ...
        fheroes::ButtonSprite buttonAbout;
        translatedText = fheroes::getSupportedText( gettext_noop( "ABOUT" ), fheroes::FontType::buttonReleasedWhite() );
        background.renderTextAdaptedButtonSprite( buttonAbout, translatedText, { 21, 12 }, fheroes::StandardWindow::Padding::TOP_RIGHT );

        auto renderMapName = [&text, &mapFormat, &display, &scenarioBox, &mapNameRoi, &scenarioBoxRoi]() {
            text.set( mapFormat.name, fheroes::FontType::normalWhite(), mapFormat.mainLanguage );
            fheroes::Copy( scenarioBox, 0, 0, display, scenarioBoxRoi );
            text.fitToOneRow( mapNameRoi.width );
            text.drawInRoi( mapNameRoi.x, mapNameRoi.y + 3, mapNameRoi.width, display, mapNameRoi );
        };
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( victoryConditionUI.processEvents() ) {
                ...
                const fheroes::SupportedLanguage language = fheroes::selectLanguage( supportedLanguages, mapFormat.mainLanguage, false );
                if ( language != mapFormat.mainLanguage ) {
                    ...
                    StringReplace( differentLanguageWarning, "%{oldLanguage}", fheroes::getLanguageName( mapFormat.mainLanguage ) );
                    StringReplace( differentLanguageWarning, "%{newLanguage}", fheroes::getLanguageName( language ) );

                    if ( fheroes::showStandardTextMessage( _( "Warning" ), differentLanguageWarning, Dialog::YES | Dialog::NO ) == Dialog::YES ) {
                        mapFormat.mainLanguage = language;

                        renderMapName();
                        renderMapDescription();

                        display.render( fheroes::getBoundaryRect( scenarioBoxRoi, descriptionTextRoi ) );
                    }
                }
            }
                ...
                if ( Dialog::inputString( fheroes::Text{}, body, editableMapName, maxMapNameLength, false, mapFormat.mainLanguage ) ) {
                ...
                const fheroes::Text body{ _( "Change Map Description" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, descripton, 150, true, mapFormat.mainLanguage ) ) {
                ...
                if ( result != mapFormat.victoryConditionType ) {
                    ...
                    fheroes::Copy( itemBackground, 2, 3, display, victoryTextRoi );
                    redrawVictoryCondition( mapFormat.victoryConditionType, victoryTextRoi, false, display );
                }

                display.render( fheroes::getBoundaryRect( victoryTextRoi, victoryConditionUIRoi ) );
            }
                ...
                if ( result != mapFormat.lossConditionType ) {
                    ...
                    fheroes::Copy( itemBackground, 2, 3, display, lossTextRoi );
                    redrawLossCondition( mapFormat.lossConditionType, lossTextRoi, false, display );
                }

                display.render( fheroes::getBoundaryRect( lossTextRoi, lossConditionUIRoi ) );
            }
            else if ( le.MouseClickLeft( buttonAbout.area() ) ) {
                std::string notes = mapFormat.creatorNotes;

                const fheroes::Text body{ _( "About" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, notes, 150, true, mapFormat.mainLanguage ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to accept the changes made." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonRumors.area() ) ) {
                fheroes::showStandardTextMessage( _( "Rumors" ), _( "Click to edit custom rumors." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonEvents.area() ) ) {
                fheroes::showStandardTextMessage( _( "Events" ), _( "Click to edit daily events." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonLanguage.area() ) ) {
                fheroes::showStandardTextMessage( _( "Language" ), _( "Click to change the language of the map." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonAbout.area() ) ) {
                fheroes::showStandardTextMessage( _( "About" ),
                ...
                message.add( fheroes::Text{ mapFormat.name, fheroes::FontType::normalWhite(), mapFormat.mainLanguage } );
                ...
                fheroes::showStandardTextMessage( _( "Map Description" ), _( "Click to change the description of the current map." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( victoryDroplistButtonRoi ) ) {
                fheroes::showStandardTextMessage( _( "Special Victory Condition" ), _( "Click to change the victory condition of the current map." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( lossDroplistButtonRoi ) ) {
                fheroes::showStandardTextMessage( _( "Special Loss Condition" ), _( "Click to change the loss condition of the current map." ), Dialog::ZERO );
            }
            ...
            for ( int32_t i = 0; i < availablePlayersCount; ++i ) {
                if ( le.MouseClickLeft( playerRects[i] ) ) {
                    ...
                    if ( victoryConditionUI.updateCondition( mapFormat ) ) {
                        ...
                        renderRoi = fheroes::getBoundaryRect( renderRoi, victoryConditionUIRoi );
                        renderRoi = fheroes::getBoundaryRect( renderRoi, victoryTextRoi );
                    }
                    ...
                    if ( lossConditionUI.updateCondition( mapFormat ) ) {
                        ...
                        renderRoi = fheroes::getBoundaryRect( renderRoi, lossConditionUIRoi );
                        renderRoi = fheroes::getBoundaryRect( renderRoi, lossTextRoi );
                    }
                    ...
                    renderRoi = fheroes::getBoundaryRect( renderRoi, playerRects[i] );
                ...
                if ( le.isMouseRightButtonPressedInArea( playerRects[i] ) ) {
                    fheroes::showStandardTextMessage( _( "Player Type" ), _( "Indicates the player types in the scenario. Click to change." ), Dialog::ZERO );
                }
            }
