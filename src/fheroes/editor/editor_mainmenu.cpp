...
namespace
{
    ...
    fheroes::GameMode editNewMapFromScratch( const Maps::MapSize & mapSize )
    {
...
namespace Editor
{
    fheroes::GameMode menuMain( const bool straightToSelectMapSize )
    {
        ...
        fheroes::drawEditorMainMenuScreen();
        ...
        const fheroes::ButtonBase & buttonNewMap = mainModeButtons.button( 0 );
        const fheroes::ButtonBase & buttonLoadMap = mainModeButtons.button( 1 );
        ...
        fheroes::StandardWindow background( mainModeButtons, true, ( buttonNewMap.area().height + spaceBetweenButtons ) * 4 );
        ...
        fheroes::Button buttonMainMenu( buttonNewMap.area().x,
        ...
        fheroes::Button buttonBack( buttonMainMenu.area().x, buttonMainMenu.area().y, menuButtonsIcnIndex, 6, 7 );

        // Add From Scratch and Random buttons. Currently unused until Random map generator has been implemented.
        fheroes::ButtonGroup mapCreationModeButtons;
        for ( int32_t i = 0; i < 2; ++i ) {
        ...
        const fheroes::ButtonBase & buttonScratchMap = mapCreationModeButtons.button( 0 );
        const fheroes::ButtonBase & buttonRandomMap = mapCreationModeButtons.button( 1 );

        // Add map size buttons
        fheroes::ButtonGroup mapSizeButtons;
        ...
        while ( le.HandleEvents() ) {
            if ( buttonNewMap.isEnabled() ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( buttonNewMap.area() ) ) {
                    ...
                    fheroes::showStandardTextMessage( _( "Main Menu" ), _( "Exit the Editor and return to the game's Main Menu." ), Dialog::ZERO );
                }
            }
                ...
                if ( le.isMouseRightButtonPressedInArea( buttonScratchMap.area() ) ) {
                    fheroes::showStandardTextMessage( _( "From Scratch" ), _( "Start from scratch with a blank map." ), Dialog::ZERO );
                }
                else if ( le.isMouseRightButtonPressedInArea( buttonRandomMap.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Random" ), _( "Create a randomly generated map." ), Dialog::ZERO );
                }
                else if ( le.isMouseRightButtonPressedInArea( buttonBack.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Back" ), _( "Return to the Editor's main menu options." ), Dialog::ZERO );
                }
            }
                ...
                for ( size_t i = 0; i < mapSizeCount; ++i ) {
                    ...
                    if ( le.isMouseRightButtonPressedInArea( mapSizeButtons.button( i ).area() ) ) {
                        ...
                        mapSize += " x " + mapSize;
                        fheroes::showStandardTextMessage( mapSize, message, Dialog::ZERO );
                    }
                }
