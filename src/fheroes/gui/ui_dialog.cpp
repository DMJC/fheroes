...
namespace
{
    ...
    void outputInTextSupportMode( const fheroes::TextBase & header, const fheroes::TextBase & body, const int buttonTypes )
    {
...
namespace fheroes
{
    int showMessage( const TextBase & header, const TextBase & body, const int buttons, const std::vector<const DialogElement *> & elements /* = {} */ )
    {
        ...
        const int32_t headerHeight = header.empty() ? 0 : header.height( fheroes::boxAreaWidthPx ) + textOffsetY;

        int overallTextHeight = headerHeight;

        const int32_t bodyTextHeight = body.height( fheroes::boxAreaWidthPx );
        if ( bodyTextHeight > 0 ) {
        ...
        for ( const DialogElement * element : elements ) {
            ...
            else if ( ( std::max( rowMaxElementWidth.back(), currentElementWidth ) + elementOffsetX ) * ( rowElementCount.back() + 1 ) <= fheroes::boxAreaWidthPx ) {
        ...
        header.draw( pos.x, pos.y + textOffsetY, fheroes::boxAreaWidthPx, display );
        body.draw( pos.x, pos.y + textOffsetY + headerHeight, fheroes::boxAreaWidthPx, display );
        ...
        for ( const DialogElement * element : elements ) {
            ...
            const int32_t emptyWidth = fheroes::boxAreaWidthPx - currentRowElementCount * currentRowMaxElementWidth;
    ...
    int32_t getDialogHeight( const TextBase & header, const TextBase & body, const int buttons, const std::vector<const DialogElement *> & elements )
    {
        ...
        return overallTextHeight + elementHeight + ( ( buttons != 0 ) ? Dialog::FrameBox::getButtonAreaHeight() : 0 ) + fheroes::borderWidthPx * 4;
    }
        ...
        : _text( text )
    {
        ...
        _area = { fheroes::boxAreaWidthPx, _text->height( fheroes::boxAreaWidthPx ) };
    }

    void TextDialogElement::draw( Image & output, const Point & offset ) const
    {
        _text->draw( offset.x, offset.y, fheroes::boxAreaWidthPx, output );
    }
    ...
    void ValueSelectionDialogElement::setOffset( const fheroes::Point & offset )
    {
