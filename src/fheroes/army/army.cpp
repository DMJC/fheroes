...
int Army::GetMoraleModificator( std::string * strs ) const
{
    ...
    hasUndead = hasUndead || ( GetCommander() && GetCommander()->GetBagArtifacts().isArtifactCursePresent( fheroes::ArtifactCurseType::UNDEAD_MORALE_PENALTY ) );
...
{
    fheroes::drawMiniMonsters( troops, cx, cy, width, 0, 0, false, true, false, 0, fheroes::Display::instance() );
}
                                     ...
                                     const bool isGarrisonView /* = false */, const uint32_t thievesGuildsCount /* = 0 */ )
{
    ...
    if ( count < 3 ) {
        fheroes::drawMiniMonsters( troops, posX + offsetX, posY + offsetY / 2 + 1, lineWidth * 2 / 3, 0, 0, isCompact, isDetailedView, isGarrisonView,
                                    thievesGuildsCount, fheroes::Display::instance() );
    }
    else {
        ...
        fheroes::Image & output = fheroes::Display::instance();

        fheroes::drawMiniMonsters( troops, posX + offsetX, posY, lineWidth * 2 / 3, 0, firstLineTroopCount, isCompact, isDetailedView, isGarrisonView,
                                    thievesGuildsCount, output );
        fheroes::drawMiniMonsters( troops, posX, posY + offsetY, secondLineWidth, firstLineTroopCount, secondLineTroopCount, isCompact, isDetailedView, isGarrisonView,
                                    thievesGuildsCount, output );
    }
}
...
NeutralMonsterJoiningCondition Army::GetJoinSolution( const Heroes & hero, const Maps::Tile & tile, const Troop & troop )
{
    ...
    const bool canJoin = [&hero, &tile, &troop, armyStrengthRatio]() {
        ...
        if ( hero.GetBagArtifacts().isArtifactCursePresent( fheroes::ArtifactCurseType::NO_JOINING_ARMIES ) ) {
            return false;
        }
