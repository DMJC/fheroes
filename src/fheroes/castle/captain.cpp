...
fheroes::Sprite Captain::GetPortrait( const PortraitType type ) const
{
    switch ( type ) {
    case PORT_BIG: {
        ...
        fheroes::Sprite portrait = fheroes::AGG::GetICN( portraitIcnId, 0 );
        const fheroes::Image & flag = fheroes::AGG::GetICN( ICN::getFlagIcnId( GetColor() ), 0 );

        const fheroes::Point & offset = GetFlagOffset( GetRace() );
        fheroes::Blit( flag, portrait, offset.x, offset.y );
        return portrait;
    }
    case PORT_MEDIUM:
    case PORT_SMALL:
        switch ( GetRace() ) {
        case Race::KNGT:
            return fheroes::AGG::GetICN( ICN::MINICAPT, 0 );
        case Race::BARB:
            return fheroes::AGG::GetICN( ICN::MINICAPT, 1 );
        case Race::SORC:
            return fheroes::AGG::GetICN( ICN::MINICAPT, 2 );
        case Race::WRLK:
            return fheroes::AGG::GetICN( ICN::MINICAPT, 3 );
        case Race::WZRD:
            return fheroes::AGG::GetICN( ICN::MINICAPT, 4 );
        case Race::NECR:
            return fheroes::AGG::GetICN( ICN::MINICAPT, 5 );
        default:
            break;
        }
        break;
    ...
    assert( 0 );
    return fheroes::AGG::GetICN( -1, 0 );
}

void Captain::PortraitRedraw( const int32_t px, const int32_t py, const PortraitType type, fheroes::Image & dstsf ) const
{
    if ( !isValid() )
        return;

    const fheroes::Sprite & port = GetPortrait( type );
    if ( PORT_SMALL != type ) { // a normal portrait in a castle or in battle
        fheroes::Blit( port, dstsf, px, py );
        return;
    }
    ...
    // background
    fheroes::Fill( dstsf, px, py, iconWidth, iconHeight, 0 );

    // mobility is always 0
    const uint8_t blueColor = fheroes::GetColorId( 15, 30, 120 );
    fheroes::Fill( dstsf, px, py, barWidth, iconHeight, blueColor );

    // portrait
    fheroes::Blit( port, dstsf, px + barWidth + 1, py );

    // spell points
    fheroes::Fill( dstsf, px + barWidth + port.width() + 2, py, barWidth, iconHeight, blueColor );
    fheroes::Blit( mana, dstsf, px + barWidth + port.width() + 2, py + mana.y() );
}
