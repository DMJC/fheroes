...
namespace
{
    fheroes::Sprite getMonsterFrame( const Monster & monster, const int32_t count )
    {
        const fheroes::Point frameOffset = { 6, 6 };

        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::STRIP, 12 );
        renderMonsterFrame( monster, output, frameOffset );

        if ( count > 0 ) {
            const fheroes::Text monsterCountText( "+" + std::to_string( count ), fheroes::FontType::normalWhite() );
