...
struct ComparisonDistance
{
    ...
    {
        const fheroes::Point point1( Maps::GetPoint( index1 ) );
        const fheroes::Point point2( Maps::GetPoint( index2 ) );
    ...
    const fheroes::Point centerPoint;
};
...
fheroes::Point Maps::getDirectionPoint( const fheroes::Point & from, const int direction )
{
    switch ( direction ) {
    case Direction::TOP:
...
fheroes::Point Maps::GetPoint( const int32_t index )
{
...
int32_t Maps::GetIndexFromAbsPoint( const fheroes::Point & mp )
{
    if ( mp.x < 0 || mp.y < 0 ) {
        return -1;
    }
...
void Maps::ClearFog( const int32_t tileIndex, const int32_t scoutingDistance, const PlayerColor playerColor )
{
    ...
    const fheroes::Point center = Maps::GetPoint( tileIndex );
    ...
    fheroes::Point fogRevealMinPos( world.h(), worldWidth );
    fheroes::Point fogRevealMaxPos( 0, 0 );
...
{
    const fheroes::Point point1( GetPoint( pos1 ) );
    const fheroes::Point point2( GetPoint( pos2 ) );
...
void Maps::ReplaceRandomCastleObjectId( const fheroes::Point & center )
{
    // Reset castle ID
...
void Maps::UpdateCastleSprite( const fheroes::Point & center, int race, bool isCastle, bool isRandom )
{
    /*
