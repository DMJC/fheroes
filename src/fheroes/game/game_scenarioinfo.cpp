...
namespace
{
    ...
    void DrawScenarioStaticInfo( const fheroes::Rect & rt )
    {
        fheroes::Display & display = fheroes::Display::instance();

        const fheroes::FontType normalWhiteFont = fheroes::FontType::normalWhite();

        // text scenario
        fheroes::Text text( _( "Scenario:" ), normalWhiteFont );
    ...
    void RedrawMapTitle( const Settings & conf, const fheroes::Rect & maxRoi, const fheroes::Rect & centeredRoi )
    {
        const auto & info = conf.getCurrentMapInfo();
        fheroes::Text text{ info.name, fheroes::FontType::normalWhite(), info.getSupportedLanguage() };

        if ( text.width() > centeredRoi.width ) {
            text.fitToOneRow( maxRoi.width );
            text.draw( maxRoi.x + ( maxRoi.width - text.width() ), maxRoi.y + 3, text.width(), fheroes::Display::instance() );
        }
        else {
            text.draw( centeredRoi.x, centeredRoi.y + 3, centeredRoi.width, fheroes::Display::instance() );
        }
    }

    void RedrawDifficultyInfo( const fheroes::Point & dst )
    {
        ...
        for ( int32_t current = Difficulty::EASY; current <= Difficulty::IMPOSSIBLE; ++current ) {
            ...
            const fheroes::Text text( Difficulty::String( current ), fheroes::FontType::smallWhite() );
            text.draw( dst.x + 31 + offset + normalSpecificOffset - ( text.width() / 2 ), dst.y + height, fheroes::Display::instance() );
        }
    }

    fheroes::Rect RedrawRatingInfo( const fheroes::Point & offset, int32_t width_ )
    {
        ...
        text.draw( offset.x, y, width_, fheroes::Display::instance() );
    ...
    fheroes::GameMode ChooseNewMap( MapsFileInfoList & lists, const int humanPlayerCount )
    {
        assert( !lists.empty() );
        ...
        fheroes::StandardWindow background( 388, 397, true, display );

        const fheroes::Rect roi( background.activeArea() );

        const fheroes::Point pointDifficultyInfo( roi.x + 8, roi.y + 79 );
        const fheroes::Point pointOpponentInfo( roi.x + 8, roi.y + 181 );
        const fheroes::Point pointClassInfo( roi.x + 8, roi.y + 265 );
        ...
        const fheroes::Rect scenarioBoxRoi( roi.x + ( roi.width - scenarioBox.width() ) / 2, roi.y + 24, scenarioBox.width(), scenarioBox.height() );
        ...
        const fheroes::Sprite & difficultyCursor = fheroes::AGG::GetICN( ICN::NGEXTRA, 62 );
        ...
        std::vector<fheroes::Rect> coordDifficulty;
        ...
        const int32_t buttonSelectWidth = fheroes::AGG::GetICN( ICN::BUTTON_MAP_SELECT_GOOD, 0 ).width();

        fheroes::Button buttonSelectMaps( scenarioBoxRoi.x + scenarioBoxRoi.width - 6 - buttonSelectWidth, scenarioBoxRoi.y + 5,
        ...
        const fheroes::Point buttonOffset( 20, 6 );

        const int buttonOkIcn = isEvilInterface ? ICN::BUTTON_SMALL_OKAY_EVIL : ICN::BUTTON_SMALL_OKAY_GOOD;
        background.renderButton( buttonOk, buttonOkIcn, 0, 1, buttonOffset, fheroes::StandardWindow::Padding::BOTTOM_LEFT );

        const int buttonCancelIcn = isEvilInterface ? ICN::BUTTON_SMALL_CANCEL_EVIL : ICN::BUTTON_SMALL_CANCEL_GOOD;
        background.renderButton( buttonCancel, buttonCancelIcn, 0, 1, buttonOffset, fheroes::StandardWindow::Padding::BOTTOM_RIGHT );
        ...
        for ( int i = 0; i < 5; ++i ) {
            const fheroes::Sprite & icon = fheroes::AGG::GetICN( ICN::DIFFICULTY_ICON_EASY + i, icnIndex );
            fheroes::Copy( icon, 0, 0, display, coordDifficulty[i] );
            fheroes::addGradientShadow( icon, display, { coordDifficulty[i].x, coordDifficulty[i].y }, { -5, 5 } );
        }
        ...
        const fheroes::Rect maxTextRoi{ scenarioBoxRoi.x + boxBorder, scenarioBoxRoi.y + 5, maxTextAreaWidth, 19 };
        ...
        const fheroes::Rect centeredTextRoi{ scenarioBoxRoi.x + boxBorder + buttonSelectWidth, scenarioBoxRoi.y + 5, 2 * rightSideAvailableTextWidth, 19 };

        // Set up restorers.
        fheroes::ImageRestorer mapTitleArea( display, maxTextRoi.x, maxTextRoi.y, maxTextRoi.width, maxTextRoi.height );
        fheroes::ImageRestorer opponentsArea( display, roi.x, pointOpponentInfo.y, roi.width, 65 );
        fheroes::ImageRestorer classArea( display, roi.x, pointClassInfo.y, roi.width, 69 );
        fheroes::ImageRestorer handicapArea( display, roi.x, pointClassInfo.y + 69, roi.width, 31 );
        fheroes::ImageRestorer ratingArea( display, buttonOk.area().x + buttonOk.area().width, buttonOk.area().y,
        ...
        fheroes::Rect ratingRoi = RedrawRatingInfo( roi.getPosition(), roi.width );

        fheroes::MovableSprite levelCursor( difficultyCursor );
        ...
        while ( true ) {
            ...
            if ( HotKeyPressEvent( Game::HotKeyEvent::MAIN_MENU_SELECT_MAP ) || le.MouseClickLeft( buttonSelectMaps.area() ) ) {
                ...
                if ( lists.empty() ) {
                    ...
                    result = fheroes::GameMode::MAIN_MENU;
                    break;
                }
            ...
            if ( Game::HotKeyPressEvent( Game::HotKeyEvent::DEFAULT_OKAY ) || le.MouseClickLeft( buttonOk.area() ) ) {
                ...
                result = fheroes::GameMode::START_GAME;
            ...
            if ( le.isMouseRightButtonPressedInArea( roi ) ) {
                if ( le.isMouseRightButtonPressedInArea( buttonSelectMaps.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Scenario" ), _( "Click here to select which scenario to play." ), Dialog::ZERO );
                }
                    ...
                    fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to accept these settings and start a new game." ), Dialog::ZERO );
                }
                else if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Cancel" ), _( "Click to return to the main menu." ), Dialog::ZERO );
                }
                else {
    ...
    fheroes::GameMode LoadNewMap()
    {
        ...
        if ( mapInfo.version == GameVersion::SUCCESSION_WARS || mapInfo.version == GameVersion::PRICE_OF_LOYALTY ) {
            ...
            fheroes::drawMainMenuScreen();
            fheroes::showStandardTextMessage( _( "Warning" ), _( "The map is corrupted or doesn't exist." ), Dialog::OK );
...
fheroes::GameMode Game::SelectScenario( const uint8_t humanPlayerCount )
{
    ...
    if ( maps.empty() ) {
        fheroes::showStandardTextMessage( _( "Warning" ), _( "No maps available!" ), Dialog::OK );
        return fheroes::GameMode::MAIN_MENU;
    }

    // We must release UI resources for this window before loading a new map. That's why all UI logic is in a separate function.
    const fheroes::GameMode result = ChooseNewMap( maps, humanPlayerCount );
    if ( result != fheroes::GameMode::START_GAME ) {
        return result;
    }
