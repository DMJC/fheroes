...
namespace
{
    const fheroes::Size riddleArea{ 250, 180 };

    const fheroes::Size answerArea{ 250, 155 };
    ...
    public:
        ...
        AnswerListBox( const fheroes::Point & pt, const fheroes::SupportedLanguage language )
            : ListBox( pt )
            , _language( language )
        {
            // Do nothing.
        ...
        {
            fheroes::Text text{ answer, ( current ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() ), _language };
...
namespace Editor
{
    bool openSphinxWindow( Maps::Map_Format::SphinxMetadata & metadata, const fheroes::SupportedLanguage language )
    {
        const CursorRestorer cursorRestorer( true, Cursor::POINTER );

        fheroes::Display & display = fheroes::Display::instance();
        fheroes::StandardWindow background{ riddleArea.width + answerArea.width + 50, riddleArea.height + 220, true, display };

        const fheroes::Rect windowArea{ background.activeArea() };

        int32_t offsetY = windowArea.y + elementOffset;

        const fheroes::Text title( MP2::StringObject( MP2::OBJ_SPHINX ), fheroes::FontType::normalYellow() );
        title.draw( windowArea.x + ( windowArea.width - title.width() ) / 2, offsetY, display );

        offsetY += title.height() + elementOffset;

        fheroes::Text text{ _( "Riddle:" ), fheroes::FontType::normalWhite() };

        const fheroes::Rect riddleRoi{ windowArea.x + elementOffset, offsetY + text.height(), riddleArea.width, riddleArea.height };
        background.applyTextBackgroundShading( riddleRoi );

        fheroes::ImageRestorer riddleRoiRestorer( display, riddleRoi.x, riddleRoi.y, riddleRoi.width, riddleRoi.height );

        text.draw( riddleRoi.x + ( riddleRoi.width - text.width() ) / 2, offsetY, display );

        text.set( metadata.riddle, fheroes::FontType::normalWhite(), language );
        text.draw( riddleRoi.x + 5, riddleRoi.y + 5, riddleRoi.width - 10, display );

        const fheroes::Rect answerRoi{ windowArea.x + elementOffset + riddleRoi.width + elementOffset, offsetY + text.height(), answerArea.width, answerArea.height };
        background.applyTextBackgroundShading( answerRoi );

        text.set( _( "Answers:" ), fheroes::FontType::normalWhite() );
        ...
        answerList.setScrollBarImage( fheroes::AGG::GetICN( listIcnId, 4 ) );
        ...
        fheroes::Button buttonAdd( answerRoi.x, answerRoi.y + answerRoi.height + 5, minibuttonIcnId, 13, 14 );
        buttonAdd.draw();

        fheroes::Button buttonEdit( answerRoi.x + buttonOffset, answerRoi.y + answerRoi.height + 5, minibuttonIcnId, 15, 16 );
        buttonEdit.draw();

        fheroes::Button buttonDelete( answerRoi.x + answerArea.width - buttonWidth, answerRoi.y + answerRoi.height + 5, minibuttonIcnId, 17, 18 );
        buttonDelete.draw();

        offsetY += text.height() + riddleArea.height + elementOffset;

        text.set( _( "Reward:" ), fheroes::FontType::normalWhite() );
        ...
        const fheroes::Rect artifactRoi{ riddleRoi.x + ( riddleRoi.width - artifactFrame.width() ) / 2, offsetY + text.height(), artifactFrame.width(),
                                          artifactFrame.height() };

        fheroes::Blit( artifactFrame, display, artifactRoi.x, artifactRoi.y );

        auto redrawArtifactImage = [&display, &artifactRoi]( const int32_t artifactId ) {
            const fheroes::Sprite & artifactImage = fheroes::AGG::GetICN( ICN::ARTIFACT, Artifact( artifactId ).IndexSprite64() );
        ...
        const fheroes::Rect resourceRoi{ answerRoi.x, offsetY + text.height(), answerRoi.width, 99 };
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.MouseClickLeft( riddleRoi ) ) {
                ...
                const fheroes::Text body{ _( "Riddle:" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, temp, longestRiddle, true, language ) ) {
                ...
                const fheroes::Text body{ _( "Answer:" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, newAnswer, longestAnswer, false, language ) ) {
                    if ( std::any_of( metadata.answers.begin(), metadata.answers.end(), [&newAnswer]( const auto & answer ) { return answer == newAnswer; } ) ) {
                        fheroes::showStandardTextMessage( _( "Answer" ), _( "This answer exists in the list." ), Dialog::OK );
                        continue;
                    }
                ...
                const fheroes::Text body{ _( "Answer:" ), fheroes::FontType::normalWhite() };
                if ( Dialog::inputString( fheroes::Text{}, body, temp, longestAnswer, false, language ) ) {
                ...
                const fheroes::Sprite & artifactImage = fheroes::AGG::GetICN( ICN::ARTIFACT, Artifact( metadata.artifact ).IndexSprite64() );
                fheroes::Copy( artifactImage, 0, 0, display, artifactRoi.x + 6, artifactRoi.y + 6, artifactImage.width(), artifactImage.height() );
                ...
                fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to save the Sphinx properties." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonAdd.area() ) ) {
                fheroes::showStandardTextMessage( _( "Add Answer" ), _( "Add an additional answer for the question." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonEdit.area() ) ) {
                fheroes::showStandardTextMessage( _( "Edit Answer" ), _( "Edit an existing answer for the question." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonDelete.area() ) ) {
                fheroes::showStandardTextMessage( _( "Delete Answer" ), _( "Delete an existing answer for the question." ), Dialog::ZERO );
            }
                ...
                if ( artifact.isValid() ) {
                    fheroes::ArtifactDialogElement artifactUI( artifact );

                    fheroes::showStandardTextMessage( artifact.GetName(), fheroes::getArtifactData( metadata.artifact ).getDescription( metadata.artifactMetadata ),
                                                       Dialog::ZERO, { &artifactUI } );
                }
                else {
                    fheroes::showStandardTextMessage( _( "Artifact" ), _( "No artifact will be given as a reward." ), Dialog::ZERO );
                }
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonDeleteArtifact.area() ) ) {
                fheroes::showStandardTextMessage( _( "Delete Artifact" ), _( "Delete an artifact from the reward." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( resourceRoi ) ) {
                if ( metadata.resources.GetValidItemsCount() == 0 ) {
                    fheroes::showStandardTextMessage( _( "Resources" ), _( "No resources will be given as a reward." ), Dialog::ZERO );
                }
                else {
                    fheroes::showResourceMessage( fheroes::Text( _( "Resources" ), fheroes::FontType::normalYellow() ),
                                                   fheroes::Text{ _( "Resources will be given as a reward." ), fheroes::FontType::normalWhite() }, Dialog::ZERO,
                                                   metadata.resources );
                }
            }
