...
Settings::Settings()
    : _resolutionInfo( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT )
    , _windowPos( -1, -1 )
...
bool Settings::Read( const std::string & filePath )
{
    ...
    if ( config.Exists( "videomode" ) ) {
        _resolutionInfo = config.ResolutionParams( "videomode", { fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT } );
    }
    ...
    if ( config.Exists( "cursor soft rendering" ) ) {
        if ( config.StrParams( "cursor soft rendering" ) == "on" ) {
            ...
            fheroes::cursor().enableSoftwareEmulation( false );
        }
    }
...
std::string Settings::String() const
{
    ...
    os << "# fheroes configuration file (saved by version " << GetVersion() << ")" << std::endl;
    os << std::endl
       << "# !!! WARNING !!!" << std::endl
...
{
    fheroes::updateAlphabet( language );
...
const std::vector<std::string> & Settings::GetRootDirs()
{
    static const std::vector<std::string> rootDirs = []() {
...
#ifdef FHEROES_DATA
        // Macro-defined path.
        result.emplace_back( EXPANDDEF( FHEROES_DATA ) );
#endif
        ...
        const char * dataEnvPath = getenv( "FHEROES_DATA" );
...
void Settings::setFullScreen( const bool enable )
{
    ...
    if ( enable != fheroes::engine().isFullScreen() ) {
        fheroes::engine().toggleFullScreen();
...
void Settings::setVSync( const bool enable )
{
    if ( enable ) {
        ...
        fheroes::engine().setVSync( true );
    }
    else {
        _gameOptions.ResetModes( GAME_RENDER_VSYNC );
        fheroes::engine().setVSync( false );
    }
}
...
void Settings::setSystemInfo( const bool enable )
{
    if ( enable ) {
        ...
        fheroes::RenderProcessor::instance().enableRenderers();
    }
    else {
        _gameOptions.ResetModes( GAME_SYSTEM_INFO );
        fheroes::RenderProcessor::instance().disableRenderers();
    }
}
...
void Settings::setScreenScalingTypeNearest( const bool enable )
{
    if ( enable ) {
        ...
        fheroes::engine().setNearestScaling( true );
    }
    else {
        _gameOptions.ResetModes( GAME_SCREEN_SCALING_TYPE_NEAREST );
        fheroes::engine().setNearestScaling( false );
    }
}
