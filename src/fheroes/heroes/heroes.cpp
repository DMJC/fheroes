...
namespace
{
    ...
    int getObjectMoraleModifiers( const std::set<MP2::MapObjectType> & objectTypes, std::string * output )
    {
        ...
        for ( const MP2::MapObjectType objectType : objectTypes ) {
            ...
            fheroes::appendModifierToString( *output, modifier );
            output->append( "\n" );
        }
...
uint32_t Heroes::GetMaxMovePoints( const bool onWater ) const
{
    ...
    if ( onWater ) {
        ...
        // Artifact bonuses
        result += GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::SEA_MOBILITY );
        ...
        // Artifact bonuses
        result += GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::LAND_MOBILITY );
...
int Heroes::getMoraleWithModifiers( std::string * text ) const
{
    ...
    const Artifact maxMoraleArtifact = bag_artifacts.getFirstArtifactWithBonus( fheroes::ArtifactBonusType::MAXIMUM_MORALE );
...
int Heroes::getLuckWithModifiers( std::string * text ) const
{
    ...
    const Artifact maxLuckArtifact = bag_artifacts.getFirstArtifactWithBonus( fheroes::ArtifactBonusType::MAXIMUM_LUCK );
...
bool Heroes::Recruit( const PlayerColor col, const fheroes::Point & pt )
{
...
uint32_t Heroes::getDailyRestoredSpellPoints() const
{
    ...
    points += static_cast<uint32_t>( GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::SPELL_POINTS_DAILY_GENERATION ) );
...
bool Heroes::PickupArtifact( const Artifact & art )
{
    ...
    if ( !bag_artifacts.PushArtifact( art ) ) {
        if ( isControlHuman() ) {
            if ( art.GetID() == Artifact::MAGIC_BOOK ) {
                if ( HaveSpellBook() ) {
                    fheroes::showStandardTextMessage( art.GetName(), _( "You cannot have multiple spell books." ), Dialog::OK );
                }
                else {
                ...
                fheroes::showStandardTextMessage( art.GetName(), _( "You cannot pick up this artifact, you already have a full load!" ), Dialog::OK );
            }
        }
    ...
    const auto scout = [this]( const int32_t artifactID ) {
        ...
        if ( std::find( bonuses.begin(), bonuses.end(), fheroes::ArtifactBonus( fheroes::ArtifactBonusType::AREA_REVEAL_DISTANCE ) ) == bonuses.end() ) {
            return false;
        }
...
bool Heroes::BuySpellBook( const Castle & castle )
{
    ...
    if ( !kingdom.AllowPayment( payment ) ) {
        if ( isControlHuman() ) {
            ...
            const fheroes::ArtifactDialogElement artifactUI( Artifact::MAGIC_BOOK );
            fheroes::showStandardTextMessage( GetName(), std::move( header ), Dialog::OK, { &artifactUI } );
        }
        return false;
    }
    ...
    if ( isControlHuman() ) {
        ...
        const fheroes::ArtifactDialogElement artifactUI( Artifact::MAGIC_BOOK );
        if ( fheroes::showStandardTextMessage( GetName(), std::move( header ), Dialog::YES | Dialog::NO, { &artifactUI } ) == Dialog::NO ) {
            return false;
        }
    }
...
{
    return GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::AREA_REVEAL_DISTANCE )
...
fheroes::Rect Heroes::GetScoutRoi() const
{
    const int32_t scoutRange = GetScoutingDistance();
    const fheroes::Point heroPosition = GetCenter();
...
const fheroes::Sprite & Heroes::GetPortrait( const int heroId, const int portraitType )
{
    if ( isValidId( heroId ) )
        switch ( portraitType ) {
        case PORT_BIG:
            return fheroes::AGG::GetICN( ICN::getHeroPortraitIcnId( heroId ), 0 );
        case PORT_MEDIUM: {
            ...
            static std::map<int, fheroes::Sprite> mediumSizePortrait;
            auto iter = mediumSizePortrait.find( heroId );
            if ( iter != mediumSizePortrait.end() ) {
                return iter->second;
            }

            const fheroes::Sprite & original = fheroes::AGG::GetICN( ICN::getHeroPortraitIcnId( heroId ), 0 );
            fheroes::Sprite output( 50, 47 );
            fheroes::Resize( original, output );

            return mediumSizePortrait.try_emplace( heroId, std::move( output ) ).first->second;
        }
        case PORT_SMALL:
            if ( heroId == Heroes::DEBUG_HERO ) {
                return fheroes::AGG::GetICN( ICN::MINIPORT, BRAX - 1 );
            }

            // Since hero IDs start from 1 we have to deduct 1 from the ID.
            return fheroes::AGG::GetICN( ICN::MINIPORT, heroId - 1 );
        default:
            break;
        }
...
void Heroes::PortraitRedraw( const int32_t px, const int32_t py, const PortraitType type, fheroes::Image & dstsf ) const
{
    const fheroes::Sprite & port = GetPortrait( _portrait, type );
    fheroes::Point mp;

    if ( !port.empty() ) {
        if ( PORT_BIG == type ) {
            fheroes::Copy( port, 0, 0, dstsf, px, py, port.width(), port.height() );
            mp.y = 2;
            mp.x = port.width() - 12;
        }
            ...
            const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::PORTXTRA, 0 );
            const fheroes::Sprite & mobility = fheroes::AGG::GetICN( ICN::MOBILITY, GetMobilityIndexSprite() );
            const fheroes::Sprite & mana = fheroes::AGG::GetICN( ICN::MANA, GetManaIndexSprite() );

            const int barw = 7;

            // Draw background.
            fheroes::Blit( background, dstsf, px, py );

            // Draw mobility.
            fheroes::Copy( mobility, 0, 0, dstsf, px, py + mobility.y(), mobility.width(), mobility.height() );

            // Draw hero's portrait.
            fheroes::Copy( port, 0, 0, dstsf, px + barw + 1, py, port.width(), port.height() );

            // Draw mana.
            fheroes::Copy( mana, 0, 0, dstsf, px + barw + port.width() + 2, py + mana.y(), mana.width(), mana.height() );
    ...
    if ( Modes( Heroes::SLEEPER ) ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::MISC4, 14 );
        fheroes::Image sleeperBG( sprite.width() - 4, sprite.height() - 4 );
        sleeperBG.fill( 0 );

        fheroes::Blit( sleeperBG, dstsf, px + mp.x + 3, py + mp.y - 1 );
        fheroes::Blit( sprite, dstsf, px + mp.x + 1, py + mp.y - 3 );
    }
}
...
Heroes * AllHeroes::Get( const fheroes::Point & center ) const
{
...
{
    fheroes::fixFrenchCharactersForMP2Map( _name );
}
...
IStreamBase & operator>>( IStreamBase & stream, Heroes & hero )
{
    ...
    if ( Game::GetVersionOfCurrentSaveFile() < FORMAT_VERSION_1010_RELEASE ) {
        ...
        hero._patrolCenter = fheroes::Point( patrolX, patrolY );
    }
    else {
