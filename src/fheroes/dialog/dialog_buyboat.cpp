...
int Dialog::BuyBoat( bool enable )
{
    ...
    Resource::BoxSprite rbs( PaymentConditions::BuyBoat(), fheroes::boxAreaWidthPx );

    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::BOATWIND, 0 );
    fheroes::Text text{ _( "Build a new ship:" ), fheroes::FontType::normalWhite() };
    const int spacer = 10;
    ...
    fheroes::Point dst_pt( box_rt.x + ( box_rt.width - text.width() ) / 2, box_rt.y );
    ...
    text.set( _( "Resource cost:" ), fheroes::FontType::normalWhite() );
    ...
    // buttons
    fheroes::ButtonGroup buttonGroup( box_rt, Dialog::OK | Dialog::CANCEL );
