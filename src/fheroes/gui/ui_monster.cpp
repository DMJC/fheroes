...
namespace fheroes
{
    void renderMonsterFrame( const Monster & monster, Image & output, const Point & offset )
    {
        ...
        const fheroes::Sprite & monsterImage = fheroes::AGG::GetICN( monster.ICNMonh(), 0 );
        fheroes::Blit( monsterImage, output, offset.x + monsterImage.x(), offset.y + monsterImage.y() );
    }
}
