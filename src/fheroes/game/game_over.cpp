...
namespace
{
    void DialogWins( const uint32_t cond )
    {
        ...
        std::vector<fheroes::LocalizedString> strings;
        ...
        if ( strings.empty() ) {
            ...
            case GameOver::WINS_TOWN: {
                ...
                assert( town != nullptr );
                if ( town ) {
                    strings = fheroes::getLocalizedStrings( _( "You captured %{name}!\nYou are victorious." ), gameLanguage, "%{name}", town->GetName(),
            ...
            case GameOver::WINS_HERO: {
                ...
                if ( hero ) {
                    strings = fheroes::getLocalizedStrings( _( "You have captured the enemy hero %{name}!\nYour quest is complete." ), gameLanguage, "%{name}",
        ...
        if ( !strings.empty() ) {
            ...
            const fheroes::Text header( _( "Victory!" ), fheroes::FontType::normalYellow() );
            const auto body = fheroes::getLocalizedText( strings, fheroes::FontType::normalWhite() );
    ...
    void DialogLoss( const uint32_t cond )
    {
        ...
        case GameOver::LOSS_ENEMY_WINS_TOWN: {
            ...
            if ( town ) {
                strings = fheroes::getLocalizedStrings( _( "The enemy has captured %{name}!\nThey are triumphant." ), gameLanguage, "%{name}", town->GetName(),
                                                         gameLanguage );
            }
        ...
        case GameOver::LOSS_HERO: {
            ...
            if ( hero ) {
                strings = fheroes::getLocalizedStrings( _( "You have lost the hero %{name}.\nYour quest is over." ), gameLanguage, "%{name}", hero->GetName(),
                                                         mapInfo.getSupportedLanguage().value_or( gameLanguage ) );
            }
        ...
        if ( !strings.empty() ) {
            ...
            const fheroes::Text header( _( "Defeat!" ), fheroes::FontType::normalYellow() );
            const auto body = fheroes::getLocalizedText( strings, fheroes::FontType::normalWhite() );
            fheroes::showMessage( header, *body, Dialog::OK );
        }
    }
    ...
    Video::Subtitle standardGameResults()
    {
        ...
        StringReplace( textBody, "%{rating}", fheroes::HighScoreDataContainer::getMonsterByRating( score ).GetName() );
        ratingText.add( { textBody, fheroes::FontType::normalWhite() } );
...
std::vector<fheroes::LocalizedString> GameOver::GetActualDescription( const uint32_t conditions, const std::optional<fheroes::SupportedLanguage> mapLanguage )
{
    // This should be populated only for strings that are set for objects within the Editor.
    std::optional<std::pair<std::string, fheroes::LocalizedString>> translationReplacement;
    ...
    if ( conditions == WINS_ALL ) {
        ...
        if ( town ) {
            ...
            translationReplacement.emplace( "%{name}", fheroes::LocalizedString( town->GetName(), mapLanguage ) );
        }
    }
        ...
        if ( hero ) {
            ...
            translationReplacement.emplace( "%{name}", fheroes::LocalizedString( hero->GetName(), mapLanguage ) );
        }
    }
    ...
    if ( translationReplacement.has_value() ) {
        return fheroes::getLocalizedStrings( std::move( msg ), gameLanguage, translationReplacement->first, translationReplacement->second.text,
                                              *( translationReplacement->second.language ) );
    }

    return { fheroes::LocalizedString( std::move( msg ), gameLanguage ) };
}
...
fheroes::GameMode GameOver::Result::checkGameOver()
{
    ...
    if ( isSinglePlayer ) {
        ...
        if ( kingdom.isControlHuman() || isAIAutoControlMode ) {
            ...
            if ( result != GameOver::COND_NONE ) {
                ...
                if ( conf.isCampaignGameType() ) {
                    return fheroes::GameMode::COMPLETE_CAMPAIGN_SCENARIO;
                }
