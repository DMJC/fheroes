 ...
 *   Copyright (C) 2011 by Andrey Afletdinov <fheroes@gmail.com>          *
 *                                                                         *
...
namespace
{
    ...
    fheroes::Sprite renderMonsterOnBackground( const fheroes::Sprite & monsterSprite )
    {
        const fheroes::Sprite & backgroundOriginal = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );
        fheroes::Sprite image = fheroes::Crop( backgroundOriginal, 36, 267, 43, 43 );
        image.setPosition( 0, 0 );

        fheroes::Blit( monsterSprite, 0, 0, image, ( image.width() - monsterSprite.width() ) / 2, ( image.height() - monsterSprite.height() ) / 2, monsterSprite.width(),
    ...
    {
    public:
        explicit SelectEnumMonster( const fheroes::Size & rt, std::string title )
        ...
        void ActionListPressRight( int & index ) override
        {
            ...
            if ( !monster.isValid() ) {
                fheroes::showStandardTextMessage( monster.GetName(), "", Dialog::ZERO );
                return;
            }
    ...
    private:
        ...
        virtual fheroes::Sprite getImage( const int index )
        {
            const Monster mons( index );
            const fheroes::Sprite & monsterSprite = fheroes::AGG::GetICN( ICN::MONS32, mons.GetSpriteIndex() );
    ...
    private:
        ...
        fheroes::Sprite getImage( const int index ) override
        {
            const Monster mons( index );
            const fheroes::Sprite & monsterSprite = fheroes::AGG::GetICN( ICN::MONS32, mons.GetSpriteIndex() );
            fheroes::Sprite image = renderMonsterOnBackground( monsterSprite );

            if ( _selected.count( index ) == 0 ) {
                fheroes::ApplyPalette( image, PAL::GetPalette( PAL::PaletteType::GRAY ) );
            }
    ...
    {
    public:
        explicit SelectEnumHeroes( const fheroes::Size & rt, std::string title )
            : Dialog::ItemSelectionWindow( rt, std::move( title ) )
        {
            SetAreaMaxItems( rtAreaItems.height / _offsetY );
        }
        ...
        void RedrawItem( const int & index, int32_t dstx, int32_t dsty, bool current ) override
        {
            const fheroes::Sprite & port = Heroes::GetPortrait( index, PORT_SMALL );
    ...
    {
    public:
        explicit SelectEnumArtifact( const fheroes::Size & rt, std::string title )
            : Dialog::ItemSelectionWindow( rt, std::move( title ) )
        {
            SetAreaMaxItems( rtAreaItems.height / _offsetY );
        }
        ...
        void RedrawItem( const int & index, int32_t dstx, int32_t dsty, bool current ) override
        {
            ...
            const fheroes::Sprite & artifactSprite = fheroes::AGG::GetICN( ICN::ARTFX, art.IndexSprite32() );
        ...
        void ActionListPressRight( int & index ) override
        {
            fheroes::ArtifactDialogElement( Artifact( index ) ).showPopup( Dialog::ZERO );
        }
    ...
    {
    public:
        explicit SelectEnumSpell( const fheroes::Size & rt, std::string title )
            : Dialog::ItemSelectionWindow( rt, std::move( title ) )
        {
            SetAreaMaxItems( rtAreaItems.height / _offsetY );
        }
        ...
        void RedrawItem( const int & index, int32_t dstx, int32_t dsty, bool current ) override
        {
            ...
            const fheroes::Sprite & spellSprite = fheroes::AGG::GetICN( ICN::SPELLS, spell.IndexSprite() );
        ...
        void ActionListPressRight( int & index ) override
        {
            fheroes::SpellDialogElement( Spell( index ), nullptr ).showPopup( Dialog::ZERO );
        }
    ...
    public:
        ...
        explicit SelectEnumSecSkill( const fheroes::Size & rt, std::string title )
            : Dialog::ItemSelectionWindow( rt, std::move( title ) )
        {
            SetAreaMaxItems( rtAreaItems.height / _offsetY );
        }
        ...
        void RedrawItem( const int & index, int32_t dstx, int32_t dsty, bool current ) override
        {
            ...
            const fheroes::Sprite & skillSprite = fheroes::AGG::GetICN( ICN::MINISS, skill.GetIndexSprite2() );
        ...
        void ActionListPressRight( int & index ) override
        {
            fheroes::SecondarySkillDialogElement( Skill::Secondary( getSkillFromListIndex( index ), getLevelFromListIndex( index ) ), Heroes() )
    ...
    {
    public:
        explicit SelectKingdomCastle( const fheroes::Size & rt, std::string title, std::string description )
        ...
        void RedrawItem( const int & index, int32_t dstx, int32_t dsty, bool current ) override
        {
            ...
            fheroes::Sprite castleIcon( fheroes::AGG::GetICN( _townFrameIcnId, 23 ) );
            fheroes::drawCastleIcon( *castle, castleIcon, { 4, 4 } );
    ...
    {
    public:
        ObjectTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title, const int32_t imageOffsetX,
        ...
        void RedrawItem( const int & objectId, int32_t posX, int32_t posY, bool isSelected ) override
        {
            ...
            const fheroes::Sprite & image = getObjectImage( _objectInfo[objectId] );

            const int32_t imageHeight = image.height();
            const int32_t imageWidth = image.width();
            if ( imageHeight > fheroes::tileWidthPx * 3 || imageWidth > fheroes::tileWidthPx * 5 ) {
                // Reduce the size of very tall images to fit the list.
                const double ratio = std::max( imageHeight / ( fheroes::tileWidthPx * 3. ), imageWidth / ( fheroes::tileWidthPx * 5. ) );
                fheroes::Sprite resized( static_cast<int32_t>( imageWidth / ratio ), static_cast<int32_t>( imageHeight / ratio ) );
                fheroes::Resize( image, resized );
    ...
    private:
        ...
        virtual fheroes::Sprite getObjectImage( const Maps::ObjectInfo & object ) const
        {
            return fheroes::generateMapObjectImage( object );
        }
    ...
    {
    public:
        MonsterTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title )
    ...
    private:
        ...
        fheroes::Sprite getObjectImage( const Maps::ObjectInfo & object ) const override
        {
            const fheroes::Sprite & image = fheroes::generateMapObjectImage( object );
    ...
    {
    public:
        ArtifactTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title )
    ...
    private:
        void showPopupWindow( const Maps::ObjectInfo & info ) override
        {
            ...
            case MP2::OBJ_ARTIFACT:
                fheroes::ArtifactDialogElement( Artifact( static_cast<int>( info.metadata[0] ) ) ).showPopup( Dialog::ZERO );
                break;
            ...
            case MP2::OBJ_RANDOM_ULTIMATE_ARTIFACT:
                fheroes::showStandardTextMessage( MP2::StringObject( info.objectType ), "", Dialog::ZERO );
                break;
            default:
    ...
    {
    public:
        TreasureTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title )
    ...
    private:
        void showPopupWindow( const Maps::ObjectInfo & info ) override
        {
            ...
            case MP2::OBJ_RESOURCE:
                fheroes::showResourceMessage( fheroes::Text{ getObjectName( info ), fheroes::FontType::normalYellow() },
                                               fheroes::Text{ Resource::getDescription(), fheroes::FontType::normalWhite() }, Dialog::ZERO,
            ...
            case MP2::OBJ_TREASURE_CHEST:
                fheroes::showStandardTextMessage( getObjectName( info ), "", Dialog::ZERO );
                break;
            default:
                // Did you expand the list of treasures? Add the corresponding logic!
                assert( 0 );
    ...
    {
    public:
        OceanObjectTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title )
    ...
    {
    public:
        GenericObjectTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title )
    ...
    private:
        ...
        std::string getObjectName( const Maps::ObjectInfo & info ) override
        {
            ...
            if ( info.objectType == MP2::OBJ_BARRIER ) {
                ...
                StringReplace( str, "%{color}", fheroes::getBarrierColorName( static_cast<int32_t>( info.metadata[0] ) ) );
                return str;
            }

            if ( info.objectType == MP2::OBJ_TRAVELLER_TENT ) {
                std::string str = _( "%{color} Tent" );
                StringReplace( str, "%{color}", fheroes::getTentColorName( static_cast<int32_t>( info.metadata[0] ) ) );
                return str;
            }
    ...
    {
    public:
        PowerUpObjectTypeSelection( const std::vector<Maps::ObjectInfo> & objectInfo, const fheroes::Size & size, std::string title )
    ...
    public:
        ...
        void ActionListPressRight( Maps::ObjectInfo & info ) override
        {
            if ( info.objectType == MP2::OBJ_MINE ) {
                ...
                fheroes::showStandardTextMessage( Maps::GetMineName( static_cast<int>( info.metadata[0] ) ), "", Dialog::ZERO );
                return;
            }
            fheroes::showStandardTextMessage( MP2::StringObject( info.objectType ), "", Dialog::ZERO );
        }
        ...
        {
            const fheroes::Sprite mineSprite( fheroes::generateMapObjectImage( info ) );

            fheroes::Display & display = fheroes::Display::instance();

            fheroes::Blit( mineSprite, display, dstx + fheroes::tileWidthPx * 5 + 5 - mineSprite.width(), dsty + 1 );

            // Mine type selection mark background.
            const fheroes::Sprite & markBackground = fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::CELLWIN_EVIL : ICN::CELLWIN, 4 );
            fheroes::Blit( markBackground, display, dstx + 10, dsty + 24 );

            if ( current ) {
                const fheroes::Sprite & mark = fheroes::AGG::GetICN( ICN::CELLWIN, 5 );
                fheroes::Blit( mark, display, dstx + 13, dsty + 27 );
            }
        }
                                     ...
                                     int32_t castlePositionIndex /* = -1 */ )
{
    ...
    const int32_t maxHeight = std::min( 100 + SelectKingdomCastle::itemsOffsetY * 12, fheroes::Display::instance().height() - dialogHeightDeduction );
...
namespace Dialog
{
    ItemSelectionWindow::ItemSelectionWindow( const fheroes::Size & dialogSize, std::string title, std::string description )
    {
        assert( !title.empty() );

        fheroes::Display & display = fheroes::Display::instance();
        _window = std::make_unique<fheroes::StandardWindow>( dialogSize.width, dialogSize.height, true, display );

        const fheroes::Rect & area = _window->activeArea();

        int32_t listOffsetY = 0;

        fheroes::Text text( std::move( title ), fheroes::FontType::normalYellow() );
        text.draw( area.x + ( area.width - text.width() ) / 2, area.y + 10, display );

        // The additional text under the title.
        if ( !description.empty() ) {
            text.set( std::move( description ), fheroes::FontType::normalWhite() );
            text.draw( area.x + ( area.width - text.width() ) / 2, area.y + 30, display );
            listOffsetY = text.height() + 3;
        }

        const fheroes::Rect listRoi( area.x + 10, area.y + 30 + listOffsetY, area.width - 40, area.height - 70 - listOffsetY );

        _window->applyTextBackgroundShading( listRoi );

        _backgroundRestorer = std::make_unique<fheroes::ImageRestorer>( display, listRoi.x, listRoi.y, listRoi.width, listRoi.height );
        ...
        setScrollBarImage( fheroes::AGG::GetICN( listIcnId, 4 ) );

        // Render dialog buttons.
        _window->renderOkayCancelButtons( _buttonOk, _buttonCancel );
    }

    void ItemSelectionWindow::renderItem( const fheroes::Sprite & itemSprite, std::string itemText, const fheroes::Point & destination,
                                          const int32_t middleImageOffsetX, const int32_t textOffsetX, const int32_t itemOffsetY, const bool current ) const
    {
        fheroes::Display & display = fheroes::Display::instance();

        fheroes::Blit( itemSprite, display, destination.x + middleImageOffsetX - ( itemSprite.width() / 2 ), destination.y + itemOffsetY - ( itemSprite.height() / 2 ) );

        fheroes::Text text( std::move( itemText ), current ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() );
        renderText( text, destination, textOffsetX, itemOffsetY );
    }

    void ItemSelectionWindow::renderItem( const fheroes::Sprite & itemSprite, std::vector<fheroes::LocalizedString> itemText, const fheroes::Point & destination,
                                          const int32_t middleImageOffsetX, const int32_t textOffsetX, const int32_t itemOffsetY, const bool current ) const
    {
        fheroes::Display & display = fheroes::Display::instance();
        ...
        auto text = fheroes::getLocalizedText( std::move( itemText ), current ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() );
        renderText( *text, destination, textOffsetX, itemOffsetY );
    }

    void ItemSelectionWindow::renderText( fheroes::TextBase & text, const fheroes::Point & destination, const int32_t textOffsetX, const int32_t itemOffsetY ) const
    {
        fheroes::Display & display = fheroes::Display::instance();

        text.fitToOneRow( _window->activeArea().width - textOffsetX - 55 );
        const fheroes::Rect & roi = _window->activeArea();
    ...
    int32_t ItemSelectionWindow::selectItemsEventProcessing()
    {
        ...
        while ( !isDoubleClicked() && le.HandleEvents() ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( _buttonOk.area() ) ) {
                fheroes::showStandardTextMessage( _( "Okay" ), _( "Accept the choice made." ), Dialog::ZERO );
            }
    ...
    fheroes::Rect ItemSelectionWindow::getBackgroundArea() const
    {
        assert( _window );
...
Skill::Secondary Dialog::selectSecondarySkill( const Heroes & hero, const int skillId /* = Skill::Secondary::UNKNOWN */ )
{
    ...
    SelectEnumSecSkill listbox( { 350, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Skill:" ) );
                           ...
                           const int32_t spellsLevel /* = -1 */ )
{
    ...
    SelectEnumSpell listbox( { 340, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Spell:" ) );
...
Artifact Dialog::selectArtifact( const int artifactId, const bool isForVictoryConditions )
{
    ...
    for ( int id = Artifact::UNKNOWN + 1; id < Artifact::ARTIFACT_COUNT; ++id ) {
        ...
        if ( !isPriceofLoyaltyArtifactAllowed && fheroes::isPriceOfLoyaltyArtifact( id ) ) {
            continue;
        }
    ...
    SelectEnumArtifact listbox( { 370, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Artifact:" ) );
...
Monster Dialog::selectMonster( const int monsterId )
{
    ...
    SelectEnumMonster listbox( { 320, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Monster:" ) );
...
int Dialog::selectHeroes( const int heroId /* = Heroes::UNKNOWN */ )
{
    ...
    SelectEnumHeroes listbox( { 300, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Hero:" ) );
...
{
    MultiMonsterSelection monsterList( { 320, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Monsters:" ) );
...
{
    fheroes::Display & display = fheroes::Display::instance();
    fheroes::StandardWindow background( 520, 250, true, display );

    const fheroes::Rect & area = background.activeArea();

    fheroes::Text text( _( "Select Hero:" ), fheroes::FontType::normalYellow() );
    text.draw( area.x + ( area.width - text.width() ) / 2, area.y + 10, display );

    // Render color and race selection sprites.
    const int32_t stepX = 70;
    const int32_t raceOffsetY = 80;
    fheroes::Point pos( area.x + 20, area.y + 30 );
    const fheroes::Sprite & colorSpriteBorderSelected = fheroes::AGG::GetICN( ICN::BRCREST, 6 );
    const fheroes::Sprite & raceShadow = fheroes::AGG::GetICN( ICN::NGEXTRA, 61 );
    fheroes::Sprite colorSpriteBorder( colorSpriteBorderSelected );
    ...
    fheroes::ApplyPalette( colorSpriteBorder, darkGrayPalette );

    // Make race selection borders.
    const fheroes::Sprite & randomRaceSprite = fheroes::AGG::GetICN( ICN::NGEXTRA, 58 );
    const int32_t raceBorderWidth = randomRaceSprite.width();
    const int32_t raceBorderHeight = randomRaceSprite.height();
    fheroes::Image raceSpriteBorder( raceBorderWidth, raceBorderHeight );
    raceSpriteBorder.reset();
    const int32_t raceImageWidth = raceBorderWidth - 8;
    const int32_t raceImageHeight = raceBorderHeight - 8;
    fheroes::Copy( randomRaceSprite, 0, 0, raceSpriteBorder, 0, 0, raceBorderWidth, 4 );
    fheroes::Copy( randomRaceSprite, 0, 4, raceSpriteBorder, 0, 4, 4, raceImageHeight );
    int32_t imageCopyOffset = raceBorderWidth - 4;
    fheroes::Copy( randomRaceSprite, imageCopyOffset, 4, raceSpriteBorder, imageCopyOffset, 4, 4, raceImageHeight );
    imageCopyOffset = raceBorderHeight - 4;
    fheroes::Copy( randomRaceSprite, 0, imageCopyOffset, raceSpriteBorder, 0, imageCopyOffset, raceBorderWidth, 4 );
    fheroes::Image raceSpriteBorderSelected( raceSpriteBorder );
    fheroes::ApplyPalette( raceSpriteBorder, darkGrayPalette );
    fheroes::ApplyPalette( raceSpriteBorderSelected, PAL::GetPalette( PAL::PaletteType::RED ) );

    std::array<fheroes::Rect, 6> colorRect;
    std::array<fheroes::Rect, 7> raceRect;

    // Random race.
    fheroes::Rect & randomRaceRect = raceRect.back();
    randomRaceRect = { pos.x - 2, pos.y + raceOffsetY, raceBorderWidth, raceBorderHeight };
    fheroes::Copy( randomRaceSprite, 4, 4, display, randomRaceRect.x + 4, randomRaceRect.y + 4, raceImageWidth, raceImageHeight );
    fheroes::Blit( raceShadow, display, pos.x - 5, pos.y + raceOffsetY + 2 );
    text.set( _( "race|Random" ), fheroes::FontType::smallWhite() );
    text.draw( randomRaceRect.x, randomRaceRect.y + randomRaceRect.height + 4, raceBorderWidth, display );

    pos.x += stepX;
    for ( uint32_t i = 0; i < 6; ++i ) {
        const fheroes::Sprite & colorSprite = fheroes::AGG::GetICN( ICN::BRCREST, i );

        fheroes::addGradientShadow( colorSpriteBorder, display, { pos.x - stepX / 2, pos.y }, { -5, 5 } );
        colorRect[i] = { pos.x - stepX / 2, pos.y, colorSpriteBorder.width(), colorSpriteBorder.height() };
        fheroes::Copy( colorSprite, 0, 0, display, colorRect[i].x + 4, colorRect[i].y + 4, colorSprite.width(), colorSprite.height() );
        text.set( Color::String( Color::IndexToColor( static_cast<int>( i ) ) ), fheroes::FontType::smallWhite() );
        text.draw( colorRect[i].x + ( colorRect[i].width - text.width() ) / 2, colorRect[i].y + colorRect[i].height + 4, display );

        const fheroes::Sprite & raceSprite = fheroes::AGG::GetICN( ICN::NGEXTRA, 51U + i );
        raceRect[i] = { pos.x - 2, pos.y + raceOffsetY, raceBorderWidth, raceBorderHeight };
        fheroes::Copy( raceSprite, 4, 4, display, raceRect[i].x + 4, raceRect[i].y + 4, raceImageWidth, raceImageHeight );
        fheroes::Blit( raceShadow, display, pos.x - 5, pos.y + raceOffsetY + 2 );
        text.set( Race::DoubleLinedString( Race::IndexToRace( static_cast<int>( i ) ) ), fheroes::FontType::smallWhite() );
...

    {
        ...
        const fheroes::Sprite heroImage = fheroes::generateMapObjectImage( objectInfo[0] );
        heroRoi = { heroImage.x(), heroImage.y(), heroImage.width(), heroImage.height() };
    }

    fheroes::ImageRestorer heroBackground( display, pos.x + heroRoi.x, pos.y + heroRoi.y, heroRoi.width, heroRoi.height );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to start placing the selected hero." ), Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "You will place" ), std::move( name ), Dialog::ZERO );
        }
        else {
            for ( size_t i = 0; i < 7; ++i ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( raceRect[i] ) ) {
                    fheroes::showStandardTextMessage( Race::String( Race::IndexToRace( static_cast<int>( i == 6 ? 7 : i ) ) ), _( "Click to select this class." ),
                                                       Dialog::ZERO );
                }
            }
            ...
            for ( size_t i = 0; i < 6; ++i ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( colorRect[i] ) ) {
                    fheroes::showStandardTextMessage( Color::String( Color::IndexToColor( static_cast<int>( i ) ) ), _( "Click to select this color." ), Dialog::ZERO );
                }
            }
        }
        ...
        // Update hero image.
        const fheroes::Sprite heroImage = fheroes::generateMapObjectImage( objectInfo[heroColor * 7 + heroRace] );
        fheroes::Blit( heroImage, display, pos.x + heroImage.x(), pos.y + heroImage.y() );

        // Update color and race borders.
        for ( int i = 0; i < 6; ++i ) {
            fheroes::Blit( ( i == heroColor ) ? colorSpriteBorderSelected : colorSpriteBorder, display, colorRect[i].x, colorRect[i].y );
        }

        for ( int i = 0; i < 7; ++i ) {
            fheroes::Blit( ( i == heroRace ) ? raceSpriteBorderSelected : raceSpriteBorder, display, raceRect[i].x, raceRect[i].y );
        }
...
int Dialog::selectMonsterType( const int monsterType )
{
    ...
    MonsterTypeSelection listbox( objectInfo, { 350, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Monster:" ) );
...
int Dialog::selectArtifactType( const int artifactType )
{
    ...
    ArtifactTypeSelection listbox( objectInfo, { 350, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Artifact:" ) );
...
int Dialog::selectTreasureType( const int resourceType )
{
    ...
    TreasureTypeSelection listbox( objectInfo, { 350, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Treasure:" ) );
...
int Dialog::selectOceanObjectType( const int objectType )
{
    ...
    OceanObjectTypeSelection listbox( objectInfo, { 350, fheroes::Display::instance().height() - dialogHeightDeduction }, _( "Select Ocean Object:" ) );
...
{
    fheroes::Display & display = fheroes::Display::instance();
    fheroes::StandardWindow background( 520, 370, true, display );

    const fheroes::Rect & area = background.activeArea();

    fheroes::Text text( _( "Castle/town placing" ), fheroes::FontType::normalYellow() );
    text.draw( area.x + ( area.width - text.width() ) / 2, area.y + 10, display );
    ...
    std::array<fheroes::Rect, 7> colorRect;
    std::array<fheroes::Rect, 7> raceRect;

    // Neutral color.
    fheroes::addGradientShadow( colorSpriteBorder, display, pos, { -5, 5 } );
    const fheroes::Sprite & neutralColorSprite = fheroes::AGG::GetICN( ICN::BRCREST, 7 );
    fheroes::Rect & neutralColorRect = colorRect.back();
    neutralColorRect = { pos.x, pos.y, colorSpriteBorder.width(), colorSpriteBorder.height() };
    fheroes::Copy( neutralColorSprite, 0, 0, display, neutralColorRect.x + 4, neutralColorRect.y + 4, neutralColorSprite.width(), neutralColorSprite.height() );
    text.set( _( "doubleLinedRace|Neutral" ), fheroes::FontType::smallWhite() );
    ...
    fheroes::ButtonGroup castleTownButtons( { fheroes::getSupportedText( gettext_noop( "TOWN" ), fheroes::FontType::buttonReleasedWhite() ),
                                               fheroes::getSupportedText( gettext_noop( "CASTLE" ), fheroes::FontType::buttonReleasedWhite() ) } );
    fheroes::ButtonBase & buttonTown = castleTownButtons.button( 0 );
    fheroes::ButtonBase & buttonCastle = castleTownButtons.button( 1 );

    const int32_t buttonWidth = buttonTown.area().width;
    const int32_t buttonHeight = buttonTown.area().height;

    const fheroes::Point buttonOffset{ area.x + ( area.width - buttonWidth ) / 2, area.y + area.height - buttonHeight - 7 };
    ...
    const fheroes::Rect castleRoi{ pos.x - 2 * fheroes::tileWidthPx - fheroes::tileWidthPx / 2, pos.y - 4 * fheroes::tileWidthPx + fheroes::tileWidthPx / 2,
                                    5 * fheroes::tileWidthPx, 5 * fheroes::tileWidthPx };

    fheroes::ImageRestorer castleBackground( display, pos.x - 5 * fheroes::tileWidthPx, pos.y - 4 * fheroes::tileWidthPx + fheroes::tileWidthPx / 2,
                                              8 * fheroes::tileWidthPx, 5 * fheroes::tileWidthPx );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to start placing the selected castle/town." ), Dialog::ZERO );
        }
        else if ( buttonTown.isEnabled() && le.isMouseRightButtonPressedInArea( buttonTown.area() ) ) {
            fheroes::showStandardTextMessage( _( "Town" ), _( "Click to select town placing." ), Dialog::ZERO );
        }
        else if ( buttonCastle.isEnabled() && le.isMouseRightButtonPressedInArea( buttonCastle.area() ) ) {
            fheroes::showStandardTextMessage( _( "Castle" ), _( "Click to select castle placing." ), Dialog::ZERO );
        }
            ...
            for ( size_t i = 0; i < 7; ++i ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( raceRect[i] ) ) {
                    ...
                    fheroes::showStandardTextMessage( i == 6 ? _( "race|Neutral" ) : Color::String( Color::IndexToColor( static_cast<int>( i ) ) ),
        ...
        // Update town image.
        const fheroes::Sprite townImage = fheroes::generateTownObjectImage( getPackedTownType( townRace, isCastle ), townColor, basementGround );
        fheroes::Blit( townImage, display, pos.x + townImage.x(), pos.y + townImage.y() );

        // Update color and race borders.
        for ( int i = 0; i < 7; ++i ) {
            fheroes::Blit( ( i == townColor ) ? colorSpriteBorderSelected : colorSpriteBorder, display, colorRect[i].x, colorRect[i].y );
            fheroes::Blit( ( i == townRace ) ? raceSpriteBorderSelected : raceSpriteBorder, display, raceRect[i].x, raceRect[i].y );
        }
...
int Dialog::selectDwellingType( const int dwellingType )
{
    ...
    GenericObjectTypeSelection listbox( objectInfo, { 480, fheroes::Display::instance().height() - 180 }, _( "Select Dwelling:" ) );
...
int Dialog::selectLandscapeMiscellaneousObjectType( const int objectType )
{
    ...
    GenericObjectTypeSelection listbox( objectInfo, { 420, fheroes::Display::instance().height() - 180 }, _( "Select Landscape Object:" ) );
...
{
    fheroes::Display & display = fheroes::Display::instance();
    fheroes::StandardWindow background( 380, 395, true, display );

    const fheroes::Rect & area = background.activeArea();

    fheroes::Text text( _( "Mine placing" ), fheroes::FontType::normalYellow() );
    ...
    offsetY += 50;
    const fheroes::Rect resourceSelectRoi( area.x + 22, offsetY - 5, 100, stepY * resourceCount + 10 );
    background.applyTextBackgroundShading( resourceSelectRoi );

    text.set( _( "Resource\ntype:" ), fheroes::FontType::smallWhite() );
    text.draw( resourceSelectRoi.x, resourceSelectRoi.y - text.height( resourceSelectRoi.width ), resourceSelectRoi.width, display );

    const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();
    std::array<fheroes::Rect, resourceCount> resourceRoi;

    // Resource type selection mark background.
    const fheroes::Sprite & markBackground = fheroes::AGG::GetICN( isEvilInterface ? ICN::CELLWIN_EVIL : ICN::CELLWIN, 4 );
    ...
    for ( uint32_t i = 0; i < resourceCount - 1; ++i ) {
        const fheroes::Sprite & resourceImage = fheroes::generateMapObjectImage( resourceInfo[i] );
        const fheroes::Point imagePosition( area.x + iconOffsetX + ( resourceSelectRoi.width - fheroes::tileWidthPx * 3 ) / 2,
                                             offsetY + ( stepY - fheroes::tileWidthPx ) / 2 );
        resourceRoi[i] = { resourceSelectRoi.x + 5, imagePosition.y, resourceSelectRoi.width - 10, fheroes::tileWidthPx };
        fheroes::Blit( resourceImage, display, imagePosition.x, imagePosition.y - 4 );
        fheroes::Blit( markBackground, display, resourceRoi[i].x + 2, resourceRoi[i].y + ( resourceRoi[i].height - markBackground.height() ) / 2 );
        offsetY += stepY;
    }

    // Render ghosts to select Abandoned mine type.
    const fheroes::Sprite & ghostImage = fheroes::AGG::GetICN( ICN::MONS32, 59 );
    ...
    for ( int32_t i = 0; i < ghostsOnIcon; ++i ) {
        fheroes::Blit( ghostImage, display, area.x + iconOffsetX + ( resourceSelectRoi.width - ghostsIconWight ) / 2 + ghostsStepX * i, resourceRoi[7].y, i < 2 );
    }
    fheroes::Blit( markBackground, display, resourceRoi[7].x + 2, resourceRoi[7].y + ( resourceRoi[7].height - markBackground.height() ) / 2 );
    ...
    fheroes::MovableSprite resourceTypeSelection( mark );
    auto redrawResourceSelection = [&resourceTypeSelection, &mark]( const fheroes::Rect & pos ) {
    ...
    const fheroes::Rect listRoi( area.x + area.width - 5 * fheroes::tileWidthPx - 75, resourceSelectRoi.y, 5 * fheroes::tileWidthPx + 20, resourceSelectRoi.height );
    ...
    fheroes::ImageRestorer appearanceTextBackground( display, 0, 0, 0, 0 );
    ...
    auto redrawAppearanceText = [&appearanceTextBackground, &listRoi, &display]( const Maps::ObjectInfo & info ) {
        ...
        const fheroes::Text appearanceText( std::move( mineText ), fheroes::FontType::smallWhite() );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Okay" ), _( "Click to select the color." ), Dialog::ZERO );
        }
        else {
            for ( uint32_t i = 0; i < resourceCount; ++i ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( resourceRoi[i] ) ) {
                    ...
                    fheroes::showStandardTextMessage( std::move( objectName ), std::move( description ), Dialog::ZERO );
...
int Dialog::selectMountainType( const int mountainType )
{
    ...
    GenericObjectTypeSelection listbox( objectInfo, { 420, fheroes::Display::instance().height() - 180 }, _( "Select Mountain Object:" ) );
...
int Dialog::selectRockType( const int rockType )
{
    ...
    GenericObjectTypeSelection listbox( objectInfo, { 420, fheroes::Display::instance().height() - 180 }, _( "Select Rock Object:" ) );
...
int Dialog::selectTreeType( const int treeType )
{
    ...
    GenericObjectTypeSelection listbox( objectInfo, { 420, fheroes::Display::instance().height() - 180 }, _( "Select Tree Object:" ) );
...
int Dialog::selectPowerUpObjectType( const int powerUpObjectType )
{
    ...
    PowerUpObjectTypeSelection listbox( objectInfo, { 420, fheroes::Display::instance().height() - 180 }, _( "Select Power Up Object:" ) );
...
int Dialog::selectAdventureMiscellaneousObjectType( const int objectType )
{
    ...
    GenericObjectTypeSelection listbox( objectInfo, { 420, fheroes::Display::instance().height() - 180 }, _( "Select Adventure Object:" ) );
...
PlayerColor Dialog::selectPlayerColor( const PlayerColor color, const uint8_t availableColors )
{
    ...
    fheroes::StandardWindow background( std::max( minWidth, colorsWidth ), 160, true, display );

    const fheroes::Rect & area = background.activeArea();

    fheroes::Text text( _( "Select color" ), fheroes::FontType::normalYellow() );
    text.draw( area.x + ( area.width - text.width() ) / 2, area.y + 10, display );

    // Render color selection sprites.
    fheroes::Point pos( area.x + 20 + ( area.width - colorsWidth ) / 2, area.y + 40 );
    const fheroes::Sprite & colorSpriteBorderSelected = fheroes::AGG::GetICN( ICN::BRCREST, 6 );
    fheroes::Sprite colorSpriteBorder( colorSpriteBorderSelected );
    ...
    for ( int32_t i = 0; i < 7; ++i ) {
        ...
        colorRect[i] = { pos.x, pos.y, colorSpriteBorder.width(), colorSpriteBorder.height() };
        fheroes::Copy( colorSprite, 0, 0, display, colorRect[i].x + 4, colorRect[i].y + 4, colorSprite.width(), colorSprite.height() );
        text.set( Color::String( currentColor ), fheroes::FontType::smallWhite() );
        text.draw( colorRect[i].x + ( colorRect[i].width - text.width() ) / 2, colorRect[i].y + colorRect[i].height + 4, display );
    ...
    for ( int32_t i = 0; i < 7; ++i ) {
        if ( colorRect[i] != fheroes::Rect{} ) {
            fheroes::Blit( ( i == selectedColorIndex ) ? colorSpriteBorderSelected : colorSpriteBorder, display, colorRect[i].x, colorRect[i].y );
        }
    }
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            ...
            for ( size_t i = 0; i < 7; ++i ) {
                if ( static_cast<int>( i ) != selectedColorIndex && colorRect[i] != fheroes::Rect{} && le.MouseClickLeft( colorRect[i] ) ) {
                    fheroes::Blit( colorSpriteBorder, display, colorRect[selectedColorIndex].x, colorRect[selectedColorIndex].y );
                    fheroes::Blit( colorSpriteBorderSelected, display, colorRect[i].x, colorRect[i].y );

                    display.render( fheroes::getBoundaryRect( colorRect[selectedColorIndex], colorRect[i] ) );
