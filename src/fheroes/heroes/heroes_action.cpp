...
namespace
{
    ...
    void DialogCaptureResourceObject( std::string hdr, std::string msg, const int32_t resourceType )
    {
        ...
        fheroes::ResourceDialogElement resourceUI( resourceType, std::move( perday ) );

        fheroes::showStandardTextMessage( std::move( hdr ), std::move( msg ), Dialog::OK, { &resourceUI } );
    }
    ...
    void BattleLose( Heroes & hero, const Battle::Result & res, bool attacker )
    {
        ...
        if ( !hero.GetKingdom().isLoss() ) {
            ...
            const fheroes::Point heroPosition = hero.GetCenter();
    ...
    void RecruitMonsterFromTile( Heroes & hero, Maps::Tile & tile, std::string msg, const Troop & troop, const bool remove )
    {
        if ( !hero.GetArmy().CanJoinTroop( troop ) )
            fheroes::showStandardTextMessage( std::move( msg ), _( "You are unable to recruit at this time, your ranks are full." ), Dialog::OK );
        else {
    ...
    void WhirlpoolTroopLoseEffect( Heroes & hero )
    {
        ...
        if ( 1 == Rand::Get( 1, 3 ) ) {
            fheroes::showStandardTextMessage( MP2::StringObject( MP2::MapObjectType::OBJ_WHIRLPOOL ),
    ...
    void showSignMessage( const int32_t tileIndex )
    {
        ...
        const fheroes::Text title{ MP2::StringObject( MP2::OBJ_SIGN ), fheroes::FontType::normalYellow() };
        const fheroes::Text body{ sign->message.text, fheroes::FontType::normalWhite(), sign->message.language };
        fheroes::showMessage( title, body, Dialog::OK );
    }
    ...
    void ActionToMonster( Heroes & hero, int32_t dst_index )
    {
        ...
        if ( join.reason == NeutralMonsterJoiningCondition::Reason::Alliance ) {
            if ( hero.GetArmy().CanJoinTroop( troop ) ) {
                ...
                fheroes::showStandardTextMessage( "", join.joiningMessage, Dialog::OK );
                ...
                fheroes::showStandardTextMessage( "", join.fleeingMessage, Dialog::OK );
            }
            ...
            if ( Dialog::YES == Dialog::ArmyJoinFree( troop ) ) {
                ...
                fheroes::showStandardTextMessage( "", _( "Insulted by your refusal of their offer, the monsters attack!" ), Dialog::OK );
            }
        }
            ...
            if ( fheroes::showStandardTextMessage( "", std::move( message ), Dialog::YES | Dialog::NO ) == Dialog::NO ) {
                destroy = true;
            }
        }
    ...
    void ActionToCastle( Heroes & hero, const int32_t dstIndex )
    {
        ...
        const auto captureCastle = [&hero, dstIndex, castle]() {
            ...
            if ( !enemyKingdom.isLoss() ) {
                ...
                const fheroes::Point castlePosition = Maps::GetPoint( dstIndex );
    ...
    void ActionToBoat( Heroes & hero, const int32_t dst_index )
    {
        ...
        const fheroes::Point offset( Maps::GetPoint( dst_index ) - hero.GetCenter() );
    ...
    void ActionToPickupResource( const Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( objectType == MP2::OBJ_BOTTLE ) {
            ...
            if ( objectType == MP2::OBJ_CAMPFIRE ) {
                const fheroes::Text header( MP2::StringObject( objectType ), fheroes::FontType::normalYellow() );
                const fheroes::Text body( _( "Ransacking an enemy camp, you discover a hidden cache of treasures." ), fheroes::FontType::normalWhite() );

                fheroes::showResourceMessage( header, body, Dialog::OK, funds );
            }
            else if ( objectType == MP2::OBJ_BARREL ) {
                const fheroes::Text header( MP2::StringObject( objectType ), fheroes::FontType::normalYellow() );
                const fheroes::Text body( _( "Your ship bumps into a barrel drifting at sea. Inside, you discover a stash of lost treasure." ),
        ...
        if ( funds.GetValidItemsCount() > 0 ) {
            {
                ...
                fheroes::showResourceMessage( fheroes::Text( std::move( caption ), fheroes::FontType::normalYellow() ),
                                               fheroes::Text( std::move( msg ), fheroes::FontType::normalWhite() ), Dialog::OK, funds );
            }

            hero.GetKingdom().AddFundsResource( funds );
        }
        else {
            fheroes::showStandardTextMessage( std::move( caption ), std::move( msg ), Dialog::OK );
        }
    ...
    void ActionToSkeleton( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( doesTileContainValuableItems( tile ) ) {
            if ( hero.IsFullBagArtifacts() ) {
                ...
                fheroes::showResourceMessage( fheroes::Text( std::move( title ), fheroes::FontType::normalYellow() ),
                                               fheroes::Text( _( "Treasure" ), fheroes::FontType::normalWhite() ), Dialog::OK, funds );
                ...
                const fheroes::ArtifactDialogElement artifactUI( art );

                fheroes::showStandardTextMessage( std::move( title ), std::move( message ), Dialog::OK, { &artifactUI } );
            ...
            fheroes::showStandardTextMessage( std::move( title ), std::move( message ), Dialog::OK );
        }
    ...
    void ActionToWagon( Heroes & hero, int32_t dst_index )
    {
        ...
        if ( doesTileContainValuableItems( tile ) ) {
            ...
            if ( art.isValid() ) {
                ...
                fheroes::showResourceMessage( fheroes::Text( std::move( title ), fheroes::FontType::normalYellow() ),
                                               fheroes::Text( std::move( message ), fheroes::FontType::normalWhite() ), Dialog::OK, funds );
            ...
            message += '\n';
            message.append( _( "Unfortunately, others have found it first, and the wagon is empty." ) );
            fheroes::showStandardTextMessage( title, message, Dialog::OK );
        }
    ...
    void ActionToFlotSam( const Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( funds.GetValidItemsCount() > 0 ) {
            ...
            fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK );
        }
    ...
    void ActionToShrine( Heroes & hero, int32_t dst_index )
    {
        ...
        if ( hero.HaveSpellBook() ) {
            ...
            if ( 3 == spellLevel && Skill::Level::NONE == hero.GetLevelSkill( Skill::Secondary::WISDOM ) ) {
                ...
                fheroes::showStandardTextMessage( std::move( head ), std::move( body ), Dialog::OK );
            }
            // already know (skip bag artifacts)
                ...
                const fheroes::SpellDialogElement spellUI( spell, &hero );
                fheroes::showStandardTextMessage( std::move( head ), std::move( body ), Dialog::OK, { &spellUI } );
            }
        }
        else {
        ...
        if ( skill.isValid() ) {
            ...
            // Skill has already been learned
...

                {
                    ...
                    const fheroes::SecondarySkillDialogElement secondarySkillUI( skill, hero );
                    fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &secondarySkillUI } );
                }
            }
        }
        else {
    ...
    void ActionToGoodLuckObject( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( visited ) {
            ...
            const fheroes::LuckDialogElement luckUI( true );
            fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &luckUI } );
        }
    }
    ...
    void ActionToPyramid( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
...

        {
            ...
            enter = ( fheroes::showStandardTextMessage( title, std::move( ask ), Dialog::YES | Dialog::NO ) == Dialog::YES );
        }
        ...
        if ( enter ) {
            if ( spell.isValid() ) {
                ...
                if ( res.isAttackerWin() ) {
                    ...
                    if ( valid ) {
                        const fheroes::SpellDialogElement spellUI( spell, &hero );
                        fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &spellUI } );
                ...
                const fheroes::LuckDialogElement luckUI( false );
                fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &luckUI, &luckUI } );
    ...
    void ActionToMagicWell( Heroes & hero, int32_t dst_index )
    {
        ...
        if ( hero.GetSpellPoints() >= max ) {
            fheroes::showStandardTextMessage( std::move( title ), _( "A drink at the well is supposed to restore your spell points, but you are already at maximum." ),
                                               Dialog::OK );
        }
        else {
            if ( hero.isObjectTypeVisited( MP2::OBJ_MAGIC_WELL ) ) {
                ...
                fheroes::showStandardTextMessage( std::move( title ), _( "A second drink at the well in one day will not help you." ), Dialog::OK );
            }
            else {
...

                {
                    ...
                    fheroes::showStandardTextMessage( std::move( title ), _( "A drink from the well has restored your spell points to maximum." ), Dialog::OK );
                }
    ...
    void ActionToPrimarySkillObject( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( visited ) {
...

            {
                ...
                const fheroes::PrimarySkillDialogElement primarySkillUI( skill, "+1" );
                fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &primarySkillUI } );
            }
    ...
    void ActionToPoorMoraleObject( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        const std::string title( MP2::StringObject( objectType ) );
        {
            ...
            if ( fheroes::showStandardTextMessage( title, std::move( ask ), Dialog::YES | Dialog::NO ) != Dialog::YES ) {
                return;
            }
        }
            ...
            if ( res.isAttackerWin() ) {
                ...
                if ( art.isValid() ) {
                    if ( hero.IsFullBagArtifacts() ) {
                        ...
                        const fheroes::ResourceDialogElement goldUI( Resource::GOLD, std::to_string( gold ) );

                        fheroes::showStandardTextMessage( title, std::move( win ), Dialog::OK, { &goldUI } );
                    }
                    else {
                        const fheroes::ResourceDialogElement goldUI( Resource::GOLD, std::to_string( gold ) );
                        const fheroes::ArtifactDialogElement artifactUI( art );

                        fheroes::showStandardTextMessage( title, std::move( win ), Dialog::OK, { &artifactUI, &goldUI } );
        ...
        if ( complete ) {
            ...
            const fheroes::MoraleDialogElement moraleUI( false );
            fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::OK, { &moraleUI } );
        }
    }
    ...
    void ActionToGoodMoraleObject( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( visited ) {
            ...
            const fheroes::MoraleDialogElement moraleUI( true );
            std::vector<const fheroes::DialogElement *> elementUI{ &moraleUI };
            if ( objectType == MP2::OBJ_TEMPLE ) {
                elementUI.emplace_back( &moraleUI );
            }

            fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, elementUI );
    ...
    void ActionToExperienceObject( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( visited ) {
        ...
        else {
            {
                ...
                const fheroes::ExperienceDialogElement experienceUI( exp );
                fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &experienceUI } );
            }
    ...
    void ActionToShipwreckSurvivor( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( hero.IsFullBagArtifacts() ) {
            ...
            fheroes::showStandardTextMessage( std::move( title ),
            ...
            fheroes::showStandardTextMessage( std::move( title ), std::move( str ), Dialog::OK, { &artifactUI } );
    ...
    void ActionToArtifact( Heroes & hero, int32_t dst_index )
    {
        ...
        if ( art.GetID() == Artifact::MAGIC_BOOK && hero.HaveSpellBook() ) {
            fheroes::showStandardTextMessage( std::move( title ), _( "You cannot have multiple spell books." ), Dialog::OK );

            return;
        }

        if ( hero.IsFullBagArtifacts() ) {
            fheroes::showStandardTextMessage( std::move( title ), _( "You cannot pick up this artifact, you already have a full load!" ), Dialog::OK );
        ...
        if ( condition == Maps::ArtifactCaptureCondition::PAY_2000_GOLD || condition == Maps::ArtifactCaptureCondition::PAY_2500_GOLD_AND_3_RESOURCES
            ...
            if ( Dialog::YES == fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::YES | Dialog::NO, { &artifactUI } ) ) {
            ...
            if ( hero.HasSecondarySkill( skill.Skill() ) ) {
                ...
                fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::OK, { &artifactUI } );
            ...
            if ( troop ) {
                if ( Monster::ROGUE == troop->GetID() )
                    ...
                    battle = ( Dialog::YES == fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::YES | Dialog::NO ) );
                }
            }
            ...
            if ( battle ) {
                ...
                fheroes::showStandardTextMessage( std::move( title ), _( "Discretion is the better part of valor, and you decide to avoid this fight for today." ),
                                                   Dialog::OK );
            }
        }
        else {
    ...
    void ActionToTreasureChest( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        const auto [goldReward, experienceReward, artifactReward]
            ...
            if ( tile.isWater() ) {
                if ( gold == 0 ) {
                    fheroes::showStandardTextMessage( std::move( hdr ),
                ...
                if ( isArtValid && !isBagFull ) {
                    ...
                    fheroes::showStandardTextMessage( std::move( hdr ), std::move( msg ), Dialog::OK, { &artifactUI, &goldUI } );
                ...
                fheroes::showStandardTextMessage( std::move( hdr ), std::move( msg ), Dialog::OK, { &goldUI } );
            ...
            fheroes::showStandardTextMessage( std::move( hdr ), std::move( msg ), Dialog::OK, { &artifactUI } );
    ...
    void ActionToGenieLamp( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
...

        {
            ...
            recruit
                = ( fheroes::showStandardTextMessage( title, _( "You stumble upon a dented and tarnished lamp lodged deep in the earth. Do you wish to rub the lamp?" ),
    ...
    void ActionToTeleports( Heroes & hero, int32_t index_from )
    {
        ...
        const fheroes::Point fromPoint = Maps::GetPoint( index_from );
    ...
    void ActionToCaptureObject( Heroes & hero, const MP2::MapObjectType objectType, const int32_t dstIndex )
    {
        ...
        if ( !hero.isFriends( getColorFromTile( tile ) ) ) {
            const auto updateRadar = [objectType, dstIndex]() {
                ...
                fheroes::Rect radarRoi( Maps::GetPoint( dstIndex ), { 1, 1 } );
            ...
            const auto captureObject = [&hero, objectType, &tile, &updateRadar, &removeObjectProtection]() {
...

                {
                    ...
                    if ( resource == Resource::UNKNOWN ) {
                        fheroes::showStandardTextMessage( std::move( header ), std::move( body ), Dialog::OK );
                    }
                    else {
    ...
    void ActionToAbandonedMine( Heroes & hero, const MP2::MapObjectType objectType, const int32_t dstIndex )
    {
...

        {
            ...
            enter = ( fheroes::showStandardTextMessage( MP2::StringObject( objectType ),
        ...
        if ( enter ) {
            ...
            if ( result.isAttackerWin() ) {
                ...
                // TODO: make a function that will automatically get the object size in tiles and return a ROI for radar update.
                // Set the radar update ROI according to captured object size and position.
                const fheroes::Point tilePoint = Maps::GetPoint( dstIndex );
                const fheroes::Rect radarRoi( tilePoint.x - 1, tilePoint.y - 1, 3, 2 );
                ...
                fheroes::showStandardTextMessage( MP2::StringObject( objectType ), _( "You beat the Ghosts and are able to restore the mine to production." ),
                                                   Dialog::OK );
            }
            else {
                BattleLose( hero, result, true );
    ...
    void ActionToDwellingJoinMonster( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( troop.isValid() ) {
...

            {
                ...
                recruit = ( fheroes::showStandardTextMessage( std::move( title ), std::move( message ), Dialog::YES | Dialog::NO ) == Dialog::YES );
            }

            if ( recruit ) {
                if ( !hero.GetArmy().CanJoinTroop( troop ) ) {
                    fheroes::showStandardTextMessage( troop.GetName(), _( "You are unable to recruit at this time, your ranks are full." ), Dialog::OK );
                }
                else {
            ...
            fheroes::showStandardTextMessage( std::move( title ), _( "As you approach the dwelling, you notice that there is no one here." ), Dialog::OK );
        }
    ...
    void ActionToDwellingRecruitMonster( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( !troop.isValid() ) {
            fheroes::showStandardTextMessage( title, std::move( msg_void ), Dialog::OK );
        }
        else {
            bool recruit = false;

            {
                ...
                recruit = ( fheroes::showStandardTextMessage( title, std::move( msg_full ), Dialog::YES | Dialog::NO ) == Dialog::YES );
            }
    ...
    void ActionToDwellingBattleMonster( Heroes & hero, const MP2::MapObjectType objectType, const int32_t dst_index )
    {
        ...
        const Outcome outcome = [dst_index, &title, objectIsEmptyMsg, recruitmentAvailableMsg, warningMsg]() {
            ...
            if ( getColorFromTile( tile ) != PlayerColor::NONE ) {
                ...
                if ( !troop.isValid() ) {
                    fheroes::showStandardTextMessage( title, objectIsEmptyMsg, Dialog::OK );
                ...
                if ( fheroes::showStandardTextMessage( title, recruitmentAvailableMsg, Dialog::YES | Dialog::NO ) == Dialog::YES ) {
            ...
            // In the original game, this dialog window has no sound, this is an improvement specific to fheroes
            const MusicalEffectPlayer musicalEffectPlayer( MUS::DUNGEON );

            if ( fheroes::showStandardTextMessage( title, warningMsg, Dialog::YES | Dialog::NO ) == Dialog::YES ) {
        ...
        case Outcome::Fight: {
            ...
            if ( res.isAttackerWin() ) {
                ...
                if ( fheroes::showStandardTextMessage( title, victoryMsg, Dialog::YES | Dialog::NO ) == Dialog::YES ) {
                    const Troop troop = getTroopFromTile( tile );
                    assert( troop.isValid() );
    ...
    void ActionToObservationTower( const Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
...

        {
            ...
            fheroes::showStandardTextMessage( MP2::StringObject( objectType ), _( "From the observation tower, you are able to see distant lands." ), Dialog::OK );
        }
        ...
        Interface::AdventureMap & I = Interface::AdventureMap::Get();
        const fheroes::Point towerPosition = Maps::GetPoint( dst_index );
        const fheroes::Rect towerRoi( towerPosition.x - scoutRange, towerPosition.y - scoutRange, 2 * scoutRange + 1, 2 * scoutRange + 1 );
    ...
    void ActionToArtesianSpring( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( world.isAnyKingdomVisited( objectType, dst_index ) ) {
            fheroes::showStandardTextMessage( std::move( title ), _( "The spring only refills once a week, and someone's already been here this week." ), Dialog::OK );
        }
        else {
            const uint32_t max = hero.GetMaxSpellPoints();
            ...
            if ( GameStatic::isHeroWorthyToVisitXanadu( hero ) ) {
                {
                    ...
                    const fheroes::SmallPrimarySkillDialogElement attackUI( Skill::Primary::ATTACK, "+1" );
                    const fheroes::SmallPrimarySkillDialogElement defenseUI( Skill::Primary::DEFENSE, "+1" );
                    const fheroes::SmallPrimarySkillDialogElement powerUI( Skill::Primary::POWER, "+1" );
                    const fheroes::SmallPrimarySkillDialogElement knowledgeUI( Skill::Primary::KNOWLEDGE, "+1" );
    ...
    void ActionToUpgradeArmyObject( Heroes & hero, const MP2::MapObjectType objectType, const std::string & defaultMessage )
    {
        ...
        if ( !mons.empty() ) {
            ...
            int32_t offsetX = 0;
            int32_t offsetY = 0;
            const fheroes::Sprite & border = fheroes::AGG::GetICN( ICN::STRIP, 12 );
            ...
            fheroes::Image surface( border.width() * monsterPerX + ( monsterPerX - 1 ) * 4, border.height() * monsterPerY + ( monsterPerY - 1 ) * 4 );
            surface.reset();
            ...
            for ( size_t i = 0; i < mons.size(); ++i ) {
                ...
                // border
                fheroes::Blit( border, surface, offsetX, offsetY );
                fheroes::renderMonsterFrame( mons[i]->GetUpgrade(), surface, { 6 + offsetX, 6 + offsetY } );
...

            {
                ...
                const fheroes::CustomImageDialogElement imageUI( std::move( surface ) );
                fheroes::showStandardTextMessage( std::move( title ), std::move( msg1 ), Dialog::OK, { &imageUI } );
            }
        }
        else {
            fheroes::showStandardTextMessage( std::move( title ), std::move( msg2 ), Dialog::OK );
        }
    }
        ...
        if ( hero.isObjectTypeVisited( objectType, Visit::GLOBAL ) ) {
            ...
            if ( kingdom.AllowPayment( payment ) ) {
...

                {
                    ...
                    buy = ( fheroes::showStandardTextMessage(
                                std::move( title ),
    ...
    void ActionToEvent( Heroes & hero, const int32_t tileIndex )
    {
        ...
        const std::vector<fheroes::ResourceDialogElement> resourceUI = fheroes::getResourceDialogElements( fundsToUpdate );

        std::vector<const fheroes::DialogElement *> elementUI;
        elementUI.reserve( resourceUI.size() );
        for ( const fheroes::ResourceDialogElement & element : resourceUI ) {
            elementUI.emplace_back( &element );
        }

        // Check for the presence of an artifact in the event and display it in the dialog.
        std::unique_ptr<fheroes::ArtifactDialogElement> artifactUI;
        const Artifact & art = mapEvent->artifact;
        if ( art.isValid() ) {
            artifactUI = std::make_unique<fheroes::ArtifactDialogElement>( art );
            AudioManager::PlaySound( M82::TREASURE );
            elementUI.emplace_back( artifactUI.get() );
        }

        std::unique_ptr<fheroes::SecondarySkillDialogElement> secondarySkillUI;
        ...
        if ( skill.isValid() ) {
            ...
            if ( addSkill ) {
                secondarySkillUI = std::make_unique<fheroes::SecondarySkillDialogElement>( skill, hero );
        ...
        std::unique_ptr<fheroes::ExperienceDialogElement> experienceUI;
        if ( mapEvent->experience > 0 ) {
            experienceUI = std::make_unique<fheroes::ExperienceDialogElement>( mapEvent->experience );
            elementUI.emplace_back( experienceUI.get() );
        }

        const fheroes::Text emptyText;
        const fheroes::Text body( mapEvent->message, fheroes::FontType::normalWhite(), Settings::Get().getCurrentMapInfo().getSupportedLanguage() );

        int32_t dialogHeight = fheroes::getDialogHeight( emptyText, body, Dialog::OK, elementUI );
        const int32_t displayHeight = fheroes::Display::instance().height();
        if ( dialogHeight > displayHeight && !elementUI.empty() ) {
            // We have to split the message into 2 as it is too big.
            std::vector<const fheroes::DialogElement *> secondDialogUIElement;
            while ( dialogHeight > displayHeight && !elementUI.empty() ) {
                secondDialogUIElement.push_back( elementUI.back() );
                elementUI.pop_back();
                dialogHeight = fheroes::getDialogHeight( emptyText, body, Dialog::OK, elementUI );
            }

            fheroes::showMessage( emptyText, body, Dialog::OK, elementUI );
            fheroes::showMessage( emptyText, emptyText, Dialog::OK, secondDialogUIElement );
        }
        else {
            fheroes::showMessage( emptyText, body, Dialog::OK, elementUI );
        }
    ...
    void ActionToObelisk( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( !hero.isVisited( world.getTile( dst_index ), Visit::GLOBAL ) ) {
            ...
            fheroes::showStandardTextMessage( std::move( title ), _( "You have already been to this obelisk." ), Dialog::OK );
        }
    }
        ...
        if ( hero.isVisited( tile ) ) {
...

            {
                ...
                if ( increaseExperience ) {
                    const fheroes::ExperienceDialogElement experienceUI( static_cast<int32_t>( possibleExperience ) );

                    // In the original game, there was no way to refuse to level up for free, this is an improvement specific to fheroes
                    increaseExperience
                        = ( fheroes::showStandardTextMessage(
                                std::move( title ),
                    ...
                    if ( hero.GetKingdom().AllowPayment( payment ) ) {
                        ...
                        increaseExperience
                            = ( fheroes::showStandardTextMessage( std::move( title ), std::move( msg ), Dialog::YES | Dialog::NO, { &experienceUI } ) == Dialog::YES );
                    }
                    else {
    ...
    void ActionToDaemonCave( Heroes & hero, const MP2::MapObjectType objectType, const int32_t dst_index )
    {
        ...
        const Outcome outcome = [&hero = std::as_const( hero ), dst_index, &title]() {
            ...
            if ( fheroes::showStandardTextMessage( title,
            ...
            if ( !doesTileContainValuableItems( tile ) ) {
                fheroes::showStandardTextMessage( title, _( "Except for evidence of a terrible battle, the cave is empty." ), Dialog::OK );
            ...
            case Maps::DaemonCaveCaptureBonus::GET_1000_EXPERIENCE: {
                ...
                const fheroes::ExperienceDialogElement experienceUI( demonSlayingExperience );
                fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::OK, { &experienceUI } );
            ...
            case Maps::DaemonCaveCaptureBonus::GET_1000_EXPERIENCE_AND_2500_GOLD: {
                ...
                const fheroes::ExperienceDialogElement experienceUI( demonSlayingExperience );
                const fheroes::ResourceDialogElement goldUI( Resource::GOLD, std::to_string( gold ) );
                fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::OK, { &experienceUI, &goldUI } );
            ...
            case Maps::DaemonCaveCaptureBonus::GET_1000_EXPERIENCE_AND_ARTIFACT: {
                ...
                const fheroes::ExperienceDialogElement experienceUI( demonSlayingExperience );
                const fheroes::ArtifactDialogElement artifactUI( art );
                fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::OK, { &experienceUI, &artifactUI } );
            ...
            case Maps::DaemonCaveCaptureBonus::PAY_2500_GOLD: {
                ...
                if ( !kingdom.AllowPayment( payment ) ) {
                    ...
                    fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::OK );
                ...
                if ( fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::YES | Dialog::NO ) == Dialog::YES ) {
            ...
            if ( outcome != Outcome::Empty ) {
                ...
                case Outcome::BattleWithServants: {
                    ...
                    if ( res.isAttackerWin() ) {
                        ...
                        fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::OK, { &goldUI } );
    ...
    void ActionToAlchemistTower( Heroes & hero )
    {
        ...
        if ( cursed ) {
            ...
            if ( Dialog::YES == fheroes::showStandardTextMessage( title, std::move( msg ), Dialog::YES | Dialog::NO ) ) {
                if ( hero.GetKingdom().AllowPayment( payment ) ) {
                    hero.GetKingdom().OddFundsResource( payment );
            ...
            fheroes::showStandardTextMessage( std::move( title ), _( "You hear a voice from high above in the tower, \"Go away! I can't help you!\"" ), Dialog::OK );
        }
    }
    ...
    void ActionToStables( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( isCavalryPresent ) {
            ...
            fheroes::showStandardTextMessage( MP2::StringObject( objectType ), std::move( body ), Dialog::OK );
        }
    }
    ...
    void ActionToArena( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( hero.isObjectTypeVisited( objectType ) ) {
            fheroes::showStandardTextMessage( MP2::StringObject( objectType ), _( "The Arena guards turn you away." ), Dialog::OK );
        }
        else {
            hero.SetVisited( dst_index );
            AudioManager::PlaySound( M82::EXPERNCE );
    ...
    void ActionToSirens( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( hero.isObjectTypeVisited( objectType ) ) {
            ...
            if ( experience == 0 ) {
                ...
                const fheroes::ExperienceDialogElement experienceUI( static_cast<int32_t>( experience ) );
                ...
                fheroes::showStandardTextMessage( std::move( title ), std::move( str ), Dialog::OK, { &experienceUI } );
    ...
    void ActionToJail( const Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( kingdom.AllowRecruitHero( false ) ) {
            ...
            fheroes::showStandardTextMessage( std::move( title ), std::move( str ), Dialog::OK );
        }
    }
    ...
    void ActionToHutMagi( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( !hero.isObjectTypeVisited( objectType, Visit::GLOBAL ) ) {
            ...
            if ( !eyeMagiIndexes.empty() ) {
                ...
                fheroes::Rect radarRenderArea;

                for ( const int32_t eyeIndex : eyeMagiIndexes ) {
                    Maps::ClearFog( eyeIndex, scoutRange, hero.GetColor() );

                    const fheroes::Point eyePosition = Maps::GetPoint( eyeIndex );
                    const fheroes::Rect eyeRoi( eyePosition.x - scoutRange, eyePosition.y - scoutRange, 2 * scoutRange + 1, 2 * scoutRange + 1 );

                    if ( skipAnimation ) {
                        radarRenderArea = fheroes::getBoundaryRect( radarRenderArea, eyeRoi );
                        continue;
                    }
    ...
    void ActionToEyeMagi( const Heroes & hero, const MP2::MapObjectType objectType )
    {
        ...
        fheroes::showStandardTextMessage( MP2::StringObject( objectType ), _( "This eye seems to be intently studying its surroundings." ), Dialog::OK );
    }
    ...
    void ActionToSphinx( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        const Outcome outcome = [objectType, riddle]() {
            ...
            if ( riddle == nullptr || !riddle->valid ) {
                fheroes::showStandardTextMessage( title, _( "You come across a giant Sphinx. The Sphinx remains strangely quiet." ), Dialog::OK );
            ...
            fheroes::MultiFontText questionText;
            questionText.add( { _( "The Sphinx asks you the following riddle:\n\n'" ), fheroes::FontType::normalWhite() } );
            questionText.add( { riddle->riddle, fheroes::FontType::normalWhite(), language } );
            questionText.add( { _( "sphinx|'\n\nYour answer?" ), fheroes::FontType::normalWhite() } );

            std::string answer;
            Dialog::inputString( fheroes::Text{ title, fheroes::FontType::normalYellow() }, questionText, answer, 0, false, {} );
            ...
            if ( count || art.isValid() ) {
                const std::vector<fheroes::ResourceDialogElement> resourceUiElements = fheroes::getResourceDialogElements( res );

                std::vector<const fheroes::DialogElement *> uiElements;
                uiElements.reserve( resourceUiElements.size() + 1 );

                for ( const fheroes::ResourceDialogElement & element : resourceUiElements ) {
    ...
    void ActionToTravellersTent( const Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        fheroes::showStandardTextMessage(
            std::string( MP2::StringObject( objectType ) ) + " (" + fheroes::getTentColorName( tentColor ) + ")",
    ...
    void actionToBlackCat( Heroes & hero, int32_t dst_index )
    {
        ...
        const fheroes::MoraleDialogElement moraleUI( true );
        const fheroes::LuckDialogElement luckUI( false );
        const std::vector<const fheroes::DialogElement *> elementUI{ &moraleUI, &moraleUI, &moraleUI, &luckUI, &luckUI };
