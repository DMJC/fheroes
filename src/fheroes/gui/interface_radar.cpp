...
namespace
{
    ...
    bool getCastleColor( uint8_t & fillColor, const fheroes::Point & position )
    {
...
Interface::Radar::Radar( BaseInterface & interface )
    : BorderWindow( { 0, 0, fheroes::radarWidthPx, fheroes::radarWidthPx } )
    , _radarType( RadarType::WorldMap )
    , _interface( interface )
{
    // Initialize radar image (_map) as a single-layer image.
    _map._disableTransformLayer();
    _map.resize( fheroes::radarWidthPx, fheroes::radarWidthPx );
}

Interface::Radar::Radar( const Radar & radar, const fheroes::Display & display )
    : BorderWindow( { display.width() - fheroes::borderWidthPx - fheroes::radarWidthPx, fheroes::borderWidthPx, fheroes::radarWidthPx, fheroes::radarWidthPx } )
...
void Interface::Radar::SetRenderArea( const fheroes::Rect & roi )
{
    ...
    if ( !conf.isHideInterfaceEnabled() || conf.ShowRadar() ) {
        ...
        _roi = roi ^ fheroes::Rect( 0, 0, world.w(), world.h() );
    }
}
...
void Interface::Radar::_redraw( const bool redrawMapObjects )
{
    ...
    const fheroes::Rect & rect = GetArea();
    if ( _hide ) {
        fheroes::Blit( fheroes::AGG::GetICN( ( conf.isEvilInterfaceEnabled() ? ICN::HEROLOGE : ICN::HEROLOGO ), 0 ), display, rect.x, rect.y );
        ...
        fheroes::Copy( _map, 0, 0, display, rect.x, rect.y, _map.width(), _map.height() );
...
void Interface::Radar::RedrawForViewWorld( const ViewWorld::ZoomROIs & roi, const ViewWorldMode mode, const bool renderMapObjects )
{
    ...
    const fheroes::Rect roiInTiles = roi.GetROIinTiles();
    _cursorArea.show();
...
void Interface::Radar::redrawForEditor( const bool renderMapObjects )
{
    ...
    if ( renderMapObjects ) {
        ...
        const fheroes::Rect & rect = GetArea();
        fheroes::Copy( _map, 0, 0, fheroes::Display::instance(), rect.x, rect.y, _map.width(), _map.height() );
    }
...
void Interface::Radar::RedrawCursor( const fheroes::Rect * roiRectangle /* = nullptr */ )
{
    const Settings & conf = Settings::Get();
    ...
    const fheroes::Rect worldSize{ 0, 0, world.w(), world.h() };
    if ( worldSize.width < 1 || worldSize.height < 1 ) {
        return;
    }

    const fheroes::Rect & viewableWorldArea = ( roiRectangle == nullptr ) ? _interface.getGameArea().GetVisibleTileROI() : *roiRectangle;
    ...
    if ( ( viewableWorldArea.width > worldSize.width ) && ( viewableWorldArea.height > worldSize.height ) ) {
        ...
        const fheroes::Rect radarWorldArea = worldSize ^ viewableWorldArea;

        const fheroes::Rect & totalRenderingArea = GetArea();
        const fheroes::Size actualRenderingArea{ totalRenderingArea.width, totalRenderingArea.height };

        const fheroes::Size cursorSize{ ( radarWorldArea.width * actualRenderingArea.width ) / worldSize.width,
        ...
        if ( _cursorArea.width() != cursorSize.width || _cursorArea.height() != cursorSize.height ) {
            ...
            _cursorArea.reset();
            fheroes::DrawBorder( _cursorArea, RADARCOLOR, 6 );
        }
...
void Interface::Radar::QueueEventProcessing()
{
    ...
    if ( le.MouseClickLeft( rect ) || le.isMouseLeftButtonPressedInArea( rect ) ) {
        const fheroes::Point & pt = le.getMouseCursorPos();

        if ( rect & pt ) {
            GameArea & gamearea = _interface.getGameArea();
            fheroes::Rect visibleROI( gamearea.GetVisibleTileROI() );
            const fheroes::Point prev( visibleROI.x, visibleROI.y );
        ...
        fheroes::showStandardTextMessage( _( "World Map" ), _( "A miniature view of the known world. Left click to move viewing area." ), Dialog::ZERO );
    }
}
...
bool Interface::Radar::QueueEventProcessingForWorldView( ViewWorld::ZoomROIs & roi ) const
{
    ...
    if ( le.isMouseCursorPosInArea( rect ) ) {
        if ( le.MouseClickLeft( rect ) || le.isMouseLeftButtonPressedInArea( rect ) ) {
            ...
            if ( rect & pt ) {
                const fheroes::Rect & initROI = roi.GetROIinPixels();
                const fheroes::Point prevCoordsTopLeft( initROI.x, initROI.y );
                const fheroes::Point newCoordsCenter( ( pt.x - rect.x ) * world.w() / rect.width, ( pt.y - rect.y ) * world.h() / rect.height );
                const fheroes::Point newCoordsTopLeft( newCoordsCenter.x - initROI.width / 2, newCoordsCenter.y - initROI.height / 2 );

                if ( prevCoordsTopLeft != newCoordsTopLeft ) {
                    return roi.ChangeCenter( { newCoordsCenter.x * fheroes::tileWidthPx - fheroes::tileWidthPx / 2,
                                               newCoordsCenter.y * fheroes::tileWidthPx - fheroes::tileWidthPx / 2 } );
                }
            }
        }
        else if ( le.isMouseRightButtonPressedInArea( GetRect() ) ) {
