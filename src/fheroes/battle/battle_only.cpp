...
void Battle::ControlInfo::Redraw() const
{
    ...
    const fheroes::Sprite & cell = fheroes::AGG::GetICN( ICN::CELLWIN, 1 );
    const fheroes::Sprite & mark = fheroes::AGG::GetICN( ICN::CELLWIN, 2 );

    fheroes::Blit( cell, display, rtLocal.x, rtLocal.y );
    if ( result & CONTROL_HUMAN )
        fheroes::Blit( mark, display, rtLocal.x + 3, rtLocal.y + 2 );
    fheroes::Text text( _( "Human" ), fheroes::FontType::smallWhite() );
    text.draw( rtLocal.x + cell.width() + 5, rtLocal.y + 5, display );

    fheroes::Blit( cell, display, rtAI.x, rtAI.y );
    if ( result & CONTROL_AI )
        fheroes::Blit( mark, display, rtAI.x + 3, rtAI.y + 2 );
    text.set( _( "AI" ), fheroes::FontType::smallWhite() );
...
bool Battle::Only::setup( const bool allowBackup, bool & reset )
{
    ...
    const fheroes::StandardWindow frameborder( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT, false );

    const fheroes::Point cur_pt( frameborder.activeArea().x, frameborder.activeArea().y );
    ...
    const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );
    fheroes::Copy( background, 0, 0, display, cur_pt.x, cur_pt.y, background.width(), background.height() );
    ...
    if ( armyInfo[1].hero != nullptr ) {
        attackedArmyControlInfo = std::make_unique<ControlInfo>( fheroes::Point{ cur_pt.x + 500, cur_pt.y + 425 }, armyInfo[1].player.GetControl() );
    }
    ...
    const fheroes::Sprite & stoneBackground = fheroes::AGG::GetICN( ICN::STONEBAK, 0 );
    fheroes::Copy( stoneBackground, 292, 270, display, cur_pt.x + 292, cur_pt.y + 270, 48, 44 );
    fheroes::Copy( stoneBackground, 292, 363, display, cur_pt.x + 292, cur_pt.y + 363, 48, 44 );

    // hide the shadow from the original EXIT button
    const fheroes::Sprite buttonOverride = fheroes::Crop( fheroes::AGG::GetICN( ICN::SWAPWIN, 0 ), 122, 428, 84, 32 );
    fheroes::Copy( buttonOverride, 0, 0, display, cur_pt.x + 276, cur_pt.y + 428, 84, 32 );

    fheroes::Button buttonReset( cur_pt.x + 30, cur_pt.y + 428, ICN::BUTTON_RESET_GOOD, 0, 1 );
    fheroes::Button buttonStart( cur_pt.x + 178, cur_pt.y + 428, ICN::BUTTON_START_GOOD, 0, 1 );
    fheroes::Button buttonExit( cur_pt.x + 366, cur_pt.y + 428, ICN::BUTTON_SMALL_EXIT_GOOD, 0, 1 );

    fheroes::addGradientShadow( fheroes::AGG::GetICN( ICN::BUTTON_RESET_GOOD, 0 ), display, buttonReset.area().getPosition(), { -5, 5 } );
    fheroes::addGradientShadow( fheroes::AGG::GetICN( ICN::BUTTON_START_GOOD, 0 ), display, buttonStart.area().getPosition(), { -5, 5 } );
    fheroes::addGradientShadow( fheroes::AGG::GetICN( ICN::BUTTON_SMALL_EXIT_GOOD, 0 ), display, buttonExit.area().getPosition(), { -5, 5 } );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonStart.area() ) ) {
            fheroes::showStandardTextMessage( _( "Start" ), _( "Start the battle." ), 0 );
        }
            ...
            fheroes::showStandardTextMessage( _( "Reset" ), _( "Reset to default settings." ), 0 );
        }
        ...
        for ( const auto & [firstId, secondId] : { std::pair<int32_t, int32_t>( 0, 1 ), std::pair<int32_t, int32_t>( 1, 0 ) } ) {
            ...
            if ( le.MouseClickLeft( first.portraitRoi ) ) {
                ...
                if ( second.hero && hid == second.hero->GetID() ) {
                    fheroes::showStandardTextMessage( _( "Error" ), _( "Please select another hero." ), Dialog::OK );
                }
...
void Battle::Only::updateHero( ArmyInfo & info, const fheroes::Point & offset )
{
...
void Battle::Only::redrawOpponents( const fheroes::Point & top ) const
{
    fheroes::Display & display = fheroes::Display::instance();

    const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );
    const fheroes::Rect textRoi( top.x + 89, top.y + 27, 462, 17 );
    fheroes::Copy( background, 89, 27, display, textRoi );
    ...
    for ( const size_t idx : { 0, 1 } ) {
        if ( armyInfo[idx].hero ) {
            const fheroes::Sprite & port = armyInfo[idx].hero->GetPortrait( PORT_BIG );
            if ( !port.empty() ) {
                fheroes::Copy( port, 0, 0, display, armyInfo[idx].portraitRoi );
            }
        }
        else {
            fheroes::Fill( display, armyInfo[idx].portraitRoi.x, armyInfo[idx].portraitRoi.y, armyInfo[idx].portraitRoi.width, armyInfo[idx].portraitRoi.height, 0 );
            text.set( _( "N/A" ), fheroes::FontType::normalWhite() );
...
void Battle::Only::redrawOpponentsStats( const fheroes::Point & top ) const
{
    fheroes::Display & display = fheroes::Display::instance();
    const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );

    fheroes::Copy( background, 262, 61, display, top.x + 262, top.y + 61, 115, 109 );
    fheroes::RedrawPrimarySkillInfo( top, armyInfo[0].ui.primarySkill.get(), armyInfo[1].ui.primarySkill.get() );
}
...
void Battle::Only::updateArmyUI( ArmyUI & ui, Heroes * hero, const fheroes::Point & offset, const uint8_t armyId )
{
    assert( hero != nullptr );
...
void Battle::Only::ArmyUI::redraw( fheroes::Image & output ) const
{
    if ( morale ) {
        morale->Redraw();
    }
