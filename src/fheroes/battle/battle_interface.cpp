...
namespace
{
    ...
    const fheroes::Point bridgeDestroySmokeOffset( -45, 65 );
    ...
    const int32_t battleLogElementWidth{ fheroes::Display::DEFAULT_WIDTH - 32 - 16 };

    struct LightningPoint
    {
        explicit LightningPoint( const fheroes::Point & p = fheroes::Point(), const int32_t thick = 1 )
            : point( p )
            , thickness( thick )
        {
            // Do nothing.
    ...
    double getDistance( const fheroes::Point & p1, const fheroes::Point & p2 )
    {
        return std::hypot( p1.x - p2.x, p1.y - p2.y );
    }

    fheroes::Point rotate( const fheroes::Point & point, double angle )
    {
    ...
    double getAngle( const fheroes::Point & start, const fheroes::Point & end )
    {
        return std::atan2( end.y - start.y, end.x - start.x );
    }

    std::vector<std::pair<LightningPoint, LightningPoint>> GenerateLightning( const fheroes::Point & src, const fheroes::Point & dst )
    {
        ...
        for ( int step = 0; step < iterationCount; ++step ) {
            ...
            for ( const auto & [firstPoint, secondPoint] : oldLines ) {
                fheroes::Point middle( firstPoint.point + secondPoint.point );
                middle.x /= 2;
                middle.y /= 2;
    ...
    void RedrawLightning( const std::vector<std::pair<LightningPoint, LightningPoint>> & lightning, const uint8_t color, fheroes::Image & surface,
                          const fheroes::Rect & roi = fheroes::Rect() )
    {
        for ( const auto & [firstPoint, secondPoint] : lightning ) {
            const fheroes::Point & first = firstPoint.point;
            const fheroes::Point & second = secondPoint.point;
            const bool isHorizontal = std::abs( first.x - second.x ) >= std::abs( first.y - second.y );

            fheroes::DrawLine( surface, first, second, color, roi );
            ...
            for ( int32_t thickness = 1; thickness < secondPoint.thickness; ++thickness ) {
                ...
                if ( isHorizontal ) {
                    fheroes::DrawLine( surface, { first.x, first.y + offset }, { second.x, second.y + offset }, color, roi );
                }
                else {
                    fheroes::DrawLine( surface, { first.x + offset, first.y }, { second.x + offset, second.y }, color, roi );
                }
            }
            ...
            for ( int32_t thickness = secondPoint.thickness; thickness < firstPoint.thickness; ++thickness ) {
                ...
                if ( isHorizontal ) {
                    fheroes::DrawLine( surface, { first.x, first.y + offset }, second, color, roi );
                }
                else {
                    fheroes::DrawLine( surface, { first.x + offset, first.y }, second, color, roi );
                }
            }
        }
    }
    ...
    fheroes::Image DrawRainbow( const std::vector<int32_t> & rainbowArc, const int32_t rainbowThickness, const bool isVertical, const bool flipHorizontally )
    {
        ...
        fheroes::Image rainbow( rainbowImgWidth, rainbowImgHeight );
        rainbow.reset();

        // Get the original good luck sprite, since it has a rainbow image which will be used to get line.
        const fheroes::Sprite & luckSprite = fheroes::AGG::GetICN( ICN::EXPMRL, 0 );

        // Get a single rainbow line from the center of the luckSprite.
        fheroes::Image croppedRainbow( 1, rainbowThickness );
        croppedRainbow._disableTransformLayer();
        fheroes::Copy( luckSprite, luckSprite.width() / 2, 0, croppedRainbow, 0, 0, 1, rainbowThickness );
        fheroes::Image rainbowLine;

        if ( isVertical ) {
            rainbowLine = fheroes::Image( croppedRainbow.height(), croppedRainbow.width() );
            rainbowLine._disableTransformLayer();

            // For a vertical rainbow orientation the line needs to be transposed.
            fheroes::Transpose( croppedRainbow, rainbowLine );
            if ( !flipHorizontally ) {
                rainbowLine = fheroes::Flip( rainbowLine, true, false );
            }
        }
        else {
        ...
        for ( int32_t x = 0; pnt != pntEnd; ++x, ++pnt ) {
            ...
            fheroes::Copy( rainbowLine, 0, 0, rainbow, imgX, imgY, rainbowLineWidth, rainbowLineHeight );
        }
        return rainbow;
    }
    ...
    {
        const uint32_t frameCount = fheroes::AGG::GetICNCount( icnId );
        if ( frameCount == 0 ) {
            return 0;
        }
        ...
        for ( uint32_t i = 0; i < frameCount; ++i ) {
            const int32_t offset = -fheroes::AGG::GetICN( icnId, i ).y();
            if ( offset > height ) {
                height = offset;
            }
        }
    ...
    fheroes::Point CalculateSpellPosition( const Battle::Unit & target, const int spellICN, const fheroes::Sprite & spellSprite )
    {
        const fheroes::Rect & pos = target.GetRectPosition();

        // Get the sprite for the first frame, so its center won't shift if the creature is animating (instead of target.GetFrame()).
        const fheroes::Sprite & unitSprite = fheroes::AGG::GetICN( target.GetMonsterSprite(), target.animation.firstFrame() );

        // Bottom-left corner (default) position with spell offset applied
        fheroes::Point result( pos.x + spellSprite.x(), pos.y + pos.height + cellYOffset + spellSprite.y() );
        ...
        case ICN::BLIND: {
            // unit's eyes
            const fheroes::Point & offset = target.animation.getBlindOffset();
    ...
    fheroes::Image DrawHexagon( const uint8_t colorId )
    {
        const fheroes::Sprite & originalGrid = fheroes::AGG::GetICN( ICN::CMBTMISC, 0 );

        // Move hexagon by 1 pixel down to match the original game's grid.
        fheroes::Image hexagonSprite( originalGrid.width(), originalGrid.height() + 1 );

        // Make the upper image line transparent. The other part will be filled with the copied image.
        fheroes::FillTransform( hexagonSprite, 0, 0, originalGrid.width(), 1, 1 );
        fheroes::Copy( originalGrid, 0, 0, hexagonSprite, 0, 1, originalGrid.width(), originalGrid.height() );

        fheroes::ReplaceColorId( hexagonSprite, 0, colorId );

        return hexagonSprite;
    }

    fheroes::Image DrawHexagonShadow( const uint8_t alphaValue, const int32_t horizSpace )
    {
        const int32_t l = 13;
        const int32_t w = Battle::Cell::widthPx;
        const int32_t h = Battle::Cell::heightPx;

        fheroes::Image sf( w, h );
        sf.reset();
        fheroes::Rect rt( horizSpace, l - 1, w + 1 - horizSpace * 2, 2 * l + 4 );
        for ( int32_t i = 0; i < w / 2; i += 2 ) {
            for ( int32_t x = 0; x < rt.width; ++x ) {
                for ( int32_t y = 0; y < rt.height; ++y ) {
                    fheroes::SetTransformPixel( sf, rt.x + x, rt.y + y, alphaValue );
                }
            }
            --rt.y;
            rt.height += 2;
    ...
    fheroes::Point GetTroopPosition( const Battle::Unit & unit, const fheroes::Sprite & sprite )
    {
        const fheroes::Rect & rt = unit.GetRectPosition();
...
namespace Battle
{
    ...
    public:
        ...
        void SetPosition( int32_t px, int32_t py )
        {
            ...
            _border.SetPosition( px, py - totalElementHeight - 32, fheroes::Display::DEFAULT_WIDTH - 32, totalElementHeight );
            const fheroes::Rect & area = _border.GetArea();

            SetTopLeft( area.getPosition() );
            SetAreaMaxItems( maxElementsInBattleLog );

            const int32_t ax = area.x + area.width - 12;
            SetScrollButtonUp( ICN::DROPLISL, 6, 7, fheroes::Point( ax, area.y - 10 ) );
            SetScrollButtonDn( ICN::DROPLISL, 8, 9, fheroes::Point( ax, area.y + area.height - 11 ) );

            const fheroes::Rect & buttonPgUpArea = _buttonPgUp.area();
            ...
            const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( ICN::DROPLISL, 13 );
            const fheroes::Image scrollbarSlider
                = fheroes::generateScrollbarSlider( originalSlider, false, _scrollbarSliderAreaLength, VisibleItemCount(), static_cast<int32_t>( _messages.size() ),
        ...
        void AddMessage( std::string str )
        {
            ...
            fheroes::Text text( str, fheroes::FontType::normalWhite() );
            ...
            while ( text.width() > battleLogElementWidth ) {
                ...
                text.set( str.substr( 0, pos ), fheroes::FontType::normalWhite() );
            }
        ...
        {
            const fheroes::Text text( str, fheroes::FontType::normalWhite() );
            text.draw( px, py, fheroes::Display::instance() );
        }
        ...
        void RedrawBackground( const fheroes::Point & /* unused*/ ) override
        {
            ...
            const fheroes::Rect & borderRect = _border.GetRect();
            Dialog::FrameBorder::RenderOther( fheroes::AGG::GetICN( ICN::TEXTBAK2, 0 ), borderRect );

            const fheroes::Sprite & sp3 = fheroes::AGG::GetICN( ICN::DROPLISL, 11 );
            for ( int32_t i = 0; i < ( ah / sp3.height() ); ++i ) {
                fheroes::Copy( sp3, 0, 0, display, ax, buttonPgUpBottom + ( sp3.height() * i ), sp3.width(), sp3.height() );
            }

            const fheroes::Sprite & sp1 = fheroes::AGG::GetICN( ICN::DROPLISL, 10 );
            const fheroes::Sprite & sp2 = fheroes::AGG::GetICN( ICN::DROPLISL, 12 );

            fheroes::Copy( sp1, 0, 0, display, ax, buttonPgUpBottom, sp1.width(), sp1.height() );
            fheroes::Copy( sp2, 0, 0, display, ax, buttonPgDnAreaY - sp2.height(), sp2.width(), sp2.height() );
        }
...
Battle::OpponentSprite::OpponentSprite( const fheroes::Rect & area, HeroBase * hero, const bool isReflect )
    : _heroBase( hero )
    ...
    , _offset( area.x, area.y )
{
    ...
    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( _heroIcnId, _currentAnim.getFrame() );

    _area.width = sprite.width();
    _area.height = sprite.height();

    if ( _isFlippedHorizontally ) {
        _area.x = _offset.x + fheroes::Display::DEFAULT_WIDTH - RIGHT_HERO_X_OFFSET - ( sprite.x() + _area.width );
...
fheroes::Point Battle::OpponentSprite::GetCastPosition() const
{
...
void Battle::OpponentSprite::Redraw( fheroes::Image & dst ) const
{
    const fheroes::Sprite & hero = fheroes::AGG::GetICN( _heroIcnId, _currentAnim.getFrame() );

    fheroes::Point offset( _offset );
    ...
    if ( _isFlippedHorizontally ) {
        fheroes::Blit( hero, dst, offset.x + fheroes::Display::DEFAULT_WIDTH - RIGHT_HERO_X_OFFSET - ( hero.x() + hero.width() ),
                        offset.y + RIGHT_HERO_Y_OFFSET + hero.y(), _isFlippedHorizontally );
    }
    else {
        fheroes::Blit( hero, dst, offset.x + LEFT_HERO_X_OFFSET + hero.x(), offset.y + LEFT_HERO_Y_OFFSET + hero.y() );
    }
}
...
Battle::Status::Status()
    : _upperBackground( fheroes::AGG::GetICN( ICN::TEXTBAR, 8 ) )
    , _lowerBackground( fheroes::AGG::GetICN( ICN::TEXTBAR, 9 ) )
{
...
{
    if ( top ) {
        _upperText.set( messageString, fheroes::FontType::normalWhite() );
        ...
        _lowerText.set( messageString, fheroes::FontType::normalWhite() );
        // The text cannot go beyond the text area so it is important to truncate it when necessary.
...
void Battle::Status::redraw( fheroes::Image & output ) const
{
    fheroes::Copy( _upperBackground, 0, 0, output, x, y, width, _upperBackground.height() );
    fheroes::Copy( _lowerBackground, 0, 0, output, x, y + _upperBackground.height(), width, _lowerBackground.height() );
...
{
    _upperText.set( "", fheroes::FontType::normalWhite() );
    _lowerText.set( "", fheroes::FontType::normalWhite() );
    _lastMessage = "";
}

bool Battle::TurnOrder::queueEventProcessing( Interface & interface, std::string & msg, const fheroes::Point & offset ) const
{
    ...
    for ( const auto & [unit, unitPos] : _rects ) {
        ...
        const fheroes::Rect unitRoi = _isInsideBattleField ? ( unitPos + offset ) : unitPos;
...
void Battle::TurnOrder::_redrawUnit( const fheroes::Rect & pos, const Battle::Unit & unit, const bool revert, const uint8_t unitColor, fheroes::Image & output )
{
    // Render background.
    const fheroes::Sprite & backgroundOriginal = fheroes::AGG::GetICN( ICN::SWAPWIN, 0 );
    fheroes::Copy( backgroundOriginal, 37, 268, output, pos.x + 1, pos.y + 1, pos.width - 2, pos.height - 2 );

    // Draw a monster's sprite.
    const fheroes::Sprite & monsterSprite = fheroes::AGG::GetICN( ICN::MONS32, unit.GetSpriteIndex() );
    const int32_t monsterSpriteHeight = monsterSprite.height();
    if ( unit.Modes( Battle::CAP_MIRRORIMAGE ) ) {
        fheroes::Sprite mirroredMonster = monsterSprite;

        fheroes::ApplyPalette( mirroredMonster, PAL::GetPalette( PAL::PaletteType::MIRROR_IMAGE ) );
        fheroes::Blit( mirroredMonster, output, pos.x + ( pos.width - monsterSprite.width() ) / 2,
                        pos.y + pos.height - monsterSpriteHeight - ( monsterSpriteHeight + 3 < pos.height ? 3 : 0 ), revert );
    }
    else {
        fheroes::Blit( monsterSprite, output, pos.x + ( pos.width - monsterSprite.width() ) / 2,
                        pos.y + pos.height - monsterSpriteHeight - ( monsterSpriteHeight + 3 < pos.height ? 3 : 0 ), revert );
    }

    const fheroes::Text number( fheroes::abbreviateNumber( static_cast<int32_t>( unit.GetCount() ) ), fheroes::FontType::smallWhite() );
    number.drawInRoi( pos.x + 2, pos.y + 4, output, pos );

    fheroes::DrawRect( output, pos, unitColor );

    if ( unit.Modes( Battle::TR_MOVED ) ) {
        fheroes::ApplyPalette( output, pos.x, pos.y, output, pos.x, pos.y, turnOrderMonsterIconSize, turnOrderMonsterIconSize,
                                PAL::GetPalette( PAL::PaletteType::GRAY ) );
        fheroes::ApplyPalette( output, pos.x, pos.y, output, pos.x, pos.y, turnOrderMonsterIconSize, turnOrderMonsterIconSize, 3 );
    }
}

void Battle::TurnOrder::redraw( const Unit * current, const uint8_t currentUnitColor, const Unit * underCursor, fheroes::Image & output,
                                const fheroes::Rect & dialogRoi )
{
    if ( _restorer ) {
        _restorer->restore();
    }
    ...
    auto & display = fheroes::Display::instance();
    ...
    if ( _rects.size() != static_cast<size_t>( unitsToDraw ) ) {
        ...
        for ( int32_t index = 0; index < unitsToDraw; ++index ) {
            ...
            if ( _isInsideBattleField ) {
                _rects.emplace_back( nullptr, fheroes::Rect( offsetX, 0, turnOrderMonsterIconSize, turnOrderMonsterIconSize ) );
            }
            else {
                _rects.emplace_back( nullptr, fheroes::Rect( offsetX, _renderingRoi.y, turnOrderMonsterIconSize, turnOrderMonsterIconSize ) );
            }
        ...
        if ( !_isInsideBattleField ) {
            _restorer = std::make_unique<fheroes::ImageRestorer>( display, _renderingRoi.x, _renderingRoi.y, _renderingRoi.width, _renderingRoi.height );
        }
    }
    ...
    : arena( battleArena )
{
    ...
    _interfacePosition = { ( display.width() - fheroes::Display::DEFAULT_WIDTH ) / 2, ( display.height() - fheroes::Display::DEFAULT_HEIGHT ) / 2,
                           _surfaceInnerArea.width, _surfaceInnerArea.height };
    border.SetPosition( _interfacePosition.x - fheroes::borderWidthPx, _interfacePosition.y - fheroes::borderWidthPx, fheroes::Display::DEFAULT_WIDTH,
    ...
    // hexagon
    _hexagonGrid = DrawHexagon( fheroes::GetColorId( 0x68, 0x8C, 0x04 ) );
    ...
    if ( Arena::GetCastle() ) {
        _ballistaTowerRect = fheroes::Rect{ _interfacePosition.x + 570, _interfacePosition.y + 145, 70, 160 };
    }

    const fheroes::Rect & area = border.GetArea();

    const fheroes::Rect & settingsRect = _buttonSettings.area();
...
Battle::Interface::~Interface()
{
    ...
    const bool isDefaultScreenSize = fheroes::Display::instance().isDefaultSize();

    fheroes::fadeOutDisplay( _background->activeArea(), !isDefaultScreenSize );
...
fheroes::Point Battle::Interface::getRelativeMouseCursorPos() const
{
...
{
    if ( !_background ) {
        _background = std::make_unique<fheroes::StandardWindow>( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT, false );
    }
    ...
    if ( !isDefaultScreenSize ) {
        ...
        display.updateNextRenderRoi( fheroes::getBoundaryRect( _background->totalArea(), _turnOrder.getRenderingRoi() ) );
    }

    fheroes::fadeInDisplay( _background->activeArea(), !isDefaultScreenSize );
}
...
void Battle::Interface::RedrawPartialFinish()
{
    ...
    auto & display = fheroes::Display::instance();
    display.render( fheroes::getBoundaryRect( _interfacePosition, _turnOrder.getRenderingRoi() ) );
}
...
void Battle::Interface::redrawPreRender()
{
    ...
    if ( IS_DEVEL() ) {
        ...
        for ( const Cell & cell : *board ) {
            const fheroes::Text text( std::to_string( cell.GetIndex() ), fheroes::FontType::smallWhite() );
            text.draw( cell.GetPos().x + 20, cell.GetPos().y + 24, _mainSurface );
        }
    }
#endif

    fheroes::Copy( _mainSurface, 0, 0, fheroes::Display::instance(), _interfacePosition.x, _interfacePosition.y, _mainSurface.width(), _mainSurface.height() );
    RedrawInterface();
}

void Battle::Interface::RedrawInterface()
{
    status.redraw( fheroes::Display::instance() );
    ...
    for ( int32_t cellRowId = 0; cellRowId < Board::heightInCells; ++cellRowId ) {
        ...
        if ( castle != nullptr ) {
            ...
            for ( const Battle::UnitSpellEffectInfo * overlaySprite : troopOverlaySpriteBeforeWall ) {
                ...
                const fheroes::Sprite & spellSprite = fheroes::AGG::GetICN( overlaySprite->icnId, overlaySprite->icnIndex );
                fheroes::Blit( spellSprite, _mainSurface, overlaySprite->position.x, overlaySprite->position.y, overlaySprite->isReflectedImage );
            }
...
void Battle::Interface::RedrawOpponentsFlags()
{
    ...
    if ( _attackingOpponent ) {
        ...
        const fheroes::Sprite & flag = fheroes::AGG::GetICN( icn, ICN::getAnimatedIcnIndex( icn, 0, animation_flags_frame ) );
        fheroes::Blit( flag, _mainSurface, _attackingOpponent->Offset().x + OpponentSprite::LEFT_HERO_X_OFFSET + flag.x(),
    ...
    if ( _defendingOpponent ) {
        ...
        const fheroes::Sprite & flag = fheroes::AGG::GetICN( icn, ICN::getAnimatedIcnIndex( icn, 0, animation_flags_frame ) );
        fheroes::Blit( flag, _mainSurface,
                        _defendingOpponent->Offset().x + fheroes::Display::DEFAULT_WIDTH - OpponentSprite::RIGHT_HERO_X_OFFSET - ( flag.x() + flag.width() ),
...
void Battle::Interface::RedrawTroopSprite( const Unit & unit )
{
    ...
    const fheroes::Sprite & monsterSprite = isCurrentMonsterAction ? *_spriteInsteadCurrentUnit : fheroes::AGG::GetICN( unit.GetMonsterSprite(), unit.GetFrame() );

    fheroes::Point drawnPosition;

    if ( unit.Modes( SP_STONE | CAP_MIRRORIMAGE ) ) {
        // Apply Stone or Mirror image visual effect.
        fheroes::Sprite modifiedMonsterSprite( monsterSprite );
        const PAL::PaletteType paletteType = unit.Modes( SP_STONE ) ? PAL::PaletteType::GRAY : PAL::PaletteType::MIRROR_IMAGE;

        fheroes::ApplyPalette( modifiedMonsterSprite, PAL::GetPalette( paletteType ) );
    ...
    if ( _unitToHighlight == &unit ) {
        ...
        const fheroes::Sprite & monsterContour = fheroes::CreateContour( monsterSprite, GetArmyColorFromPlayerColor( unit.GetArmyColor() ) );
        fheroes::Blit( monsterContour, _mainSurface, drawnPosition.x, drawnPosition.y, unit.isReflect() );
        ...
        const fheroes::Sprite & monsterContour = fheroes::CreateContour( monsterSprite, _contourColor );
        fheroes::Blit( monsterContour, _mainSurface, drawnPosition.x, drawnPosition.y, unit.isReflect() );
    }
}

fheroes::Point Battle::Interface::_drawTroopSprite( const Unit & unit, const fheroes::Sprite & troopSprite )
{
    // Get the sprite rendering offset.
    fheroes::Point offset = GetTroopPosition( unit, troopSprite );
    ...
    if ( _movingUnit == &unit ) {
        ...
        if ( _movingUnit->animation.animationLength() ) {
            ...
            const fheroes::Rect & unitPosition = unit.GetRectPosition();
            ...
            if ( _movingUnit->isAbilityPresent( fheroes::MonsterAbilityType::FLYING ) ) {
    ...
    fheroes::AlphaBlit( troopSprite, _mainSurface, offset.x, offset.y, unit.GetCustomAlpha(), unit.isReflect() );
...
{
    const fheroes::Rect & rt = unit.GetRectPosition();
    const fheroes::Sprite & bar = fheroes::AGG::GetICN( ICN::TEXTBAR, GetIndexIndicator( unit ) );
    ...
    fheroes::Copy( bar, 0, 0, _mainSurface, sx, sy, bar.width(), bar.height() );

    const fheroes::Text text( fheroes::abbreviateNumber( static_cast<int32_t>( unit.GetCount() ) ), fheroes::FontType::smallWhite() );
...
void Battle::Interface::RedrawCover()
{
    ...
    if ( bridge && ( bridge->isDown() || _bridgeAnimation.animationIsRequired ) ) {
        ...
        const fheroes::Sprite & bridgeImage = fheroes::AGG::GetICN( ICN::getCastleIcnId( Arena::GetCastle()->GetRace() ), spriteIndex );
        fheroes::Blit( bridgeImage, _mainSurface, bridgeImage.x(), bridgeImage.y() );
    }
    ...
    if ( cell && _currentUnit && Settings::Get().BattleShowMouseShadow() ) {
        ...
        else if ( _currentUnit->isAbilityPresent( fheroes::MonsterAbilityType::AREA_SHOT )
        ...
        for ( const Cell * highlightedCell : highlightedCells ) {
            ...
            if ( isApplicable ) {
                fheroes::Blit( _hexagonCursorShadow, _mainSurface, highlightedCell->GetPos().x, highlightedCell->GetPos().y );
            }
        }
    }
}
...
void Battle::Interface::_redrawBattleGround()
{
    ...
    if ( _battleGroundIcn != ICN::UNKNOWN ) {
        const fheroes::Sprite & cbkg = fheroes::AGG::GetICN( _battleGroundIcn, 0 );
        fheroes::Copy( cbkg, _battleGround );
    }

    // Objects near the left and right borders of the Battlefield.
    if ( _borderObjectsIcn != ICN::UNKNOWN ) {
        const fheroes::Sprite & frng = fheroes::AGG::GetICN( _borderObjectsIcn, 0 );
        fheroes::Blit( frng, _battleGround, frng.x(), frng.y() );
    }

    // Big obstacles in the center of the Battlefield.
    if ( arena.GetICNCovr() != ICN::UNKNOWN ) {
        const fheroes::Sprite & cover = fheroes::AGG::GetICN( arena.GetICNCovr(), 0 );
        fheroes::Blit( cover, _battleGround, cover.x(), cover.y() );
    }
    ...
    if ( castle != nullptr ) {
        ...
        const fheroes::Sprite & castleBackground = fheroes::AGG::GetICN( castleBackgroundIcnId, 1 );
        fheroes::Blit( castleBackground, _battleGround, castleBackground.x(), castleBackground.y() );

        // Moat.
        if ( castle->isBuild( BUILD_MOAT ) ) {
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::MOATWHOL, 0 );
            fheroes::Blit( sprite, _battleGround, sprite.x(), sprite.y() );
        }
    }
    ...
    if ( Settings::Get().BattleShowGrid() ) {
        ...
        for ( const Cell & cell : board ) {
            fheroes::Blit( _hexagonGrid, _battleGround, cell.GetPos().x, cell.GetPos().y );
        }
    }
    ...
    // Castle top wall.
    if ( castle != nullptr ) {
        const fheroes::Sprite & sprite2 = fheroes::AGG::GetICN( castleBackgroundIcnId, castle->isFortificationBuilt() ? 4 : 3 );
        fheroes::Blit( sprite2, _battleGround, sprite2.x(), sprite2.y() );
    }
}

void Battle::Interface::_redrawCoverStatic()
{
    fheroes::Copy( _battleGround, _mainSurface );

    const Settings & conf = Settings::Get();

    // Movement shadow.
    if ( !_movingUnit && conf.BattleShowMoveShadow() && _currentUnit && !( _currentUnit->GetCurrentControl() & CONTROL_AI ) ) {
        const fheroes::Image & shadowImage = conf.BattleShowGrid() ? _hexagonGridShadow : _hexagonShadow;
        const Board & board = *Arena::GetBoard();
        ...
        for ( const Cell & cell : board ) {
            ...
            if ( pos.GetHead() != nullptr ) {
                ...
                fheroes::Blit( shadowImage, _mainSurface, cell.GetPos().x, cell.GetPos().y );
            }
        }
    }
}
...
void Battle::Interface::RedrawCastle( const Castle & castle, const int32_t cellId )
{
    ...
    if ( Arena::CATAPULT_POS == cellId ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::CATAPULT, catapult_frame );
        fheroes::Blit( sprite, _mainSurface, 22 + sprite.x(), 390 + sprite.y() );
    }
    else if ( Arena::CASTLE_GATE_POS == cellId ) {
        const Bridge * bridge = Arena::GetBridge();
        assert( bridge != nullptr );
        if ( bridge != nullptr && !bridge->isDestroyed() ) {
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( castleIcnId, 4 );
            fheroes::Blit( sprite, _mainSurface, sprite.x(), sprite.y() );
        }
    }
        ...
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( castleIcnId, index );
        fheroes::Blit( sprite, _mainSurface, sprite.x(), sprite.y() );
    }
        ...
        const fheroes::Sprite & towerSprite = fheroes::AGG::GetICN( castleIcnId, index );
        fheroes::Blit( towerSprite, _mainSurface, 443 + towerSprite.x(), 153 + towerSprite.y() );
    }
        ...
        const fheroes::Sprite & towerSprite = fheroes::AGG::GetICN( castleIcnId, index );
        fheroes::Blit( towerSprite, _mainSurface, 443 + towerSprite.x(), 405 + towerSprite.y() );
    }
    else if ( Arena::CASTLE_TOP_GATE_TOWER_POS == cellId ) {
        const int index = ( Board::GetCell( cellId )->GetObject() == 1 ) ? 19 : 17;
        const fheroes::Sprite & towerSprite = fheroes::AGG::GetICN( castleIcnId, index );
        fheroes::Blit( towerSprite, _mainSurface, 399 + towerSprite.x(), 237 + towerSprite.y() );
    }
    else if ( Arena::CASTLE_BOTTOM_GATE_TOWER_POS == cellId ) {
        const int index = ( Board::GetCell( cellId )->GetObject() == 1 ) ? 19 : 17;
        const fheroes::Sprite & towerSprite = fheroes::AGG::GetICN( castleIcnId, index );
        fheroes::Blit( towerSprite, _mainSurface, 399 + towerSprite.x(), 321 + towerSprite.y() );
    }
}

void Battle::Interface::RedrawCastleMainTower( const Castle & castle )
{
    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::getCastleIcnId( castle.GetRace() ), ( Arena::GetTower( TowerType::TWR_CENTER )->isValid() ? 20 : 26 ) );
...
void Battle::Interface::_redrawGroundObjects( const int32_t cellId )
{
    ...
    const fheroes::Sprite & objectSprite = fheroes::AGG::GetICN( objectIcnId, 0 );
    const fheroes::Rect & pt = cell->GetPos();
    fheroes::Blit( objectSprite, _battleGround, pt.x + pt.width / 2 + objectSprite.x(), pt.y + pt.height + objectSprite.y() + cellYOffset );
}
...
void Battle::Interface::_redrawHighObjects( const int32_t cellId )
{
    ...
    const fheroes::Sprite & objectSprite = fheroes::AGG::GetICN( objectIcnId, 0 );
    const fheroes::Rect & pt = cell->GetPos();
    fheroes::Blit( objectSprite, _mainSurface, pt.x + pt.width / 2 + objectSprite.x(), pt.y + pt.height + objectSprite.y() + cellYOffset );
}
...
void Battle::Interface::HumanTurn( const Unit & unit, Actions & actions )
{
    ...
    while ( !humanturn_exit && le.HandleEvents( Game::isDelayNeeded( delayTypes ) ) ) {
        ...
        if ( humanturn_redraw ) {
            ...
            listlog->Redraw();
            fheroes::Display::instance().render( listlog->GetArea() );
        }
    }
...
void Battle::Interface::HumanBattleTurn( const Unit & unit, Actions & actions, std::string & msg )
{
    ...
    if ( le.isAnyKeyPressed() ) {
        ...
        // Switch the auto combat mode on
            ...
            if ( fheroes::showStandardTextMessage( {}, _( "Are you sure you want to enable the auto combat mode?" ), Dialog::YES | Dialog::NO ) == Dialog::YES ) {
        ...
        // Resolve the combat in quick combat mode
            ...
            if ( fheroes::showStandardTextMessage( {}, _( "Are you sure you want to resolve the battle in the quick combat mode?" ), Dialog::YES | Dialog::NO )
                 == Dialog::YES ) {
    ...
    fheroes::Rect battleFieldRect{ _interfacePosition.x, _interfacePosition.y, _interfacePosition.width, _interfacePosition.height - status.height };
    ...
    if ( doListlogProcessing ) {
        const fheroes::Rect & lislogRect = listlog->GetArea();
    ...
    if ( doListlogProcessing ) {
        ...
        if ( le.MouseClickLeft( _ballistaTowerRect ) || le.isMouseRightButtonPressedInArea( _ballistaTowerRect ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Ballista" ), ballistaMessage, le.isMouseRightButtonPressed() ? Dialog::ZERO : Dialog::OK );
        }
    }
        ...
        if ( le.MouseClickLeft( _buttonAuto.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Automatic Combat Modes" ), _( "Choose between proceeding the combat in auto combat mode or in quick combat mode." ),
                                               Dialog::ZERO );
        }
    }
        ...
        if ( le.MouseClickLeft( _buttonSettings.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "System Options" ), _( "Allows you to customize the combat screen." ), Dialog::ZERO );
        }
    }
        ...
        if ( le.MouseClickLeft( _buttonSkip.area() ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Skip" ),
        ...
        const fheroes::Rect attackingOpponentArea = _attackingOpponent->GetArea() + _interfacePosition.getPosition();
        ...
        const fheroes::Rect defendingOpponentArea = _defendingOpponent->GetArea() + _interfacePosition.getPosition();
        ...
        if ( listlog ) {
            ...
            if ( le.MouseClickLeft( status ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Message Bar" ), _( "Shows the results of individual monster's actions." ), Dialog::ZERO );
            }
        }
...
void Battle::Interface::FadeArena( const bool clearMessageLog )
{
    ...
    const fheroes::Rect srt = border.GetArea();
    fheroes::Image top;
    top._disableTransformLayer();
    top.resize( srt.width, srt.height );

    fheroes::Copy( display, srt.x, srt.y, top, 0, 0, srt.width, srt.height );
    fheroes::FadeDisplayWithPalette( top, srt.getPosition(), 5, 300, 5 );
...
void Battle::Interface::OpenAutoModeDialog( const Unit & unit, Actions & actions )
{
    ...
    fheroes::ButtonGroup autoButtons(
        { fheroes::getSupportedText( gettext_noop( "AUTO\nCOMBAT" ), buttonFontType ), fheroes::getSupportedText( gettext_noop( "QUICK\nCOMBAT" ), buttonFontType ) } );

    const int32_t titleYOffset = 16;

    const fheroes::Text title( _( "Automatic Combat Modes" ), { fheroes::FontSize::NORMAL, fheroes::FontColor::YELLOW } );

    fheroes::ButtonBase & autoCombatButton = autoButtons.button( 0 );
    fheroes::ButtonBase & quickCombatButton = autoButtons.button( 1 );

    fheroes::StandardWindow background( autoButtons, false, titleYOffset + title.height() );
    ...
    const fheroes::Rect roiArea = background.activeArea();
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            fheroes::showStandardTextMessage( _( "Cancel" ), _( "Exit this menu." ), Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "Auto Combat" ), msg, Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "Quick Combat" ), msg, Dialog::ZERO );
        }
    }
}
...
void Battle::Interface::RedrawMissileAnimation( const fheroes::Point & startPos, const fheroes::Point & endPos, const double angle, const uint32_t monsterID )
{
    LocalEvent & le = LocalEvent::Get();
    ...
    fheroes::Sprite missile;
    fheroes::Point endPosShift{ 0, 0 };

    // Mage is channeling the bolt; doesn't have missile sprite
    if ( isMage ) {
        fheroes::delayforMs( Game::ApplyBattleSpeed( 115 ) );
    }
    else {
        missile = fheroes::AGG::GetICN( static_cast<int>( Monster::GetMissileICN( monsterID ) ),
    ...
    const std::vector<fheroes::Point> points = getLinePoints( startPos, endPos + endPosShift, isMage ? 50 : std::max( missile.width(), 25 ) );
    std::vector<fheroes::Point>::const_iterator pnt = points.begin();
    ...
    const fheroes::Point missileOffset( reverse ? ( -missile.width() - missile.x() ) : missile.x(), ( angle > 0 ) ? ( -missile.height() - missile.y() ) : missile.y() );
    ...
    while ( le.HandleEvents( false ) && pnt != points.end() ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_MISSILE_DELAY ) ) {
            RedrawPartialStart();
            if ( isMage ) {
                fheroes::DrawLine( _mainSurface, { startPos.x, startPos.y - 2 }, { pnt->x, pnt->y - 2 }, 0x77 );
                fheroes::DrawLine( _mainSurface, { startPos.x, startPos.y - 1 }, { pnt->x, pnt->y - 1 }, 0xB5 );
                fheroes::DrawLine( _mainSurface, startPos, *pnt, 0xBC );
                fheroes::DrawLine( _mainSurface, { startPos.x, startPos.y + 1 }, { pnt->x, pnt->y + 1 }, 0xB5 );
                fheroes::DrawLine( _mainSurface, { startPos.x, startPos.y + 2 }, { pnt->x, pnt->y + 2 }, 0x77 );
            }
            else {
                // Coordinates in 'pnt' corresponds to the front side of the projectile (arrowhead).
                fheroes::Blit( missile, _mainSurface, pnt->x + missileOffset.x, pnt->y + missileOffset.y, reverse );
            }
            RedrawPartialFinish();
            ++pnt;
        }
...
void Battle::Interface::RedrawActionAttackPart1( Unit & attacker, const Unit & defender, const TargetsInfo & targets )
{
    ...
    const fheroes::Rect & pos1 = attacker.GetRectPosition();
    const fheroes::Rect & pos2 = defender.GetRectPosition();
    ...
    if ( archer ) {
        ...
        const fheroes::Sprite & attackerSprite = fheroes::AGG::GetICN( attacker.GetMonsterSprite(), attacker.GetFrame() );
        const fheroes::Point attackerPos = GetTroopPosition( attacker, attackerSprite );
        ...
        const fheroes::Point shooterPos( pos1.x + rearCenterX, attackerPos.y - attackerSprite.y() );

        // Use the front one to calculate the angle, then overwrite
        fheroes::Point offset = attacker.GetStartMissileOffset( Monster_Info::FRONT );

        const fheroes::Point targetPos = defender.GetCenterPoint();

        double angle = GetAngle( fheroes::Point( shooterPos.x + offset.x, shooterPos.y + offset.y ), targetPos );
        ...
        const fheroes::Point missileStart( shooterPos.x + ( attacker.isReflect() ? -offset.x : offset.x ), shooterPos.y + offset.y );
...
void Battle::Interface::RedrawActionAttackPart2( Unit & attacker, const Unit & defender, const TargetsInfo & targets, const uint32_t resurrects )
{
    ...
    if ( !isMirror && !targets.empty() ) {
        ...
        if ( resurrects != 0 ) {
            ...
            if ( attacker.isAbilityPresent( fheroes::MonsterAbilityType::SOUL_EATER ) ) {
            ...
            else if ( attacker.isAbilityPresent( fheroes::MonsterAbilityType::HP_DRAIN ) ) {
...
void Battle::Interface::_redrawActionWincesKills( const TargetsInfo & targets, Unit * attacker /* = nullptr */, const Unit * defender /* = nullptr */ )
{
    ...
    const bool drawLichCloud = ( attacker != nullptr ) && ( defender != nullptr ) && attacker->isArchers() && !Unit::isHandFighting( *attacker, *defender )
                               && attacker->isAbilityPresent( fheroes::MonsterAbilityType::AREA_SHOT );
    ...
    const uint32_t lichCloudMaxFrame = fheroes::AGG::GetICNCount( ICN::LICHCLOD );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_FRAME_DELAY } ) ) ) {
        ...
        if ( drawLichCloud && lichCloudFrame < lichCloudMaxFrame ) {
            const fheroes::Sprite & spellSprite = fheroes::AGG::GetICN( ICN::LICHCLOD, lichCloudFrame );
            const fheroes::Point & pos = CalculateSpellPosition( *defender, ICN::LICHCLOD, spellSprite );
            fheroes::Blit( spellSprite, _mainSurface, pos.x, pos.y, false );
            ++lichCloudFrame;
        }
...
void Battle::Interface::RedrawActionMove( Unit & unit, const Indexes & path )
{
    ...
    const bool canFly = unit.isAbilityPresent( fheroes::MonsterAbilityType::FLYING );
...
void Battle::Interface::RedrawActionFly( Unit & unit, const Position & pos )
{
    ...
    const fheroes::Point destPos = unit.GetRectPosition().getPosition();
    fheroes::Point targetPos = Board::GetCell( destIndex )->GetPos().getPosition();
    ...
    const std::vector<fheroes::Point> points = getLinePoints( destPos, targetPos, step );
    std::vector<fheroes::Point>::const_iterator currentPoint = points.begin();
...
void Battle::Interface::RedrawActionLuck( const Unit & unit )
{
    ...
    const fheroes::Rect & pos = unit.GetRectPosition();
    ...
    if ( isGoodLuck ) {
        ...
        const fheroes::Rect & battleArea = border.GetArea();
        const fheroes::Point rainbowDescendPoint( pos.x + pos.width / 2, pos.y - pos.height / 2 );
        ...
        const fheroes::Size rainbowArcBegin( rainbowTop, rainbowDescend + rainbowAscend );
        const fheroes::Size rainbowArcEnd( rainbowLength - rainbowTop, rainbowDescend );
        ...
        const fheroes::Image luckSprite = DrawRainbow( rainbowArc, rainbowThickness, isVerticalRainbow, isRainbowFromRight );
        ...
        while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_MISSILE_DELAY } ) )
            ...
            if ( Game::validateAnimationDelay( Game::BATTLE_MISSILE_DELAY ) ) {
                ...
                if ( isVerticalRainbow ) {
                    fheroes::Blit( luckSprite, 0, 0, _mainSurface, drawOffset, 0, luckSprite.width(), drawWidth );
                }
                else {
                    if ( isRainbowFromRight ) {
                        fheroes::Blit( luckSprite, rainbowLength - drawWidth, 0, _mainSurface, battleArea.width - drawWidth, drawOffset, drawWidth,
                                        luckSprite.height() );
                    }
                    else {
                        fheroes::Blit( luckSprite, 0, 0, _mainSurface, 0, drawOffset, drawWidth, luckSprite.height() );
                    }
                }
        ...
        while ( le.HandleEvents( Game::isCustomDelayNeeded( animationDelay ) ) && ( frameId < frameLimit || ( soundOn && Mixer::isPlaying( -1 ) ) ) ) {
            ...
            if ( Game::validateCustomAnimationDelay( animationDelay ) ) {
                ...
                if ( frameId < frameLimit ) {
                    const fheroes::Sprite & luckSprite = fheroes::AGG::GetICN( ICN::CLOUDLUK, frameId );
                    fheroes::Blit( luckSprite, _mainSurface, offsetX + luckSprite.x(), offsetY + luckSprite.y() );
...
void Battle::Interface::RedrawActionTowerPart1( const Tower & tower, const Unit & defender )
{
    ...
    const fheroes::Point missileStart = tower.GetPortPosition();
    const fheroes::Point targetPos = defender.GetCenterPoint();
...
void Battle::Interface::RedrawActionCatapultPart1( const CastleDefenseStructure catapultTarget, const bool isHit )
{
    ...
    const fheroes::Rect & area = GetArea();
    ...
    // boulder animation
    fheroes::Point pt1( 30, 290 );
    fheroes::Point pt2 = Catapult::GetTargetPosition( catapultTarget, isHit );
    ...
    const std::vector<fheroes::Point> points = GetArcPoints( pt1, pt2, boulderArcHeight, boulderArcStep );
    std::vector<fheroes::Point>::const_iterator pnt = points.begin();

    uint32_t boulderFrameId = 0;
    const uint32_t boulderFramesCount = fheroes::AGG::GetICNCount( ICN::BOULDER );
    ...
    while ( le.HandleEvents( false ) && pnt != points.end() ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_CATAPULT_BOULDER_DELAY ) ) {
            ...
            RedrawPartialStart();
            fheroes::Blit( fheroes::AGG::GetICN( ICN::BOULDER, boulderFrameId ), _mainSurface, pnt->x, pnt->y );
            RedrawPartialFinish();
            ++pnt;
            ++boulderFrameId;
    ...
    uint32_t maxFrame = isHit ? castleBuildingDestroyFrame : fheroes::AGG::GetICNCount( icn );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_CATAPULT_CLOUD_DELAY } ) ) && frame < maxFrame ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_CATAPULT_CLOUD_DELAY ) ) {
            ...
            if ( isBridgeDestroyed && frame >= bridgeDestroySmokeDelay ) {
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icn, frame - bridgeDestroySmokeDelay );
                fheroes::Blit( sprite, _mainSurface, pt1.x + sprite.x(), pt1.y + sprite.y() );
            }
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icn, frame );
            fheroes::Blit( sprite, _mainSurface, pt2.x + sprite.x(), pt2.y + sprite.y() );
            RedrawPartialFinish();
...
void Battle::Interface::RedrawActionCatapultPart2( const CastleDefenseStructure catapultTarget )
{
    ...
    const fheroes::Point pt1 = Catapult::GetTargetPosition( catapultTarget, true ) + GetArea().getPosition();
    fheroes::Point pt2;
    ...
    const uint32_t maxFrame = fheroes::AGG::GetICNCount( icnId );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_CATAPULT_CLOUD_DELAY } ) ) && frame < maxAnimationFrame ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_CATAPULT_CLOUD_DELAY ) ) {
            ...
            if ( isBridgeDestroyed ) {
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icnId, frame - bridgeDestroySmokeDelay );
                fheroes::Blit( sprite, _mainSurface, pt2.x + sprite.x(), pt2.y + sprite.y() );
            }
            // Don't draw the smoke cloud after its animation has ended (in case the second smoke cloud is still animating).
            if ( frame <= maxFrame ) {
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icnId, frame );
                fheroes::Blit( sprite, _mainSurface, pt1.x + sprite.x(), pt1.y + sprite.y() );
            }
            RedrawPartialFinish();
...
void Battle::Interface::_redrawActionArrowSpell( const Unit & target )
{
    ...
    if ( caster ) {
        const fheroes::Point missileStart = caster == _attackingOpponent->GetHero() ? _attackingOpponent->GetCastPosition() : _defendingOpponent->GetCastPosition();

        const fheroes::Point targetPos = target.GetCenterPoint();
        const double angle = GetAngle( missileStart, targetPos );
...
void Battle::Interface::_redrawActionTeleportSpell( Unit & target, const int32_t dst )
{
    ...
    const fheroes::Sprite & unitSprite = fheroes::AGG::GetICN( target.GetMonsterSprite(), target.GetFrame() );
    fheroes::Sprite rippleSprite;
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && ( frame <= frameLimit || ( soundOn && Mixer::isPlaying( -1 ) ) ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            if ( frame >= frameLimit ) {
                ...
                rippleSprite = fheroes::createRippleEffect( unitSprite, amplitude, -phaseCoeff * frame, wavePeriod );

                if ( frame > frameLimit - imageCutFrames ) {
                    // Animate disappearing by making sprite transparent starting from the top.
                    fheroes::FillTransform( rippleSprite, 0, 0, rippleSprite.width(), spriteHeight * ( frame - frameLimit + imageCutFrames ) / imageCutFrames, 1 );
                }
            }
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && ( frame < frameLimit || ( soundOn && Mixer::isPlaying( -1 ) ) ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            if ( frame == frameLimit ) {
                ...
                rippleSprite = fheroes::createRippleEffect( unitSprite, amplitude, phaseCoeff * ( frame + frameLimit ), wavePeriod );

                if ( frame < imageCutFrames ) {
                    // Animate appearing from bottom to top by making the top part transparent.
                    fheroes::FillTransform( rippleSprite, 0, 0, rippleSprite.width(), spriteHeight * ( imageCutFrames - frame ) / imageCutFrames, 1 );
                }
...
void Battle::Interface::redrawActionMirrorImageSpell( const HeroBase & caster, const int32_t targetCell, const Unit & originalUnit, Unit & mirrorUnit )
{
    ...
    fheroes::Sprite mirrorUnitSprite = fheroes::AGG::GetICN( originalUnit.GetMonsterSprite(), originalUnit.GetFrame() );
    ...
    const fheroes::Point & unitPos = originalUnit.GetRectPosition().getPosition();
    const fheroes::Point & mirrorPos = mirrorUnit.GetRectPosition().getPosition();
    const fheroes::Point spriteOffet( mirrorUnitSprite.x(), mirrorUnitSprite.y() );

    // Get spell animation points.
    const std::vector<fheroes::Point> points = getLinePoints( unitPos - mirrorPos + spriteOffet, spriteOffet, 5 );
    std::vector<fheroes::Point>::const_iterator pnt = points.cbegin();
...
void Battle::Interface::_redrawLightningOnTargets( const std::vector<fheroes::Point> & points, const fheroes::Rect & drawRoi )
{
    if ( points.size() < 2 )
        return;

    const fheroes::Point roiOffset( drawRoi.x, drawRoi.y );
    ...
    for ( size_t i = 1; i < points.size(); ++i ) {
        const fheroes::Point & startingPos = points[i - 1];
        const fheroes::Point & endPos = points[i];
        ...
        while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) )
            ...
            if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
                ...
                RedrawLightning( lightningBolt, fheroes::GetColorId( 0xff, 0xff, 0 ), _mainSurface,
                                 { roi.x + roiOffset.x, roi.y + roiOffset.y, roi.width, roi.height } );
                fheroes::ApplyPalette( _mainSurface, 7 );
    ...
    fheroes::delayforMs( 100 );

    uint32_t frame = 0;
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && frame < fheroes::AGG::GetICNCount( ICN::SPARKS ) ) {
        CheckGlobalEvents( le );

        if ( ( frame == 0 ) || Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            RedrawPartialStart();

            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::SPARKS, frame );
            const int32_t spriteWidth = sprite.width();

            for ( size_t i = 1; i < points.size(); ++i ) {
                const fheroes::Point pt = points[i] - fheroes::Point( spriteWidth / 2, 0 ) + roiOffset;
                fheroes::Blit( sprite, _mainSurface, pt.x, pt.y );
            }
            RedrawPartialFinish();
...
void Battle::Interface::_redrawActionLightningBoltSpell( const Unit & target )
{
    ...
    const fheroes::Point startingPos
    ...
    const fheroes::Rect & pos = target.GetRectPosition();
    const fheroes::Point endPos( pos.x + pos.width / 2, pos.y );

    const std::vector<fheroes::Point> points{ startingPos, endPos };
...
void Battle::Interface::_redrawActionChainLightningSpell( const TargetsInfo & targets )
{
    const fheroes::Point startingPos
        = arena.GetCurrentCommander() == _attackingOpponent->GetHero() ? _attackingOpponent->GetCastPosition() : _defendingOpponent->GetCastPosition();
    std::vector<fheroes::Point> points;
    points.reserve( size( targets ) + 1 );
    points.push_back( startingPos );

    for ( const TargetInfo & target : targets ) {
        const fheroes::Rect & pos = target.defender->GetRectPosition();
...
void Battle::Interface::_redrawActionBloodLustSpell( const Unit & target )
{
    ...
    fheroes::Sprite bloodlustEffect( unitSprite );
    fheroes::ApplyPalette( bloodlustEffect, PAL::GetPalette( PAL::PaletteType::RED ) );

    fheroes::Sprite mixSprite;
    ...
    while ( le.HandleEvents( Game::isCustomDelayNeeded( bloodlustDelay ) ) && Mixer::isPlaying( -1 ) ) {
        ...
        if ( frame < 20 && Game::validateCustomAnimationDelay( bloodlustDelay ) ) {
            ...
            fheroes::AlphaBlit( bloodlustEffect, mixSprite, alpha );
            Redraw();
...
void Battle::Interface::_redrawActionStoneSpell( const Unit & target )
{
    ...
    fheroes::Sprite stoneEffect( unitSprite );
    fheroes::ApplyPalette( stoneEffect, PAL::GetPalette( PAL::PaletteType::GRAY ) );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && Mixer::isPlaying( -1 ) ) {
        ...
        if ( frame < 25 && Game::validateCustomAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            mixSprite = unitSprite;
            fheroes::AlphaBlit( stoneEffect, mixSprite, alpha );
            Redraw();
...
void Battle::Interface::_redrawRaySpell( const Unit & target, const int spellICN, const int spellSound, const int32_t size )
{
    ...
    const std::vector<fheroes::Point> path = getLinePoints( startingPos, targetPos, size );
    const uint32_t spriteCount = fheroes::AGG::GetICNCount( spellICN );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && i < path.size() ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            ...
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( spellICN, frame );
            fheroes::Blit( sprite, _mainSurface, path[i].x - sprite.width() / 2, path[i].y - sprite.height() / 2 );
            RedrawPartialFinish();
            ++i;
        }
    }
...
void Battle::Interface::_redrawActionDisruptingRaySpell( Unit & target )
{
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && frame < frameLimit ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            ...
            rippleSprite = fheroes::createRippleEffect( unitSprite, amplitude, -0.4 * frame, 60 );
...
void Battle::Interface::_redrawActionDeathWaveSpell( const int32_t strength )
{
    ...
    fheroes::Image battleFieldCopy;
    battleFieldCopy._disableTransformLayer();
    battleFieldCopy.resize( area.width, area.height );
    fheroes::Copy( _mainSurface, 0, 0, battleFieldCopy, 0, 0, area.width, area.height );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_DISRUPTING_DELAY } ) ) && position < area.width + waveLength ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_DISRUPTING_DELAY ) ) {
            ...
            const fheroes::Rect renderArea( _interfacePosition.x + restorePositionX, _interfacePosition.y + area.y, waveWidth + restoreWidth, area.height );

            // Place a copy of the original image where the Death Wave effect was on the previous frame.
            fheroes::Copy( battleFieldCopy, restorePositionX, area.y, display, renderArea.x, renderArea.y, restoreWidth, renderArea.height );

            // Place the Death Wave effect to its new position.
            fheroes::CreateDeathWaveEffect( spellEffect, battleFieldCopy, position, deathWaveCurve );
            fheroes::Copy( spellEffect, 0, 0, display, renderArea.x + restoreWidth, renderArea.y, waveWidth, area.height );
...
void Battle::Interface::_redrawActionColdRingSpell( const int32_t dst, const TargetsInfo & targets )
{
    ...
    uint32_t frame = 0;
    const fheroes::Rect & center = Board::GetCell( dst )->GetPos();
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && frame < fheroes::AGG::GetICNCount( icn ) ) {
        CheckGlobalEvents( le );

        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            RedrawPartialStart();

            const fheroes::Sprite & sprite1 = fheroes::AGG::GetICN( icn, frame );
            fheroes::Blit( sprite1, _mainSurface, center.x + center.width / 2 + sprite1.x(), center.y + center.height / 2 + sprite1.y() );
            const fheroes::Sprite & sprite2 = fheroes::AGG::GetICN( icn, frame );
            fheroes::Blit( sprite2, _mainSurface, center.x + center.width / 2 - sprite2.width() - sprite2.x(), center.y + center.height / 2 + sprite2.y(), true );
            RedrawPartialFinish();
...
void Battle::Interface::_redrawActionHolyShoutSpell( const uint8_t strength )
{
    ...
    std::vector<fheroes::Image> spellEffect;
    ...
    spellEffect.emplace_back( fheroes::CreateHolyShoutEffect( battleFieldCopy, 4, strength ) );
    ...
    fheroes::Display & display = fheroes::Display::instance();
    const fheroes::Rect renderArea( _interfacePosition.x + area.x, _interfacePosition.y + area.y, area.width, area.height );
        ...
        if ( Game::validateCustomAnimationDelay( spellcastDelay ) ) {
            ...
            if ( frame != halfMaxFrame ) {
                ...
                if ( frame < spellEffectLastFrame ) {
                    fheroes::AlphaBlit( spellEffect[spellEffectLastFrame], spellEffect[frame], alpha );
                    alpha += alphaStep;
                }

                const uint32_t spellEffectFrame = ( frame < halfMaxFrame ) ? frame : ( maxFrame - frame - 1 );
                fheroes::Copy( spellEffect[spellEffectFrame], area.x, area.y, display, renderArea.x, renderArea.y, renderArea.width, renderArea.height );
...
void Battle::Interface::_redrawActionElementalStormSpell( const TargetsInfo & targets )
{
    ...
    const uint32_t icnCount = fheroes::AGG::GetICNCount( icn );

    std::vector<fheroes::Sprite> spriteCache;
    spriteCache.reserve( icnCount );
    for ( uint32_t i = 0; i < icnCount; ++i ) {
        spriteCache.push_back( fheroes::AGG::GetICN( icn, i ) );
    }
            ...
            if ( icnCount > 0 ) {
                for ( int x = 0; x * spriteSize < _surfaceInnerArea.width; ++x ) {
                    ...
                    for ( int y = 0; y * spriteSize < _surfaceInnerArea.height; ++y ) {
                        const fheroes::Sprite & sprite = spriteCache[( idX + y ) % icnCount];
                        fheroes::Blit( sprite, _mainSurface, offsetX + sprite.x(), y * spriteSize + sprite.y() );
                    }
                }
            }
...
void Battle::Interface::_redrawActionArmageddonSpell()
{
    ...
    fheroes::Image spriteWhitening;
    spriteWhitening._disableTransformLayer();
    spriteWhitening.resize( area.width, area.height );

    fheroes::Copy( _mainSurface, area.x, area.y, spriteWhitening, 0, 0, area.width, area.height );
    fheroes::Image spriteReddish = spriteWhitening;
    fheroes::ApplyPalette( spriteReddish, PAL::GetPalette( PAL::PaletteType::RED ) );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && alpha < 100 ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            fheroes::ApplyPalette( spriteWhitening, 9 );
            fheroes::Copy( spriteWhitening, 0, 0, _mainSurface, area.x, area.y, area.width, area.height );
            RedrawPartialFinish();
    ...
    fheroes::Copy( spriteReddish, 0, 0, _mainSurface, area.x, area.y, area.width, area.height );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && Mixer::isPlaying( -1 ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            fheroes::Copy( spriteReddish, std::max( 0, -offsetX ), std::max( 0, -offsetY ), _mainSurface, std::max( 0, offsetX ), std::max( 0, offsetY ),
...
void Battle::Interface::redrawActionEarthquakeSpellPart1( const HeroBase & caster, const std::vector<CastleDefenseStructure> & targets )
{
    ...
    fheroes::Image battlefieldImage;
    battlefieldImage._disableTransformLayer();
    battlefieldImage.resize( area.width, area.height );
    fheroes::Copy( _mainSurface, area.x, area.y, battlefieldImage, area );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && frame < 18 ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            fheroes::Copy( battlefieldImage, std::max( 0, -offsetX ), std::max( 0, -offsetY ), _mainSurface, std::max( 0, offsetX ), std::max( 0, offsetY ),
                            area.width - std::abs( offsetX ), area.height - std::abs( offsetY ) );
    ...
    const uint32_t maxFrame = fheroes::AGG::GetICNCount( icn ) / 2;
    frame = 0;
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY, Game::BATTLE_OPPONENTS_DELAY } ) ) && frame < maxFrame ) {
        ...
        if ( isNeedSpellAnimationUpdate || Game::validateAnimationDelay( Game::BATTLE_OPPONENTS_DELAY ) ) {
            ...
            for ( const CastleDefenseStructure target : targets ) {
                const fheroes::Point pt2 = Catapult::GetTargetPosition( target, true );
                ...
                if ( target == CastleDefenseStructure::BRIDGE && frame >= bridgeDestroySmokeDelay ) {
                    ...
                    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icn, frame - bridgeDestroySmokeDelay );
                    fheroes::Blit( sprite, _mainSurface, pt2.x + bridgeDestroySmokeOffset.x + sprite.x(), pt2.y + bridgeDestroySmokeOffset.y + sprite.y() );
                }

                const fheroes::Sprite & spriteCloud = fheroes::AGG::GetICN( icn, frame );
                fheroes::Blit( spriteCloud, _mainSurface, pt2.x + spriteCloud.x(), pt2.y + spriteCloud.y() );
            }
...
void Battle::Interface::redrawActionEarthquakeSpellPart2( const std::vector<CastleDefenseStructure> & targets )
{
    ...
    LocalEvent & le = LocalEvent::Get();
    const int icn = ICN::LICHCLOD;
    uint32_t maxFrame = fheroes::AGG::GetICNCount( icn );
        ...
        if ( isNeedSpellAnimationUpdate || Game::validateAnimationDelay( Game::BATTLE_OPPONENTS_DELAY ) ) {
            ...
            for ( const CastleDefenseStructure target : targets ) {
                ...
                if ( target == CastleDefenseStructure::BRIDGE ) {
                    // When a bridge is demolished, there is one additional smoke explosion at the point where the bridge falls.
                    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icn, frame - bridgeDestroySmokeDelay + 1 );
                    fheroes::Blit( sprite, _mainSurface, pt2.x + bridgeDestroySmokeOffset.x + sprite.x(), pt2.y + bridgeDestroySmokeOffset.y + sprite.y() );
                }
...
void Battle::Interface::RedrawTargetsWithFrameAnimation( const int32_t dst, const TargetsInfo & targets, const int icn, const int m82, int repeatCount /* = 0 */ )
{
    ...
    uint32_t frameCount = fheroes::AGG::GetICNCount( icn );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && frame < frameCount ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            ...
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icn, frame );
            fheroes::Blit( sprite, _mainSurface, center.x + center.width / 2 + sprite.x(), center.y + center.height / 2 + sprite.y() );
            RedrawPartialFinish();
...
void Battle::Interface::RedrawTargetsWithFrameAnimation( const TargetsInfo & targets, const int icn, const int m82, const bool wnce )
{
    ...
    const uint32_t maxFrame = fheroes::AGG::GetICNCount( icn );
    uint32_t frame = 0;
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && ( frame < maxFrame || isDefenderAnimating ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            if ( frame < maxFrame ) {
                ...
                const fheroes::Sprite & spellSprite = fheroes::AGG::GetICN( icn, frame );
...
void Battle::Interface::RedrawTroopWithFrameAnimation( Unit & unit, const int icn, const int m82, const CreatureSpellAnimation animation )
{
    ...
    const uint32_t maxICNFrame = fheroes::AGG::GetICNCount( icn );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::BATTLE_SPELL_DELAY } ) ) && ( frame < maxICNFrame || unit.GetAnimationState() == Monster_Info::WNCE_DOWN ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::BATTLE_SPELL_DELAY ) ) {
            if ( frame < maxICNFrame ) {
                _unitSpellEffectInfos.back().position = CalculateSpellPosition( unit, icn, fheroes::AGG::GetICN( icn, frame ) );
...
void Battle::Interface::InterruptAutoCombatIfRequested( LocalEvent & le )
{
    ...
    const int interrupt = fheroes::showStandardTextMessage( {}, _( "Are you sure you want to interrupt the auto combat mode?" ), Dialog::YES | Dialog::NO );
...
void Battle::Interface::ProcessingHeroDialogResult( const int result, Actions & actions )
{
        ...
        if ( hero ) {
            if ( hero->HaveSpellBook() ) {
                ...
                if ( arena.isDisableCastSpell( Spell::NONE, &msg ) ) {
                    fheroes::showStandardTextMessage( "", msg, Dialog::OK );
                }
                else {
                    ...
                    if ( spell.isValid() ) {
                        ...
                        if ( arena.isDisableCastSpell( spell, &msg ) ) {
                            fheroes::showStandardTextMessage( spell.GetName(), msg, Dialog::OK );
                        }
                        else {
                            std::string error;
                            ...
                            if ( hero->CanCastSpell( spell, &error ) ) {
                                ...
                                fheroes::showStandardTextMessage( _( "Error" ), error, Dialog::OK );
                            }
                        }
                    }
                }
                ...
                fheroes::showStandardTextMessage( "", _( "No spells to cast." ), Dialog::OK );
        }
        break;
    }
    ...
    case 2: {
        if ( arena.CanRetreatOpponent( _currentUnit->GetCurrentOrArmyColor() ) ) {
            if ( Dialog::YES == fheroes::showStandardTextMessage( "", _( "Are you sure you want to retreat?" ), Dialog::YES | Dialog::NO ) ) {
            ...
            fheroes::showStandardTextMessage( "", _( "Retreat disabled" ), Dialog::OK );
        }
        break;
    }
    ...
    case 3: {
        if ( arena.CanSurrenderOpponent( _currentUnit->GetCurrentOrArmyColor() ) ) {
            ...
            fheroes::showStandardTextMessage( "", _( "Surrender disabled" ), Dialog::OK );
        }
        break;
    }
...
void Battle::PopupDamageInfo::setSpellAttackInfo( const HeroBase * hero, const Unit * defender, const Spell & spell )
{
    ...
    const int spellPoints = hero ? hero->GetPower() : fheroes::spellPowerForBuiltinMonsterSpells;
...
void Battle::PopupDamageInfo::_makeDamageImage()
{
    ...
    const fheroes::Text damageText( str, fheroes::FontType::smallWhite() );
    ...
    const fheroes::Text killedText( str, fheroes::FontType::smallWhite() );

    const fheroes::Rect & unitRect = _defender->GetRectPosition();
    ...
    const fheroes::Rect borderRect( isLeftSidePopup ? ( x - w - unitRect.width - borderWidth ) : x, y, w, h );

    const fheroes::Sprite & backgroundImage = fheroes::AGG::GetICN( ICN::CELLWIN, 1 );

    _damageImage = fheroes::Stretch( backgroundImage, 0, 0, backgroundImage.width(), backgroundImage.height(), borderRect.width, borderRect.height );
...
void Battle::PopupDamageInfo::redraw() const
{
    ...
    fheroes::Copy( _damageImage, 0, 0, display, _damageImage.x(), _damageImage.y(), _damageImage.width(), _damageImage.height() );
    fheroes::addGradientShadowForArea( display, { _damageImage.x(), _damageImage.y() }, _damageImage.width(), _damageImage.height(), 5 );
}
