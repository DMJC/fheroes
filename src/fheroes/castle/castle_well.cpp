...
bool Castle::_recruitCastleMax( const Troops & currentCastleArmy )
{
    ...
    const fheroes::FontType normalWhite = fheroes::FontType::normalWhite();
    ...
    if ( monstersRecruitedText.empty() ) {
        ...
        if ( isCreaturePresent ) {
            if ( !canAffordOneCreature ) {
                fheroes::showStandardTextMessage( {}, _( "Not enough resources to recruit creatures." ), Dialog::OK );
            }
            else {
                fheroes::showStandardTextMessage( {}, _( "You are unable to recruit at this time, your ranks are full." ), Dialog::OK );
            }
        }
        else {
            fheroes::showStandardTextMessage( {}, _( "No creatures available for purchase." ), Dialog::OK );
        }
    }
    else if ( fheroes::showResourceMessage( fheroes::Text( _( "Recruit Creatures" ), fheroes::FontType::normalYellow() ),
                                             fheroes::Text( std::move( monstersRecruitedText ), normalWhite ), Dialog::YES | Dialog::NO, totalMonstersCost )
              == Dialog::YES ) {
...
void Castle::_openWell()
{
    ...
    const fheroes::Rect roi = restorer.rect();

    const int32_t buttonOffsetY = roi.y + bottomBarOffsetY;

    // MAX button.
    fheroes::Button buttonMax( roi.x, buttonOffsetY, ICN::BUTTON_WELL_MAX, 0, 1 );

    // EXIT button.
    fheroes::Button buttonExit( roi.x + roi.width - fheroes::AGG::GetICN( ICN::BUTTON_GUILDWELL_EXIT, 0 ).width(), buttonOffsetY, ICN::BUTTON_GUILDWELL_EXIT, 0, 1 );

    const std::array<fheroes::Rect, maxNumOfDwellings> rectMonster
        = { fheroes::Rect( roi.x + 20, roi.y + 18, 288, 124 ),   fheroes::Rect( roi.x + 20, roi.y + 168, 288, 124 ),
            fheroes::Rect( roi.x + 20, roi.y + 318, 288, 124 ),  fheroes::Rect( roi.x + 334, roi.y + 18, 288, 124 ),
            fheroes::Rect( roi.x + 334, roi.y + 168, 288, 124 ), fheroes::Rect( roi.x + 334, roi.y + 318, 288, 124 ) };
    ...
    std::array<fheroes::RandomMonsterAnimation, maxNumOfDwellings> monsterAnimInfo
        = { fheroes::RandomMonsterAnimation( castleMonster[0] ), fheroes::RandomMonsterAnimation( castleMonster[1] ),
            fheroes::RandomMonsterAnimation( castleMonster[2] ), fheroes::RandomMonsterAnimation( castleMonster[3] ),
            fheroes::RandomMonsterAnimation( castleMonster[4] ), fheroes::RandomMonsterAnimation( castleMonster[5] ) };
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::CASTLE_UNIT_DELAY } ) ) ) {
        ...
        if ( Game::validateAnimationDelay( Game::CASTLE_UNIT_DELAY ) ) {
            fheroes::Copy( background, 0, 0, display, roi.x, roi.y, roi.width, roi.height );
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonMax.area() ) ) {
            fheroes::showStandardTextMessage( _( "Max" ), _( "Hire all creatures in the town." ), Dialog::ZERO );
        }
    }
}

void Castle::_wellRedrawAvailableMonsters( const uint32_t dwellingType, const bool restoreBackground, fheroes::Image & background ) const
{
    ...
    if ( restoreBackground ) {
        ...
        const fheroes::Sprite & wellBackground = fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::WELLBKG_EVIL : ICN::WELLBKG, 0 );

        fheroes::Copy( wellBackground, offset.x, offset.y, background, offset.x, offset.y, 137, fheroes::getFontHeight( fheroes::FontSize::NORMAL ) );
    }

    std::string textString = _( "Available" );
    textString += ": ";

    fheroes::Text text( std::move( textString ), fheroes::FontType::smallWhite() );
    text.draw( offset.x + 24, offset.y + 2, background );

    text.set( std::to_string( population ), fheroes::FontType::normalYellow() );
    text.draw( offset.x + 109 - text.width() / 2, offset.y, background );
}

void Castle::_wellRedrawBackground( fheroes::Image & background ) const
{
    const fheroes::Sprite & wellBackground = fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::WELLBKG_EVIL : ICN::WELLBKG, 0 );
    const int32_t backgroundWidth = wellBackground.width();

    fheroes::Copy( wellBackground, 0, 0, background, 0, 0, backgroundWidth, bottomBarOffsetY );
    ...
    const int32_t buttonMaxWidth = fheroes::AGG::GetICN( ICN::BUTTON_WELL_MAX, 0 ).width();
    // ICN::SMALLBAR image's first column contains all black pixels. This should not be drawn.
    fheroes::Copy( bottomBar, 1, 0, background, buttonMaxWidth, bottomBarOffsetY, backgroundWidth / 2 - buttonMaxWidth, barHeight );
    fheroes::Copy( bottomBar, bottomBar.width() - backgroundWidth / 2 + exitWidth - 1, 0, background, backgroundWidth / 2, bottomBarOffsetY,
    ...
    fheroes::Fill( background, background.width() - 1, bottomBarOffsetY, 1, bottomBar.height(), static_cast<uint8_t>( 0 ) );

    const fheroes::Rect textRoi{ 0, bottomBarOffsetY, backgroundWidth, bottomBar.height() - 1 };
    fheroes::Text text( _( "Town Population Information and Statistics" ), fheroes::FontType::normalWhite() );
    text.drawInRoi( 315 - text.width() / 2, bottomBarOffsetY + 3, background, textRoi );

    const fheroes::FontType statsFontType = fheroes::FontType::smallWhite();

    for ( const uint32_t dwellingType : castleDwellings ) {
        // By default the 'icnIndex' and 'offset' values are set for DWELLING_MONSTER1.
        uint32_t icnIndex = 19;
        fheroes::Point offset{ 0, 1 };
        ...
        fheroes::Point renderPoint( offset.x + 21, offset.y + 35 );
        const fheroes::Sprite & dwellingImage = fheroes::AGG::GetICN( ICN::getBuildingIcnId( _race ), icnIndex );
        fheroes::Copy( dwellingImage, 0, 0, background, renderPoint.x, renderPoint.y, dwellingImage.width(), dwellingImage.height() );
...
void Castle::_wellRedrawMonsterAnimation( const fheroes::Rect & roi, std::array<fheroes::RandomMonsterAnimation, maxNumOfDwellings> & monsterAnimInfo ) const
{
    fheroes::Display & display = fheroes::Display::instance();

    for ( size_t monsterId = 0; monsterId < maxNumOfDwellings; ++monsterId ) {
        fheroes::Point outPos( roi.x, roi.y + 1 );
        ...
        const fheroes::Sprite & smonster = fheroes::AGG::GetICN( monsterAnimInfo[monsterId].icnFile(), monsterAnimInfo[monsterId].frameId() );
        ...
        fheroes::Size inSize( smonster.width(), smonster.height() );

        if ( fheroes::FitToRoi( smonster, inPos, display, outPos, inSize, roi ) ) {
            fheroes::Blit( smonster, inPos, display, outPos, inSize, flipMonsterSprite );
        }
