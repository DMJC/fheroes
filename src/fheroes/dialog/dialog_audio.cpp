...
namespace
{
    void drawDialog( const std::vector<fheroes::Rect> & rects )
    {
        assert( rects.size() == 4 );
        ...
        const fheroes::Sprite & musicVolumeIcon = fheroes::AGG::GetICN( ICN::SPANEL, isMusicOn ? 1 : 0 );
        std::string value;
        if ( isMusicOn ) {
        ...
        fheroes::drawOption( rects[0], musicVolumeIcon, _( "Music" ), value, fheroes::UiOptionTextWidth::TWO_ELEMENTS_ROW );

        // Sound volume.
        const bool isAudioOn = ( Audio::isValid() && conf.SoundVolume() > 0 );
        const fheroes::Sprite & soundVolumeOption = fheroes::AGG::GetICN( ICN::SPANEL, isAudioOn ? 3 : 2 );
        if ( isAudioOn ) {
        ...
        fheroes::drawOption( rects[1], soundVolumeOption, _( "Effects" ), value, fheroes::UiOptionTextWidth::TWO_ELEMENTS_ROW );

        // Music Type.
        const MusicSource musicType = conf.MusicType();
        const fheroes::Sprite & musicTypeIcon = fheroes::AGG::GetICN( ICN::SPANEL, musicType == MUSIC_EXTERNAL ? 11 : 10 );
        ...
        fheroes::drawOption( rects[2], musicTypeIcon, _( "Music Type" ), value, fheroes::UiOptionTextWidth::TWO_ELEMENTS_ROW );

        // 3D Audio.
        const bool is3DAudioEnabled = conf.is3DAudioEnabled();
        const fheroes::Sprite & interfaceStateIcon = is3DAudioEnabled ? fheroes::AGG::GetICN( ICN::SPANEL, 11 ) : fheroes::AGG::GetICN( ICN::SPANEL, 10 );
        if ( is3DAudioEnabled ) {
            value = _( "On" );
        }
        else {
        ...
        fheroes::drawOption( rects[3], interfaceStateIcon, _( "3D Audio" ), value, fheroes::UiOptionTextWidth::TWO_ELEMENTS_ROW );
    }
}
...
namespace Dialog
{
    bool openAudioSettingsDialog( const bool fromAdventureMap )
    {
        ...
        const fheroes::Sprite & optionSprite = fheroes::AGG::GetICN( ICN::SPANEL, 0 );
        const fheroes::Point optionOffset( windowRoi.x + 53, windowRoi.y + 31 );
        const fheroes::Point optionStep( 118, 110 );

        std::vector<fheroes::Rect> roi;
        roi.reserve( 4 );
        ...
        const fheroes::Rect & musicVolumeRoi = roi[0];
        const fheroes::Rect & soundVolumeRoi = roi[1];
        const fheroes::Rect & musicTypeRoi = roi[2];
        const fheroes::Rect & audio3D = roi[3];
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( musicVolumeRoi ) ) {
                fheroes::showStandardTextMessage( _( "Music" ), _( "Toggle ambient music level." ), 0 );
            }

            else if ( le.isMouseRightButtonPressedInArea( soundVolumeRoi ) ) {
                fheroes::showStandardTextMessage( _( "Effects" ), _( "Toggle foreground sounds level." ), 0 );
            }
            else if ( le.isMouseRightButtonPressedInArea( musicTypeRoi ) ) {
                fheroes::showStandardTextMessage( _( "Music Type" ), _( "Change the type of music." ), 0 );
            }
            else if ( le.isMouseRightButtonPressedInArea( audio3D ) ) {
                fheroes::showStandardTextMessage( _( "3D Audio" ), _( "Toggle the 3D effect of foreground sounds." ), 0 );
            }
