...
namespace
{
    ...
    void AITownPortal( Heroes & hero, const int32_t targetIndex )
    {
        ...
        const Spell spellToUse = [&hero, targetIndex]() {
            const Castle * nearestCastle = fheroes::getNearestCastleTownGate( hero );
    ...
    void AIBattleLose( Heroes & hero, const Battle::Result & res, bool attacker, const fheroes::Point * centerOn = nullptr, const bool playSound = false )
    {
    ...
    void AIToCoast( Heroes & hero, const int32_t dst_index )
    {
        ...
        const fheroes::Point prevPos = hero.GetCenter();
...
fheroes::GameMode AI::HeroesMove( Heroes & hero )
{
    const Route::Path & path = hero.GetPath();
    ...
    while ( le.HandleEvents( !hideAIMovements && Game::isDelayNeeded( delayTypes ) ) ) {
        ...
        if ( hideAIMovements || !AIIsShowAnimationForHero( hero, colors ) ) {
            ...
            if ( hero.isAction() ) {
                ...
                const fheroes::GameMode gameState = GameOver::Result::Get().checkGameOver();
                if ( gameState != fheroes::GameMode::CANCEL ) {
                    return gameState;
                }
            }
            ...
            if ( heroAnimationFrameCount == 0 ) {
                ...
                if ( hero.Move( noMovementAnimation ) ) {
                    ...
                    const fheroes::Point movement( hero.MovementDirection() );
                    if ( movement != fheroes::Point() ) { // don't waste resources for no movement
                        const int32_t heroMovementSkipValue = Game::AIHeroAnimSpeedMultiplier();
