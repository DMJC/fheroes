...
namespace
{
    fheroes::Sprite getObjectImage( const Maps::ObjectGroup group, const int32_t type )
    {
        if ( type == -1 ) {
            return {};
        }
        ...
        return fheroes::generateMapObjectImage( objectInfo[type] );
    }
    ...
    void setCustomCursor( const Maps::ObjectGroup group, const int32_t type )
    {
        ...
        const fheroes::Sprite & image = getObjectImage( group, type );

        Cursor::Get().setCustomImage( image, { image.x(), image.y() } );
    }

    fheroes::Rect getObjectOccupiedArea( const Maps::ObjectGroup group, const int32_t objectType )
    {
        ...
        fheroes::Point minPos{ offsets.front() };
        fheroes::Point maxPos{ offsets.front() };
    ...
    fheroes::Image makeInstrumentPanelBackground( const int32_t width, const int32_t height )
    {
        fheroes::Image background;
        background._disableTransformLayer();
        background.resize( width, height );
        fheroes::StandardWindow::renderBackgroundImage( background, { 0, 0, width, height }, 0, Settings::Get().isEvilInterfaceEnabled() );

        // Make background borders: it consists of rectangles with different transform shading.
        auto applyRectTransform = [width, height]( fheroes::Image & output, const int32_t offset, const uint8_t transformId ) {
    ...
    void drawInstrumentName( fheroes::Image & output, const fheroes::Point & pos, std::string text )
    {
        const fheroes::Sprite & originalPanel = fheroes::AGG::GetICN( ICN::EDITPANL, 0 );
        ...
        if ( Settings::Get().isEvilInterfaceEnabled() ) {
            fheroes::ApplyPalette( originalPanel, nameBackgroundOffsetX, nameBackgroundOffsetY, output, pos.x, pos.y, nameBackgroundWidth, nameBackgroundHeight,
                                    PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );
        }
        else {
            fheroes::Copy( originalPanel, nameBackgroundOffsetX, nameBackgroundOffsetY, output, pos.x, pos.y, nameBackgroundWidth, nameBackgroundHeight );
        }
        const fheroes::Text terrainText( std::move( text ), fheroes::FontType::smallWhite() );
        terrainText.draw( pos.x + ( nameBackgroundWidth - terrainText.width() ) / 2, pos.y + 3, output );
    }

    void drawObjectTypeSelectionRect( fheroes::Image & output, const fheroes::Point & pos )
    {
        const fheroes::Sprite & selection = fheroes::AGG::GetICN( ICN::TERRAINS, 9 );
        fheroes::Blit( selection, 0, 0, output, pos.x - 2, pos.y - 2, selection.width(), selection.height() );
    }
    ...
    void showObjectTypeInfoText( std::string objectName )
    {
        ...
        fheroes::showStandardTextMessage( std::move( objectName ), std::move( text ), Dialog::ZERO );
    }

    template <size_t TSize>
    void updateObjectTypeSelection( const int8_t objectId, const std::array<fheroes::Rect, TSize> & buttonAreas,
                                    const std::function<const char *( uint8_t )> & getObjectTypeName, const fheroes::Point & ObjectTypeNamePosition,
...
namespace Interface
{
    ...
    fheroes::Rect EditorPanel::getBrushArea() const
    {
    ...
    void EditorPanel::setPos( const int32_t displayX, int32_t displayY )
    {
        ...
        const int32_t instrumentPanelWidth = display.width() - displayX - fheroes::borderWidthPx;
        const int32_t bottomBorderOffset = ( display.height() > fheroes::Display::DEFAULT_HEIGHT + fheroes::borderWidthPx ) ? fheroes::borderWidthPx : 0;
        const int32_t instrumentPanelHeight = display.height() - displayY - fheroes::AGG::GetICN( ICN::EDITBTNS, 0 ).height() * 5 - bottomBorderOffset;
        ...
        icnIndex = 24;
        for ( fheroes::Button & button : _brushSizeButtons ) {
    ...
    {
        for ( const fheroes::Button & button : _instrumentButtons ) {
            button.draw();
        }

        fheroes::Display & display = fheroes::Display::instance();

        fheroes::Copy( _instrumentPanelBackground, 0, 0, display, _rectInstrumentPanel.x, _rectInstrumentPanel.y, _rectInstrumentPanel.width,
                        _rectInstrumentPanel.height );

        if ( _selectedInstrument == Instrument::TERRAIN || _selectedInstrument == Instrument::ERASE ) {
            for ( const fheroes::Button & button : _brushSizeButtons ) {
                button.draw();
            }
        }
        ...
        if ( _selectedInstrument == Instrument::TERRAIN ) {
            ...
            for ( size_t i = 0; i < TerrainBrush::TERRAIN_COUNT; ++i ) {
                ...
                fheroes::Copy( originalPanel, originalOffsetX, originalOffsetY, display, _terrainButtonsRect[i] );
            }
            ...
            for ( uint32_t i = 0; i < LandscapeObjectBrush::LANDSCAPE_COUNT; ++i ) {
                fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, i + 6 ), 0, 0, display, _landscapeObjectButtonsRect[i] );
            }
            ...
            const fheroes::Sprite & originalPanel = fheroes::AGG::GetICN( ICN::EDITPANL, 1 );
            ...
            fheroes::Copy( originalPanel, originalArtifactsImageOffsetX, originalImagesOffsetY, display, _adventureObjectButtonsRect[AdventureObjectBrush::ARTIFACTS] );
            for ( uint32_t i = AdventureObjectBrush::DWELLINGS; i < AdventureObjectBrush::TREASURES; ++i ) {
                fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, i + 10 ), 0, 0, display, _adventureObjectButtonsRect[i] );
            }
            fheroes::Copy( originalPanel, originalTreasuresImageOffsetX, originalImagesOffsetY, display, _adventureObjectButtonsRect[AdventureObjectBrush::TREASURES] );
            for ( uint32_t i = AdventureObjectBrush::WATER_ADVENTURE; i < AdventureObjectBrush::ADVENTURE_COUNT; ++i ) {
                fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, i + 9 ), 0, 0, display, _adventureObjectButtonsRect[i] );
            }
            ...
            fheroes::Copy( originalPanel, originalHeroesImageOffsetX, originalImagesOffsetY, display, _kingdomObjectButtonsRect[KingdomObjectBrush::HEROES] );
            fheroes::Copy( originalPanel, originalTownsImageOffsetX, originalImagesOffsetY, display, _kingdomObjectButtonsRect[KingdomObjectBrush::TOWNS] );
            ...
            const fheroes::Text instrumentName( getObjectGroupName( Maps::ObjectGroup::MONSTERS ), fheroes::FontType::normalWhite() );
            ...
            if ( _selectedMonsterType < 0 ) {
                // Show a tip.
                const fheroes::Text text( _( "Click here to\nselect a monster." ), fheroes::FontType::smallWhite() );
                ...
                const fheroes::Sprite & image = getObjectImage( Maps::ObjectGroup::MONSTERS, _selectedMonsterType );
                fheroes::Blit( image, display, _rectInstrumentPanel.x + ( _rectInstrumentPanel.width - image.width() ) / 2,
                                _rectInstrumentPanel.y + 67 - image.height() );

                fheroes::Text text( Monster( _selectedMonsterType + 1 ).GetName(), fheroes::FontType::smallWhite() );
                text.draw( _rectInstrumentPanel.x + 5, _rectInstrumentPanel.y + 70, _rectInstrumentPanel.width - 10, display );
                text.set( _( "Click here to\nselect another monster." ), fheroes::FontType::smallWhite() );
            ...
            const fheroes::Text instrumentName( _( "Cell\nDetails" ), fheroes::FontType::normalWhite() );
            ...
            const fheroes::Text instrumentName( getObjectGroupName( Maps::ObjectGroup::ROADS ), fheroes::FontType::normalWhite() );
            instrumentName.draw( _rectInstrumentPanel.x + ( _rectInstrumentPanel.width - instrumentName.width() ) / 2, _rectInstrumentPanel.y + 8, display );
        }
        else if ( _selectedInstrument == Instrument::STREAM ) {
            const fheroes::Text instrumentName( getObjectGroupName( Maps::ObjectGroup::STREAMS ), fheroes::FontType::normalWhite() );
            instrumentName.draw( _rectInstrumentPanel.x + ( _rectInstrumentPanel.width - instrumentName.width() ) / 2, _rectInstrumentPanel.y + 8, display );
        }
        else if ( _selectedInstrument == Instrument::ERASE ) {
            const fheroes::Text instrumentName( _( "Erase" ), fheroes::FontType::normalWhite() );
            instrumentName.draw( _rectInstrumentPanel.x + ( _rectInstrumentPanel.width - instrumentName.width() ) / 2, _rectInstrumentPanel.y + 8, display );
            ...
            fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, 8 ), 0, 0, display, _eraseButtonsRect[0] );
            // Adventure non pickable objects icon.
            fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, 13 ), 0, 0, display, _eraseButtonsRect[1] );
            // Castle objects icon.
            fheroes::Copy( originalPanel, originalTownsImageOffsetX, originalTownsMonstersHeroesOffsetY, display, _eraseButtonsRect[2] );
            // Adventure pickable objects icon.
            fheroes::Copy( originalPanel, originalHeroesTreasuresImageOffsetX, originalArtifactsTreasresOffsetY, display, _eraseButtonsRect[3] );
            // Monster objects icon.
            fheroes::Copy( originalPanel, originalMonstersImageOffsetX, originalTownsMonstersHeroesOffsetY, display, _eraseButtonsRect[4] );
            // Hero objects icon.
            fheroes::Copy( originalPanel, originalHeroesTreasuresImageOffsetX, originalTownsMonstersHeroesOffsetY, display, _eraseButtonsRect[5] );
            // Road objects icon.
            fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, 16 ), 0, 0, display, _eraseButtonsRect[6] );
            // Stream objects icon.
            fheroes::Copy( fheroes::AGG::GetICN( ICN::EDITPANL, 17 ), 0, 0, display, _eraseButtonsRect[7] );

            // Object type to erase selection marks.
            const fheroes::Sprite & selectionMark = fheroes::AGG::GetICN( ICN::TOWNWIND, 11 );
            for ( size_t i = 0; i < _eraseButtonsRect.size(); ++i ) {
                if ( _eraseButtonObjectTypes[i] & _eraseTypes ) {
                    fheroes::Blit( selectionMark, 0, 0, display, _eraseButtonsRect[i].x + 10, _eraseButtonsRect[i].y + 11, selectionMark.width(),
        ...
        case Instrument::KINGDOM_OBJECTS:
            ...
            case KingdomObjectBrush::HEROES:
                _interface.setCursorUpdater( [type = getSelectedObjectType()]( const int32_t /*tileIndex*/ ) {
                    ...
                    const fheroes::Sprite & image = fheroes::AGG::GetICN( ICN::MINIHERO, type );
            ...
            case KingdomObjectBrush::TOWNS:
                _interface.setCursorUpdater( [this]( const int32_t tileIndex ) {
                    ...
                    const fheroes::Sprite & image = fheroes::generateTownObjectImage( type, color, world.getTile( tileIndex ).GetGround() );
    ...
    {
        auto updateButtonState = []( fheroes::ButtonBase & button, const bool isAvailable ) {
            if ( isAvailable ) {
    ...
    fheroes::GameMode EditorPanel::queueEventProcessing()
    {
        LocalEvent & le = LocalEvent::Get();
        fheroes::GameMode res = fheroes::GameMode::CANCEL;
        ...
        if ( _selectedInstrument == Instrument::TERRAIN || _selectedInstrument == Instrument::ERASE ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( _brushSizeButtonsRect[BrushSize::SMALL] ) ) {
                fheroes::showStandardTextMessage( _( "Small Brush" ), brushSizeText( 1, _selectedInstrument == Instrument::TERRAIN ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _brushSizeButtonsRect[BrushSize::MEDIUM] ) ) {
                fheroes::showStandardTextMessage( _( "Medium Brush" ), brushSizeText( 2, _selectedInstrument == Instrument::TERRAIN ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _brushSizeButtonsRect[BrushSize::LARGE] ) ) {
                fheroes::showStandardTextMessage( _( "Large Brush" ), brushSizeText( 4, _selectedInstrument == Instrument::TERRAIN ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _brushSizeButtonsRect[BrushSize::AREA] ) ) {
                if ( _selectedInstrument == Instrument::TERRAIN ) {
                    fheroes::showStandardTextMessage( _( "Area Fill" ), _( "Used to click and drag for filling in large areas." ), Dialog::ZERO );
                }
                else {
                    fheroes::showStandardTextMessage( _( "Clear Area" ), _( "Used to click and drag for clearing large areas." ), Dialog::ZERO );
                }
            }
        }
        ...
        if ( _selectedInstrument == Instrument::TERRAIN ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::WATER] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::WATER ), _( "Traversable only by boat." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::GRASS] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::GRASS ), _( "No special modifiers." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::SNOW] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::SNOW ), movePenaltyText( "1.5" ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::SWAMP] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::SWAMP ), movePenaltyText( "1.75" ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::LAVA] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::LAVA ), _( "No special modifiers." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::DESERT] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::DESERT ), movePenaltyText( "2" ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::DIRT] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::DIRT ), _( "No special modifiers." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::WASTELAND] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::WASTELAND ), movePenaltyText( "1.25" ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( _terrainButtonsRect[TerrainBrush::BEACH] ) ) {
                fheroes::showStandardTextMessage( _getTerrainTypeName( TerrainBrush::BEACH ), movePenaltyText( "1.25" ), Dialog::ZERO );
            }
        }
        else if ( _selectedInstrument == Instrument::LANDSCAPE_OBJECTS ) {
            for ( size_t i = 0; i < _landscapeObjectButtonsRect.size(); ++i ) {
        ...
        if ( le.MouseClickLeft( _rectMagnify ) ) {
            ...
            fheroes::showStandardTextMessage( _( "Terrain Mode" ), _( "Used to draw the underlying grass, dirt, water, etc. on the map." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::LANDSCAPE_OBJECTS] ) ) {
            fheroes::showStandardTextMessage( _( "Landscape Objects Mode" ), _( "Used to place landscape objects (mountains, rocks, trees, etc.) on the map." ),
                                               Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::DETAIL] ) ) {
            fheroes::showStandardTextMessage( _( "Detail Mode" ), _( "Used for special editing of action objects." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::ADVENTURE_OBJECTS] ) ) {
            fheroes::showStandardTextMessage( _( "Adventure Objects Mode" ),
            ...
            fheroes::showStandardTextMessage( _( "Kingdom Objects Mode" ), _( "Used to place kingdom objects (towns, castles and heroes) on the map." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::MONSTERS] ) ) {
            fheroes::showStandardTextMessage( _( "Monsters Mode" ), _( "Used to place monsters on the map." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::STREAM] ) ) {
            fheroes::showStandardTextMessage( _( "Stream Mode" ), _( "Allows you to draw streams by clicking and dragging." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::ROAD] ) ) {
            fheroes::showStandardTextMessage( _( "Road Mode" ), _( "Allows you to draw roads by clicking and dragging." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _instrumentButtonsRect[Instrument::ERASE] ) ) {
            fheroes::showStandardTextMessage( _( "Erase Mode" ), _( "Used to erase objects from the map." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _rectMagnify ) ) {
            fheroes::showStandardTextMessage( _( "Magnify" ), _( "Change between zoom and normal view." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _rectUndo ) ) {
            fheroes::showStandardTextMessage( _( "Undo" ), _( "Undo your last action." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _rectRedo ) ) {
            fheroes::showStandardTextMessage( _( "Redo" ), _( "Redo the last undone action." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _rectSpecs ) ) {
            fheroes::showStandardTextMessage( _( "Specifications" ), _( "Edit map title, description, and other general information." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _rectFile ) ) {
            fheroes::showStandardTextMessage( _( "File Options" ), _( "Open the file options menu, where you can save or load maps, or quit out of the editor." ),
                                               Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( _rectSystem ) ) {
            fheroes::showStandardTextMessage( _( "System Options" ), _( "View the editor system options, which let you customize the editor." ), Dialog::ZERO );
        }
