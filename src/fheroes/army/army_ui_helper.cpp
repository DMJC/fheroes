...
void fheroes::drawMiniMonsters( const Troops & troops, int32_t cx, const int32_t cy, const int32_t width, uint32_t first, uint32_t count, const bool isCompact,
                                 const bool isDetailedView, const bool isGarrisonView, const uint32_t thievesGuildsCount, Image & output )
{
    ...
    for ( size_t slot = 0; slot < slots; ++slot ) {
        ...
        const fheroes::Sprite & monster = fheroes::AGG::GetICN( ICN::MONS32, troop->GetSpriteIndex() );
        const fheroes::Text text( std::move( monstersCountRepresentation ), fheroes::FontType::smallWhite() );
        ...
        if ( isCompact ) {
            ...
            fheroes::Blit( monster, output, posX + offset, cy + offsetY + monster.y() );
            text.draw( posX - text.width() - offset + static_cast<int32_t>( chunk ), cy + 23, output );
        }
        else {
            const int32_t offsetY = 28 - monster.height() + monster.y();
            fheroes::Blit( monster, output, posX - monster.width() / 2 + monster.x() + 2, cy + offsetY );
