...
void Game::Init()
{
    ...
    HotKeysLoad( Settings::GetLastFile( "", "fheroes.key" ) );
}
...
void Game::EnvironmentSoundMixer()
{
    ...
    fheroes::Point center;
    fheroes::Point tilePixelOffset;
    ...
    if ( tilePixelOffset != fheroes::Point() ) {
        ++scanningOffset;
    }

    std::vector<fheroes::Point> positions;
    ...
    std::stable_sort( positions.begin(), positions.end(),
                      []( const fheroes::Point & p1, const fheroes::Point & p2 ) { return p1.x * p1.x + p1.y * p1.y < p2.x * p2.x + p2.y * p2.y; } );

    const double maxDistance = std::sqrt( ( maxOffset * maxOffset + maxOffset * maxOffset ) * fheroes::tileWidthPx * fheroes::tileWidthPx );

    const bool is3DAudioEnabled = Settings::Get().is3DAudioEnabled();

    for ( const fheroes::Point & pos : positions ) {
        ...
        fheroes::Point actualPosition = pos;
        actualPosition.x *= fheroes::tileWidthPx;
        actualPosition.y *= fheroes::tileWidthPx;
...
{
    if ( isDetailedView ) {
        return ( abbreviateNumber ? fheroes::abbreviateNumber( count ) : std::to_string( count ) );
    }
