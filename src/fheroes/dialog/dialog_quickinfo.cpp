...
namespace
{
    ...
    {
    public:
        RadarUpdater( const bool performUpdate, const fheroes::Point & updatedPosition, const fheroes::Rect & areaToRestore )
            ...
            , _restorer( fheroes::Display::instance(), areaToRestore.x, areaToRestore.y, areaToRestore.width, areaToRestore.height )
        {
    ...
    private:
        ...
        const fheroes::Point _updatedPosition;
        const fheroes::Point _prevPosition;
    ...
    std::string showBarrierInfo( const Maps::Tile & tile, const Kingdom & kingdom )
    {
        ...
        StringReplace( str, "%{color}", fheroes::getBarrierColorName( barrierColor ) );
    ...
    std::string showTentInfo( const Maps::Tile & tile, const Kingdom & kingdom )
    {
        ...
        StringReplace( str, "%{color}", fheroes::getTentColorName( tentColor ) );
    ...
    fheroes::Rect makeRectQuickInfo( const LocalEvent & le, const fheroes::Sprite & imageBox, const fheroes::Point & position = fheroes::Point() )
    {
    ...
    void showQuickInfo( const Castle & castle, const fheroes::Point & position, const bool showOnRadar, const fheroes::Rect & areaToRestore )
    {
        ...
        // image box
        const fheroes::Sprite & box = fheroes::AGG::GetICN( ICN::QWIKTOWN, 0 );

        LocalEvent & le = LocalEvent::Get();
        fheroes::Rect cur_rt = makeRectQuickInfo( le, box, position );

        fheroes::Display & display = fheroes::Display::instance();
        fheroes::ImageRestorer back( display, cur_rt.x, cur_rt.y, cur_rt.width, cur_rt.height );
        fheroes::Blit( box, display, cur_rt.x, cur_rt.y );

        cur_rt = fheroes::Rect( cur_rt.x + 22, cur_rt.y + 9, 192, 154 );
        fheroes::Point dst_pt;

        // castle name
        fheroes::Text text( castle.GetName(), fheroes::FontType::smallWhite() );
        ...
        const fheroes::Sprite & castleIcon = fheroes::AGG::GetICN( conf.isEvilInterfaceEnabled() ? ICN::LOCATORE : ICN::LOCATORS, 23 );

        dst_pt.x = cur_rt.x + ( cur_rt.width - castleIcon.width() ) / 2;
        dst_pt.y += 10;
        fheroes::Blit( castleIcon, display, dst_pt.x, dst_pt.y );
        fheroes::drawCastleIcon( castle, display, fheroes::Point( dst_pt.x + 4, dst_pt.y + 4 ) );

        // color flags
        const uint32_t flagIcnIndex = fheroes::getCastleLeftFlagIcnIndex( castle.GetColor() );

        const fheroes::Point flagOffset( 5, 4 );

        const fheroes::Sprite & l_flag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex );
        fheroes::Blit( l_flag, display, dst_pt.x - flagOffset.x - l_flag.width(), dst_pt.y + flagOffset.y );

        const fheroes::Sprite & r_flag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex + 1 );
        fheroes::Blit( r_flag, display, dst_pt.x + flagOffset.x + castleIcon.width(), dst_pt.y + flagOffset.y );
        ...
        text.set( _( "Defenders:" ), fheroes::FontType::smallWhite() );
        dst_pt.x = cur_rt.x + ( cur_rt.width - text.width() ) / 2;
        ...
        if ( isDetailedView || thievesGuildsCount > 0 ) {
            ...
            if ( castleArmy.isValid() ) {
                ...
                text.set( _( "None" ), fheroes::FontType::smallWhite() );
                dst_pt.x = cur_rt.x + ( cur_rt.width - text.width() ) / 2;
                dst_pt.y += 47;
                text.draw( dst_pt.x, dst_pt.y, display );
            }
            ...
            text.set( _( "Unknown" ), fheroes::FontType::smallWhite() );
            dst_pt.x = cur_rt.x + ( cur_rt.width - text.width() ) / 2;
            dst_pt.y += 47;
            text.draw( dst_pt.x, dst_pt.y, display );
        }
    ...
    void showQuickInfo( const HeroBase & hero, const fheroes::Point & position, const bool showOnRadar, const fheroes::Rect & areaToRestore,
                        const std::optional<bool> showFullInfo )
    {
        ...
        // image box
        const fheroes::Sprite & box = fheroes::AGG::GetICN( ICN::QWIKHERO, 0 );

        LocalEvent & le = LocalEvent::Get();
        fheroes::Rect cur_rt = makeRectQuickInfo( le, box, position );

        fheroes::Display & display = fheroes::Display::instance();
        fheroes::ImageRestorer restorer( display, cur_rt.x, cur_rt.y, cur_rt.width, cur_rt.height );
        fheroes::Blit( box, display, cur_rt.x, cur_rt.y );

        cur_rt = fheroes::Rect( restorer.x() + 28, restorer.y() + 10, 146, 144 );
        fheroes::Point dst_pt;
        ...
        // hero's name
        const fheroes::FontType smallWhite = fheroes::FontType::smallWhite();
        fheroes::Text text( hero.GetName(), smallWhite );
        // hero's level
        if ( isFullInfo && isActiveHero ) {
            ...
            text.fitToOneRow( box.width() - boxShadowAndBorder - fheroes::Text{ heroLevel, smallWhite }.width() );
        ...
        const fheroes::Sprite & heroPortraitFrame = fheroes::AGG::GetICN( conf.isEvilInterfaceEnabled() ? ICN::LOCATORE : ICN::LOCATORS, 22 );

        // mini port heroes
        const fheroes::Sprite & port = isActiveHero ? activeHero->GetPortrait( PORT_SMALL ) : activeCaptain->GetPortrait( PORT_SMALL );
        if ( !port.empty() ) {
            fheroes::Blit( heroPortraitFrame, display, cur_rt.x + ( cur_rt.width - heroPortraitFrame.width() ) / 2, cur_rt.y + 12 );
            fheroes::Blit( port, display, cur_rt.x + ( cur_rt.width - port.width() ) / 2, cur_rt.y + 16 );
        }

        // luck
        if ( isFullInfo ) {
            const int32_t luck = hero.GetLuck();
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::MINILKMR, ( 0 > luck ? 0 : ( 0 < luck ? 1 : 2 ) ) );
            uint32_t count = ( 0 == luck ? 1 : std::abs( luck ) );
            dst_pt.x = cur_rt.x + 120;
            dst_pt.y = cur_rt.y + ( count == 1 ? 20 : 13 );

            while ( count-- ) {
                fheroes::Blit( sprite, display, dst_pt.x, dst_pt.y );
        ...
        if ( isFullInfo ) {
            ...
            fheroes::Sprite sprite = fheroes::AGG::GetICN( ICN::MINILKMR, spriteInx );
            if ( hero.GetArmy().AllTroopsAreUndead() ) {
                fheroes::ApplyPalette( sprite, PAL::GetPalette( PAL::PaletteType::GRAY ) );
                fheroes::ApplyPalette( sprite, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
            }
        ...
        if ( !isNeutralHero ) {
            ...
            const uint32_t flagIcnIndex = fheroes::getCastleLeftFlagIcnIndex( hero.GetColor() );

            const fheroes::Sprite & l_flag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex );
            dst_pt.x = cur_rt.x + ( cur_rt.width - 40 ) / 2 - l_flag.width();
            fheroes::Blit( l_flag, display, dst_pt.x, dst_pt.y );

            const fheroes::Sprite & r_flag = fheroes::AGG::GetICN( ICN::FLAG32, flagIcnIndex + 1 );
            dst_pt.x = cur_rt.x + ( cur_rt.width + 40 ) / 2;
            fheroes::Blit( r_flag, display, dst_pt.x, dst_pt.y );
        }
...
void Dialog::QuickInfoWithIndicationOnRadar( const Castle & castle, const fheroes::Rect & areaToRestore )
{
    showQuickInfo( castle, {}, true, areaToRestore );
}

void Dialog::QuickInfoWithIndicationOnRadar( const HeroBase & hero, const fheroes::Rect & areaToRestore )
{
    showQuickInfo( hero, {}, true, areaToRestore, {} );
}

void Dialog::QuickInfoAtPosition( const Castle & castle, const fheroes::Point & position )
{
    showQuickInfo( castle, position, true, {} );
}

void Dialog::QuickInfoAtPosition( const HeroBase & hero, const fheroes::Point & position )
{
