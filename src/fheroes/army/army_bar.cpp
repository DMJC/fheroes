...
ArmyBar::ArmyBar( Army * ptr, const bool miniSprites, const bool readOnly, const bool isEditMode /* false */, const bool saveLastTroop /* true */ )
    : spcursor( fheroes::AGG::GetICN( ICN::STRIP, 1 ) )
...
{
    if ( use_mini_sprite )
        SetBackground( { 43, 43 }, fheroes::GetColorId( 0, 45, 0 ) );
    else {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::STRIP, 2 );
...
void ArmyBar::SetBackground( const fheroes::Size & sz, const uint8_t fillColor )
{
    if ( !use_mini_sprite ) {
        // Nothing to draw.
        return;
...
void ArmyBar::RedrawBackground( const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    if ( use_mini_sprite ) {
        ...
        if ( can_change && !_saveLastTroop && _army->GetOccupiedSlotCount() == 0 ) {
            ...
            fheroes::ApplyPalette( fheroes::AGG::GetICN( ICN::STRIP, 2 ), 0, 0, dstsf, pos.x, pos.y, pos.width, pos.height,
                                    PAL::GetPalette( PAL::PaletteType::DARKENING ) );

            const fheroes::Text text( _( "Default\ntroop" ), fheroes::FontType::normalWhite() );
            text.drawInRoi( pos.x, pos.y + pos.height / 2 - 17, pos.width, dstsf, pos );
        }
        else {
            fheroes::Copy( fheroes::AGG::GetICN( ICN::STRIP, 2 ), 0, 0, dstsf, pos );
        }
    }
}

void ArmyBar::RedrawItem( ArmyTroop & troop, const fheroes::Rect & pos, bool selected, fheroes::Image & dstsf )
{
    if ( !troop.isValid() ) {
        // Nothing to draw.
        return;
    ...
    const fheroes::Text text( std::to_string( troop.GetCount() ), use_mini_sprite ? fheroes::FontType::smallWhite() : fheroes::FontType::normalWhite() );
    ...
    if ( use_mini_sprite ) {
        ...
        fheroes::Blit( mons32, srcrt.x, srcrt.y, dstsf, pos.x + ( pos.width - mons32.width() ) / 2, pos.y + pos.height - mons32.height() - 1, srcrt.width,
                        srcrt.height );

        text.draw( pos.x + pos.width - text.width() - 3, pos.y + pos.height - text.height() + 2, dstsf );
    }
    else {
        fheroes::renderMonsterFrame( troop, dstsf, pos.getPosition() );
...
void ArmyBar::Redraw( fheroes::Image & dstsf )
{
    spcursor.hide();
    ...
    if ( isSelected() ) {
        ...
        if ( can_change ) // add troop
        {
            ...
            if ( mons.isValid() ) {
                ...
                int32_t count = 1;
                const fheroes::MonsterDialogElement monsUi{ mons };
