...
namespace
{
    void drawSprite( fheroes::Image & output, const int icnId, const uint32_t index )
    {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icnId, index );
        fheroes::Blit( sprite, 0, 0, output, sprite.x(), sprite.y(), sprite.width(), sprite.height() );
    }

    void renderWindowBackground( fheroes::Image & output, const fheroes::Rect & roi )
    {
        ...
        const fheroes::Sprite & background = fheroes::AGG::GetICN( isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK, 0 );
        ...
        fheroes::Point offset{ roi.x, roi.y };
        ...
        for ( int y = 0; y < stepY; ++y ) {
            ...
            for ( int x = 0; x < stepX; ++x ) {
                fheroes::Copy( background, 0, 0, output, offset.x, offset.y, std::min( background.width(), roi.width - x * background.width() ), height );
    ...
    void fillScreenBorders( fheroes::Image & output, const fheroes::Rect & backgroundRoi )
    {
