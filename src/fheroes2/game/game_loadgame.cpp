...
fheroes2::GameMode Game::LoadGame()
{
    ...
    const fheroes2::ButtonBase & buttonCampaignGame = gameModeButtons.button( 1 );
    const fheroes2::ButtonBase & buttonMultiplayerGame = gameModeButtons.button( 2 );

    const int32_t spaceBetweenButtons = 10;

    fheroes2::StandardWindow background( gameModeButtons, true, buttonStandardGame.area().height + spaceBetweenButtons );
    ...
    while ( le.HandleEvents() ) {
        ...
        else if ( le.MouseClickLeft( buttonCampaignGame.area() ) || HotKeyPressEvent( HotKeyEvent::MAIN_MENU_CAMPAIGN ) ) {
