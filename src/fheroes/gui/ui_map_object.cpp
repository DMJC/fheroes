...
namespace
{
    ...
    void renderObjectPart( fheroes::Sprite & image, const Maps::ObjectPartInfo & objectPart, const fheroes::Point & minOffset )
    {
        const int icnId = MP2::getIcnIdFromObjectIcnType( objectPart.icnType );

        const fheroes::Sprite & imagePart = fheroes::AGG::GetICN( icnId, objectPart.icnIndex );
        ...
        if ( objectPart.animationFrames > 0 ) {
            const fheroes::Sprite & animationFrame = fheroes::AGG::GetICN( icnId, objectPart.icnIndex + 1 );
    ...
    void renderObject( fheroes::Sprite & image, const Maps::ObjectInfo & object, const fheroes::Point & minOffset )
    {
    ...
    void getMinMaxOffsets( const Maps::ObjectInfo & object, fheroes::Point & minOffset, fheroes::Point & maxOffset )
    {
        for ( const auto & objectPart : object.groundLevelParts ) {
...
namespace fheroes
{
    ...
    Sprite generateTownObjectImage( const int townType, const int color, const int groundId )
    {
        ...
        fheroes::Point basementMinOffset;
        fheroes::Point townMinOffset;
        fheroes::Point shadowMinOffset;
        const fheroes::Point leftFlagMinOffset{ 1, 0 };
        const fheroes::Point rightFlagMinOffset{ -1, 0 };
        ...
        fheroes::Point minOffset = basementMinOffset;
        ...
        fheroes::Sprite outputImage( width, height, imageOffset.x, imageOffset.y );
        outputImage.reset();
