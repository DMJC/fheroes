...
namespace
{
    ...
    public:
            ...
            , _race( race )
        {
            ...
            for ( BuildingType upgradedBuildingType = fheroes::getUpgradeForBuilding( _race, _mainBuildingType ); upgradedBuildingType != buildingType;
                  upgradedBuildingType = fheroes::getUpgradeForBuilding( _race, upgradedBuildingType ) ) {
        ...
        bool queueEventProcessing( const bool restrictionMode )
        {
            ...
            if ( le.isMouseRightButtonPressedInArea( _area ) ) {
                ...
                const std::string requirement = fheroes::getBuildingRequirementString( _race, building );
                ...
                fheroes::showStandardTextMessage( Castle::GetStringBuilding( building, _race ), std::move( description ), Dialog::ZERO );
            }
        ...
        {
            return fheroes::getBuildingName( _race, _getBuildindTypeForRender() );
        }

        void redraw( const bool isNotDefault ) const
        {
            fheroes::Display & display = fheroes::Display::instance();
            const int index = fheroes::getIndexBuildingSprite( _getBuildindTypeForRender() );

            const int buildingIcnId = ICN::getBuildingIcnId( _race );
            const fheroes::Sprite & buildingImage = ( buildingIcnId == ICN::UNKNOWN )
                                                         ? fheroes::AGG::GetICN( Settings::Get().isEvilInterfaceEnabled() ? ICN::CASLXTRA_EVIL : ICN::CASLXTRA, 0 )
                                                         : fheroes::AGG::GetICN( buildingIcnId, index );

            const fheroes::Rect buildingImageRoi( _area.x + 1, _area.y + 1, 135, 57 );

            if ( isNotDefault ) {
                fheroes::Copy( buildingImage, 0, 0, display, buildingImageRoi );
            }
            else {
                // Apply the blur effect originally used for the Holy Shout spell.
                fheroes::Copy( fheroes::CreateHolyShoutEffect( buildingImage, 1, 0 ), 0, 0, display, buildingImageRoi );
                fheroes::ApplyPalette( display, buildingImageRoi.x, buildingImageRoi.y, display, buildingImageRoi.x, buildingImageRoi.y, buildingImageRoi.width,
            ...
            for ( int32_t i = 0; i < maxUpgrades; ++i ) {
                if ( _restrictedId != -1 && i >= _restrictedId ) {
                    ...
                    fheroes::Sprite denySign( fheroes::AGG::GetICN( ICN::TOWNWIND, 12 ) );
                    fheroes::ApplyPalette( denySign, PAL::CombinePalettes( PAL::GetPalette( PAL::PaletteType::GRAY ), PAL::GetPalette( PAL::PaletteType::DARKENING ) ) );

                    fheroes::Blit( denySign, display, _area.x + _area.width - ( maxUpgrades - i ) * 20 - denySign.width() / 2, _area.y + 48 - denySign.height() / 2 );
                    continue;
                }

                const int icnId = ( i > _builtId ) ? ICN::CELLWIN : ICN::TOWNWIND;
                const uint32_t icnIndex = ( i > _builtId ) ? 5 : 11;
                const fheroes::Sprite & mark = fheroes::AGG::GetICN( icnId, icnIndex );
                fheroes::Blit( mark, display, _area.x + _area.width - ( maxUpgrades - i ) * 20 - mark.width() / 2, _area.y + 48 - mark.height() / 2 );
            }

            if ( _builtId > -1 ) {
                const fheroes::Sprite & textBackground = fheroes::AGG::GetICN( ICN::BLDGXTRA, 0 );
                fheroes::Copy( textBackground, 0, 58, display, _area.x, _area.y + 58, _area.width, textBackground.height() );
            }
            else if ( _restrictedId != 0 ) {
                const fheroes::Sprite & textBackground = fheroes::AGG::GetICN( ICN::CASLXTRA, 1 );
                fheroes::Copy( textBackground, 0, 0, display, _area.x, _area.y + 58, _area.width, textBackground.height() );
            }

            if ( _restrictedId > -1 ) {
                fheroes::Sprite textBackground( fheroes::AGG::GetICN( ICN::CASLXTRA, 2 ) );
                fheroes::ApplyPalette( textBackground, 6, 1, textBackground, 6, 1, 125, 12,
                                        PAL::CombinePalettes( PAL::GetPalette( PAL::PaletteType::GRAY ), PAL::GetPalette( PAL::PaletteType::DARKENING ) ) );

                if ( _restrictedId == 0 ) {
                    fheroes::Copy( textBackground, 0, 0, display, _area.x, _area.y + 58, _area.width - 5, textBackground.height() );
                }
                else {
                    fheroes::CreateDitheringTransition( textBackground, _buildArea.width - 5, 0, display, _banArea.x - 5, _area.y + 58, 10, textBackground.height(),
                                                         true, false );
                    fheroes::Copy( textBackground, _buildArea.width + 5, 0, display, _banArea.x + 5, _area.y + 58, _banArea.width - 5, textBackground.height() );
                }
            }

            if ( !isNotDefault ) {
                // Make the header darker for the default buildings.
                fheroes::ApplyPalette( display, _area.x + 6, _area.y + 59, display, _area.x + 6, _area.y + 59, 125, 12, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
            }

            const fheroes::Text buildingName( Castle::GetStringBuilding( _getBuildindTypeForRender(), _race ), fheroes::FontType::smallWhite() );
    ...
    private:
        ...
        BuildingType _getBuildingType( const int8_t level ) const
        {
            ...
            for ( int8_t i = 0; i < level; ++i ) {
                building = fheroes::getUpgradeForBuilding( _race, building );
            }
        ...
        fheroes::Rect _area{ 0, 0, 137, 70 };
        fheroes::Rect _buildArea{ 0, 0, 69, 70 };
        fheroes::Rect _banArea{ 0, 0, 68, 70 };
    };
    ...
    bool mageGuildSpellsDialog( std::map<uint8_t, int32_t> & mustHaveSpells, std::vector<int32_t> & bannedSpells, const int race, const fheroes::Rect & dialogRoi )
    {
        const CursorRestorer cursorRestorer( true, Cursor::POINTER );

        fheroes::Display & display = fheroes::Display::instance();

        fheroes::ImageRestorer backgroundRestorer( display, dialogRoi.x, dialogRoi.y, dialogRoi.width, dialogRoi.height );

        const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();
        const fheroes::Sprite & backgroundSprite = fheroes::AGG::GetICN( ( isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK ), 0 );
        const fheroes::Sprite & statusBarSprite = fheroes::AGG::GetICN( ICN::CASLBAR, 0 );
        const int32_t statusBarHeight = statusBarSprite.height();

        fheroes::Copy( backgroundSprite, 0, 0, display, dialogRoi.x, dialogRoi.y, dialogRoi.width, dialogRoi.height - statusBarHeight );

        const bool hasLibraryCapability = ( race == Race::WZRD ) || ( race == Race::RAND );

        // Guild image.
        fheroes::renderMageGuildBuilding( race, 5, dialogRoi.getPosition() );
        ...
        // OKAY button.
        const fheroes::Point statusBarOffset{ dialogRoi.x, dialogRoi.y + dialogRoi.height - statusBarHeight };

        fheroes::Button buttonOkay( statusBarOffset.x, statusBarOffset.y, ICN::BUTTON_OKAY_TOWN, 0, 1 );
        buttonOkay.draw();

        // EXIT button.
        const int32_t buttonExitWidth = fheroes::AGG::GetICN( ICN::BUTTON_GUILDWELL_EXIT, 0 ).width();
        fheroes::Button buttonExit( statusBarOffset.x + dialogRoi.width - buttonExitWidth, statusBarOffset.y, ICN::BUTTON_GUILDWELL_EXIT, 0, 1 );
        buttonExit.draw();

        fheroes::ButtonSprite buttonBannedSpells;
        {
            const char * translatedText = fheroes::getSupportedText( gettext_noop( "BANNED SPELLS" ), fheroes::FontType::buttonReleasedWhite() );

            fheroes::Sprite released;
        ...
        const int32_t buttonOkayWidth = fheroes::AGG::GetICN( ICN::BUTTON_OKAY_TOWN, 0 ).width();
        const int32_t statusBarWidth = dialogRoi.width - buttonExitWidth - buttonOkayWidth;
        fheroes::Copy( statusBarSprite, 0, 0, display, statusBarOffset.x + buttonOkayWidth, statusBarOffset.y, statusBarWidth / 2, statusBarHeight );
        const int32_t statusBarSecondPart = statusBarWidth - statusBarWidth / 2;
        fheroes::Copy( statusBarSprite, statusBarSprite.width() - statusBarSecondPart, 0, display, statusBarOffset.x + buttonOkayWidth + statusBarWidth / 2,
        ...
        while ( eventHandler.HandleEvents() ) {
            ...
            if ( eventHandler.MouseClickLeft( buttonBannedSpells.area() ) ) {
                ...
                auto getMageGuildTitle = []( const int level ) {
                    switch ( level ) {
                    case 1:
                        return fheroes::getBuildingName( 0, BUILD_MAGEGUILD1 );
                    case 2:
                        return fheroes::getBuildingName( 0, BUILD_MAGEGUILD2 );
                    case 3:
                        return fheroes::getBuildingName( 0, BUILD_MAGEGUILD3 );
                    case 4:
                        return fheroes::getBuildingName( 0, BUILD_MAGEGUILD4 );
                    case 5:
                        return fheroes::getBuildingName( 0, BUILD_MAGEGUILD5 );
                    default:
                        assert( 0 );
                        return "";
                    }
            ...
            if ( eventHandler.isMouseCursorPosInArea( buttonOkay.area() ) ) {
                ...
                if ( eventHandler.isMouseRightButtonPressedInArea( buttonOkay.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Okay" ), statusBarMessage, Dialog::ZERO );
                }
            }
                ...
                if ( eventHandler.isMouseRightButtonPressedInArea( buttonExit.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Exit" ), statusBarMessage, Dialog::ZERO );
                }
            }
                ...
                if ( eventHandler.isMouseRightButtonPressedInArea( buttonBannedSpells.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Banned Spells" ), statusBarMessage, Dialog::ZERO );
                }
            }
...
namespace Editor
{
    bool castleDetailsDialog( Maps::Map_Format::CastleMetadata & castleMetadata, const int race, const PlayerColor color, const fheroes::SupportedLanguage language )
    {
        const CursorRestorer cursorRestorer( true, Cursor::POINTER );

        fheroes::Display & display = fheroes::Display::instance();

        fheroes::StandardWindow window( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT, false );
        const fheroes::Rect dialogRoi = window.activeArea();
        const fheroes::Rect dialogWithShadowRoi = window.totalArea();

        const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();

        const fheroes::Sprite & constructionBackground = fheroes::AGG::GetICN( isEvilInterface ? ICN::CASLWIND_EVIL : ICN::CASLWIND, 0 );
        ...
        fheroes::Blit( constructionBackground, 0, 0, display, dialogRoi.x, dialogRoi.y, rightPartOffsetX, backgroundHeight );
        // Use the right part of standard background dialog.
        fheroes::Copy( fheroes::AGG::GetICN( isEvilInterface ? ICN::STONEBAK_EVIL : ICN::STONEBAK, 0 ), rightPartOffsetX, 0, display, dialogRoi.x + rightPartOffsetX,
                        dialogRoi.y, rightPartWidth, backgroundHeight );
        // Add horizontal separators.
        fheroes::Copy( constructionBackground, rightPartOffsetX, 251, display, dialogRoi.x + rightPartOffsetX, dialogRoi.y + 226, rightPartWidth, 4 );
        fheroes::Copy( constructionBackground, rightPartOffsetX, 251, display, dialogRoi.x + rightPartOffsetX, dialogRoi.y + 306, rightPartWidth, 4 );

        // Castle name background.
        const fheroes::Sprite & statusBarSprite = fheroes::AGG::GetICN( ICN::CASLBAR, 0 );
        const int32_t statusBarHeight = statusBarSprite.height();
        const fheroes::Rect nameArea( dialogRoi.x + rightPartOffsetX, dialogRoi.y + 1, rightPartWidth, statusBarHeight - 2 );
        fheroes::Copy( statusBarSprite, 17, 0, display, nameArea.x, dialogRoi.y, nameArea.width, statusBarHeight );
        ...
        auto drawCastleName = [&castleMetadata, &display, &nameArea, isTown, language]() {
            const fheroes::LanguageSwitcher switcher( language );
            fheroes::Text text( getCastleName( castleMetadata.customName, isTown ), fheroes::FontType::normalWhite() );
        ...
        fheroes::ButtonSprite buttonSpells;
        {
            const char * translatedText = fheroes::getSupportedText( gettext_noop( "SET SPELLS" ), fheroes::FontType::buttonReleasedWhite() );
            window.renderTextAdaptedButtonSprite( buttonSpells, translatedText, { 219, 78 }, fheroes::StandardWindow::Padding::TOP_CENTER );
        }

        // Allow castle building checkbox.
        fheroes::Point dstPt( dialogRoi.x + rightPartOffsetX + 10, dialogRoi.y + 130 );
        fheroes::MovableSprite allowCastleSign;
        fheroes::Rect allowCastleArea;
        if ( isTown ) {
        ...
        dstPt.y += 30;
        fheroes::MovableSprite defaultBuildingsSign;
        const fheroes::Rect defaultBuildingsArea
        ...
        fheroes::ButtonSprite buttonRestrictBuilding;

        const char * translatedText = fheroes::getSupportedText( gettext_noop( "RESTRICT" ), fheroes::FontType::buttonReleasedWhite() );
        window.renderTextAdaptedButtonSprite( buttonRestrictBuilding, translatedText, { 219, -32 }, fheroes::StandardWindow::Padding::CENTER_CENTER );

        const bool isNeutral = ( color == PlayerColor::NONE );

        // Castle army.
        dstPt.y = dialogRoi.y + 311;
        fheroes::MovableSprite defaultArmySign;
        fheroes::Rect defaultArmyArea;
        if ( isNeutral ) {
            ...
            const fheroes::Text armyText( isTown ? _( "Town Army" ) : _( "Castle Army" ), fheroes::FontType::normalWhite() );
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.isMouseCursorPosInArea( nameArea ) ) {
                ...
                if ( le.MouseClickLeft( nameArea ) ) {
                    ...
                    const fheroes::Text body{ _( "Enter Castle name" ), fheroes::FontType::normalWhite() };
                    if ( Dialog::inputString( fheroes::Text{}, body, res, 30, false, language ) && !res.empty() ) {
                        castleMetadata.customName = std::move( res );
                        redrawName = true;
                    }
                }
                else if ( le.isMouseRightButtonPressedInArea( nameArea ) ) {
                    fheroes::showStandardTextMessage( getCastleName( castleMetadata.customName, isTown ), {}, Dialog::ZERO );
                }

                if ( redrawName ) {
                    // Restore name background
                    fheroes::Copy( statusBarSprite, 17, 1, display, nameArea );
                    drawCastleName();
                ...
                if ( le.MouseClickLeft( buttonResetCastleName.area() ) ) {
                    ...
                    fheroes::showStandardTextMessage( _( "Reset" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.MouseClickLeft( allowCastleArea ) ) {
                    ...
                    fheroes::showStandardTextMessage( _( "Allow Castle build" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.MouseClickLeft( defaultBuildingsArea ) ) {
                    ...
                    fheroes::showStandardTextMessage( _( "Default Buildings" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.isMouseRightButtonPressedInArea( buttonRestrictBuilding.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Restrict Building Construction" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.MouseClickLeft( defaultArmyArea ) ) {
                    ...
                    fheroes::showStandardTextMessage( _( "Default Army" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.isMouseRightButtonPressedInArea( buttonSpells.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Set Spells" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.isMouseRightButtonPressedInArea( buttonOkay.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Okay" ), message, Dialog::ZERO );
                }
            }
                ...
                if ( le.isMouseRightButtonPressedInArea( buttonExit.area() ) ) {
                    fheroes::showStandardTextMessage( _( "Exit" ), message, Dialog::ZERO );
                }
            }
            else {
