...
namespace
{
    ...
    const fheroes::Size messageArea{ 300, 210 };
    const int32_t elementOffset{ 9 };
    const int32_t playerAreaWidth{ fheroes::Display::DEFAULT_WIDTH - fheroes::borderWidthPx * 2 - 3 * elementOffset - messageArea.width };
...
namespace Editor
{
                         ...
                         const fheroes::SupportedLanguage language )
    {
        ...
        fheroes::StandardWindow background( fheroes::Display::DEFAULT_WIDTH - fheroes::borderWidthPx * 2, messageArea.height + 220, true, display );
        ...
        const fheroes::Rect resourceRoi{ messageRoi.x, text.height() + offsetY + messageArea.height + 2 * elementOffset, messageRoi.width, 99 };
        ...
        text.set( _( "Computer colors allowed to get event:" ), fheroes::FontType::normalWhite() );
        ...
        text.set( _( "First day of occurrence:" ), fheroes::FontType::normalWhite() );
        ...
        const fheroes::Point firstDayOccurrencePos{ playerAreaOffsetX + ( playerAreaWidth - fheroes::ValueSelectionDialogElement::getArea().width ) / 2, offsetY };

        fheroes::ValueSelectionDialogElement firstDaySelection( 1, lastDayForEvents, static_cast<int32_t>( eventMetadata.firstOccurrenceDay ), 1,
                                                                 firstDayOccurrencePos );

        firstDaySelection.draw( display );

        offsetY += 10 + fheroes::ValueSelectionDialogElement::getArea().height;

        fheroes::ImageRestorer firstDateDescription( display, playerAreaOffsetX, offsetY, playerAreaWidth, 35 );

        text.set( getDateDescription( firstDaySelection.getValue() ), fheroes::FontType::normalWhite() );
        text.draw( playerAreaOffsetX + ( playerAreaWidth - text.width() ) / 2, offsetY, text.width(), display );

        offsetY += 35;

        text.set( _( "Repeat period (days):" ), fheroes::FontType::normalWhite() );
        ...
        const fheroes::Point repeatPeriodPos{ playerAreaOffsetX + ( playerAreaWidth - fheroes::ValueSelectionDialogElement::getArea().width ) / 2, offsetY };

        fheroes::ValueSelectionDialogElement repeatPeriodSelection( 0, daysInYear, static_cast<int32_t>( eventMetadata.repeatPeriodInDays ), 1, repeatPeriodPos );
