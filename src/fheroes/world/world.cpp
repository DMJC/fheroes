...
std::list<MapBaseObject *> MapObjects::get( const fheroes::Point & pos ) const
{
...
const Castle * World::getCastleEntrance( const fheroes::Point & tilePosition ) const
{
...
Castle * World::getCastleEntrance( const fheroes::Point & tilePosition )
{
    if ( !isValidCastleEntrance( tilePosition ) ) {
        return nullptr;
    }
...
bool World::isValidCastleEntrance( const fheroes::Point & tilePosition ) const
{
...
fheroes::LocalizedString World::getCurrentRumor() const
{
    ...
    case 1: {
        ...
        const fheroes::Point artifactPos = Maps::GetPoint( artifactIndex );
...
bool World::DiggingForUltimateArtifact( const fheroes::Point & center )
{
...
MapEvent * World::GetMapEvent( const fheroes::Point & pos )
{
...
void World::fixFrenchCharactersInStrings()
{
    ...
    for ( std::string & str : _customRumors ) {
        fheroes::fixFrenchCharactersForMP2Map( str );
    }
    for ( EventDate & event : vec_eventsday ) {
        fheroes::fixFrenchCharactersForMP2Map( event.message );
    }
    ...
    for ( const auto & tile : vec_tiles ) {
        ...
        case MP2::OBJ_BOTTLE: {
            ...
            if ( sign != nullptr ) {
                fheroes::fixFrenchCharactersForMP2Map( sign->message.text );
            }
            break;
        }
        case MP2::OBJ_EVENT: {
        ...
        case MP2::OBJ_SPHINX: {
            ...
            if ( sphinx != nullptr ) {
                for ( std::string & text : sphinx->answers ) {
                    fheroes::fixFrenchCharactersForMP2Map( text );
                }
                fheroes::fixFrenchCharactersForMP2Map( sphinx->riddle );
            }
            break;
        }
        default:
