...
namespace
{
    void RedrawResourceSprite( const fheroes::Image & sf, const fheroes::Point & pos, int32_t count, int32_t width, int32_t offset, int32_t value )
    {
        fheroes::Display & display = fheroes::Display::instance();

        const fheroes::Point dst_pt( pos.x + width / 2 + count * width, pos.y + offset );
        fheroes::Blit( sf, display, dst_pt.x - sf.width() / 2, dst_pt.y - sf.height() );
...
const fheroes::Rect & Resource::BoxSprite::GetArea() const
{
    return *this;
}
...
void Resource::BoxSprite::Redraw() const
{
    ...
    while ( valueVsSprite.size() - id > 2 ) {
        ...
        const fheroes::Sprite & res1 = fheroes::AGG::GetICN( ICN::RESOURCE, valueVsSprite[id].second );
        const fheroes::Sprite & res2 = fheroes::AGG::GetICN( ICN::RESOURCE, valueVsSprite[id + 1].second );
        const fheroes::Sprite & res3 = fheroes::AGG::GetICN( ICN::RESOURCE, valueVsSprite[id + 2].second );
