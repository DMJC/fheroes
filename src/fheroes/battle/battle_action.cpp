...
Battle::TargetsInfo Battle::Arena::GetTargetsForDamage( const Unit & attacker, Unit & defender, const int32_t dst, const Battle::CellDirection dir ) const
{
    ...
    {
        const std::vector<fheroes::MonsterAbility> & attackerAbilities = fheroes::getMonsterData( attacker.GetID() ).battleStats.abilities;

        if ( const auto abilityIter = std::find( attackerAbilities.begin(), attackerAbilities.end(), fheroes::MonsterAbilityType::ENEMY_HALVING );
    ...
    // lich cloud damage
    else if ( attacker.isAbilityPresent( fheroes::MonsterAbilityType::AREA_SHOT ) && !Unit::isHandFighting( attacker, defender ) ) {
