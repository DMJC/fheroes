...
namespace
{
    ...
    fheroes::Rect CastleGetMaxArea( const Castle & castle, const fheroes::Point & top )
    {
        const fheroes::Sprite & townbkg = fheroes::AGG::GetICN( getTownIcnId( castle.GetRace() ), 0 );
    ...
    void redrawBuildingConnection( const Castle & castle, const fheroes::Point & position, const uint32_t building, const uint32_t buildingCurrentlyUnderConstruction,
                                   const uint8_t alpha = 255 )
    {
        const fheroes::Rect & roi = CastleGetMaxArea( castle, position );
        ...
        if ( race == Race::BARB ) {
            if ( building & BUILD_MAGEGUILD || building == BUILD_SPEC ) {
                ...
                fheroes::drawCastleDialogBuilding( ICN::TWNBEXT2, 0, castle, position, roi, alpha );
            }
            ...
            if ( building == DWELLING_MONSTER3 || building == BUILD_THIEVESGUILD ) {
                ...
                fheroes::drawCastleDialogBuilding( ICN::TWNBEXT3, 0, castle, position, roi, alpha );
            }
        }
            ...
            fheroes::drawCastleDialogBuilding( ICN::NECROMANCER_CASTLE_CAPTAIN_QUARTERS_BRIDGE, 0, castle, position, roi, alpha );
        }
    }

    void redrawCastleBuilding( const Castle & castle, const fheroes::Point & offset, const uint32_t building, const uint32_t frame,
                               const uint32_t buildingCurrentlyUnderConstruction, const uint8_t alpha = 255 )
    {
        ...
        const fheroes::Rect roi = CastleGetMaxArea( castle, offset );

        // Building main sprite.
        fheroes::drawCastleDialogBuilding( icn, index, castle, offset, roi, alpha );
        ...
        if ( const uint32_t index2 = ICN::getAnimatedIcnIndex( icn, index, frame ); index2 != 0 ) {
            fheroes::drawCastleDialogBuilding( icn, index2, castle, offset, roi, alpha );
        }
        ...
        if ( knightCastleCase && building == BUILD_RIGHTTURRET ) {
            ...
            if ( flagAnimFrame > 0 ) {
                fheroes::drawCastleDialogBuilding( castleIcn, flagAnimFrame, castle, offset, roi, alpha );
            }
        }
    }

    void redrawCastleBuildingExtended( const Castle & castle, const fheroes::Point & offset, const uint32_t building, const uint32_t frame,
                                       const uint32_t buildingCurrentlyUnderConstruction, const uint8_t alpha = 255 )
    {
        if ( building == BUILD_TENT ) {
            // We don't need to draw a tent because it's on the background image.
        ...
        if ( building == BUILD_SHIPYARD ) {
            if ( castle.HasBoatNearby() ) {
                ...
                fheroes::drawCastleDialogBuilding( icn2, 0, castle, offset, roi, alpha );

                if ( const uint32_t index2 = ICN::getAnimatedIcnIndex( icn2, 0, frame ) ) {
                    fheroes::drawCastleDialogBuilding( icn2, index2, castle, offset, roi, alpha );
                }
            }
            else {
            ...
            if ( castleRace == Race::SORC && buildingCurrentlyUnderConstruction == BUILD_STATUE
                ...
                fheroes::drawCastleDialogBuilding( gardenIcn, 0, castle, offset, roi );
                if ( const uint32_t index = ICN::getAnimatedIcnIndex( gardenIcn, 0, frame ) ) {
                    fheroes::drawCastleDialogBuilding( gardenIcn, index, castle, offset, roi );
                }

                // Fade-in Statue.
                fheroes::drawCastleDialogBuilding( ICN::TWNSEXT1, 0, castle, offset, roi, alpha );
            }
        }
            ...
            if ( castleRace == Race::SORC ) {
                if ( buildingCurrentlyUnderConstruction != BUILD_STATUE && isBuildingFullyBuilt( castle, BUILD_STATUE, buildingCurrentlyUnderConstruction ) ) {
                    ...
                    if ( const uint32_t index2 = ICN::getAnimatedIcnIndex( ICN::TWNSEXT1, 0, frame ) ) {
                        fheroes::drawCastleDialogBuilding( ICN::TWNSEXT1, index2, castle, offset, roi, alpha );
                    }
                }
            }
            else if ( castleRace == Race::KNGT && !isBuildingFullyBuilt( castle, BUILD_CASTLE, buildingCurrentlyUnderConstruction ) ) {
                const fheroes::Sprite & rightFarm = fheroes::AGG::GetICN( ICN::KNIGHT_CASTLE_RIGHT_FARM, 0 );
                const fheroes::Sprite & leftFarm = fheroes::AGG::GetICN( ICN::KNIGHT_CASTLE_LEFT_FARM, 0 );
                fheroes::drawCastleDialogBuilding( ICN::KNIGHT_CASTLE_LEFT_FARM, 0, castle, { offset.x + rightFarm.x() - leftFarm.width(), offset.y + rightFarm.y() },
                                                    roi, alpha );
            }
        }
            ...
            if ( castleRace == Race::BARB ) {
                if ( !isBuildingFullyBuilt( castle, BUILD_CASTLE, buildingCurrentlyUnderConstruction ) ) {
                    const fheroes::Sprite & rightCaptainQuarters = fheroes::AGG::GetICN( ICN::TWNBCAPT, 0 );
                    const fheroes::Sprite & leftCaptainQuarters = fheroes::AGG::GetICN( ICN::BARBARIAN_CASTLE_CAPTAIN_QUARTERS_LEFT_SIDE, 0 );
                    fheroes::drawCastleDialogBuilding( ICN::BARBARIAN_CASTLE_CAPTAIN_QUARTERS_LEFT_SIDE, 0, castle,
                ...
                fheroes::drawCastleDialogBuilding( ICN::SORCERESS_CASTLE_CAPTAIN_QUARTERS_LEFT_SIDE, 0, castle, offset, roi, alpha );
            }
        }
    }

    void redrawCastleBuildings( const Castle & castle, const fheroes::Point & offset, const CastleDialog::BuildingsRenderQueue & buildings,
                                const CastleDialog::FadeBuilding & fadeBuilding, const uint32_t animationIndex )
    {
        ...
        const fheroes::Sprite & townbkg = fheroes::AGG::GetICN( getTownIcnId( castleRace ), 0 );
        const fheroes::Rect max( offset.x, offset.y, townbkg.width(), townbkg.height() );
        fheroes::Copy( townbkg, 0, 0, display, offset.x, offset.y, max.width, max.height );

        if ( castleRace == Race::BARB ) {
            // Render the river animation for the Barbarian castle.
            const fheroes::Sprite & sprite0 = fheroes::AGG::GetICN( ICN::TWNBEXT1, 1 + animationIndex % 5 );
            fheroes::Blit( sprite0, display, offset.x + sprite0.x(), offset.y + sprite0.y() );
        }
        ...
        // which must be animated, even if the castle itself is not located on the seashore.
            ...
            fheroes::drawCastleDialogBuilding( riverIcnId, riverExtraIndex, castle, offset, max );
        }
        ...
        if ( fadingInBuildingId == BUILD_SHIPYARD || ( castle.HasSeaAccess() && ( castleRace == Race::BARB || !castle.isBuild( BUILD_SHIPYARD ) ) ) ) {
            ...
            fheroes::drawCastleDialogBuilding( bayIcnId, 0, castle, offset, max );
            fheroes::drawCastleDialogBuilding( bayIcnId, bayExtraIndex, castle, offset, max );
        }
        ...
        if ( fadingInBuildingId == BUILD_NOTHING ) {
            for ( const CastleDialog::BuildingRenderInfo & currentBuild : buildings ) {
                ...
                const BuildingType upgradeForCurrentBuilding = fheroes::getUpgradeForBuilding( castleRace, currentBuild.id );
...
CastleDialog::BuildingsRenderQueue::BuildingsRenderQueue( const Castle & castle, const fheroes::Point & top )
{
    const int castleRace = castle.GetRace();
    const std::vector<BuildingType> ordersBuildings = fheroes::getBuildingDrawingPriorities( castleRace, Settings::Get().getCurrentMapInfo().version );

    for ( const BuildingType buildingId : ordersBuildings ) {
        emplace_back( buildingId, fheroes::getCastleBuildingArea( castleRace, buildingId ) + top );
    }
}
...
void CastleDialog::redrawAllBuildings( const Castle & castle, const fheroes::Point & offset, const BuildingsRenderQueue & buildings,
                                       const CastleDialog::FadeBuilding & alphaBuilding, const uint32_t animationIndex )
{
    ...
    fheroes::drawCastleName( castle, fheroes::Display::instance(), offset );
}
