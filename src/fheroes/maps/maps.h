...
namespace Maps
{
    ...
    fheroes::Point getDirectionPoint( const fheroes::Point & from, const int direction );
    ...
    fheroes::Point GetPoint( const int32_t index );

    // Convert maps point to index maps. Returns -1 if x or y is negative.
    int32_t GetIndexFromAbsPoint( const fheroes::Point & mp );
    ...
    void UpdateCastleSprite( const fheroes::Point & center, int race, bool isCastle = false, bool isRandom = false );
    void ReplaceRandomCastleObjectId( const fheroes::Point & );
}
