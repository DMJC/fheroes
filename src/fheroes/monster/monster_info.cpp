...
namespace
{
    std::vector<fheroes::MonsterData> monsterData;

    double getMonsterBaseStrength( const fheroes::MonsterData & data )
    {
        const fheroes::MonsterBattleStats & battleStats = data.battleStats;
        const std::vector<fheroes::MonsterAbility> & abilities = battleStats.abilities;

        const double effectiveHP = battleStats.hp * ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION ) ? 1.4 : 1 );
        const bool isArchers = ( battleStats.shots > 0 );

        double damagePotential = ( battleStats.damageMin + battleStats.damageMax ) / 2.0;

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::DOUBLE_SHOOTING ) ) {
            // How can it be that this ability is assigned not to a shooter?
            assert( isArchers );

            damagePotential *= 2;
        }
        else if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::DOUBLE_MELEE_ATTACK ) ) {
            // Melee attacker will lose potential on second attack after retaliation
            damagePotential *= fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION ) ? 2 : 1.75;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::DOUBLE_DAMAGE_TO_UNDEAD ) ) {
            damagePotential *= 1.15; // 15% of all Monsters are Undead, deals double damage.
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK ) ) {
            damagePotential *= 1.2;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::UNLIMITED_RETALIATION ) ) {
            damagePotential *= 1.25;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::ALL_ADJACENT_CELL_MELEE_ATTACK )
             || fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::AREA_SHOT ) ) {
            damagePotential *= 1.3;
        }
        ...
        if ( isArchers ) {
            monsterSpecial += fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::NO_MELEE_PENALTY ) ? 0.5 : 0.4;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::FLYING ) ) {
            monsterSpecial += 0.3;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::ENEMY_HALVING ) ) {
            monsterSpecial += 1;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::SOUL_EATER ) ) {
            monsterSpecial += 2;
        }

        if ( fheroes::isAbilityPresent( abilities, fheroes::MonsterAbilityType::HP_DRAIN ) ) {
            monsterSpecial += 0.3;
        }

        if ( const auto abilityIter = std::find( abilities.begin(), abilities.end(), fheroes::MonsterAbilityType::SPELL_CASTER ); abilityIter != abilities.end() ) {
    ...
    void populateMonsterData()
    {
        ...
        const fheroes::MonsterSound monsterSounds[Monster::MONSTER_COUNT] = {
        ...
        const fheroes::MonsterBattleStats monsterBattleStats[Monster::MONSTER_COUNT] = {
        ...
        const fheroes::MonsterGeneralStats monsterGeneralStats[Monster::MONSTER_COUNT]
        ...
        monsterData[Monster::RANGER].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_SHOOTING );

        monsterData[Monster::CAVALRY].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );

        monsterData[Monster::CHAMPION].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );

        monsterData[Monster::PALADIN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_MELEE_ATTACK );

        monsterData[Monster::CRUSADER].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_MELEE_ATTACK );
        monsterData[Monster::CRUSADER].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_DAMAGE_TO_UNDEAD );
        monsterData[Monster::CRUSADER].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::IMMUNE_TO_CERTAIN_SPELL, 100, Spell::CURSE );
        monsterData[Monster::CRUSADER].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::IMMUNE_TO_CERTAIN_SPELL, 100, Spell::MASSCURSE );

        monsterData[Monster::WOLF].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::WOLF].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_MELEE_ATTACK );

        monsterData[Monster::TROLL].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::HP_REGENERATION );

        monsterData[Monster::WAR_TROLL].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::HP_REGENERATION );

        monsterData[Monster::CYCLOPS].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK );
        monsterData[Monster::CYCLOPS].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SPELL_CASTER, 20, Spell::PARALYZE );

        monsterData[Monster::SPRITE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION );
        monsterData[Monster::SPRITE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );

        monsterData[Monster::DWARF].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MAGIC_RESISTANCE, 25, 0 );

        monsterData[Monster::BATTLE_DWARF].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MAGIC_RESISTANCE, 25, 0 );

        monsterData[Monster::ELF].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_SHOOTING );

        monsterData[Monster::GRAND_ELF].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_SHOOTING );

        monsterData[Monster::UNICORN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::UNICORN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SPELL_CASTER, 20, Spell::BLIND );

        monsterData[Monster::PHOENIX].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::PHOENIX].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK );
        monsterData[Monster::PHOENIX].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::PHOENIX].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL_SPELL_IMMUNITY );

        monsterData[Monster::CENTAUR].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );

        monsterData[Monster::GARGOYLE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );

        monsterData[Monster::GRIFFIN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::GRIFFIN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::GRIFFIN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNLIMITED_RETALIATION );

        monsterData[Monster::HYDRA].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::HYDRA].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ALL_ADJACENT_CELL_MELEE_ATTACK );
        monsterData[Monster::HYDRA].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION );

        monsterData[Monster::GREEN_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DRAGON );
        monsterData[Monster::GREEN_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::GREEN_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::GREEN_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MAGIC_RESISTANCE, 100, 0 );
        monsterData[Monster::GREEN_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK );

        monsterData[Monster::RED_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DRAGON );
        monsterData[Monster::RED_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::RED_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::RED_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MAGIC_RESISTANCE, 100, 0 );
        monsterData[Monster::RED_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK );

        monsterData[Monster::BLACK_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DRAGON );
        monsterData[Monster::BLACK_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::BLACK_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::BLACK_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MAGIC_RESISTANCE, 100, 0 );
        monsterData[Monster::BLACK_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK );

        monsterData[Monster::BOAR].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );

        monsterData[Monster::IRON_GOLEM].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL_SPELL_DAMAGE_REDUCTION, 50, 0 );
        monsterData[Monster::IRON_GOLEM].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::CERTAIN_SPELL_DAMAGE_REDUCTION, 50, Spell::ARMAGEDDON );

        monsterData[Monster::STEEL_GOLEM].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL_SPELL_DAMAGE_REDUCTION, 50, 0 );
        monsterData[Monster::STEEL_GOLEM].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::CERTAIN_SPELL_DAMAGE_REDUCTION, 50, Spell::ARMAGEDDON );

        monsterData[Monster::ROC].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::ROC].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );

        monsterData[Monster::MAGE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_MELEE_PENALTY );

        monsterData[Monster::ARCHMAGE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_MELEE_PENALTY );
        monsterData[Monster::ARCHMAGE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SPELL_CASTER, 20, Spell::DISPEL );

        monsterData[Monster::GIANT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MIND_SPELL_IMMUNITY );

        monsterData[Monster::TITAN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_MELEE_PENALTY );
        monsterData[Monster::TITAN].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MIND_SPELL_IMMUNITY );

        monsterData[Monster::SKELETON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );

        monsterData[Monster::ZOMBIE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );

        monsterData[Monster::MUTANT_ZOMBIE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );

        monsterData[Monster::MUMMY].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::MUMMY].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SPELL_CASTER, 20, Spell::CURSE );

        monsterData[Monster::ROYAL_MUMMY].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::ROYAL_MUMMY].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SPELL_CASTER, 30, Spell::CURSE );

        monsterData[Monster::VAMPIRE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::VAMPIRE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::VAMPIRE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION );

        monsterData[Monster::VAMPIRE_LORD].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::VAMPIRE_LORD].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::VAMPIRE_LORD].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION );
        monsterData[Monster::VAMPIRE_LORD].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::HP_DRAIN );

        monsterData[Monster::LICH].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::LICH].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::AREA_SHOT );

        monsterData[Monster::POWER_LICH].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::POWER_LICH].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::AREA_SHOT );

        monsterData[Monster::BONE_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );
        monsterData[Monster::BONE_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DRAGON );
        monsterData[Monster::BONE_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::BONE_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::BONE_DRAGON].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::MORAL_DECREMENT, 100, 1 );

        monsterData[Monster::ROGUE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::NO_ENEMY_RETALIATION );

        monsterData[Monster::GHOST].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SOUL_EATER );
        monsterData[Monster::GHOST].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );
        monsterData[Monster::GHOST].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::UNDEAD );

        monsterData[Monster::GENIE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ENEMY_HALVING, 10, 0 );
        monsterData[Monster::GENIE].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FLYING );

        monsterData[Monster::MEDUSA].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );
        monsterData[Monster::MEDUSA].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::SPELL_CASTER, 20, Spell::PETRIFY );

        monsterData[Monster::NOMAD].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::DOUBLE_HEX_SIZE );

        monsterData[Monster::AIR_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL );
        monsterData[Monster::AIR_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::AIR_CREATURE );
        monsterData[Monster::AIR_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::IMMUNE_TO_CERTAIN_SPELL, 100, Spell::METEORSHOWER );
        monsterData[Monster::AIR_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::EXTRA_DAMAGE_FROM_CERTAIN_SPELL, 100,
                                                                               Spell::CHAINLIGHTNING );
        monsterData[Monster::AIR_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::EXTRA_DAMAGE_FROM_CERTAIN_SPELL, 100,
                                                                               Spell::ELEMENTALSTORM );
        monsterData[Monster::AIR_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::EXTRA_DAMAGE_FROM_CERTAIN_SPELL, 100,
                                                                               Spell::LIGHTNINGBOLT );
        monsterData[Monster::AIR_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_EARTH_CREATURES );

        monsterData[Monster::EARTH_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL );
        monsterData[Monster::EARTH_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::EARTH_CREATURE );
        monsterData[Monster::EARTH_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::IMMUNE_TO_CERTAIN_SPELL, 100, Spell::CHAINLIGHTNING );
        monsterData[Monster::EARTH_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::IMMUNE_TO_CERTAIN_SPELL, 100, Spell::ELEMENTALSTORM );
        monsterData[Monster::EARTH_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::IMMUNE_TO_CERTAIN_SPELL, 100, Spell::LIGHTNINGBOLT );
        monsterData[Monster::EARTH_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::EXTRA_DAMAGE_FROM_CERTAIN_SPELL, 100,
                                                                                 Spell::METEORSHOWER );
        monsterData[Monster::EARTH_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_AIR_CREATURES );

        monsterData[Monster::FIRE_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL );
        monsterData[Monster::FIRE_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FIRE_CREATURE );
        monsterData[Monster::FIRE_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::FIRE_SPELL_IMMUNITY );
        monsterData[Monster::FIRE_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_COLD_SPELLS );
        monsterData[Monster::FIRE_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_WATER_CREATURES );

        monsterData[Monster::WATER_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::ELEMENTAL );
        monsterData[Monster::WATER_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::WATER_CREATURE );
        monsterData[Monster::WATER_ELEMENT].battleStats.abilities.emplace_back( fheroes::MonsterAbilityType::COLD_SPELL_IMMUNITY );
        monsterData[Monster::WATER_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_FIRE_SPELLS );
        monsterData[Monster::WATER_ELEMENT].battleStats.weaknesses.emplace_back( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_FIRE_CREATURES );

        // Calculate base value of monster strength.
        for ( fheroes::MonsterData & data : monsterData ) {
