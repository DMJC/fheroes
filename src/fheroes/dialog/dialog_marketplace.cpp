...
namespace
{
    ...
    {
        return fheroes::getTradeCost( ( tradingPost ? 3 : kingdom.GetCountMarketplace() ), rs_from, rs_to );
    }
    ...
    {
    public:
        explicit TradeWindowGUI( const fheroes::Rect & rt )
            : pos_rt( rt )
            , back( fheroes::Display::instance() )
            , tradpostIcnId( Settings::Get().isEvilInterfaceEnabled() ? ICN::TRADPOSE : ICN::TRADPOST )
            , textSell( fheroes::Display::instance() )
            , textBuy( fheroes::Display::instance() )
        {
            ...
            const fheroes::Sprite & spriteGift = fheroes::AGG::GetICN( giftButtonIcnID, 0 );
            const fheroes::Sprite & spriteTrade = fheroes::AGG::GetICN( tradeButtonIcnID, 0 );
            ...
            _scrollbar.setImage( fheroes::AGG::GetICN( tradpostIcnId, 2 ) );
            _scrollbar.setArea( { pos_rt.x + ( pos_rt.width - fheroes::AGG::GetICN( tradpostIcnId, 1 ).width() ) / 2 + 22, pos_rt.y + 131, 187, 11 } );
            _scrollbar.hide();

            const fheroes::Text text( _( "Please inspect our fine wares. If you feel like offering a trade, click on the items you wish to trade with and for." ),
                                       fheroes::FontType::normalWhite() );
            text.draw( pos_rt.x, pos_rt.y + 32, pos_rt.width, fheroes::Display::instance() );
        ...
        fheroes::Rect buttonMax;
        fheroes::Rect buttonMin;
        fheroes::Button buttonTrade;
        fheroes::Button buttonLeft;
        fheroes::Button buttonRight;
        fheroes::Button buttonGift;
        fheroes::Scrollbar _scrollbar;

    private:
        fheroes::Rect pos_rt;
        fheroes::ImageRestorer back;
        const int tradpostIcnId;

        fheroes::MovableText textSell;
        fheroes::MovableText textBuy;
    };
                                        ...
                                        uint32_t count_sell, const bool fromTradingPost, const bool firstExchange )
    {
        ...
        if ( disable || resourceFrom == resourceTo || ( Resource::GOLD != resourceTo && 0 == max_buy ) ) {
            _scrollbar.hide();
            back.restore();
            const fheroes::Rect dst_rt( pos_rt.x, pos_rt.y + 30, pos_rt.width, 100 );
            ...
            const fheroes::Text displayMessage( std::move( message ), fheroes::FontType::normalWhite() );
            ...
            buttonMax = fheroes::Rect();
            buttonMin = fheroes::Rect();
        }
        else {
            back.restore();

            const fheroes::Sprite & bar = fheroes::AGG::GetICN( tradpostIcnId, 1 );
            fheroes::Point dst_pt( pos_rt.x + ( pos_rt.width - bar.width() ) / 2 - 2, pos_rt.y + 128 );
            fheroes::Blit( bar, display, dst_pt.x, dst_pt.y );

            const uint32_t maximumValue = ( Resource::GOLD == resourceTo ) ? max_sell : max_buy;

            const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( tradpostIcnId, 2 );
            const fheroes::Image scrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, true, 187, 1, static_cast<int32_t>( maximumValue + 1 ),
            ...
            const fheroes::Sprite & sprite_from = fheroes::AGG::GetICN( ICN::RESOURCE, Resource::getIconIcnIndex( resourceFrom ) );
            dst_pt.x = pos_rt.x + ( pos_rt.width - sprite_from.width() + 1 ) / 2 - 70;
            dst_pt.y = pos_rt.y + 115 - sprite_from.height();
            fheroes::Blit( sprite_from, display, dst_pt.x, dst_pt.y );
            const fheroes::Sprite & sprite_to = fheroes::AGG::GetICN( ICN::RESOURCE, Resource::getIconIcnIndex( resourceTo ) );
            dst_pt.x = pos_rt.x + ( pos_rt.width - sprite_to.width() + 1 ) / 2 + 70;
            dst_pt.y = pos_rt.y + 115 - sprite_to.height();
            fheroes::Blit( sprite_to, display, dst_pt.x, dst_pt.y );
            const fheroes::Sprite & sprite_fromto = fheroes::AGG::GetICN( tradpostIcnId, 0 );
            dst_pt.x = pos_rt.x + ( pos_rt.width - sprite_fromto.width() ) / 2;
            dst_pt.y = pos_rt.y + 90;
            fheroes::Blit( sprite_fromto, display, dst_pt.x, dst_pt.y );
            fheroes::Text text( _( "Max" ), fheroes::FontType::smallYellow() );
            dst_pt.x = pos_rt.x + ( pos_rt.width - text.width() ) / 2 - 5;
            dst_pt.y = pos_rt.y + 80;
            buttonMax = fheroes::Rect( dst_pt.x, dst_pt.y, text.width(), text.height() );
            text.draw( dst_pt.x, dst_pt.y + 2, display );
            text.set( _( "Min" ), fheroes::FontType::smallYellow() );
            dst_pt.x = pos_rt.x + ( pos_rt.width - text.width() ) / 2 - 5;
            dst_pt.y = pos_rt.y + 103;
            buttonMin = fheroes::Rect( dst_pt.x, dst_pt.y, text.width(), text.height() );
            text.draw( dst_pt.x, dst_pt.y + 2, display );
            text.set( _( "Qty to trade" ), fheroes::FontType::smallWhite() );
    ...
    {
        fheroes::Point dst_pt;

        _scrollbar.hide();

        auto text = std::make_unique<fheroes::Text>( std::string( "-" ) + std::to_string( count_sell ) + " " + "(" + std::to_string( max_sell - count_sell ) + ")",
        ...
        text = std::make_unique<fheroes::Text>( std::string( "+" ) + std::to_string( count_buy ) + " " + "(" + std::to_string( orig_buy + count_buy ) + ")",
                                                 fheroes::FontType::smallWhite() );
    ...
    void RedrawResourceSprite( const fheroes::Image & sf, int32_t px, int32_t py, int32_t value )
    {
        fheroes::Display & display = fheroes::Display::instance();
        fheroes::Blit( sf, display, px, py );

        const fheroes::Text text( std::to_string( value ), fheroes::FontType::smallWhite() );
        text.draw( px + ( 34 - text.width() ) / 2, py + 23, display );
    }

    void RedrawFromResource( const fheroes::Point & pt, const Funds & rs )
    {
        const int tradpost = Settings::Get().isEvilInterfaceEnabled() ? ICN::TRADPOSE : ICN::TRADPOST;

        // wood
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 7 ), pt.x, pt.y, rs.wood );
        // mercury
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 8 ), pt.x + 37, pt.y, rs.mercury );
        // ore
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 9 ), pt.x + 74, pt.y, rs.ore );
        // sulfur
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 10 ), pt.x, pt.y + 37, rs.sulfur );
        // crystal
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 11 ), pt.x + 37, pt.y + 37, rs.crystal );
        // gems
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 12 ), pt.x + 74, pt.y + 37, rs.gems );
        // gold
        RedrawResourceSprite( fheroes::AGG::GetICN( tradpost, 13 ), pt.x + 37, pt.y + 74, rs.gold );
    }

    void RedrawResourceSprite2( const fheroes::Image & sf, int32_t px, int32_t py, bool show, const Kingdom & kingdom, int from, int res, bool trading )
    {
        fheroes::Display & display = fheroes::Display::instance();
        fheroes::Blit( sf, display, px, py );

        if ( show ) {
            const fheroes::Text text( GetStringTradeCosts( kingdom, from, res, trading ), fheroes::FontType::smallWhite() );
            text.draw( px + ( 34 - text.width() ) / 2, py + 23, display );
        }
    }

    void RedrawToResource( const fheroes::Point & pt, bool showcost, const Kingdom & kingdom, bool tradingPost, int from_resource )
    {
        const int tradpost = Settings::Get().isEvilInterfaceEnabled() ? ICN::TRADPOSE : ICN::TRADPOST;

        // wood
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 7 ), pt.x, pt.y, showcost, kingdom, from_resource, Resource::WOOD, tradingPost );
        // mercury
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 8 ), pt.x + 37, pt.y, showcost, kingdom, from_resource, Resource::MERCURY, tradingPost );
        // ore
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 9 ), pt.x + 74, pt.y, showcost, kingdom, from_resource, Resource::ORE, tradingPost );
        // sulfur
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 10 ), pt.x, pt.y + 37, showcost, kingdom, from_resource, Resource::SULFUR, tradingPost );
        // crystal
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 11 ), pt.x + 37, pt.y + 37, showcost, kingdom, from_resource, Resource::CRYSTAL, tradingPost );
        // gems
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 12 ), pt.x + 74, pt.y + 37, showcost, kingdom, from_resource, Resource::GEMS, tradingPost );
        // gold
        RedrawResourceSprite2( fheroes::AGG::GetICN( tradpost, 13 ), pt.x + 37, pt.y + 74, showcost, kingdom, from_resource, Resource::GOLD, tradingPost );
    }
}
...
void Dialog::Marketplace( Kingdom & kingdom, bool fromTradingPost )
{
    ...
    const fheroes::Rect & pos_rt = box.GetArea();
    fheroes::Point dst_pt( pos_rt.x, pos_rt.y );

    // header
    fheroes::Text text{ header, fheroes::FontType::normalYellow() };
    dst_pt.x = pos_rt.x + ( pos_rt.width - text.width() ) / 2;
    dst_pt.y = pos_rt.y;
    text.draw( dst_pt.x, dst_pt.y + 2, display );
    ...
    int resourceFrom = 0;
    const fheroes::Point pt1( pos_rt.x, pos_rt.y + 190 );
    std::vector<fheroes::Rect> rectsFrom;
    rectsFrom.reserve( 7 );
    ...
    fheroes::MovableSprite cursorFrom( fheroes::AGG::GetICN( tradpost, 14 ) );
    text.set( _( "Your Resources" ), fheroes::FontType::smallWhite() );
    ...
    int resourceTo = 0;
    const fheroes::Point pt2( 136 + pos_rt.x, pos_rt.y + 190 );
    std::vector<fheroes::Rect> rectsTo;
    rectsTo.reserve( 7 );
    ...
    fheroes::MovableSprite cursorTo( fheroes::AGG::GetICN( tradpost, 14 ) );
    text.set( _( "Available Trades" ), fheroes::FontType::smallWhite() );
    dst_pt.x = pt2.x + ( 108 - text.width() ) / 2;
    dst_pt.y = pt2.y - 15;
    text.draw( dst_pt.x, dst_pt.y + 2, display );
    RedrawToResource( pt2, false, kingdom, fromTradingPost, 0 );
    ...
    const fheroes::Rect & buttonMax = gui.buttonMax;
    const fheroes::Rect & buttonMin = gui.buttonMin;
    fheroes::Button & buttonGift = gui.buttonGift;
    fheroes::Button & buttonTrade = gui.buttonTrade;
    fheroes::Button & buttonLeft = gui.buttonLeft;
    fheroes::Button & buttonRight = gui.buttonRight;

    fheroes::TimedEventValidator timedButtonLeft( [&buttonLeft]() { return buttonLeft.isPressed(); } );
    fheroes::TimedEventValidator timedButtonRight( [&buttonRight]() { return buttonRight.isPressed(); } );
    buttonLeft.subscribe( &timedButtonLeft );
    buttonRight.subscribe( &timedButtonRight );

    fheroes::Scrollbar & scrollbar = gui._scrollbar;

    // button exit
    const int exitButtonIcnID = isEvilInterface ? ICN::UNIFORM_EVIL_EXIT_BUTTON : ICN::UNIFORM_GOOD_EXIT_BUTTON;
    const fheroes::Sprite & spriteExit = fheroes::AGG::GetICN( exitButtonIcnID, 0 );

    dst_pt.x = pos_rt.x + 68 + ( pos_rt.width - spriteExit.width() ) / 2;
    dst_pt.y = pos_rt.y + pos_rt.height - spriteExit.height();
    fheroes::Button buttonExit( dst_pt.x, dst_pt.y, exitButtonIcnID, 0, 1 );
    ...
    while ( le.HandleEvents() ) {
        ...
        for ( uint32_t ii = 0; ii < rectsFrom.size(); ++ii ) {
            const fheroes::Rect & rect_from = rectsFrom[ii];
            ...
            if ( le.MouseClickLeft( rect_from ) ) {
                ...
                fheroes::showKingdomIncome( kingdom, 0 );
            }
        }

        // click to
        for ( uint32_t ii = 0; ii < rectsTo.size(); ++ii ) {
            const fheroes::Rect & rect_to = rectsTo[ii];
        ...
        if ( buttonLeft.isEnabled() && buttonRight.isEnabled() && max_buy && le.isMouseLeftButtonPressedInArea( scrollbar.getArea() ) ) {
            const fheroes::Point & mousePos = le.getMouseCursorPos();
