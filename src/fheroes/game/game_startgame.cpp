...
namespace
{
    ...
    void ShowNewWeekDialog()
    {
        ...
        std::unique_ptr<const fheroes::MonsterDialogElement> monsterDialogElement;
        ...
        case WeekName::MONSTERS: {
            ...
            monsterDialogElement = std::make_unique<const fheroes::MonsterDialogElement>( monster );
        ...
        fheroes::showStandardTextMessage( std::move( headerText ), std::move( messageText ), Dialog::OK,
                                           monsterDialogElement ? std::vector<const fheroes::DialogElement *>{ monsterDialogElement.get() }
                                                                : std::vector<const fheroes::DialogElement *>{} );
    }
...
fheroes::GameMode Game::StartBattleOnly()
{
...
fheroes::GameMode Game::StartGame()
{
...
void Game::DialogPlayers( const PlayerColor color, std::string title, std::string message )
{
    ...
    const fheroes::Sprite & border = fheroes::AGG::GetICN( ICN::BRCREST, 6 );
    fheroes::Sprite sign = border;

    switch ( color ) {
    case PlayerColor::BLUE:
        fheroes::Blit( fheroes::AGG::GetICN( ICN::BRCREST, 0 ), sign, 4, 4 );
        break;
    case PlayerColor::GREEN:
        fheroes::Blit( fheroes::AGG::GetICN( ICN::BRCREST, 1 ), sign, 4, 4 );
        break;
    case PlayerColor::RED:
        fheroes::Blit( fheroes::AGG::GetICN( ICN::BRCREST, 2 ), sign, 4, 4 );
        break;
    case PlayerColor::YELLOW:
        fheroes::Blit( fheroes::AGG::GetICN( ICN::BRCREST, 3 ), sign, 4, 4 );
        break;
    case PlayerColor::ORANGE:
        fheroes::Blit( fheroes::AGG::GetICN( ICN::BRCREST, 4 ), sign, 4, 4 );
        break;
    case PlayerColor::PURPLE:
        fheroes::Blit( fheroes::AGG::GetICN( ICN::BRCREST, 5 ), sign, 4, 4 );
        break;
    default:
    ...
    const fheroes::CustomImageDialogElement imageUI( std::move( sign ) );
    fheroes::showStandardTextMessage( std::move( title ), std::move( message ), Dialog::OK, { &imageUI } );
}
...
void Game::OpenCastleDialog( Castle & castle, bool updateFocus /* = true */, const bool renderBackgroundDialog /* = true */ )
{
    ...
    if ( renderBackgroundDialog ) {
        ...
        if ( fheroes::Display::instance().isDefaultSize() ) {
            setDisplayFadeIn();
        }
    }
    else {
...
void Game::OpenHeroesDialog( Heroes & hero, bool updateFocus, const bool renderBackgroundDialog, const bool disableDismiss /* = false */ )
{
    ...
    while ( it != myHeroes.end() && result != Dialog::CANCEL ) {
        result = ( *it )->OpenDialog( false, needFade, disableDismiss, false, renderBackgroundDialog, false,
                                      fheroes::getLanguageFromAbbreviation( Settings::Get().getGameLanguage() ) );
...
fheroes::GameMode Interface::AdventureMap::StartGame()
{
    ...
    fheroes::GameMode res = fheroes::GameMode::END_TURN;
    ...
    while ( res == fheroes::GameMode::END_TURN ) {
        if ( !isLoadedFromSave ) {
            world.NewDay();
        }
        ...
        for ( const Player * player : sortedPlayers ) {
            ...
            if ( kingdom.isPlay() ) {
                ...
                case CONTROL_HUMAN:
                    ...
                    if ( res == fheroes::GameMode::HIGHSCORES_STANDARD ) {
                        break;
                    }
                ...
                case CONTROL_AI:
                    ...
                    assert( res == fheroes::GameMode::END_TURN );
                ...
                if ( res != fheroes::GameMode::END_TURN ) {
                    break;
                }
    ...
    // If we are here, the res value should never be fheroes::GameMode::END_TURN
    assert( res != fheroes::GameMode::END_TURN );
...
fheroes::GameMode Interface::AdventureMap::HumanTurn( const bool isLoadedFromSave )
{
    if ( isLoadedFromSave ) {
        updateFocus();
    }
    ...
    if ( !isLoadedFromSave ) {
        ...
        myKingdom.ActionNewDayResourceUpdate( []( const EventDate & event, const Funds & funds ) {
            ...
            if ( funds.GetValidItemsCount() ) {
                fheroes::showResourceMessage( fheroes::Text( event.title, fheroes::FontType::normalYellow(), language ),
                                               fheroes::Text( event.message, fheroes::FontType::normalWhite(), language ), Dialog::OK, funds );
            }
            else if ( !event.message.empty() ) {
                const fheroes::Text header( event.title, fheroes::FontType::normalYellow(), language );
                const fheroes::Text body( event.message, fheroes::FontType::normalWhite(), language );
                fheroes::showMessage( header, body, Dialog::OK );
            }
        } );
    ...
    fheroes::GameMode res = gameResult.checkGameOver();

    const VecCastles & myCastles = myKingdom.GetCastles();
    if ( res == fheroes::GameMode::CANCEL && myCastles.empty() ) {
    ...
    fheroes::Point heroAnimationOffset;
    ...
    while ( res == fheroes::GameMode::CANCEL ) {
...
#if defined( WITH_DEBUG )
        {
            ...
            if ( player->isAIAutoControlMode() ) {
                return fheroes::GameMode::END_TURN;
            }
        }
#endif
        ...
        if ( Game::validateAnimationDelay( Game::CURRENT_HERO_DELAY ) ) {
            ...
            if ( hero ) {
                ...
                if ( heroAnimationFrameCount > 0 ) {
                    ...
                    if ( heroAnimationOffset != fheroes::Point() ) {
                ...
                if ( heroAnimationFrameCount == 0 ) {
                    ...
                    if ( hero->isMoveEnabled() ) {
                        if ( hero->Move( 10 == conf.HeroesMoveSpeed() ) ) {
                            ...
                            if ( const fheroes::Point movement( hero->MovementDirection() ); movement != fheroes::Point() ) {
                                // Do not generate a frame as we are going to do it later.
                                Interface::AdventureMap::RedrawLocker redrawLocker( Interface::AdventureMap::Get() );
        ...
        if ( res == fheroes::GameMode::CANCEL && !myKingdom.isPlay() ) {
            res = fheroes::GameMode::END_TURN;
        }
