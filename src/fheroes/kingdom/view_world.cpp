...
namespace
{
    ...
    constexpr size_t totalZoomLevels{ 4 };

    const std::array<int32_t, totalZoomLevels> tileSizePerZoomLevel{ 4, 6, 12, 32 };
    const std::array<int32_t, totalZoomLevels> icnPerZoomLevel{ ICN::MISC4, ICN::MISC6, ICN::MISC12, ICN::MISC12 };
    const std::array<int32_t, totalZoomLevels> icnLetterPerZoomLevel{ ICN::LETTER4, ICN::LETTER6, ICN::LETTER12, ICN::LETTER12 };
    const std::array<int32_t, totalZoomLevels> icnPerZoomLevelFlags{ ICN::VWFLAG4, ICN::VWFLAG6, ICN::VWFLAG12, ICN::VWFLAG12 };
    ...
    fheroes::Rect computeROI( const fheroes::Point & centerInPixel, const ZoomLevel zoomLevel, const fheroes::Rect & visibleROI )
    {
        // how many pixels from "world map" we can see in "view world" window, given current zoom
        const int32_t pixelsW = visibleROI.width * fheroes::tileWidthPx / tileSizePerZoomLevel[static_cast<uint8_t>( zoomLevel )];
        const int32_t pixelsH = visibleROI.height * fheroes::tileWidthPx / tileSizePerZoomLevel[static_cast<uint8_t>( zoomLevel )];
    ...
    ZoomLevel GetNextZoomLevel( const ZoomLevel level, const bool cycle, const size_t zoomLevels )
    {
        assert( zoomLevels == 3 || zoomLevels == 4 );

        if ( level == ZoomLevel::ZoomLevel0 ) {
            return ZoomLevel::ZoomLevel1;
        }

        if ( level == ZoomLevel::ZoomLevel1 ) {
    ...
    ZoomLevel GetPreviousZoomLevel( const ZoomLevel level, const bool cycle, const size_t zoomLevels )
    {
        if ( level == ZoomLevel::ZoomLevel1 ) {
    ...
    {
        std::vector<fheroes::Image> cachedImages; // One image per zoom Level

        CacheForMapWithResources() = delete;

        // Compute complete world map, and save it for all zoom levels
        explicit CacheForMapWithResources( const ViewWorldMode viewMode, Interface::GameArea & gameArea, const size_t zoomLevels )
        {
            ...
            const int32_t redrawAreaWidth = blockSizeX * fheroes::tileWidthPx;
            const int32_t redrawAreaHeight = blockSizeY * fheroes::tileWidthPx;
            const int32_t redrawAreaCenterX = blockSizeX * fheroes::tileWidthPx / 2;
            const int32_t redrawAreaCenterY = blockSizeY * fheroes::tileWidthPx / 2;

            // Create temporary image where we will draw blocks of the main map on
            fheroes::Image temporaryImg;
            ...
            const fheroes::Point gameAreaCenter( gameArea.getCurrentCenterInPixels() );
            ...
            for ( int32_t x = 0; x < worldWidth; x += blockSizeX ) {
                for ( int32_t y = 0; y < worldHeight; y += blockSizeY ) {
                    gameArea.SetCenterInPixels( { x * fheroes::tileWidthPx + redrawAreaCenterX, y * fheroes::tileWidthPx + redrawAreaCenterY } );
                    gameArea.Redraw( temporaryImg, drawingFlags );

                    for ( size_t i = 0; i < zoomLevels; ++i ) {
                        fheroes::Resize( temporaryImg, 0, 0, temporaryImg.width(), temporaryImg.height(), cachedImages[i], x * tileSizePerZoomLevel[i],
...
#if defined( SAVE_WORLD_MAP )
            fheroes::Save( cachedImages[3], Settings::Get().getCurrentMapInfo().name + saveFilePrefix + ".bmp" );
#endif
        }
    ...
    void DrawWorld( const ViewWorld::ZoomROIs & ROI, CacheForMapWithResources & cache, const fheroes::Rect & roiScreen )
    {
        fheroes::Display & display = fheroes::Display::instance();
        const uint8_t zoomLevelId = static_cast<uint8_t>( ROI.getZoomLevel() );
        const fheroes::Image & image = cache.cachedImages[zoomLevelId];

        const int32_t offsetPixelsX = tileSizePerZoomLevel[zoomLevelId] * ROI.GetROIinPixels().x / fheroes::tileWidthPx;
        const int32_t offsetPixelsY = tileSizePerZoomLevel[zoomLevelId] * ROI.GetROIinPixels().y / fheroes::tileWidthPx;

        const fheroes::Point inPos( offsetPixelsX < 0 ? 0 : offsetPixelsX, offsetPixelsY < 0 ? 0 : offsetPixelsY );
        const fheroes::Point outPos( fheroes::borderWidthPx + ( offsetPixelsX < 0 ? -offsetPixelsX : 0 ),
                                      fheroes::borderWidthPx + ( offsetPixelsY < 0 ? -offsetPixelsY : 0 ) );

        fheroes::Copy( image, inPos.x, inPos.y, display, outPos.x, outPos.y, roiScreen.width, roiScreen.height );
        ...
        if ( image.width() < roiScreen.width ) {
            // Left black bar.
            fillBlack( fheroes::borderWidthPx, fheroes::borderWidthPx, outPos.x - fheroes::borderWidthPx, roiScreen.height );
            // Right black bar.
            fillBlack( fheroes::borderWidthPx - offsetPixelsX + image.width(), fheroes::borderWidthPx,
                       display.width() - 3 * fheroes::borderWidthPx - fheroes::radarWidthPx + offsetPixelsX - image.width(), roiScreen.height );
        }

        if ( image.height() < roiScreen.height ) {
            // Top black bar.
            fillBlack( fheroes::borderWidthPx, fheroes::borderWidthPx, display.width() - 3 * fheroes::borderWidthPx - fheroes::radarWidthPx,
                       outPos.y - fheroes::borderWidthPx );
            // Bottom black bar.
            fillBlack( fheroes::borderWidthPx, fheroes::borderWidthPx - offsetPixelsY + image.height(), roiScreen.width,
                       display.height() - 2 * fheroes::borderWidthPx + offsetPixelsY - image.height() );
        }
    }
    ...
    void DrawObjectsIcons( const PlayerColor color, const ViewWorldMode mode, CacheForMapWithResources & cache )
    {
        ...
        const auto renderCastleFlags = [&cache]( const uint32_t icnIndex, const int32_t posX, const int32_t posY ) {
            for ( size_t zoomLevelId = 0; zoomLevelId < cache.getZoomLevels(); ++zoomLevelId ) {
                ...
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icnFlagsBase, flagIndex );

                const int32_t dstx = posX * tileSize + ( tileSize - sprite.width() ) / 2;
                const int32_t dsty = posY * tileSize + ( tileSize - sprite.height() ) / 2 + 1;

                fheroes::Blit( sprite, cache.cachedImages[zoomLevelId], dstx + tileSize, dsty, false );
                // We place a second flag, flipped horizontally.
                fheroes::Blit( sprite, cache.cachedImages[zoomLevelId], dstx - tileSize, dsty, true );
            }
        };
        ...
        const auto renderIcon = [&cache]( const uint32_t icnIndex, const int32_t posX, const int32_t posY ) {
            for ( size_t zoomLevelId = 0; zoomLevelId < cache.getZoomLevels(); ++zoomLevelId ) {
                ...
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icnPerZoomLevel[zoomLevelId], icnIndex );
                fheroes::Blit( sprite, cache.cachedImages[zoomLevelId], dstx - sprite.width() / 2, dsty - sprite.height() / 2 );
            }
        };
        ...
        const auto renderResourceIcon = [&cache]( const uint32_t icnIndex, const uint32_t resource, const int32_t posX, const int32_t posY ) {
            ...
            for ( size_t zoomLevelId = 0; zoomLevelId < cache.getZoomLevels(); ++zoomLevelId ) {
                ...
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( icnPerZoomLevel[zoomLevelId], icnIndex );
                fheroes::Blit( sprite, cache.cachedImages[zoomLevelId], dstx - sprite.width() / 2, dsty - sprite.height() / 2 );
                const fheroes::Sprite & letter = fheroes::AGG::GetICN( icnLetterPerZoomLevel[zoomLevelId], letterIndex );
                fheroes::Blit( letter, cache.cachedImages[zoomLevelId], dstx - letter.width() / 2, dsty - letter.height() / 2 );
            }
        };
    ...
    void drawViewWorldSprite( const fheroes::Sprite & viewWorldSprite, fheroes::Display & display, const bool isEvilInterface )
    {
        const int32_t dstX = display.width() - viewWorldSprite.width() - fheroes::borderWidthPx;
        int32_t dstY = 2 * fheroes::borderWidthPx + fheroes::radarWidthPx;
        const int32_t cutHeight = 275;
        fheroes::Copy( viewWorldSprite, 0, 0, display, dstX, dstY, viewWorldSprite.width(), cutHeight );
        dstY += cutHeight;

        if ( display.height() > fheroes::Display::DEFAULT_HEIGHT ) {
            const fheroes::Sprite & icnston = fheroes::AGG::GetICN( isEvilInterface ? ICN::STONBAKE : ICN::STONBACK, 0 );
            const int32_t startY = 11;
            const int32_t copyHeight = 46;
            const int32_t repeatHeight = display.height() - fheroes::borderWidthPx - dstY - ( viewWorldSprite.height() - cutHeight );
            const int32_t repeatCount = repeatHeight / copyHeight;
            for ( int32_t i = 0; i < repeatCount; ++i ) {
                fheroes::Copy( icnston, 0, startY, display, dstX, dstY, icnston.width(), copyHeight );
                dstY += copyHeight;
            }
            fheroes::Copy( icnston, 0, startY, display, dstX, dstY, icnston.width(), repeatHeight % copyHeight );
            dstY += repeatHeight % copyHeight;
        }

        fheroes::Copy( viewWorldSprite, 0, cutHeight, display, dstX, dstY, viewWorldSprite.width(), viewWorldSprite.height() - cutHeight );
    }
}

ViewWorld::ZoomROIs::ZoomROIs( const ZoomLevel zoomLevel, const fheroes::Point & centerInPixels, const fheroes::Rect & visibleScreenInPixels, const size_t zoomLevels )
    : _zoomLevel( zoomLevel )
    , _center( centerInPixels )
    , _visibleROI( visibleScreenInPixels )
{
    _roiForZoomLevels.resize( zoomLevels );
...
{
    for ( size_t i = 0; i < _roiForZoomLevels.size(); ++i ) {
...
bool ViewWorld::ZoomROIs::ChangeCenter( const fheroes::Point & centerInPixels )
{
    const fheroes::Rect & currentRect = GetROIinPixels();
    const fheroes::Size worldSize( world.w() * fheroes::tileWidthPx, world.h() * fheroes::tileWidthPx );
    fheroes::Point newCenter;
...
{
    const ZoomLevel newLevel = GetNextZoomLevel( _zoomLevel, cycle, _roiForZoomLevels.size() );
    return _changeZoom( newLevel );
}

bool ViewWorld::ZoomROIs::zoomOut( const bool cycle )
{
    const ZoomLevel newLevel = GetPreviousZoomLevel( _zoomLevel, cycle, _roiForZoomLevels.size() );
    return _changeZoom( newLevel );
}

fheroes::Rect ViewWorld::ZoomROIs::GetROIinTiles() const
{
    fheroes::Rect result = GetROIinPixels();
    result.x = ( result.x + fheroes::tileWidthPx / 2 ) / fheroes::tileWidthPx;
    result.y = ( result.y + fheroes::tileWidthPx / 2 ) / fheroes::tileWidthPx;
    result.width = ( result.width + fheroes::tileWidthPx / 2 ) / fheroes::tileWidthPx;
    result.height = ( result.height + fheroes::tileWidthPx / 2 ) / fheroes::tileWidthPx;
    return result;
}
...
void ViewWorld::ViewWorldWindow( const PlayerColor color, const ViewWorldMode mode, Interface::BaseInterface & interface )
{
    ...
    fheroes::ImageRestorer restorer( display );
    ...
    fheroes::Rect fadeRoi( 0, 0, display.width(), display.height() );

    if ( !isHideInterface ) {
        // If interface is on there is no need to fade the whole screen, just only map area.
        fadeRoi.x += fheroes::borderWidthPx;
        fadeRoi.y += fheroes::borderWidthPx;
        fadeRoi.width -= 3 * fheroes::borderWidthPx + fheroes::radarWidthPx;
        fadeRoi.height -= 2 * fheroes::borderWidthPx;
    }

    // Fade-out Adventure map screen.
    fheroes::fadeOutDisplay( fadeRoi, false );
    ...
    fheroes::RenderProcessor & renderProcessor = fheroes::RenderProcessor::instance();
    ...
    const fheroes::Rect & worldMapROI = gameArea.GetVisibleTileROI();
    const fheroes::Rect & visibleScreenInPixels = gameArea.GetROI();

    // Initial view is centered on where the player is centered
    fheroes::Point viewCenterInPixels( worldMapROI.x * fheroes::tileWidthPx + ( visibleScreenInPixels.width + fheroes::tileWidthPx ) / 2,
                                        worldMapROI.y * fheroes::tileWidthPx + ( visibleScreenInPixels.height + fheroes::tileWidthPx ) / 2 );
    ...
    if ( world.w() * tileSizePerZoomLevel[static_cast<uint8_t>( zoomLevel )] <= visibleScreenInPixels.width
        ...
        viewCenterInPixels.x = world.w() * fheroes::tileWidthPx / 2;
        viewCenterInPixels.y = world.h() * fheroes::tileWidthPx / 2;
    }
    ...
    CacheForMapWithResources cache( mode, gameArea, zoomLevels );

    if ( !interface.isEditor() ) {
        DrawObjectsIcons( color, mode, cache );
    }
    else if ( display.height() == fheroes::Display::DEFAULT_HEIGHT ) {
        // Fix borders for Editor if screen height is 480 pixels.
        const fheroes::Sprite & borderSprite = fheroes::AGG::GetICN( isEvilInterface ? ICN::ADVBORDE : ICN::ADVBORD, 0 );

        const int32_t offsetX = 480;
        const int32_t stepY = 304;
        fheroes::Rect borderRoi( display.width() - borderSprite.width() + offsetX, 160, 144, 17 );

        fheroes::Copy( borderSprite, offsetX, borderRoi.y, display, borderRoi );
        borderRoi.y += stepY;
        fheroes::Copy( borderSprite, offsetX, borderRoi.y, display, borderRoi );
    ...
    // "View world" sprite
    const fheroes::Sprite & viewWorldSprite = fheroes::AGG::GetICN( GetSpriteResource( mode, isEvilInterface ), 0 );
    drawViewWorldSprite( viewWorldSprite, display, isEvilInterface );

    // Zoom button
    const fheroes::Point buttonZoomPosition( display.width() - fheroes::radarWidthPx + 16, 2 * fheroes::borderWidthPx + fheroes::radarWidthPx + 128 );
    fheroes::Button buttonZoom( buttonZoomPosition.x, buttonZoomPosition.y, ( isEvilInterface ? ICN::LGNDXTRE : ICN::LGNDXTRA ), 0, 1 );

    buttonZoom.draw();

    // Exit button
    const fheroes::Point buttonExitPosition( display.width() - fheroes::radarWidthPx + 16, 2 * fheroes::borderWidthPx + fheroes::radarWidthPx + 236 );
    fheroes::Button buttonExit( buttonExitPosition.x, buttonExitPosition.y, ( isEvilInterface ? ICN::BUTTON_VIEWWORLD_EXIT_EVIL : ICN::BUTTON_VIEWWORLD_EXIT_GOOD ), 0,
                                 1 );
    ...
    if ( !isHideInterface ) {
        display.render( { display.width() - fheroes::radarWidthPx + fheroes::borderWidthPx, fheroes::borderWidthPx, display.height() - 2 * fheroes::borderWidthPx,
                          fheroes::radarWidthPx } );
    }

    // Render the View World map image.
    DrawWorld( currentROI, cache, visibleScreenInPixels );

    fheroes::fadeInDisplay( fadeRoi, false );

    // Use for dragging the map from main window
    bool isDrag = false;
    fheroes::Point initMousePos;
    fheroes::Point initRoiCenter;
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.MouseClickLeft( buttonZoom.area() ) ) {
            ...
            if ( isDrag ) {
                const fheroes::Point & newMousePos = le.getMouseCursorPos();
                const int32_t tileSize = tileSizePerZoomLevel[static_cast<uint8_t>( currentROI.getZoomLevel() )];
                const fheroes::Point newRoiCenter( initRoiCenter.x - ( newMousePos.x - initMousePos.x ) * fheroes::tileWidthPx / tileSize,
                                                    initRoiCenter.y - ( newMousePos.y - initMousePos.y ) * fheroes::tileWidthPx / tileSize );
            ...
            fheroes::showStandardTextMessage( _( "Zoom" ), _( "Click this button to adjust the level of zoom." ), Dialog::ZERO );
        }
