...
namespace
{
    void drawButtonShadow( fheroes::Image & output, const int buttonIcn, const uint32_t icnIndex, const fheroes::Point & offset )
    {
        const fheroes::Sprite & buttonSprite = fheroes::AGG::GetICN( buttonIcn, icnIndex );

        fheroes::addGradientShadow( buttonSprite, output, offset, { -5, 5 } );
    }

    void drawCostPerTroopFrame( fheroes::Image & output, const fheroes::Point & offset )
    {
        const fheroes::Sprite & originalBackground = fheroes::AGG::GetICN( ICN::RECRBKG, 0 );
        const fheroes::Sprite & recruitWindowTitle = fheroes::AGG::GetICN( ICN::BLDGXTRA, 0 );

        fheroes::Sprite recruitWindow( 132, 67 );
        ...
        fheroes::Copy( recruitWindowTitle, 3, 58, recruitWindow, 3, 0, 63, 14 );
        fheroes::Copy( recruitWindowTitle, recruitWindowTitle.width() - 66, 58, recruitWindow, 66, 0, 63, 14 );
        fheroes::Copy( originalBackground, 138, 54, recruitWindow, 0, 0, 3, 63 );
        fheroes::Copy( originalBackground, 267, 54, recruitWindow, 129, 0, 3, 63 );
        fheroes::Copy( originalBackground, 138, 117, recruitWindow, 0, 63, 132, 4 );

        const fheroes::Text recruitWindowText( _( "Cost per troop:" ), fheroes::FontType::smallWhite() );
        ...
        if ( Settings::Get().isEvilInterfaceEnabled() ) {
            fheroes::ApplyPalette( recruitWindow, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );
        }

        fheroes::Blit( recruitWindow, output, offset.x, offset.y );
        fheroes::addGradientShadow( recruitWindow, output, { offset.x, offset.y }, { -5, 5 } );
    }

    void RedrawCurrentInfo( const fheroes::Point & pos, const uint32_t result, const Funds & paymentMonster, const Funds & paymentCosts, const Funds & funds,
                            const std::string & label, const fheroes::Image & background = {} )
    {
        fheroes::Display & display = fheroes::Display::instance();

        fheroes::Text text( std::to_string( result ), fheroes::FontType::normalWhite() );

        // Restore the background of the text before rendering it.
        fheroes::Copy( background, 118, 147, display, pos.x + 118, pos.y + 147, 68, text.height() );
        ...
        text.set( std::move( sgold ), fheroes::FontType::smallWhite() );

        // Restore the background of the text before rendering it.
        fheroes::Copy( background, 0, 214, display, pos.x, pos.y + 214, background.width(), text.height() );
        ...
        fheroes::Copy( background, 0, 166, display, pos.x, pos.y + 166, background.width(), text.height() );

        if ( !label.empty() ) {
            text.set( label, fheroes::FontType::smallWhite() );
            text.draw( pos.x + 151 - text.width() / 2, pos.y + 166, display );
        }
    }

    void RedrawResourceInfo( const int resourceIcnIndex, const fheroes::Point & pos, const int32_t value, const int32_t px1, const int32_t py1, const int32_t px2,
                             const int32_t py2, const bool showTotalSum )
    {
        ...
        const fheroes::Sprite & sres = fheroes::AGG::GetICN( ICN::RESOURCE, Resource::getIconIcnIndex( resourceIcnIndex ) );
        fheroes::Blit( sres, fheroes::Display::instance(), pos.x + px1 + offsetX, pos.y + py1 );

        const fheroes::Text text( std::to_string( value ), fheroes::FontType::smallWhite() );
        text.draw( pos.x + px2 - text.width() / 2 + offsetX, pos.y + py2, display );

        if ( showTotalSum ) {
            fheroes::Blit( sres, display, pos.x + px1 - 45, pos.y + py1 + 125 );
        }
    }

    void RedrawMonsterInfo( const fheroes::Rect & pos, const Monster & monster, const uint32_t available, const bool showTotalSum )
    {
        fheroes::Display & display = fheroes::Display::instance();
        ...
        fheroes::Text text( std::move( str ), fheroes::FontType::normalYellow() );
        fheroes::Point dst_pt( pos.x + ( pos.width - text.width() ) / 2, pos.y + 11 );
        ...
        const fheroes::Sprite & smon = fheroes::AGG::GetICN( monster.GetMonsterSprite(), monsterInfo.animationFrames[Bin_Info::MonsterAnimInfo::STATIC][0] );
        ...
        fheroes::Blit( smon, display, dst_pt.x, dst_pt.y );
        ...
        text.set( std::move( str ), fheroes::FontType::smallWhite() );
        text.draw( pos.x + 64 - text.width() / 2, pos.y + 120 + std::max( monsterExtraOffsetY, 2 ), display );

        if ( showTotalSum ) {
            text.set( _( "Number to buy:" ), fheroes::FontType::smallWhite() );
            text.draw( pos.x + 107 - text.width(), pos.y + 149, display );
        }
    }

    const char * SwitchMaxMinButtons( fheroes::ButtonBase & btnMax, fheroes::ButtonBase & btnMin, bool max )
    {
...
Troop Dialog::RecruitMonster( const Monster & monster0, const uint32_t available, const bool allowDowngradedMonster, const int32_t windowOffsetY )
{
    ...
    const fheroes::Size windowSize{ 299, 272 };
    const fheroes::Point dialogOffset( ( display.width() - windowSize.width ) / 2, ( display.height() - windowSize.height ) / 2 + windowOffsetY );

    const fheroes::StandardWindow window( dialogOffset.x, dialogOffset.y, windowSize.width, windowSize.height, true, display );

    const fheroes::Rect roi( window.totalArea() );

    const fheroes::Rect windowActiveArea( window.activeArea() );

    const fheroes::Sprite & originalBackground = fheroes::AGG::GetICN( ICN::RECRBKG, 0 );

    // Render the recruit count background from original recruit dialog ICN.
    fheroes::Sprite background( 68, 19 );
    fheroes::Copy( originalBackground, 134, 159, background, 0, 0, background.width(), background.height() );

    const fheroes::Rect recruitCountInputArea( dialogOffset.x + 118, dialogOffset.y + 147, background.width(), background.height() );

    if ( isEvilInterface ) {
        fheroes::ApplyPalette( background, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );
    }

    fheroes::Point dst_pt( dialogOffset.x + 118, dialogOffset.y + 143 );
    fheroes::Copy( background, 0, 0, display, dst_pt.x, dst_pt.y, background.width(), background.height() );
    fheroes::addGradientShadow( background, display, dst_pt, { -5, 5 } );
    ...
    fheroes::Button buttonOk( dst_pt.x, dst_pt.y, buttonId, 0, 1 );
    ...
    const int32_t buttonCancelWidth = fheroes::AGG ::GetICN( buttonId, 0 ).width();
    dst_pt.x = dialogOffset.x + windowSize.width - backgroundMargin - buttonCancelWidth;
    fheroes::Button buttonCancel( dst_pt.x, dst_pt.y, buttonId, 0, 1 );
    drawButtonShadow( display, buttonId, 0, dst_pt );

    buttonId = isEvilInterface ? ICN::BUTTON_SMALL_MAX_EVIL : ICN::BUTTON_SMALL_MAX_GOOD;
    const int32_t buttonMaxWidth = fheroes::AGG ::GetICN( buttonId, 0 ).width();
    dst_pt.x = dialogOffset.x + 253 - buttonMaxWidth / 2;
    dst_pt.y = dialogOffset.y + 140;

    fheroes::Button buttonMax( dst_pt.x, dst_pt.y, buttonId, 0, 1 );
    fheroes::Button buttonMin( dst_pt.x, dst_pt.y, isEvilInterface ? ICN::BUTTON_SMALL_MIN_EVIL : ICN::BUTTON_SMALL_MIN_GOOD, 0, 1 );
    drawButtonShadow( display, buttonId, 0, dst_pt );

    dst_pt.x = dialogOffset.x + 189;
    dst_pt.y = dialogOffset.y + 138;
    fheroes::Button buttonUp( dst_pt.x, dst_pt.y, ICN::RECRUIT, 0, 1 );
    drawButtonShadow( display, ICN::RECRUIT, 0, dst_pt );

    dst_pt.y = dialogOffset.y + 153;
    fheroes::Button buttonDn( dst_pt.x, dst_pt.y, ICN::RECRUIT, 2, 3 );
    drawButtonShadow( display, ICN::RECRUIT, 2, dst_pt );

    fheroes::TimedEventValidator timedButtonUp( [&buttonUp]() { return buttonUp.isPressed(); } );
    fheroes::TimedEventValidator timedButtonDn( [&buttonDn]() { return buttonDn.isPressed(); } );

    buttonDn.subscribe( &timedButtonDn );
    buttonUp.subscribe( &timedButtonUp );

    const fheroes::Rect rtWheel( dialogOffset.x + 114, dialogOffset.y + 139, 100, 30 );

    // Create monster switching arrows
    fheroes::ButtonSprite monsterSwitchLeft;
    fheroes::ButtonSprite monsterSwitchRight;
    ...
    if ( showDowngradedMonsterSwitchButtons ) {
        const fheroes::Sprite & leftButtonSprite = fheroes::AGG::GetICN( ICN::MONSTER_SWITCH_LEFT_ARROW, 0 );
        monsterSwitchLeft.setSprite( leftButtonSprite, fheroes::AGG::GetICN( ICN::MONSTER_SWITCH_LEFT_ARROW, 1 ) );

        dst_pt.x = dialogOffset.x + 6;
        dst_pt.y = dialogOffset.y + 64;
        monsterSwitchLeft.setPosition( dst_pt.x, dst_pt.y );
        fheroes::addGradientShadow( leftButtonSprite, display, dst_pt, { -5, 5 } );

        const fheroes::Sprite & rightButtonSprite = fheroes::AGG::GetICN( ICN::MONSTER_SWITCH_RIGHT_ARROW, 0 );
        monsterSwitchRight.setSprite( rightButtonSprite, fheroes::AGG::GetICN( ICN::MONSTER_SWITCH_RIGHT_ARROW, 1 ) );
        dst_pt.x = dialogOffset.x + 105;
        monsterSwitchRight.setPosition( dst_pt.x, dst_pt.y );
        fheroes::addGradientShadow( rightButtonSprite, display, dst_pt, { -5, 5 } );
    ...
    fheroes::Copy( display, dialogOffset.x, dialogOffset.y, background, 0, 0, windowSize.width, windowSize.height );
    ...
    const fheroes::Rect monsterArea( dialogOffset.x + 24, dialogOffset.y + 19, 75, 95 );

    const auto buttonReleaseRestore = [&display, &background, &dialogOffset]( fheroes::ButtonBase & button ) {
        ...
        const fheroes::Rect & buttonRoi = button.area();

        fheroes::Copy( background, buttonRoi.x - dialogOffset.x, buttonRoi.y - dialogOffset.y, display, buttonRoi.x, buttonRoi.y, buttonRoi.width, buttonRoi.height );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( updateMonsterInfo ) {
            ...
            fheroes::Copy( background, 0, 0, display, dialogOffset.x, dialogOffset.y, windowSize.width, windowSize.height );
        ...
        if ( const auto value = fheroes::processIntegerValueTyping( 0, static_cast<int32_t>( max ), typedValueBuf ); value ) {
            result = *value;
            ...
            fheroes::openVirtualNumpad( temp, 0, static_cast<int32_t>( max ) );
            ...
            fheroes::showStandardTextMessage( _( "Okay" ), _( "Recruit selected monsters." ), 0 );
        }
        else if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            fheroes::showStandardTextMessage( _( "Cancel" ), _( "Exit this menu without doing anything." ), 0 );
        }
        else if ( buttonMax.isEnabled() && le.isMouseRightButtonPressedInArea( buttonMax.area() ) ) {
            fheroes::showStandardTextMessage( _( "MAX" ), _( "Select maximum monsters to be recruited." ), 0 );
        }
        else if ( buttonMin.isEnabled() && le.isMouseRightButtonPressedInArea( buttonMin.area() ) ) {
            fheroes::showStandardTextMessage( _( "MIN" ), _( "Select only 1 monster to be recruited." ), 0 );
        }
...
{
    const fheroes::Size windowSize{ 289, 141 };
    ...
    const fheroes::Point dialogOffset( ( display.width() - windowSize.width ) / 2,
                                        display.height() / 2 - fheroes::Display::DEFAULT_HEIGHT / 2 + fheroes::borderWidthPx );
