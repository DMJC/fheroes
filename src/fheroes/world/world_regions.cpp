...
namespace
{
    ...
    void FindMissingRegions( std::vector<MapRegionNode> & rawData, const fheroes::Size & mapSize, std::vector<MapRegion> & regions )
    {
...
void World::ComputeStaticAnalysis()
{
    ...
    for ( const TileData & castleTile : castleCenters ) {
        ...
        // getCastle( fheroes::Point( val % width, val / width ) )->GetColor();
