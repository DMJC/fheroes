...
namespace
{
    ...
    {
    public:
        SpellContainerUI( fheroes::Point offset, std::vector<std::pair<Spell, bool>> & spells )
            : _spells( spells )
        {
            ...
            offset.x += ( fheroes::Display::DEFAULT_WIDTH - static_cast<int32_t>( _spellsPerRow ) * spellItemWidth ) / 2;

            const int32_t rowCount{ static_cast<int32_t>( ( _spells.size() + _spellsPerRow - 1 ) / _spellsPerRow ) };
            offset.y += ( fheroes::Display::DEFAULT_HEIGHT - 50 - spellRowOffsetY * rowCount ) / 2;

            // Calculate all areas where we are going to render spells.
            _spellRoi.reserve( _spells.size() );

            const fheroes::Sprite & scrollImage = fheroes::AGG::GetICN( ICN::TOWNWIND, 0 );
        ...
        void draw( fheroes::Image & output )
        {
            ...
            fheroes::Sprite inactiveScrollImage( scrollImage );
            fheroes::ApplyPalette( inactiveScrollImage, PAL::GetPalette( PAL::PaletteType::GRAY ) );

            for ( size_t i = 0; i < _spells.size(); ++i ) {
                if ( !_spells[i].second ) {
                    // The spell is being inactive.
                    fheroes::Blit( inactiveScrollImage, output, _spellRoi[i].x, _spellRoi[i].y );
                }
                else {
                    fheroes::Blit( scrollImage, output, _spellRoi[i].x, _spellRoi[i].y );
                }

                const fheroes::Sprite & spellImage = fheroes::AGG::GetICN( ICN::SPELLS, _spells[i].first.IndexSprite() );

                if ( !_spells[i].second ) {
                    // The spell is being inactive.
                    fheroes::Sprite inactiveSpellImage( spellImage );
                    fheroes::ApplyPalette( inactiveSpellImage, PAL::GetPalette( PAL::PaletteType::GRAY ) );

                    fheroes::Blit( inactiveSpellImage, output, _spellRoi[i].x + 3 + ( _spellRoi[i].width - inactiveSpellImage.width() ) / 2,
                                    _spellRoi[i].y + 31 - inactiveSpellImage.height() / 2 );
                }
                else {
                    fheroes::Blit( spellImage, output, _spellRoi[i].x + 3 + ( _spellRoi[i].width - spellImage.width() ) / 2,
                                    _spellRoi[i].y + 31 - spellImage.height() / 2 );
                }

                const fheroes::Text text( _spells[i].first.GetName(), fheroes::FontType::smallWhite() );
                text.draw( _spellRoi[i].x + 18, _spellRoi[i].y + 57, 78, fheroes::Display::instance() );
            }
        }
        ...
        bool processEvents( LocalEvent & eventProcessor )
        {
            ...
            const fheroes::Rect spellRoi = _spellRoi[spellIndex];
            ...
            if ( eventProcessor.isMouseRightButtonPressedInArea( spellRoi ) ) {
                fheroes::SpellDialogElement( _spells[spellIndex].first, nullptr ).showPopup( Dialog::ZERO );
            }
    ...
    private:
        ...
        std::vector<fheroes::Rect> _spellRoi;
...
namespace Editor
{
                                   ...
                                   const int32_t minimumEnabledSpells, const bool pickDisabledSpells )
    {
        ...
        fheroes::ButtonGroup levelSelection;
        ...
        fheroes::ImageRestorer restorer( display, activeArea.x, activeArea.y, activeArea.width, activeArea.height );
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( isMultiLevelSelectionEnabled ) {
                ...
                for ( int32_t i = 0; i < 5; ++i ) {
                    if ( le.isMouseRightButtonPressedInArea( levelSelection.button( i ).area() ) ) {
                        ...
                        fheroes::showStandardTextMessage( _( "Spells level" ), std::move( str ), Dialog::ZERO );
                        break;
                    }
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Enable All Spells" ), _( "Click to enable all spells." ), Dialog::ZERO );
            }
            else if ( buttonToggleOff.isEnabled() && le.isMouseRightButtonPressedInArea( buttonToggleOff.area() ) ) {
                fheroes::showStandardTextMessage( _( "Disable All Spells" ), _( "Click to disable all spells." ), Dialog::ZERO );
            }
