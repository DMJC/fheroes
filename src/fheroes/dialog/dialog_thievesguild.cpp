...
namespace
{
    ...
    void drawFlags( const std::vector<ValueColors> & v, const fheroes::Point & pos, const int32_t step, const size_t count, fheroes::Image & output )
    {
        ...
        const int32_t sptireWidth = fheroes::AGG::GetICN( ICN::TOWNWIND, 22 ).width();
        ...
        for ( size_t i = 0; i < flagGroups; ++i ) {
            ...
            for ( const PlayerColor color : colors ) {
                const fheroes::Sprite & flag = fheroes::AGG::GetICN( ICN::TOWNWIND, 22 + Color::GetIndex( color ) );
                fheroes::Blit( flag, output, offsetX, offsetY );
                offsetX += sptireWidth;
            }
        }
    }

    void drawHeroStats( const Heroes * hero, const int32_t offsetX, int32_t offsetY, fheroes::Image & output )
    {
        fheroes::Text text( _( "Att." ), fheroes::FontType::smallWhite() );
        text.draw( offsetX, offsetY, output );
        text.set( std::to_string( hero->GetAttack() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX + 50 - text.width(), offsetY, output );

        offsetY += 11;
        text.set( _( "Def." ), fheroes::FontType::smallWhite() );
        text.draw( offsetX, offsetY, output );
        text.set( std::to_string( hero->GetDefense() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX + 50 - text.width(), offsetY, output );

        offsetY += 11;
        text.set( _( "Power" ), fheroes::FontType::smallWhite() );
        text.draw( offsetX, offsetY, output );
        text.set( std::to_string( hero->GetPower() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX + 50 - text.width(), offsetY, output );

        offsetY += 11;
        text.set( _( "Knowl" ), fheroes::FontType::smallWhite() );
        text.draw( offsetX, offsetY, output );
        text.set( std::to_string( hero->GetKnowledge() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX + 50 - text.width(), offsetY, output );
    }

    void drawHeroIcons( const PlayerColorsVector & colors, const bool drawStats, const fheroes::Point & pos, const int32_t step, const int frameIcnID,
                        fheroes::Image & output )
    {
        ...
        for ( const PlayerColor color : colors ) {
            ...
            const fheroes::Sprite & window = fheroes::AGG::GetICN( frameIcnID, 22 );
            fheroes::Blit( window, output, offsetX - window.width() / 2, pos.y - 4 );

            const fheroes::Sprite & icon = hero->GetPortrait( PORT_SMALL );
            fheroes::Copy( icon, 0, 0, output, offsetX - icon.width() / 2, pos.y, icon.width(), icon.height() );
    ...
    void drawPersonality( const PlayerColorsVector & colors, const fheroes::Point & pos, const int32_t step, fheroes::Image & output )
    {
        int32_t offsetX = pos.x;

        for ( const PlayerColor color : colors ) {
            const Player * player = Players::Get( color );
            const fheroes::Text text( player->isControlHuman() ? _( "Human" ) : player->GetPersonalityString(), fheroes::FontType::smallWhite() );
    ...
    void drawBestMonsterIcons( const PlayerColorsVector & colors, const fheroes::Point & pos, const int32_t step, fheroes::Image & output )
    {
        int32_t offsetX = pos.x;
        ...
        for ( const PlayerColor color : colors ) {
            ...
            if ( monster.isValid() ) {
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::MONS32, monster.GetSpriteIndex() );
                fheroes::Blit( sprite, output, offsetX - sprite.width() / 2, pos.y - sprite.height() / 2 );
            }
...
void Dialog::ThievesGuild( const bool oracle )
{
    ...
    const fheroes::Sprite & backgroundSprite = fheroes::AGG::GetICN( isEvilInterfaceTown ? ICN::STONEBAK_EVIL : ICN::STONEBAK, 0 );
    fheroes::Copy( backgroundSprite, 0, 0, display, dialogRoi.x, dialogRoi.y, backgroundSprite.width(), backgroundSprite.height() );
    ...
    fheroes::Text text;
    fheroes::Point offset( startOffsetX, dialogRoi.y + 3 );

    for ( size_t player = 0; player < playersCount; ++player ) {
        text.set( getPlayerOrderString( player ), fheroes::FontType::normalWhite() );
    ...
    fheroes::Copy( bottomBar, 1, 0, display, dialogRoi.x, offset.y, fheroes::Display::DEFAULT_WIDTH / 2, barHeight );
    fheroes::Copy( bottomBar, bottomBar.width() - fheroes::Display::DEFAULT_WIDTH / 2 + exitWidth - 1, 0, display, dialogRoi.x + fheroes::Display::DEFAULT_WIDTH / 2,
                    offset.y, fheroes::Display::DEFAULT_WIDTH / 2 - exitWidth + 1, barHeight );

    // Exit button.
    offset.x = dialogRoi.x + fheroes::Display::DEFAULT_WIDTH - exitWidth;
    fheroes::Button buttonExit( offset.x, offset.y, ICN::BUTTON_GUILDWELL_EXIT, 0, 1 );

    // Text on status bar.
    text.set( oracle ? _( "Oracle: Player Rankings" ) : _( "Thieves' Guild: Player Rankings" ), fheroes::FontType::normalWhite() );
    offset.x = dialogRoi.x + ( fheroes::Display::DEFAULT_WIDTH - exitWidth - text.width() ) / 2;
    text.draw( offset.x, dialogRoi.y + 464, display );

    text.set( _( "Number of Towns:" ), fheroes::FontType::normalWhite() );
    ...
    text.set( _( "Number of Castles:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Number of Heroes:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Gold in Treasury:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Wood & Ore:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Gems, Cr, Slf & Mer:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Obelisks Found:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Artifacts:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Total Army Strength:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Income:" ), fheroes::FontType::normalWhite() );
    offset.y += 24;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Best Hero Stats:" ), fheroes::FontType::normalWhite() );
    offset.y += 47;
    text.draw( textOffsetX - text.width(), offset.y, display );

    text.set( _( "Personality:" ), fheroes::FontType::normalWhite() );
    offset.y += 42;
    text.draw( textOffsetX - text.width(), offset.y, display );
    ...
    text.set( _( "Best Monster:" ), fheroes::FontType::normalWhite() );
    offset.y += 32;
    text.draw( textOffsetX - text.width(), offset.y, display );
