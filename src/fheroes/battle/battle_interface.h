...
namespace Battle
{
    ...
    {
    public:
        OpponentSprite( const fheroes::Rect & area, HeroBase * hero, const bool isReflect );
        ...
        fheroes::Point GetCastPosition() const;
        void Redraw( fheroes::Image & dst ) const;
    ...
    private:
        ...
        fheroes::Rect _area;
        fheroes::Point _offset;
    };

    class Status final : public fheroes::Rect
    {
    public:
        Status();
    ...
    private:
        fheroes::Text _upperText;
        fheroes::Text _lowerText;
        const fheroes::Sprite & _upperBackground;
        const fheroes::Sprite & _lowerBackground;
    ...
    public:
        ...
        void set( const fheroes::Rect & roi, const std::shared_ptr<const Units> & units, const PlayerColor opponentColor )
        {
            _battleRoi = roi;
            _orderOfUnits = units;
        ...
        void redraw( const Unit * current, const uint8_t currentUnitColor, const Unit * underCursor, fheroes::Image & output, const fheroes::Rect & dialogRoi );

        bool queueEventProcessing( Interface & interface, std::string & msg, const fheroes::Point & offset ) const;

        const fheroes::Rect & getRenderingRoi() const
        {
    ...
    private:
        using UnitPos = std::pair<const Unit *, fheroes::Rect>;

        static void _redrawUnit( const fheroes::Rect & pos, const Battle::Unit & unit, const bool revert, const uint8_t currentUnitColor, fheroes::Image & output );

        std::weak_ptr<const Units> _orderOfUnits;
        PlayerColor _opponentColor{ PlayerColor::NONE };
        fheroes::Rect _renderingRoi;
        fheroes::Rect _battleRoi;
    ...
    public:
        ...
        void setBattleUIRect( const fheroes::Rect & battleUIRect )
        {
    ...
    private:
        ...
        fheroes::Sprite _damageImage;
        fheroes::Rect _battleUIRect;
    ...
    public:
        ...
        const fheroes::Rect & GetInterfaceRoi() const
        {
            return _interfacePosition;
        }

        fheroes::Point getRelativeMouseCursorPos() const;
        ...
        void RedrawMissileAnimation( const fheroes::Point & startPos, const fheroes::Point & endPos, const double angle, const uint32_t monsterID );
    ...
    private:
        ...
        fheroes::Point _drawTroopSprite( const Unit & unit, const fheroes::Sprite & troopSprite );
        ...
        void _redrawLightningOnTargets( const std::vector<fheroes::Point> & points, const fheroes::Rect & drawRoi ); // helper function
        ...
        fheroes::Rect _interfacePosition;
        fheroes::Rect _surfaceInnerArea{ 0, 0, fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT };
        fheroes::Image _mainSurface;
        fheroes::Image _battleGround;
        fheroes::Image _hexagonGrid;
        fheroes::Image _hexagonShadow;
        fheroes::Image _hexagonGridShadow;
        fheroes::Image _hexagonCursorShadow;

        int _battleGroundIcn{ ICN::UNKNOWN };
        int _borderObjectsIcn{ ICN::UNKNOWN };

        fheroes::Button _buttonAuto;
        fheroes::Button _buttonSettings;
        fheroes::Button _buttonSkip;
        Status status;
        ...
        const fheroes::Sprite * _spriteInsteadCurrentUnit{ nullptr };
        fheroes::Point _movingPos;
        fheroes::Point _flyingPos;

        int32_t _currentCellIndex{ -1 };
        // Index of the cell selected as the source for the Teleport spell
        int32_t _teleportSpellSrcIdx{ -1 };
        fheroes::Rect _ballistaTowerRect;
        ...
        std::unique_ptr<fheroes::StandardWindow> _background;
