...
namespace
{
    void transformToBlack( fheroes::Image & out )
    {
        if ( out.empty() ) {
            return;
        }
    ...
    fheroes::Sprite generateHeader()
    {
        const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::CBKGLAVA, 0 );
        assert( background.height() < fheroes::Display::DEFAULT_HEIGHT );

        fheroes::Sprite output;
        output._disableTransformLayer();
        output.resize( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT - background.height() );
        output.fill( 0 );

        const fheroes::Text caption( "fheroes engine (" + Settings::GetVersion() + ")", fheroes::FontType::normalYellow() );
        caption.draw( output.width() / 2 - caption.width() / 2, 17, output );

        return output;
    }

    int32_t renderText( fheroes::Image & output, const int32_t offsetX, const int32_t offsetY, const int32_t textWidth, const char * titleText, const char * bodyText )
    {
        const fheroes::Text title( titleText, fheroes::FontType::normalYellow() );
        const fheroes::Text name( bodyText, fheroes::FontType::normalWhite() );
    ...
    fheroes::Sprite generateResurrectionCreditsFirstPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGLAVA, 0 );
        output._disableTransformLayer();
        ...
        const fheroes::Sprite & blackDragon = fheroes::AGG::GetICN( ICN::DRAGBLAK, 5 );
        fheroes::Blit( blackDragon, output, ( columnStep - blackDragon.width() ) / 2, offsetY );
        ...
        const fheroes::Sprite & minotaur = fheroes::AGG::GetICN( ICN::MINOTAUR, 14 );
        fheroes::Blit( minotaur, output, ( columnStep - minotaur.width() ) / 2, offsetY );
        offsetY += minotaur.height();

        offsetY += 40;

        const fheroes::Text websiteInto( _( "Visit us at " ), fheroes::FontType::normalWhite() );
        const fheroes::Text website( "https://github.com/ihhub/fheroes", fheroes::FontType::normalYellow() );
        ...
        const fheroes::Sprite & missile = fheroes::AGG::GetICN( ICN::ARCH_MSL, 4 );
        fheroes::Blit( missile, output, websiteOffsetX - 10 - missile.width(), offsetY + websiteHeight / 2 - missile.height() / 2 );
        fheroes::Blit( missile, output, websiteOffsetX + websiteIntoWidth + websiteWidth + 10, offsetY + websiteHeight / 2 - missile.height() / 2, true );
        ...
        const fheroes::Sprite & cyclop = fheroes::AGG::GetICN( ICN::CYCLOPS, 38 );
        fheroes::Blit( cyclop, output, offsetX + ( columnStep - cyclop.width() ) / 2, offsetY );
        ...
        const fheroes::Sprite & crusader = fheroes::AGG::GetICN( ICN::PALADIN2, 23 );
        fheroes::Blit( crusader, output, offsetX + ( columnStep - crusader.width() ) / 2, offsetY );
        ...
        const fheroes::Sprite & mage = fheroes::AGG::GetICN( ICN::MAGE1, 24 );
        fheroes::Blit( mage, output, offsetX + ( columnStep - mage.width() ) / 2, offsetY );
        ...
        const fheroes::Sprite & phoenix = fheroes::AGG::GetICN( ICN::PHOENIX, 4 );
        fheroes::Blit( phoenix, output, offsetX + ( columnStep - phoenix.width() ) / 2, offsetY - 10 );

        const fheroes::Sprite & goblin = fheroes::AGG::GetICN( ICN::GOBLIN, 27 );
        fheroes::Blit( goblin, output, output.width() - goblin.width() * 2, output.height() - goblin.height() - 10, true );

        return output;
    }

    fheroes::Sprite generateResurrectionCreditsSecondPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGLAVA, 0 );
        output._disableTransformLayer();
        ...
        const fheroes::Text title( _( "Special Thanks to" ), fheroes::FontType::normalYellow() );
        ...
        fheroes::Text name( std::move( contributors ), fheroes::FontType::normalWhite() );
        ...
        name.set( std::move( supporters ), fheroes::FontType::normalWhite() );
        ...
        name.set( _( "and many-many other contributors and supporters!" ), fheroes::FontType::normalWhite() );
        name.draw( output.width() / 2 - name.width() / 2, offsetY, output );

        const fheroes::Sprite & hydra = fheroes::AGG::GetICN( ICN::HYDRA, 11 );

        offsetY = output.height() - hydra.height() - 40;

        fheroes::Blit( hydra, output, textInitialOffsetX - hydra.width() / 2, offsetY );

        return output;
    }

    fheroes::Sprite generateResurrectionCreditsThirdPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGSWMP, 0 );
        output._disableTransformLayer();
        ...
        offsetY += renderText( output, offsetX, offsetY, textWidth, _( "Support us at" ), _( "local-donation-platform|https://www.patreon.com/fheroes" ) );
        offsetY += 30;

        const fheroes::Sprite & wizard = fheroes::AGG::GetICN( ICN::CMBTCAPZ, 4 );
        fheroes::Blit( wizard, output, ( textInitialOffsetX - wizard.width() ) / 2, offsetY );
        offsetY += wizard.height() + 20;

        offsetY
            += renderText( output, offsetX, offsetY, textWidth - 10, _( "Connect with us at" ), _( "local-social-network|https://www.facebook.com/groups/fheroes" ) );
        offsetY += 20;

        const fheroes::Sprite & vampireLord = fheroes::AGG::GetICN( ICN::VAMPIRE2, 22 );
        fheroes::Blit( vampireLord, output, ( textInitialOffsetX - vampireLord.width() ) / 2, offsetY );

        offsetY = textInitialOffsetY;
        offsetX += textInitialOffsetX;

        offsetY += renderText( output, offsetX, offsetY, textWidth - 10, _( "Need help with the game?" ), "https://discord.com/servers/fheroes-733093692860137523" );
        offsetY += 10;

        fheroes::Sprite labyrinth = fheroes::AGG::GetICN( ICN::TWNWUP_3, 0 );
        fheroes::ApplyPalette( labyrinth, 2 );
        fheroes::Blit( labyrinth, output, textInitialOffsetX + ( textInitialOffsetX - labyrinth.width() ) / 2, offsetY );

        offsetY += labyrinth.height() + 50;

        const int32_t miniMonsterXOffset = textInitialOffsetX * 3 / 2;

        fheroes::Text name( _( "Original project before 0.7" ), fheroes::FontType::smallYellow() );
        name.draw( offsetX, offsetY, textWidth - 10, output );
        offsetY += name.height( textWidth - 10 );
        name.set( "Andrey Afletdinov\nhttps://sourceforge.net/\nprojects/fheroes/", fheroes::FontType::smallWhite() );
        name.draw( offsetX, offsetY, textWidth - 10, output );

        fheroes::Sprite creature = fheroes::AGG::GetICN( ICN::MAGE2, 4 );
        transformToBlack( creature );

        const int32_t creatureOffsetY = output.height() - 95;
        fheroes::Blit( creature, 0, 0, output, miniMonsterXOffset - creature.width() / 2, creatureOffsetY, creature.width(), creature.height() );
        name.set( "?", fheroes::FontType::normalYellow() );
        name.draw( miniMonsterXOffset - name.width() / 2, creatureOffsetY + creature.height() / 2 - 5, output );

        return output;
    }

    fheroes::Sprite generateSuccessionWarsCreditsFirstPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGWATR, 0 );
        fheroes::ApplyPalette( output, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        output._disableTransformLayer();

        const fheroes::FontType nameFontType = fheroes::FontType::normalWhite();

        const fheroes::Text title( _( "Heroes of Might and Magic II: The Succession Wars team" ), nameFontType );
    ...
    fheroes::Sprite generateSuccessionWarsCreditsSecondPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGWATR, 0 );
        fheroes::ApplyPalette( output, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        output._disableTransformLayer();
    ...
    fheroes::Sprite generatePriceOfLoyaltyCreditsFirstPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGGRAV, 0 );
        fheroes::ApplyPalette( output, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        output._disableTransformLayer();

        const fheroes::FontType titleFontType = fheroes::FontType::normalYellow();
        const fheroes::FontType nameFontType = fheroes::FontType::normalWhite();

        fheroes::Text title( _( "Heroes of Might and Magic II: The Price of Loyalty team" ), nameFontType );
        title.draw( ( output.width() - title.width() ) / 2, 10, output );
    ...
    fheroes::Sprite generatePriceOfLoyaltyCreditsSecondPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGGRAV, 0 );
        fheroes::ApplyPalette( output, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        output._disableTransformLayer();
    ...
    fheroes::Sprite generatePriceOfLoyaltyCreditsThirdPage()
    {
        fheroes::Sprite output = fheroes::AGG::GetICN( ICN::CBKGGRAV, 0 );
        fheroes::ApplyPalette( output, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        output._disableTransformLayer();
...
void Game::ShowCredits( const bool keepMainMenuBorders )
{
    ...
    const fheroes::Sprite & mainMenuBackground = fheroes::AGG::GetICN( ICN::HEROES, 0 );
    const fheroes::Rect creditsRoi( mainMenuBackground.x(), mainMenuBackground.y(), mainMenuBackground.width(), mainMenuBackground.height() );

    // Hide mouse cursor.
    const CursorRestorer cursorRestorer( false );

    std::unique_ptr<fheroes::ImageRestorer> restorer;
    ...
    if ( keepMainMenuBorders ) {
        ...
        restorer = std::make_unique<fheroes::ImageRestorer>( display, creditsRoi.x, creditsRoi.y, creditsRoi.width, creditsRoi.height );

        fheroes::fadeOutDisplay( creditsRoi, false );
    }
    ...
    std::vector<fheroes::Sprite> pages;
    ...
    while ( le.HandleEvents( Game::isCustomDelayNeeded( animationDelay ) ) ) {
        ...
        if ( Game::validateCustomAnimationDelay( animationDelay ) ) {
            ...
            if ( fadeInHeader ) {
                fheroes::ApplyAlpha( header, 0, 0, display, creditsRoi.x, creditsRoi.y, header.width(), header.height(), alpha );
            }

            const fheroes::Image & page = pages[sequence.pageId()];
            ...
            if ( alpha == 255 ) {
                ...
                fheroes::ApplyAlpha( page, 0, 0, display, creditsRoi.x, creditsRoi.y + header.height(), page.width(), page.height(), alpha );
            }
    ...
    if ( keepMainMenuBorders ) {
        ...
        fheroes::fadeInDisplay( creditsRoi, false );
    }
    else {
