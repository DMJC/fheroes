...
fheroes::GameMode Interface::AdventureMap::EventHeroMovement()
{
    if ( Heroes * hero = GetFocusHeroes(); hero ) {
        if ( hero->GetPath().isValidForMovement() && hero->MayStillMove( false, true ) ) {
            ...
            fheroes::showStandardTextMessage( "Hero Movement Points",
...
fheroes::GameMode Interface::AdventureMap::EventEndTurn() const
{
#ifndef NDEBUG
    ...
    if ( !myKingdom.HeroesMayStillMove()
         || Dialog::YES
                == fheroes::showStandardTextMessage( _( "End Turn" ), _( "One or more heroes may still move, are you sure you want to end your turn?" ),
...
fheroes::GameMode Interface::AdventureMap::EventAdventureDialog()
{
...
fheroes::GameMode Interface::AdventureMap::EventFileDialog() const
{
...
{
    fheroes::showSystemOptionsDialog();
}

fheroes::GameMode Interface::BaseInterface::EventExit()
{
    if ( Dialog::YES & fheroes::showStandardTextMessage( _( "Quit" ), _( "Are you sure you want to quit?" ), Dialog::YES | Dialog::NO ) )
...
fheroes::GameMode Interface::AdventureMap::EventNewGame() const
{
    return Dialog::YES == fheroes::showStandardTextMessage( "", _( "Are you sure you want to restart? (Your current game will be lost.)" ), Dialog::YES | Dialog::NO )
               ? fheroes::GameMode::NEW_GAME
               : fheroes::GameMode::CANCEL;
}

fheroes::GameMode Interface::AdventureMap::EventSaveGame() const
{
    while ( true ) {
        ...
        if ( System::IsFile( filename )
             && Dialog::NO == fheroes::showStandardTextMessage( "", _( "Are you sure you want to overwrite the save with this name?" ), Dialog::YES | Dialog::NO ) ) {
            continue;
        }

        if ( Game::Save( filename ) ) {
            fheroes::showStandardTextMessage( "", _( "Game saved successfully." ), Dialog::OK );
        }
        else {
...
fheroes::GameMode Interface::AdventureMap::EventLoadGame() const
{
    return Dialog::YES
                   == fheroes::showStandardTextMessage( "", _( "Are you sure you want to load a new game? (Your current game will be lost.)" ),
                                                         Dialog::YES | Dialog::NO )
               ? fheroes::GameMode::LOAD_GAME
               : fheroes::GameMode::CANCEL;
}
...
fheroes::GameMode Interface::AdventureMap::EventScenarioInformation()
{
    if ( Settings::Get().isCampaignGameType() ) {
        fheroes::Display & display = fheroes::Display::instance();
        fheroes::ImageRestorer saver( display, 0, 0, display.width(), display.height() );
        ...
        const fheroes::GameMode returnMode = Game::SelectCampaignScenario( fheroes::GameMode::CANCEL, true );
        if ( returnMode == fheroes::GameMode::CANCEL ) {
...
fheroes::GameMode Interface::AdventureMap::EventDigArtifact()
{
    Heroes * hero = GetFocusHeroes();
    if ( hero == nullptr ) {
    ...
    if ( hero->isShipMaster() ) {
        fheroes::showStandardTextMessage( "", _( "Try looking on land!!!" ), Dialog::OK );
        return fheroes::GameMode::CANCEL;
    }
    ...
    if ( hero->GetMaxMovePoints() > hero->GetMovePoints() ) {
        fheroes::showStandardTextMessage( "", _( "Digging for artifacts requires a whole day, try again tomorrow." ), Dialog::OK );
        return fheroes::GameMode::CANCEL;
    }
    ...
    if ( world.getTile( hero->GetIndex() ).isSuitableForUltimateArtifact() || ( ultimateArtifact.getPosition() == hero->GetIndex() && !ultimateArtifact.isFound() ) ) {
        ...
        if ( world.DiggingForUltimateArtifact( hero->GetCenter() ) ) {
            ...
            const fheroes::ArtifactDialogElement artifactUI( ultimate.GetID() );
            fheroes::showStandardTextMessage( _( "Congratulations!" ), std::move( msg ), Dialog::OK, { &artifactUI } );
        }
        else {
            fheroes::showStandardTextMessage( "", _( "Nothing here. Where could it be?" ), Dialog::OK );
        }
    ...
    fheroes::showStandardTextMessage( "", _( "Try searching on clear ground." ), Dialog::OK );

    return fheroes::GameMode::CANCEL;
}

fheroes::GameMode Interface::AdventureMap::EventDefaultAction()
{
    if ( Heroes * hero = GetFocusHeroes(); hero ) {
