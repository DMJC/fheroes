...
namespace
{
    const fheroes::Size giftDialogSize( 320, 234 );

    int32_t GetIndexClickRects( const std::vector<fheroes::Rect> & rects )
    {
        LocalEvent & le = LocalEvent::Get();

        const fheroes::Point & pos = le.getMouseCursorPos();
        const fheroes::Point position( pos.x, pos.y );
    ...
    struct SelectRecipientsColors
    {
        ...
        std::vector<fheroes::Rect> positions;

        SelectRecipientsColors( const fheroes::Point & pos, PlayerColor senderColor )
            : colors( Settings::Get().GetPlayers().GetActualColors() & ( ~senderColor ) )
        {
            ...
            const fheroes::Rect box( ( display.width() - giftDialogSize.width ) / 2, ( display.height() - giftDialogSize.height ) / 2, giftDialogSize.width,
                                      giftDialogSize.height );

            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::CELLWIN, 43 );
        ...
        void Redraw() const
        {
            ...
            for ( PlayerColorsVector::const_iterator it = colors.begin(); it != colors.end(); ++it ) {
                const fheroes::Rect & pos = positions[std::distance( colors.begin(), it )];

                fheroes::Blit( fheroes::AGG::GetICN( ICN::CELLWIN, 43 + Color::GetIndex( *it ) ), display, pos.x, pos.y );
                if ( recipients & *it ) {
                    fheroes::Blit( fheroes::AGG::GetICN( ICN::CELLWIN, 2 ), display, pos.x + 2, pos.y + 2 );
                }
            }
        }
    ...
    struct ResourceBar
    {
        ...
        {
            positions.reserve( 7 );
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::TRADPOST, 7 );
        ...
        {
            fheroes::Display & display = fheroes::Display::instance();
            const fheroes::Text text( std::to_string( count ), fheroes::FontType::smallWhite() );
            const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::TRADPOST, 7 + Resource::getIconIcnIndex( type ) );
            fheroes::Blit( sprite, display, posx, posy );
        ...
        bool QueueEventProcessing( Funds & funds, uint32_t mul )
        {
            ...
            if ( index >= 0 ) {
                ...
                if ( 0 == mul ) {
                    fheroes::showStandardTextMessage( {}, _( "First select recipients!" ), Dialog::OK );
                }
                else if ( 0 == max ) {
                    std::string msg = _( "You cannot select %{resource}!" );
                    StringReplace( msg, "%{resource}", Resource::String( rs ) );
                    fheroes::showStandardTextMessage( {}, std::move( msg ), Dialog::OK );
                }
                else {
...
int Dialog::MakeGiftResource( Kingdom & kingdom )
{
    ...
    const fheroes::StandardWindow frameborder( 320, 234, true );
    const fheroes::Rect box( frameborder.activeArea() );
    ...
    fheroes::Text text( _( "Select Recipients" ), normalWhite );
    text.draw( box.x + ( box.width - text.width() ) / 2, box.y + 7, display );
    SelectRecipientsColors selector( fheroes::Point( box.x + 65, box.y + 28 ), kingdom.GetColor() );
    selector.Redraw();
    ...
    const fheroes::Sprite & buttonOkSprite = fheroes::AGG::GetICN( okIcnId, 0 );
    const fheroes::Sprite & buttonCancelSprite = fheroes::AGG::GetICN( cancelIcnId, 0 );

    const int32_t border = 10;
    fheroes::ButtonGroup btnGroup;
    btnGroup.addButton( fheroes::makeButtonWithShadow( box.x + border, box.y + box.height - border - buttonOkSprite.height(), buttonOkSprite,
                                                        fheroes::AGG::GetICN( okIcnId, 1 ), display ),
                        Dialog::OK );
    btnGroup.addButton( fheroes::makeButtonWithShadow( box.x + box.width - border - buttonCancelSprite.width(),
                                                        box.y + box.height - border - buttonCancelSprite.height(), buttonCancelSprite,
                                                        fheroes::AGG::GetICN( cancelIcnId, 1 ), display ),
                        Dialog::CANCEL );
