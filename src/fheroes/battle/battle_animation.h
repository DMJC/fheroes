...
class RandomizedDelay : protected fheroes::TimeDelay
{
public:
    explicit RandomizedDelay( const uint32_t delay )
        : fheroes::TimeDelay( delay )
        , halfDelay( delay / 2 )
    {
        // Do nothing.
    }
...
public:
    ...
    fheroes::Point getBlindOffset() const
    {
        return _monsterInfo.eyePosition;
    }

    fheroes::Point getProjectileOffset( const size_t direction ) const;
