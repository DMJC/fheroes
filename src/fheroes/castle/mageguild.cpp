...
void MageGuild::initialize( const int race, const bool hasLibrary )
{
    ...
    const auto addSpell = [this, hasLibrary, &spellsInUse, &mageGuildLevels]( const Spell & spell ) {
        const size_t spellLevel = fheroes::checkedCast<size_t>( spell.Level() ).value();
    ...
    for ( size_t level = 1; level <= mageGuildLevels.size(); ++level ) {
        ...
        std::vector<int> allSpellsOfLevel = Spell::getAllSpellIdsSuitableForSpellBook( fheroes::checkedCast<int>( level ).value() );

        while ( freeSlots > 0 ) {
            assert( !allSpellsOfLevel.empty() );

            const uint32_t spellIdx = Rand::Get( 0, fheroes::checkedCast<uint32_t>( allSpellsOfLevel.size() - 1 ).value() );
...
void MageGuild::initialize( const int race, const bool hasLibrary, const std::map<uint8_t, int32_t> & mustHaveSpells, const std::vector<int32_t> & bannedSpells )
{
    ...
    const auto addSpell = [this, hasLibrary, &spellsInUse, &mageGuildLevels]( const Spell & spell ) {
        ...
        const size_t levelIndex = fheroes::checkedCast<size_t>( spellLevel - 1 ).value();
    ...
    for ( size_t level = 1; level <= mageGuildLevels.size(); ++level ) {
        ...
        std::vector<int> allSpellsOfLevel = Spell::getAllSpellIdsSuitableForSpellBook( fheroes::checkedCast<int>( level ).value(), spellsInUse );
