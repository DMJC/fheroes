...
namespace Interface
{
    ...
    {
    public:
        explicit ListBox( const fheroes::Point & pt = fheroes::Point() )
            : ptRedraw( pt )
        ...
        virtual void RedrawBackground( const fheroes::Point & ) = 0;
        ...
        virtual void ActionListDoubleClick( Item & item, const fheroes::Point & /*mousePos*/, int32_t /*itemOffsetX*/, int32_t /*itemOffsetY*/ )
        {
            ActionListDoubleClick( item );
        }

        virtual void ActionListSingleClick( Item & item, const fheroes::Point & /*mousePos*/, int32_t /*itemOffsetX*/, int32_t /*itemOffsetY*/ )
        {
            ActionListSingleClick( item );
        }

        virtual void ActionListPressRight( Item & item, const fheroes::Point & /*mousePos*/, int32_t /*itemOffsetX*/, int32_t /*itemOffsetY*/ )
        {
            ActionListPressRight( item );
        }

        virtual bool ActionListCursor( Item & /* item */, const fheroes::Point & /* cursor */ )
        {
            return false;
        }

        void SetTopLeft( const fheroes::Point & tl )
        {
            ptRedraw = tl;
        }

        void SetScrollButtonUp( int icn, uint32_t index1, uint32_t index2, const fheroes::Point & pos )
        {
            _buttonPgUp.setICNInfo( icn, index1, index2 );
            _buttonPgUp.setPosition( pos.x, pos.y );
        }

        void SetScrollButtonDn( int icn, uint32_t index1, uint32_t index2, const fheroes::Point & pos )
        {
            _buttonPgDn.setICNInfo( icn, index1, index2 );
            _buttonPgDn.setPosition( pos.x, pos.y );
        }

        void setScrollBarArea( const fheroes::Rect & area )
        {
            _scrollbar.setArea( area );
        }

        void setScrollBarImage( const fheroes::Image & image )
        {
        ...
        void updateScrollBarImage()
        {
            ...
            setScrollBarImage( fheroes::generateScrollbarSlider( _scrollbar, false, _scrollbar.getArea().height, VisibleItemCount(), _size(),
                                                                  { 0, 0, scrollBarWidth, 8 }, { 0, 7, scrollBarWidth, 8 } ) );
            _scrollbar.moveToIndex( _topId );
        }

        fheroes::Scrollbar & GetScrollbar()
        {
            return _scrollbar;
        }
        ...
        void SetAreaItems( const fheroes::Rect & rt )
        {
            rtAreaItems = rt;
        }
        ...
        Item * GetFromPosition( const fheroes::Point & mp )
        {
            Verify();
            if ( !IsValid() ) {
                return nullptr;
        ...
        bool QueueEventProcessing() override
        {
            ...
            if ( useHotkeys && le.isKeyPressed( fheroes::Key::KEY_PAGE_UP ) ) {
                SetCurrentVisible();
                if ( _topId > 0 ) {
                    needRedraw = true;
            ...
            if ( useHotkeys && le.isKeyPressed( fheroes::Key::KEY_PAGE_DOWN ) ) {
                SetCurrentVisible();
            ...
            if ( useHotkeys && le.isKeyPressed( fheroes::Key::KEY_UP ) && ( _currentId > 0 ) ) {
                needRedraw = true;
            ...
            if ( useHotkeys && le.isKeyPressed( fheroes::Key::KEY_DOWN ) && ( _currentId + 1 < _size() ) ) {
                needRedraw = true;
            ...
            if ( useHotkeys && le.isKeyPressed( fheroes::Key::KEY_HOME ) && ( _topId > 0 || _currentId > 0 ) ) {
                needRedraw = true;
            ...
            if ( useHotkeys && le.isKeyPressed( fheroes::Key::KEY_END ) && ( _topId + maxItems < _size() || _currentId < _lastVisibleId() ) ) {
                needRedraw = true;
    ...
    protected:
        ...
        fheroes::Rect rtAreaItems;

        fheroes::Button _buttonPgUp;
        fheroes::Button _buttonPgDn;
    ...
    private:
        ...
        fheroes::Point ptRedraw;
        fheroes::Point _dragStartPos;
        ...
        fheroes::TimedEventValidator _timedButtonPgUp;
        fheroes::TimedEventValidator _timedButtonPgDn;
