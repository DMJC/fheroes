...
void Interface::AdventureMap::reset()
{
    ...
    if ( isHideInterface ) {
        ...
        _controlPanel.SetPos( display.width() - _controlPanel.GetArea().width - fheroes::borderWidthPx, 0 );

        fheroes::Point radrPos = conf.PosRadar();
        fheroes::Point bttnPos = conf.PosButtons();
        fheroes::Point iconPos = conf.PosIcons();
        fheroes::Point statPos = conf.PosStatus();

        const auto isPosValid = []( const fheroes::Point & pos ) { return pos.x >= 0 && pos.y >= 0; };
        ...
        const int32_t px = display.width() - fheroes::borderWidthPx - fheroes::radarWidthPx;

        _radar.SetPos( px, fheroes::borderWidthPx );
        _iconsPanel.SetPos( px, _radar.GetArea().y + _radar.GetArea().height + fheroes::borderWidthPx );
        _buttonsPanel.SetPos( px, _iconsPanel.GetArea().y + _iconsPanel.GetArea().height + fheroes::borderWidthPx );
...
void Interface::AdventureMap::redraw( const uint32_t force )
{
    ...
    if ( combinedRedraw & REDRAW_GAMEAREA ) {
        _gameArea.Redraw( fheroes::Display::instance(), LEVEL_ALL );
...
int32_t Interface::AdventureMap::GetDimensionDoorDestination( const int32_t from, const int32_t distance, const bool water )
{
    ...
    const fheroes::Rect & radarRect = _radar.GetRect();
    const fheroes::Rect & radarArea = _radar.GetArea();

    fheroes::Button buttonExit( radarArea.x + 32, radarArea.y + radarArea.height - 37,
    ...
    const auto drawControlPanel = [&display, isEvilInterface, isHideInterface, &radarRect, &radarArea, &buttonExit]() {
        ...
        fheroes::Blit( fheroes::AGG::GetICN( ( isEvilInterface ? ICN::EVIWDDOR : ICN::VIEWDDOR ), 0 ), display, radarArea.x, radarArea.y );

        buttonExit.draw();
    };

    const fheroes::Rect & gameAreaROI = _gameArea.GetROI();
    const bool isFadingEnabled = ( gameAreaROI.width > fheroes::tileWidthPx * distance ) || ( gameAreaROI.height > fheroes::tileWidthPx * distance );

    const fheroes::Rect spellROI = [this, from, distance, isHideInterface, &gameAreaROI]() -> fheroes::Rect {
        const fheroes::Point heroPos = _gameArea.GetRelativeTilePosition( Maps::GetPoint( from ) );

        const int32_t x = heroPos.x - fheroes::tileWidthPx * ( distance / 2 );
        const int32_t y = heroPos.y - fheroes::tileWidthPx * ( distance / 2 );

        // We need to add an extra cell since the hero stands exactly in the middle of a cell
        const int32_t w = std::min( fheroes::tileWidthPx * ( distance + 1 ), gameAreaROI.width );
        const int32_t h = std::min( fheroes::tileWidthPx * ( distance + 1 ), gameAreaROI.height );

        return { isHideInterface ? x : std::max( x, fheroes::borderWidthPx ), isHideInterface ? y : std::max( y, fheroes::borderWidthPx ), w, h };
    }();
    ...
    if ( isFadingEnabled ) {
        if ( isHideInterface ) {
            ...
            fheroes::InvertedFadeWithPalette( display, gameAreaROI, spellROI, 5, 300, 9 );
        }
    }
    else {
        drawControlPanel();
