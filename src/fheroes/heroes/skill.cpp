...
int Skill::getLeadershipModifiers( int level, std::string * strs )
{
    ...
    if ( skillValue != 0 && strs ) {
        ...
        fheroes::appendModifierToString( *strs, skillValue );
        strs->append( "\n" );
    }
...
uint32_t Skill::GetNecromancyBonus( const HeroBase & hero )
{
    ...
    const uint32_t artifactEffect = hero.GetBagArtifacts().isArtifactBonusPresent( fheroes::ArtifactBonusType::NECROMANCY_SKILL ) ? 1 : 0;
    // cap bonus at 7
