...
namespace
{
    ...
    std::string getExternalMusicFile( const int musicTrackId )
    {
        ...
        const auto tryMusicFileType = [musicTrackId]( MusicFileType & musicFileType ) -> std::string {
            ...
            fheroes::replaceStringEnding( fileName, musicFileType.extension[0].c_str(), musicFileType.extension[1].c_str() );
            if ( findMusicFile( musicDirectories, fileName, fullPath ) ) {
            ...
            fheroes::replaceStringEnding( fileName, musicFileType.extension[1].c_str(), musicFileType.extension[2].c_str() );
            if ( findMusicFile( musicDirectories, fileName, fullPath ) ) {
                // Swap extensions to improve cache hit.
    ...
    fheroes::AGGFile g_midiHeroes2AGG;
    fheroes::AGGFile g_midiHeroes2xAGG;
