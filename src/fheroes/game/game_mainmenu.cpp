...
namespace
{
    ...
    {
        uint32_t frame;
        fheroes::Button & button;
        const fheroes::Rect & buttonArea;
        bool isOver;
        bool wasOver;
    };
...
{
    fheroes::GameMode result = fheroes::GameMode::MAIN_MENU;

    bool exit = false;

    while ( !exit ) {
        switch ( result ) {
        case fheroes::GameMode::QUIT_GAME:
            exit = true;
            break;
        case fheroes::GameMode::MAIN_MENU:
            result = Game::MainMenu( isFirstGameRun );
            isFirstGameRun = false;
            break;
        case fheroes::GameMode::NEW_GAME:
            result = Game::NewGame( isProbablyDemoVersion );
            isProbablyDemoVersion = false;
            break;
        case fheroes::GameMode::LOAD_GAME:
            result = Game::LoadGame();
            break;
        case fheroes::GameMode::HIGHSCORES_STANDARD:
            result = Game::DisplayHighScores( false );
            break;
        case fheroes::GameMode::HIGHSCORES_CAMPAIGN:
            result = Game::DisplayHighScores( true );
            break;
        case fheroes::GameMode::CREDITS:
            result = Game::Credits();
            break;
        case fheroes::GameMode::NEW_STANDARD:
            result = Game::NewStandard();
            break;
        case fheroes::GameMode::NEW_SUCCESSION_WARS_CAMPAIGN:
            result = Game::NewSuccessionWarsCampaign();
            break;
        case fheroes::GameMode::NEW_PRICE_OF_LOYALTY_CAMPAIGN:
            result = Game::NewPriceOfLoyaltyCampaign();
            break;
        case fheroes::GameMode::NEW_BATTLE_ONLY:
            result = Game::NewBattleOnly();
            break;
        case fheroes::GameMode::LOAD_STANDARD:
            result = Game::LoadStandard();
            break;
        case fheroes::GameMode::LOAD_CAMPAIGN:
            result = Game::LoadCampaign();
            break;
        case fheroes::GameMode::LOAD_HOT_SEAT:
            result = Game::LoadHotseat();
            break;
        case fheroes::GameMode::SELECT_SCENARIO_ONE_HUMAN_PLAYER:
        case fheroes::GameMode::SELECT_SCENARIO_TWO_HUMAN_PLAYERS:
        case fheroes::GameMode::SELECT_SCENARIO_THREE_HUMAN_PLAYERS:
        case fheroes::GameMode::SELECT_SCENARIO_FOUR_HUMAN_PLAYERS:
        case fheroes::GameMode::SELECT_SCENARIO_FIVE_HUMAN_PLAYERS:
        case fheroes::GameMode::SELECT_SCENARIO_SIX_HUMAN_PLAYERS: {
            const uint8_t humanPlayerCount
                = static_cast<uint8_t>( static_cast<int>( result ) - static_cast<int>( fheroes::GameMode::SELECT_SCENARIO_ONE_HUMAN_PLAYER ) );
            // Add +1 since we don't have zero human player option.
            result = Game::SelectScenario( humanPlayerCount + 1 );
            break;
        }
        case fheroes::GameMode::START_GAME:
            result = Game::StartGame();
            break;
        case fheroes::GameMode::SELECT_CAMPAIGN_SCENARIO:
            result = Game::SelectCampaignScenario( fheroes::GameMode::MAIN_MENU, false );
            break;
        case fheroes::GameMode::COMPLETE_CAMPAIGN_SCENARIO:
            result = Game::CompleteCampaignScenario( false );
            break;
        case fheroes::GameMode::COMPLETE_CAMPAIGN_SCENARIO_FROM_LOAD_FILE:
            result = Game::CompleteCampaignScenario( true );
            while ( result == fheroes::GameMode::SELECT_CAMPAIGN_SCENARIO ) {
                result = Game::SelectCampaignScenario( fheroes::GameMode::LOAD_CAMPAIGN, false );
            }
            break;
        case fheroes::GameMode::START_BATTLE_ONLY_MODE:
            result = Game::StartBattleOnly();
            break;
        case fheroes::GameMode::EDITOR_MAIN_MENU:
            result = Editor::menuMain( false );
            break;
        case fheroes::GameMode::EDITOR_NEW_MAP:
            result = Editor::menuMain( true );
            break;
        case fheroes::GameMode::EDITOR_LOAD_MAP:
...
fheroes::GameMode Game::MainMenu( const bool isFirstGameRun )
{
    ...
    if ( isFirstGameRun ) {
        ...
        fheroes::selectLanguage( fheroes::getSupportedLanguages(), fheroes::getLanguageFromAbbreviation( conf.getGameLanguage() ), true );

        {
            std::string body( _( "Welcome to Heroes of Might and Magic II powered by fheroes engine!" ) );
            ...
            fheroes::showStandardTextMessage( _( "Greetings!" ), std::move( body ), Dialog::OK );
    ...
    fheroes::Button buttonNewGame( 0, 0, ICN::BTNSHNGL, NEWGAME_DEFAULT, NEWGAME_DEFAULT + 2 );
    fheroes::Button buttonLoadGame( 0, 0, ICN::BTNSHNGL, LOADGAME_DEFAULT, LOADGAME_DEFAULT + 2 );
    fheroes::Button buttonHighScores( 0, 0, ICN::BTNSHNGL, HIGHSCORES_DEFAULT, HIGHSCORES_DEFAULT + 2 );
    fheroes::Button buttonCredits( 0, 0, ICN::BTNSHNGL, CREDITS_DEFAULT, CREDITS_DEFAULT + 2 );
    fheroes::Button buttonEditor( 0, 0, ICN::BTNSHNGL, EDITOR_DEFAULT, EDITOR_DEFAULT + 2 );
    fheroes::Button buttonQuit( 0, 0, ICN::BTNSHNGL, QUIT_DEFAULT, QUIT_DEFAULT + 2 );

    const fheroes::Sprite & lantern10 = fheroes::AGG::GetICN( ICN::SHNGANIM, 0 );
    fheroes::Blit( lantern10, display, lantern10.x(), lantern10.y() );

    const fheroes::Sprite & lantern11 = fheroes::AGG::GetICN( ICN::SHNGANIM, ICN::getAnimatedIcnIndex( ICN::SHNGANIM, 0, 0 ) );
    fheroes::Blit( lantern11, display, lantern11.x(), lantern11.y() );
    ...
    const double scaleX = static_cast<double>( display.width() ) / fheroes::Display::DEFAULT_WIDTH;
    const double scaleY = static_cast<double>( display.height() ) / fheroes::Display::DEFAULT_HEIGHT;

    const double scale = std::min( scaleX, scaleY );
    const int32_t offsetX = static_cast<int32_t>( std::lround( display.width() - fheroes::Display::DEFAULT_WIDTH * scale ) ) / 2;
    const int32_t offsetY = static_cast<int32_t>( std::lround( display.height() - fheroes::Display::DEFAULT_HEIGHT * scale ) ) / 2;

    const fheroes::Rect settingsArea( static_cast<int32_t>( 63 * scale ) + offsetX, static_cast<int32_t>( 202 * scale ) + offsetY, static_cast<int32_t>( 108 * scale ),
    ...
    for ( size_t i = 0; le.hasMouseMoved() && i < buttons.size(); ++i ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::BTNSHNGL, buttons[i].frame );
        fheroes::Blit( sprite, display, sprite.x(), sprite.y() );
    }

    fheroes::Sprite highlightDoor = fheroes::AGG::GetICN( ICN::SHNGANIM, 18 );
    fheroes::ApplyPalette( highlightDoor, 8 );
    ...
    while ( true ) {
        ...
        for ( ButtonInfo & button : buttons ) {
            ...
            if ( button.isOver != button.wasOver ) {
                ...
                const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::BTNSHNGL, frame );
                fheroes::Blit( sprite, display, sprite.x(), sprite.y() );
            }
        }
        ...
        if ( isPOLPresent && ( HotKeyPressEvent( HotKeyEvent::EDITOR_MAIN_MENU ) || le.MouseClickLeft( buttonEditor.area() ) ) ) {
            return fheroes::GameMode::EDITOR_MAIN_MENU;
        }

        // right info
        if ( le.isMouseRightButtonPressedInArea( buttonQuit.area() ) ) {
            fheroes::showStandardTextMessage( _( "Quit" ), _( "Quit Heroes of Might and Magic II and return to the operating system." ), Dialog::ZERO );
        }
            ...
            fheroes::showStandardTextMessage( _( "Credits" ), _( "View the credits screen." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( buttonHighScores.area() ) ) {
            fheroes::showStandardTextMessage( _( "High Scores" ), _( "View the high scores screen." ), Dialog::ZERO );
        }
            ...
            {
                fheroes::showStandardTextMessage( _( "Editor" ), _( "Create new or modify existing maps." ), Dialog::ZERO );
            }
        }
        ...
        if ( validateAnimationDelay( MAIN_MENU_DELAY ) ) {
            const fheroes::Sprite & lantern12 = fheroes::AGG::GetICN( ICN::SHNGANIM, ICN::getAnimatedIcnIndex( ICN::SHNGANIM, 0, lantern_frame ) );
            ++lantern_frame;
            fheroes::Blit( lantern12, display, lantern12.x(), lantern12.y() );
            if ( le.isMouseCursorPosInArea( settingsArea ) ) {
                const int32_t doorOffsetY = static_cast<int32_t>( 55 * scale ) + offsetY;
                fheroes::Blit( highlightDoor, 0, doorOffsetY, display, highlightDoor.x(), highlightDoor.y() + doorOffsetY, highlightDoor.width(),
