...
namespace
{
    ...
    const fheroes::Size defaultSpecialButtonSize{ 54, defaultButtonHeight };
    const fheroes::Size spacebarButtonSize{ 175, defaultButtonHeight };
    ...
    const fheroes::Point buttonShadowOffset{ -5, 5 };
    const fheroes::Point offsetFromWindowBorders{ 25, 50 };
    const fheroes::Size inputAreaSize{ 268, 21 };
    const int32_t inputAreaBorders{ 2 };
    const int32_t inputAreaOffsetFromWindowTop{ 20 };

    fheroes::SupportedLanguage lastSelectedLanguage{ fheroes::SupportedLanguage::English };

    bool isSupportedForLanguageSwitching( const fheroes::SupportedLanguage language )
    {
        switch ( language ) {
        case fheroes::SupportedLanguage::English:
            // English is a default language so it is not considered as an extra language.
            return false;
        case fheroes::SupportedLanguage::Belarusian:
        case fheroes::SupportedLanguage::Czech:
        case fheroes::SupportedLanguage::Danish:
        case fheroes::SupportedLanguage::French:
        case fheroes::SupportedLanguage::German:
        case fheroes::SupportedLanguage::Polish:
        case fheroes::SupportedLanguage::Russian:
        case fheroes::SupportedLanguage::Slovak:
        case fheroes::SupportedLanguage::Ukrainian:
            return true;
        default:
            break;
        }
    ...
    {
    public:
        KeyboardRenderer( fheroes::Display & output, std::string & info, const size_t lengthLimit, const bool evilInterface )
            : _output( output )
            , _info( info )
        ...
        fheroes::Rect getWindowRoi() const
        {
            if ( !_window ) {
        ...
        const fheroes::Rect & getTextRoi() const
        {
        ...
        bool resize( const fheroes::Size & size )
        {
            ...
            const fheroes::Point defaultOffset{ ( _output.width() - size.width ) / 2, ( _output.height() - defaultWindowHeight ) / 2 };
            const fheroes::Point offset{ defaultOffset.x, defaultOffset.y - ( size.height - defaultWindowHeight ) };

            // It is important to destroy the previous window to avoid rendering issues.
            _textUI.reset();
            _window.reset();

            _window = std::make_unique<fheroes::StandardWindow>( offset.x, offset.y, size.width, size.height, true, _output );

            _window->render();

            const fheroes::Rect & windowRoi = _window->activeArea();
            ...
            const fheroes::Sprite & initialWindow = fheroes::AGG::GetICN( ICN::REQBKG, 0 );
            fheroes::Copy( initialWindow, 40, 286, _output, _textInputArea );

            if ( _isEvilInterface ) {
                fheroes::ApplyPalette( _output, _textInputArea.x, _textInputArea.y, _output, _textInputArea.x, _textInputArea.y, _textInputArea.width,
                                        _textInputArea.height, PAL::GetPalette( PAL::PaletteType::GOOD_TO_EVIL_INTERFACE ) );
            }

            _textUI
                = std::make_unique<fheroes::TextInputField>( fheroes::Rect{ _textInputArea.x + inputAreaBorders, _textInputArea.y + inputAreaBorders,
        ...
        void setCursorPosition( const fheroes::Point & clickPosition )
        {
            if ( _textUI ) {
    ...
    private:
        fheroes::Display & _output;
        std::string & _info;
        size_t _lengthLimit{ 255 };
        std::unique_ptr<fheroes::StandardWindow> _window;
        std::unique_ptr<fheroes::TextInputField> _textUI;
        size_t _cursorPosition{ 0 };
        fheroes::Rect _textInputArea{ 0, 0, inputAreaSize.width, inputAreaSize.height };
    ...
    struct KeyboardButton
    {
        ...
        KeyboardButton( std::string input, const fheroes::Key kbKey, const fheroes::Size & buttonSize, const bool isEvilInterface,
            ...
            , action( std::move( actionEvent ) )
        {
            ...
            buttonShadow.reset();
            fheroes::addGradientShadow( released, buttonShadow, { -std::min( 0, buttonShadowOffset.x ), -std::min( 0, buttonShadowOffset.y ) }, buttonShadowOffset );
        }

        KeyboardButton( std::string input, const fheroes::Size & buttonSize, const bool isEvilInterface, std::function<DialogAction( KeyboardRenderer & )> actionEvent )
            : KeyboardButton( std::move( input ), fheroes::Key::NONE, buttonSize, isEvilInterface, std::move( actionEvent ) )
        {
            // Do nothing.
        }
        ...
        fheroes::ButtonSprite button;
        fheroes::Image buttonShadow;

        // This is used only for buttons which should have pressed state for some layouts.
        bool isInvertedRenderingLogic{ false };
    };

    std::vector<std::string> getAlphaNumericCharacterLayout( const fheroes::SupportedLanguage language )
    {
    ...
    std::vector<std::string> getCapitalCharacterLayout( const fheroes::SupportedLanguage language )
    {
        switch ( language ) {
        case fheroes::SupportedLanguage::Belarusian:
    ...
    std::vector<std::string> getNonCapitalCharacterLayout( const fheroes::SupportedLanguage language )
    {
        switch ( language ) {
        case fheroes::SupportedLanguage::Belarusian:
    ...
    void getCharacterLayout( const LayoutType layoutType, const fheroes::SupportedLanguage language, std::vector<std::string> & buttonLetters,
    ...
    fheroes::Size getDefaultButtonSize( const fheroes::SupportedLanguage language )
    {
    ...
    std::vector<std::vector<KeyboardButton>> generateButtons( const std::vector<std::string> & buttonLetters, const std::vector<std::string> & returnLetters,
                                                              const LayoutType layoutType, const fheroes::SupportedLanguage language, const bool isEvilInterface )
    {
        ...
        const auto getKeyForCharacter = [layoutType]( const char ch ) {
            ...
            switch ( ch ) {
            case '0':
                return fheroes::Key::KEY_0;
            case '1':
                return fheroes::Key::KEY_1;
            case '2':
                return fheroes::Key::KEY_2;
            case '3':
                return fheroes::Key::KEY_3;
            case '4':
                return fheroes::Key::KEY_4;
            case '5':
                return fheroes::Key::KEY_5;
            case '6':
                return fheroes::Key::KEY_6;
            case '7':
                return fheroes::Key::KEY_7;
            case '8':
                return fheroes::Key::KEY_8;
            case '9':
                return fheroes::Key::KEY_9;
            default:
                break;
            }
        ...
        const fheroes::Size buttonSize
            = ( layoutType == LayoutType::AlphaNumeric ) ? getDefaultButtonSize( fheroes::SupportedLanguage::English ) : getDefaultButtonSize( language );
                                  ...
                                  const bool isExtraLanguageSupported )
    {
        ...
        case LayoutType::SignedNumeric:
            ...
            lastButtonRow.emplace_back( "-", fheroes::Key::KEY_MINUS, getDefaultButtonSize( fheroes::SupportedLanguage::English ), isEvilInterface,
                                        []( KeyboardRenderer & renderer ) {
                                            renderer.swapSign();
                                            return DialogAction::DoNothing;
                                        } );

            lastButtonRow.emplace_back( "0", fheroes::Key::KEY_0, getDefaultButtonSize( fheroes::SupportedLanguage::English ), isEvilInterface,
                                        []( KeyboardRenderer & renderer ) {
                                            renderer.insertCharacter( '0' );
                                            return DialogAction::DoNothing;
                                        } );

            lastButtonRow.emplace_back( "\x7F", getDefaultButtonSize( fheroes::SupportedLanguage::English ), isEvilInterface,
                                        []( const KeyboardRenderer & ) { return DialogAction::DoNothing; } );
            lastButtonRow.back().button.hide();

            lastButtonRow.emplace_back( "~", fheroes::Key::KEY_BACKSPACE, defaultSpecialButtonSize, isEvilInterface, []( KeyboardRenderer & renderer ) {
                renderer.removeCharacter();
                return DialogAction::DoNothing;
            } );
            break;
        case LayoutType::UnsignedNumeric:
            ...
            lastButtonRow.emplace_back( "-", getDefaultButtonSize( fheroes::SupportedLanguage::English ), isEvilInterface,
                                        []( const KeyboardRenderer & ) { return DialogAction::DoNothing; } );
            lastButtonRow.back().button.hide();
    ...
    void addExtraButtons( std::vector<std::vector<KeyboardButton>> & buttons, const LayoutType layoutType, const fheroes::SupportedLanguage language,
    ...
    fheroes::Rect getButtonsRoi( const std::vector<std::vector<KeyboardButton>> & buttonLayout, const fheroes::Point & offset, const int32_t windowWidth )
    {
        ...
        fheroes::Rect roi{ offset.x + offsets.front(), offset.y, 1, 1 };
    ...
    void renderButtons( std::vector<std::vector<KeyboardButton>> & buttonLayout, const fheroes::Point & offset, fheroes::Image & output, const int32_t windowWidth )
    {
        std::vector<int32_t> offsets;
        ...
        for ( size_t i = 0; i < buttonLayout.size(); ++i ) {
            ...
            for ( auto & buttonInfo : buttonLayout[i] ) {
                ...
                if ( buttonInfo.button.draw( output ) ) {
                    fheroes::Blit( buttonInfo.buttonShadow, output, xOffset + buttonShadowOffset.x, yOffset );
                }
                xOffset += buttonInfo.button.area().width + buttonOffset;
            }
    ...
    {
        const fheroes::Key key = [&le]() {
            if ( !le.isAnyKeyPressed() ) {
                return fheroes::Key::NONE;
            }

            const fheroes::Key keyValue = le.getPressedKeyValue();

            // Convert the numpad keys to regular keys if needed
            switch ( keyValue ) {
            case fheroes::Key::KEY_KP_MINUS:
                return fheroes::Key::KEY_MINUS;
        ...
        for ( const auto & buttonRow : buttonLayout ) {
            for ( const auto & buttonInfo : buttonRow ) {
                ...
                if ( le.MouseClickLeft( buttonInfo.button.area() ) || ( key != fheroes::Key::NONE && buttonInfo.key == key ) ) {
    ...
    DialogAction processVirtualKeyboardEvent( const LayoutType layoutType, const fheroes::SupportedLanguage language, const bool isExtraLanguageSupported,
                                              KeyboardRenderer & renderer )
    {
        ...
        const fheroes::Rect windowRoi{ renderer.getWindowRoi() };
        const fheroes::Rect buttonsRoi = getButtonsRoi( buttons, windowRoi.getPosition() + offsetFromWindowBorders, windowWidth );
        const fheroes::Rect & textRoi = renderer.getTextRoi();

        fheroes::Display & display = fheroes::Display::instance();
        const fheroes::ImageRestorer restorer( display, buttonsRoi.x, buttonsRoi.y, buttonsRoi.width, buttonsRoi.height );
        ...
        const fheroes::Sprite & okayButtonReleasedImage = fheroes::AGG::GetICN( buttonIcnId, 0 );
        const fheroes::Sprite & okayButtonPressedImage = fheroes::AGG::GetICN( buttonIcnId, 1 );

        const fheroes::Point okayButtonPosition{ windowRoi.x + ( windowRoi.width - okayButtonReleasedImage.width() ) / 2, windowRoi.y + windowRoi.height - 35 };

        fheroes::ButtonSprite okayButton( okayButtonPosition.x, okayButtonPosition.y, okayButtonReleasedImage, okayButtonPressedImage );

        // Render OKAY button and its shadow only if the keyboard dialog was resized.
        if ( isResized ) {
            okayButton.draw();
            fheroes::addGradientShadow( okayButtonReleasedImage, display, okayButtonPosition, buttonShadowOffset );
        }
