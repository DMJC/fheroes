...
namespace
{
    ...
    std::string buildingStatusMessage( const int race, const uint32_t buildingId )
    {
        ...
        if ( ( buildingId & DWELLING_MONSTERS ) == 0 && ( buildingId & DWELLING_UPGRADES ) == 0 ) {
            return fheroes::getBuildingName( race, static_cast<BuildingType>( buildingId ) );
        }
    ...
    void RedrawIcons( const Castle & castle, const Heroes * hero, const fheroes::Point & pt )
    {
        fheroes::Display & display = fheroes::Display::instance();

        fheroes::Blit( fheroes::AGG::GetICN( ICN::STRIP, 0 ), display, pt.x, pt.y + 256 );

        if ( castle.isBuild( BUILD_CAPTAIN ) ) {
            const fheroes::Sprite & captainImage = castle.GetCaptain().GetPortrait( PORT_BIG );
            fheroes::Copy( captainImage, 0, 0, display, pt.x + 5, pt.y + 262, captainImage.width(), captainImage.height() );
        }
        else {
            const fheroes::Sprite & crestImage = fheroes::AGG::GetICN( ICN::CREST, Color::GetIndex( castle.GetColor() ) );
            fheroes::Copy( crestImage, 0, 0, display, pt.x + 5, pt.y + 262, crestImage.width(), crestImage.height() );
        }
        ...
        if ( hero ) {
            ...
            const fheroes::Sprite & heroImage = fheroes::AGG::GetICN( ICN::STRIP, 3 );
            const fheroes::Sprite & backgroundImage = fheroes::AGG::GetICN( ICN::STRIP, 11 );
            fheroes::Copy( heroImage, 0, 0, display, pt.x + 5, pt.y + 361, heroImage.width(), heroImage.height() );
            fheroes::Copy( backgroundImage, 0, 0, display, pt.x + 112, pt.y + 361, backgroundImage.width(), backgroundImage.height() );
        }
    }
    ...
    void generateHeroImage( fheroes::Image & image, Heroes * hero )
    {
        assert( hero != nullptr );
        ...
        fheroes::Blit( fheroes::AGG::GetICN( ICN::STRIP, 0 ), 0, 100, image, 0, 0, 552, 107 );
        const fheroes::Sprite & port = hero->GetPortrait( PORT_BIG );
        fheroes::Copy( port, 0, 0, image, 5, 5, port.width(), port.height() );
...
Castle::CastleDialogReturnValue Castle::OpenDialog( const bool openConstructionWindow, const bool fade, const bool renderBackgroundDialog )
{
    ...
    fheroes::Point statusBarPosition( dialogRoi.x, dialogRoi.y + 480 - 19 );

    fheroes::Button buttonPrevCastle( statusBarPosition.x, statusBarPosition.y, ICN::SMALLBAR, 1, 2 );
    ...
    fheroes::Copy( bar, 0, 0, display, statusBarPosition.x, statusBarPosition.y, bar.width(), bar.height() );
    ...
    // Next castle button.
    fheroes::Button buttonNextCastle( statusBarPosition.x + bar.width(), statusBarPosition.y, ICN::SMALLBAR, 3, 4 );
    ...
    const fheroes::Sprite & crest = fheroes::AGG::GetICN( ICN::CREST, Color::GetIndex( GetColor() ) );
    const fheroes::Rect rectSign1( dialogRoi.x + 5, dialogRoi.y + 262, crest.width(), crest.height() );
    const fheroes::Rect rectSign2( rectSign1.x, dialogRoi.y + 361, 100, 92 );
    ...
    auto setArmyBarParameters = []( ArmyBar & armyBar, const fheroes::Point & offset ) {
        armyBar.setTableSize( { 5, 1 } );
    ...
    // Exit button.
    fheroes::Button buttonExit( dialogRoi.x + 553, dialogRoi.y + 428, ICN::BUTTON_EXIT_TOWN, 0, 1 );

    // Kingdom resources.
    const fheroes::Rect & rectResource = fheroes::drawResourcePanel( GetKingdom().GetFunds(), display, dialogRoi.getPosition() );
    ...
    while ( le.HandleEvents() && result == CastleDialogReturnValue::DoNothing ) {
        ...
        if ( alphaHero >= 255 && fadeBuilding.isFadeDone() ) {
            ...
            if ( le.MouseClickLeft( resActiveArea ) ) {
                ...
                fheroes::showKingdomIncome( GetKingdom(), Dialog::OK );
            }
            else if ( le.isMouseRightButtonPressedInArea( resActiveArea ) ) {
                fheroes::showKingdomIncome( GetKingdom(), 0 );
            }
                ...
                fheroes::showStandardTextMessage( _( "Show next town" ), _( "Click to show next town." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonPrevCastle.area() ) ) {
                fheroes::showStandardTextMessage( _( "Show previous town" ), _( "Click to show previous town." ), Dialog::ZERO );
            }
                ...
                display.render( fheroes::getBoundaryRect( topArmyBar.GetArea(), bottomArmyBar.GetArea() ) );
            }
            ...
            for ( auto it = cacheBuildings.crbegin(); it != cacheBuildings.crend(); ++it ) {
                ...
                if ( le.isMouseRightButtonPressedInArea( it->coord ) ) {
                    ...
                    if ( isMonsterDwelling ) {
                        ...
                        fheroes::showStandardTextMessage( GetStringBuilding( it->id ), GetDescriptionBuilding( it->id ), Dialog::ZERO );
                    }
                ...
                if ( le.MouseClickLeft( it->coord ) || hotKeyBuilding == it->id || ( isMagicGuild && hotKeyBuilding == BUILD_MAGEGUILD ) ) {
                    ...
                    if ( isMagicGuild ) {
                        ...
                        case BUILD_SHRINE: {
                            const fheroes::ButtonRestorer exitRestorer( buttonExit );
                            fheroes::showStandardTextMessage( GetStringBuilding( it->id ), GetDescriptionBuilding( it->id ), Dialog::OK );
                            break;
                        }
                        ...
                        case BUILD_TENT: {
                            ...
                            if ( isBuildingDisabled( BUILD_CASTLE ) ) {
                                fheroes::showStandardTextMessage( _( "Town" ), _( "This town may not be upgraded to a castle." ), Dialog::OK );
                                break;
                            }
        ...
        if ( alphaHero < 255 && Game::validateAnimationDelay( Game::CASTLE_BUYHERO_DELAY ) ) {
            ...
            fheroes::AlphaBlit( surfaceHero, display, dialogRoi.x, dialogRoi.y + 356, static_cast<uint8_t>( alphaHero ) );
        ...
        if ( needRedraw ) {
            ...
            fheroes::drawResourcePanel( GetKingdom().GetFunds(), display, dialogRoi.getPosition() );
        ...
        if ( fadeBuilding.isFadeDone() ) {
            ...
            if ( build != BUILD_NOTHING ) {
                if ( BUILD_CAPTAIN == build ) {
                    ...
                    fheroes::drawCastleName( *this, fheroes::Display::instance(), dialogRoi.getPosition() );
                }
            ...
            const fheroes::Sprite & crestImage = fheroes::AGG::GetICN( ICN::CREST, Color::GetIndex( GetColor() ) );
            fheroes::Copy( crestImage, 0, 0, display, rectSign1.x, rectSign1.y, crestImage.width(), crestImage.height() );
            const fheroes::Sprite & captainImage = GetCaptain().GetPortrait( PORT_BIG );
            fheroes::AlphaBlit( captainImage, display, rectSign1.x, rectSign1.y, fadeBuilding.getAlpha() );
        }
