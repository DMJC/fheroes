...
namespace
{
    const std::string highScoreFileName = "fheroes.hgs";
    fheroes::HighScoreDataContainer highScoreDataContainer;
    ...
    fheroes::Rect getAnimationRoi( const fheroes::Point & position, const bool isCampaign )
    {
        fheroes::Rect roi;
        std::vector<uint32_t> animationIndex;

        if ( isCampaign ) {
            const std::vector<fheroes::HighscoreData> & highScoreData = highScoreDataContainer.getHighScoresCampaign();

            animationIndex.reserve( highScoreData.size() );

            for ( const fheroes::HighscoreData & data : highScoreData ) {
                animationIndex.push_back( fheroes::HighScoreDataContainer::getMonsterByDay( data.rating ).GetSpriteIndex() * 9 );
            }
        }
        else {
            const std::vector<fheroes::HighscoreData> & highScoreData = highScoreDataContainer.getHighScoresStandard();

            animationIndex.reserve( highScoreData.size() );

            for ( const fheroes::HighscoreData & data : highScoreData ) {
                animationIndex.push_back( fheroes::HighScoreDataContainer::getMonsterByRating( data.rating ).GetSpriteIndex() * 9 );
            }
        }

        // Animation frames: 0 - static part, 1-6 - animation, 7 and 8 - attacking sprite. We analyze and render here only 0-6 frames.
        for ( uint32_t i = 0; i < 7; ++i ) {
            const fheroes::Sprite & topMonsterSprite = fheroes::AGG::GetICN( ICN::MINIMON, animationIndex.front() + i );
            const fheroes::Sprite & bottomMonsterSprite = fheroes::AGG::GetICN( ICN::MINIMON, animationIndex.back() + i );
            ...
            for ( const uint32_t index : animationIndex ) {
                const fheroes::Sprite & monsterSprite = fheroes::AGG::GetICN( ICN::MINIMON, index + i );
    ...
    void redrawMonstersAnimation( const fheroes::Point & position, uint32_t & monsterAnimationFrameId, const std::vector<fheroes::HighscoreData> & highScores,
                                  const std::function<Monster( size_t )> & getMonster )
    {
        ...
        for ( const fheroes::HighscoreData & data : highScores ) {
            ...
            const fheroes::Sprite & secondaryMonsterSprite = fheroes::AGG::GetICN( ICN::MINIMON, secondaryMonsterAnimationIndex );
            fheroes::Blit( secondaryMonsterSprite, display, secondaryMonsterSprite.x() + position.x + monsterOffsetX,
    ...
    void redrawHighScoreAnimation( const fheroes::Point & position, uint32_t & monsterAnimationFrameId, const bool isCampaign )
    {
        if ( isCampaign ) {
            redrawMonstersAnimation( position, monsterAnimationFrameId, highScoreDataContainer.getHighScoresCampaign(),
                                     fheroes::HighScoreDataContainer::getMonsterByDay );
        }
        else {
            redrawMonstersAnimation( position, monsterAnimationFrameId, highScoreDataContainer.getHighScoresStandard(),
                                     fheroes::HighScoreDataContainer::getMonsterByRating );
        }
    }

    void redrawHighScoreScreen( const fheroes::Point & position, const int32_t selectedScoreIndex, const std::vector<fheroes::HighscoreData> & highScores,
                                const uint32_t titleImageIndex, const std::function<Monster( size_t )> & getMonster )
    {
        fheroes::Display & display = fheroes::Display::instance();

        // Draw background.
        const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::HSBKG, 0 );
        fheroes::Copy( background, 0, 0, display, position.x, position.y, background.width(), background.height() );
        fheroes::Blit( fheroes::AGG::GetICN( ICN::HISCORE, titleImageIndex ), display, position.x + 50, position.y + 31 );

        fheroes::Text text( "", fheroes::FontType::normalWhite() );

        int32_t scoreIndex = 0;
        int32_t offsetY = position.y;

        for ( const fheroes::HighscoreData & data : highScores ) {
            const fheroes::FontType font = ( scoreIndex == selectedScoreIndex ) ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite();

            // TODO: scenario name can have its own independent language.
            const fheroes::SupportedLanguage language = fheroes::getLanguageFromAbbreviation( data.languageAbbreviation );
            ...
            const fheroes::Sprite & baseMonsterSprite = fheroes::AGG::GetICN( ICN::MINIMON, baseMonsterAnimationIndex );
            fheroes::Blit( baseMonsterSprite, display, baseMonsterSprite.x() + position.x + 554, baseMonsterSprite.y() + offsetY + 91 );
...
fheroes::GameMode Game::DisplayHighScores( const bool isCampaign )
{
    ...
    if ( IS_DEVEL() && ( gameResult.GetResult() & GameOver::WINS ) ) {
        ...
        fheroes::showStandardTextMessage( _( "High Scores" ), std::move( msg ), Dialog::OK );
    ...
    if ( isAfterGameCompletion ) {
        const auto inputPlayerName = []( std::string & playerName ) {
            Dialog::inputString( fheroes::Text{}, fheroes::Text{ _( "Your Name" ), fheroes::FontType::normalWhite() }, playerName, 15, false, {} );
            if ( playerName.empty() ) {
                playerName = _( "Unknown Hero" );
            }
        };

        const uint32_t completionTime = fheroes::HighscoreData::generateCompletionTime();
        std::string lang = fheroes::getLanguageAbbreviation( fheroes::getCurrentLanguage() );
    ...
    const fheroes::Sprite & back = fheroes::AGG::GetICN( ICN::HSBKG, 0 );

    const fheroes::Point top{ ( display.width() - back.width() ) / 2, ( display.height() - back.height() ) / 2 };
    const fheroes::StandardWindow border( display.DEFAULT_WIDTH, display.DEFAULT_HEIGHT, false );

    uint32_t monsterAnimationFrameId = 0;
    if ( isCampaign ) {
        redrawHighScoreScreen( top, selectedEntryIndex, highScoreDataContainer.getHighScoresCampaign(), 7, fheroes::HighScoreDataContainer::getMonsterByDay );
    }
    else {
        redrawHighScoreScreen( top, selectedEntryIndex, highScoreDataContainer.getHighScoresStandard(), 6, fheroes::HighScoreDataContainer::getMonsterByRating );
    }

    const fheroes::Rect animationRoi = getAnimationRoi( top, isCampaign );

    // Make a copy of static monsters animation part with background to restore it during animations.
    fheroes::ImageRestorer backgroundCopy( display, animationRoi.x, animationRoi.y, animationRoi.width, animationRoi.height );

    // Render first animation frame.
    redrawHighScoreAnimation( top, monsterAnimationFrameId, isCampaign );

    fheroes::Button buttonOtherHighScore( top.x + 8, top.y + 315, isCampaign ? ICN::BUTTON_HSCORES_VERTICAL_CAMPAIGN : ICN::BUTTON_HSCORES_VERTICAL_STANDARD, 0, 1 );
    fheroes::Button buttonExit( top.x + back.width() - 36, top.y + 315, ICN::BUTTON_HSCORES_VERTICAL_EXIT, 0, 1 );
    ...
    if ( isAfterGameCompletion ) {
        ...
        fheroes::fadeInDisplay( border.activeArea(), !isDefaultScreenSize );
    }
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( { Game::MAPS_DELAY } ) ) ) {
        ...
        if ( le.MouseClickLeft( buttonOtherHighScore.area() ) ) {
            return isCampaign ? fheroes::GameMode::HIGHSCORES_STANDARD : fheroes::GameMode::HIGHSCORES_CAMPAIGN;
        }
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonExit.area() ) ) {
            ...
            if ( isCampaign ) {
                fheroes::showStandardTextMessage( _( "Standard" ), _( "View High Scores for Standard Maps." ), Dialog::ZERO );
            }
            else {
                fheroes::showStandardTextMessage( _( "Campaign" ), _( "View High Scores for Campaigns." ), Dialog::ZERO );
            }
        }
