...
namespace
{
    ...
    const fheroes::Sprite & getChar( const uint8_t character, const fheroes::FontType & fontType )
    {
        if ( character < 0x21 ) {
            return errorImage;
        }
        ...
        case fheroes::FontSize::SMALL:
            switch ( fontType.color ) {
            case fheroes::FontColor::WHITE:
                return fheroes::AGG::GetICN( ICN::SMALFONT, character - 0x20 );
            case fheroes::FontColor::GRAY:
                return fheroes::AGG::GetICN( ICN::GRAY_SMALL_FONT, character - 0x20 );
            case fheroes::FontColor::YELLOW:
                return fheroes::AGG::GetICN( ICN::YELLOW_SMALLFONT, character - 0x20 );
            default:
            ...
            break;
        case fheroes::FontSize::NORMAL:
            switch ( fontType.color ) {
            case fheroes::FontColor::WHITE:
                return fheroes::AGG::GetICN( ICN::FONT, character - 0x20 );
            case fheroes::FontColor::GRAY:
                return fheroes::AGG::GetICN( ICN::GRAY_FONT, character - 0x20 );
            case fheroes::FontColor::YELLOW:
                return fheroes::AGG::GetICN( ICN::YELLOW_FONT, character - 0x20 );
            case fheroes::FontColor::GOLDEN_GRADIENT:
                return fheroes::AGG::GetICN( ICN::GOLDEN_GRADIENT_FONT, character - 0x20 );
            case fheroes::FontColor::SILVER_GRADIENT:
                return fheroes::AGG::GetICN( ICN::SILVER_GRADIENT_FONT, character - 0x20 );
            default:
                // Did you add a new font color? Add the corresponding logic for it!
                assert( 0 );
                break;
            ...
            break;
        case fheroes::FontSize::LARGE:
            switch ( fontType.color ) {
            case fheroes::FontColor::WHITE:
                return fheroes::AGG::GetICN( ICN::WHITE_LARGE_FONT, character - 0x20 );
            case fheroes::FontColor::GOLDEN_GRADIENT:
                return fheroes::AGG::GetICN( ICN::GOLDEN_GRADIENT_LARGE_FONT, character - 0x20 );
            case fheroes::FontColor::SILVER_GRADIENT:
                return fheroes::AGG::GetICN( ICN::SILVER_GRADIENT_LARGE_FONT, character - 0x20 );
            default:
                // Did you add a new font color? Add the corresponding logic for it!
                assert( 0 );
                break;
            ...
            break;
        case fheroes::FontSize::BUTTON_RELEASED:
            switch ( fontType.color ) {
            case fheroes::FontColor::WHITE:
                return fheroes::AGG::GetICN( ICN::BUTTON_GOOD_FONT_RELEASED, character - 0x20 );
            case fheroes::FontColor::GRAY:
                return fheroes::AGG::GetICN( ICN::BUTTON_EVIL_FONT_RELEASED, character - 0x20 );
            default:
                // Did you add a new font color? Add the corresponding logic for it!
                assert( 0 );
                break;
            ...
            break;
        case fheroes::FontSize::BUTTON_PRESSED:
            switch ( fontType.color ) {
            case fheroes::FontColor::WHITE:
                return fheroes::AGG::GetICN( ICN::BUTTON_GOOD_FONT_PRESSED, character - 0x20 );
            case fheroes::FontColor::GRAY:
                return fheroes::AGG::GetICN( ICN::BUTTON_EVIL_FONT_PRESSED, character - 0x20 );
            default:
                // Did you add a new font color? Add the corresponding logic for it!
                assert( 0 );
                break;
    ...
    uint32_t getCharacterLimit( const fheroes::FontSize fontSize )
    {
        switch ( fontSize ) {
        case fheroes::FontSize::SMALL:
            return fheroes::AGG::GetICNCount( ICN::SMALFONT ) + 0x20 - 1;
        case fheroes::FontSize::NORMAL:
        case fheroes::FontSize::LARGE:
            return fheroes::AGG::GetICNCount( ICN::FONT ) + 0x20 - 1;
        case fheroes::FontSize::BUTTON_RELEASED:
        case fheroes::FontSize::BUTTON_PRESSED:
            return fheroes::AGG::GetICNCount( ICN::BUTTON_GOOD_FONT_RELEASED ) + 0x20 - 1;
        default:
    ...
    int32_t getLineWidth( const uint8_t * data, const int32_t size, const fheroes::FontCharHandler & charHandler, const bool keepTrailingSpaces )
    {
    ...
    fheroes::Rect getTextLineArea( const uint8_t * data, const int32_t size, const fheroes::FontCharHandler & charHandler )
    {
        assert( data != nullptr && size > 0 );

        const uint8_t * dataEnd = data + size;

        const fheroes::Sprite & firstCharSprite = charHandler.getSprite( *data );
        fheroes::Rect area( firstCharSprite.x(), firstCharSprite.y(), firstCharSprite.width(), firstCharSprite.height() );
        ++data;

        for ( ; data != dataEnd; ++data ) {
            const fheroes::Sprite & sprite = charHandler.getSprite( *data );
    ...
    int32_t getMaxCharacterCount( const uint8_t * data, const int32_t size, const fheroes::FontCharHandler & charHandler, const int32_t maxWidth )
    {
    ...
    int32_t renderSingleLine( const uint8_t * data, const int32_t size, const int32_t x, const int32_t y, fheroes::Image & output, const fheroes::Rect & imageRoi,
                              const fheroes::FontCharHandler & charHandler )
    {
        ...
        for ( ; data != dataEnd; ++data ) {
            ...
            const fheroes::Sprite & charSprite = charHandler.getSprite( *data );
            assert( !charSprite.empty() );

            const fheroes::Rect charRoi{ offsetX + charSprite.x(), y + charSprite.y(), charSprite.width(), charSprite.height() };

            const fheroes::Rect overlappedRoi = imageRoi ^ charRoi;

            fheroes::Blit( charSprite, overlappedRoi.x - charRoi.x, overlappedRoi.y - charRoi.y, output, overlappedRoi.x, overlappedRoi.y, overlappedRoi.width,
    ...
    int32_t getMaxWordWidth( const uint8_t * data, const int32_t size, const fheroes::FontType fontType )
    {
        assert( data != nullptr && size > 0 );

        const fheroes::FontCharHandler charHandler( fontType );
    ...
    std::unique_ptr<fheroes::LanguageSwitcher> getLanguageSwitcher( const fheroes::TextBase & text )
    {
        ...
        return std::make_unique<fheroes::LanguageSwitcher>( language.value() );
    }
}
...
namespace fheroes
{
    ...
    size_t TextInput::getCursorPositionInAdjacentLine( const size_t currentPos, const int32_t maxWidth, const bool moveUp )
    {
        ...
        const fheroes::FontCharHandler charHandler( _fontType );
