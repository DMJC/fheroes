                                                          ...
                                                          const Battle::Units & enemies, bool retreating ) const
{
    ...
    const uint32_t spellDamage = fheroes::getSpellDamage( spell, _commander->GetPower(), _commander );
...
int32_t AI::BattlePlanner::spellDurationMultiplier( const Battle::Unit & target ) const
{
    const int32_t duration
        = _commander->GetPower() + _commander->GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::EVERY_COMBAT_SPELL_DURATION );
...
{
    const uint32_t hpRestored = fheroes::getResurrectPoints( spell, _commander->GetPower(), _commander );
...
AI::SpellcastOutcome AI::BattlePlanner::spellSummonValue( const Spell & spell, const Battle::Arena & arena, const PlayerColor heroColor ) const
{
    ...
    const Troop summon( Monster( spell ), fheroes::getSummonMonsterCount( spell, _commander->GetPower(), _commander ) );
