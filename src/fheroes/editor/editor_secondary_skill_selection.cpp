...
namespace
{
    ...
    {
    public:
        SecondarySkillContainerUI( fheroes::Point offset, std::vector<std::pair<Skill::Secondary, bool>> & skills )
            : _skills( skills )
        {
            ...
            offset.x += ( fheroes::Display::DEFAULT_WIDTH - static_cast<int32_t>( _skillsPerRow ) * skillItemWidth ) / 2;
            offset.y += ( fheroes::Display::DEFAULT_HEIGHT - 50 - skillRowOffsetY * static_cast<int32_t>( _skills.size() / _skillsPerRow ) ) / 2;

            // Calculate all areas where we are going to render skills.
            _skillRoi.reserve( _skills.size() );

            const fheroes::Sprite & frameImage = fheroes::AGG::GetICN( ICN::SECSKILL, 15 );
        ...
        void draw( fheroes::Image & output )
        {
            const fheroes::Sprite & frameImage = fheroes::AGG::GetICN( ICN::SECSKILL, 15 );

            fheroes::Sprite inactiveFrameImage( frameImage );
            fheroes::ApplyPalette( inactiveFrameImage, PAL::GetPalette( PAL::PaletteType::GRAY ) );

            for ( size_t i = 0; i < _skills.size(); ++i ) {
                const Skill::Secondary & skill = _skills[i].first;

                const fheroes::Sprite & skillImage = fheroes::AGG::GetICN( ICN::SECSKILL, skill.GetIndexSprite1() );
                const fheroes::Point skillImagePos( _skillRoi[i].x + 3, _skillRoi[i].y + 3 );

                fheroes::Copy( skillImage, 0, 0, output, skillImagePos.x, skillImagePos.y, skillImage.width(), skillImage.height() );

                if ( _skills[i].second ) {
                    fheroes::Blit( frameImage, output, _skillRoi[i].x, _skillRoi[i].y );
                }
                else {
                    // The skill is being inactive.
                    fheroes::Blit( inactiveFrameImage, output, _skillRoi[i].x, _skillRoi[i].y );
                    fheroes::ApplyPalette( output, skillImagePos.x, skillImagePos.y, output, _skillRoi[i].x + 3, _skillRoi[i].y + 3, skillImage.width(),
        ...
        const fheroes::Rect & redrawChangedSkill( fheroes::Image & output )
        {
            ...
            if ( _skills[_skillToRedraw].second ) {
                ...
                fheroes::Blit( frameImage, output, roi.x, roi.y );
                fheroes::Copy( skillImage, 0, 0, output, skillImageOffsetX, roi.y + 3, skillImage.width(), skillImage.height() );
                ...
                fheroes::ApplyPalette( output, roi.x, roi.y, output, roi.x, roi.y, roi.width, roi.height, PAL::GetPalette( PAL::PaletteType::GRAY ) );
            }
        ...
        bool processEvents( LocalEvent & eventProcessor )
        {
            ...
            const fheroes::Rect skillRoi = _skillRoi[skillIndex];
            ...
            if ( eventProcessor.isMouseRightButtonPressedInArea( skillRoi ) ) {
                ...
                fheroes::SecondarySkillDialogElement( _skills[skillIndex].first, fakeHero ).showPopup( Dialog::ZERO );
            }
    ...
    private:
        ...
        std::vector<fheroes::Rect> _skillRoi;
...
namespace Editor
{
    bool openSecondarySkillSelectionWindow( std::string title, const int skillLevel, std::vector<int32_t> & selectedSkills )
    {
        ...
        const fheroes::Text text( std::move( title ), fheroes::FontType::normalYellow() );
        ...
        fheroes::Button buttonToggleOn;
        fheroes::Button buttonToggleOff;
        ...
        while ( le.HandleEvents() ) {
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
                ...
                fheroes::showStandardTextMessage( _( "Enable All Secondary Skills" ), _( "Click to enable all secondary skills." ), Dialog::ZERO );
            }
            else if ( buttonToggleOff.isEnabled() && le.isMouseRightButtonPressedInArea( buttonToggleOff.area() ) ) {
                fheroes::showStandardTextMessage( _( "Disable All Secondary Skills" ), _( "Click to disable all secondary skills." ), Dialog::ZERO );
            }
