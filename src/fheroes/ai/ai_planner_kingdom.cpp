...
fheroes::GameMode AI::Planner::KingdomTurn( Kingdom & kingdom )
{
    ...
    if ( const auto iter = std::find_if( heroes.begin(), heroes.end(),
        ...
        if ( const fheroes::GameMode gameState = GameOver::Result::Get().checkGameOver(); gameState != fheroes::GameMode::CANCEL ) {
            return gameState;
        }
    }
    ...
    while ( true ) {
        ...
        const fheroes::GameMode gameState = HeroesTurn( heroes, currentProgressValue, endProgressValue, moreTaskForHeroes );
        if ( gameState != fheroes::GameMode::END_TURN ) {
            return gameState;
        }
