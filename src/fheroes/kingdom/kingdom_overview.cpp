...
namespace
{
    ...
    void redrawCommonBackground( const fheroes::Point & dst, const int visibleItems, fheroes::Image & output )
    {
        // Scrollbar background.
        const fheroes::Sprite & scrollbar = fheroes::AGG::GetICN( ICN::OVERVIEW, 13 );
        fheroes::Copy( scrollbar, 0, 0, output, dst.x + scrollbarOffset + 1, dst.y + 17, scrollbar.width(), scrollbar.height() );

        // Items background.
        const fheroes::Sprite & itemsBack = fheroes::AGG::GetICN( ICN::OVERVIEW, 8 );
        const fheroes::Sprite & overback = fheroes::AGG::GetICN( ICN::OVERBACK, 0 );
        ...
        for ( int i = 0; i < visibleItems; ++i, offsetY += stepY, overbackOffsetY += stepY ) {
            fheroes::Copy( itemsBack, 0, 0, output, offsetX, offsetY, itemsBackWidth, itemsBackHeight );
            // Horizontal yellow "grid" lines.
            fheroes::Copy( overback, 29, 13, output, overbackOffsetX, overbackOffsetY, 595, 4 );
        }

        // Copy one vertical line in case of previous army selection.
        fheroes::Copy( overback, 29, 12, output, overbackOffsetX, dst.y + 12, 1, 357 );
    }
    ...
    struct HeroRow
    {
        ...
        void Init( Heroes * ptr )
        {
            ...
            armyBar->SetBackground( { 41, 53 }, fheroes::GetColorId( 72, 28, 0 ) );
    ...
    {
    public:
        StatsHeroesList( const fheroes::Rect & windowArea, const fheroes::Point & offset, const VecHeroes & heroes );
        ...
        void ActionListSingleClick( HeroRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy ) override;
        void ActionListDoubleClick( HeroRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy ) override;
        void ActionListPressRight( HeroRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy ) override;
        bool ActionListCursor( HeroRow & row, const fheroes::Point & cursor ) override;

    private:
        void SetContent( const VecHeroes & heroes );

        std::vector<HeroRow> content;
        const fheroes::Rect _windowArea;
    };

    StatsHeroesList::StatsHeroesList( const fheroes::Rect & windowArea, const fheroes::Point & offset, const VecHeroes & heroes )
        : Interface::ListBox<HeroRow>( offset )
        , _windowArea( windowArea )
    {
        const fheroes::Sprite & back = fheroes::AGG::GetICN( ICN::OVERVIEW, 13 );
        ...
        const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( ICN::SCROLL, 4 );
        const fheroes::Image scrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, false, backHeight - 2, 4, static_cast<int32_t>( heroes.size() ),
    ...
    bool StatsHeroesList::Refresh( VecHeroes & heroes )
    {
        ...
        if ( heroes.size() != contentSize ) {
            const fheroes::Sprite & back = fheroes::AGG::GetICN( ICN::OVERVIEW, 13 );
            const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( ICN::SCROLL, 4 );
            const fheroes::Image scrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, false, back.height() - 2, 4, static_cast<int32_t>( heroes.size() ),
                                                                                       { 0, 0, originalSlider.width(), 8 }, { 0, 7, originalSlider.width(), 8 } );
            setScrollBarImage( scrollbarSlider );
    ...
    void StatsHeroesList::ActionListDoubleClick( HeroRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy )
    {
        ActionListSingleClick( row, cursor, ox, oy );
    }

    void StatsHeroesList::ActionListSingleClick( HeroRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy )
    {
        if ( row.hero && ( fheroes::Rect( ox + 5, oy + 4, Interface::IconsBar::getItemWidth(), Interface::IconsBar::getItemHeight() ) & cursor ) ) {
    ...
    void StatsHeroesList::ActionListPressRight( HeroRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy )
    {
        if ( row.hero && ( fheroes::Rect( ox + 5, oy + 4, Interface::IconsBar::getItemWidth(), Interface::IconsBar::getItemHeight() ) & cursor ) ) {
    ...
    bool StatsHeroesList::ActionListCursor( HeroRow & row, const fheroes::Point & cursor )
    {
        const fheroes::Point cursorPos( cursor.x, cursor.y );
    ...
    void StatsHeroesList::RedrawItem( const HeroRow & row, int32_t dstx, int32_t dsty, bool current )
    {
        ...
        fheroes::Blit( fheroes::AGG::GetICN( ICN::OVERVIEW, 10 ), display, dstx, dsty );
        ...
        fheroes::Text text( std::to_string( row.hero->GetAttack() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX - text.width(), offsetY, display );

        offsetX += 35;
        text.set( std::to_string( row.hero->GetDefense() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX - text.width(), offsetY, display );

        offsetX += 35;
        text.set( std::to_string( row.hero->GetPower() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX - text.width(), offsetY, display );

        offsetX += 35;
        text.set( std::to_string( row.hero->GetKnowledge() ), fheroes::FontType::smallWhite() );
        text.draw( offsetX - text.width(), offsetY, display );
    ...
    void StatsHeroesList::RedrawBackground( const fheroes::Point & dst )
    {
        fheroes::Display & display = fheroes::Display::instance();

        const fheroes::Sprite & header = fheroes::AGG::GetICN( ICN::OVERVIEW, 6 );
        fheroes::Copy( header, 0, 0, display, dst.x + 30, dst.y, header.width(), header.height() );

        const int32_t offsetY = dst.y + 3;

        fheroes::Text text( _( "Hero/Stats" ), fheroes::FontType::smallWhite() );
        text.draw( dst.x + 130 - text.width() / 2, offsetY, display );

        text.set( _( "Skills" ), fheroes::FontType::smallWhite() );
        text.draw( dst.x + 300 - text.width() / 2, offsetY, display );

        text.set( _( "Artifacts" ), fheroes::FontType::smallWhite() );
    ...
    struct CstlRow
    {
        ...
        void Init( Castle * ptr )
        {
            ...
            garrisonArmyBar->SetBackground( { 41, 41 }, fheroes::GetColorId( 40, 12, 0 ) );
            ...
            dwellingsBar = std::make_unique<DwellingsBar>( *castle, fheroes::Size{ 39, 52 } );
        ...
        void updateHeroArmyBar()
        {
            ...
            if ( hero ) {
                ...
                heroArmyBar->SetBackground( { 41, 41 }, fheroes::GetColorId( 40, 12, 0 ) );
    ...
    {
    public:
        StatsCastlesList( const fheroes::Rect & windowArea, const fheroes::Point & offset, const VecCastles & castles );
        ...
        void ActionListSingleClick( CstlRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy ) override;
        void ActionListDoubleClick( CstlRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy ) override;
        void ActionListPressRight( CstlRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy ) override;
        bool ActionListCursor( CstlRow & row, const fheroes::Point & cursor ) override;
    ...
    StatsCastlesList::StatsCastlesList( const fheroes::Rect & windowArea, const fheroes::Point & offset, const VecCastles & castles )
        ...
        , _windowArea( windowArea )
    {
        ...
        const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( ICN::SCROLL, 4 );
        const fheroes::Image scrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, false, back.height() - 2, 4, static_cast<int32_t>( castles.size() ),
                                                                                   { 0, 0, originalSlider.width(), 8 }, { 0, 7, originalSlider.width(), 8 } );
    ...
    void StatsCastlesList::ActionListDoubleClick( CstlRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy )
    {
        ActionListSingleClick( row, cursor, ox, oy );
    }

    void StatsCastlesList::ActionListSingleClick( CstlRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy )
    {
        if ( row.castle ) {
            // click castle icon
            if ( fheroes::Rect( ox + 17, oy + 19, Interface::IconsBar::getItemWidth(), Interface::IconsBar::getItemHeight() ) & cursor ) {
                Game::OpenCastleDialog( *row.castle, false, false );
            }

            // click hero icon
            else if ( fheroes::Rect( ox + 82, oy + 19, Interface::IconsBar::getItemWidth(), Interface::IconsBar::getItemHeight() ) & cursor ) {
    ...
    void StatsCastlesList::ActionListPressRight( CstlRow & row, const fheroes::Point & cursor, int32_t ox, int32_t oy )
    {
        if ( row.castle ) {
            if ( fheroes::Rect( ox + 17, oy + 19, Interface::IconsBar::getItemWidth(), Interface::IconsBar::getItemHeight() ) & cursor ) {
    ...
    bool StatsCastlesList::ActionListCursor( CstlRow & row, const fheroes::Point & cursor )
    {
        const fheroes::Point cursorPos( cursor.x, cursor.y );
    ...
    void StatsCastlesList::RedrawItem( const CstlRow & row, int32_t dstx, int32_t dsty, bool current )
    {
        ...
        fheroes::Blit( fheroes::AGG::GetICN( ICN::OVERVIEW, 11 ), display, dstx, dsty );
        ...
        if ( hero ) {
            ...
            const fheroes::Text text( std::to_string( hero->GetAttack() ) + sep + std::to_string( hero->GetDefense() ) + sep + std::to_string( hero->GetPower() ) + sep
            ...
            const fheroes::Text text( std::to_string( captain.GetAttack() ) + sep + std::to_string( captain.GetDefense() ) + sep + std::to_string( captain.GetPower() )
                                           + sep + std::to_string( captain.GetKnowledge() ),
                                       fheroes::FontType::smallWhite() );
            text.draw( dstx + 104 - text.width() / 2, dsty + 45, display );
        }

        const fheroes::Text text( row.castle->GetName(), fheroes::FontType::smallWhite() );
    ...
    void StatsCastlesList::RedrawBackground( const fheroes::Point & dst )
    {
        fheroes::Display & display = fheroes::Display::instance();

        const fheroes::Sprite & header = fheroes::AGG::GetICN( ICN::OVERVIEW, 7 );
        fheroes::Copy( header, 0, 0, display, dst.x + 30, dst.y, header.width(), header.height() );

        const int32_t offsetY = dst.y + 3;

        fheroes::Text text( _( "Town/Castle" ), fheroes::FontType::smallWhite() );
        text.draw( dst.x + 105 - text.width() / 2, offsetY, display );

        text.set( _( "Garrison" ), fheroes::FontType::smallWhite() );
        text.draw( dst.x + 275 - text.width() / 2, offsetY, display );

        text.set( _( "Available" ), fheroes::FontType::smallWhite() );
        text.draw( dst.x + 500 - text.width() / 2, offsetY, display );

        redrawCommonBackground( dst, VisibleItemCount(), display );
    }

    void RedrawIncomeInfo( const fheroes::Point & pt, const Kingdom & myKingdom )
    {
        ...
        fheroes::Text text( CapturedExtInfoString( Resource::WOOD, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 54 - text.width() / 2, offsetY, display );

        text.set( CapturedExtInfoString( Resource::MERCURY, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 146 - text.width() / 2, offsetY, display );

        text.set( CapturedExtInfoString( Resource::ORE, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 228 - text.width() / 2, offsetY, display );

        text.set( CapturedExtInfoString( Resource::SULFUR, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 294 - text.width() / 2, offsetY, display );

        text.set( CapturedExtInfoString( Resource::CRYSTAL, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 360 - text.width() / 2, offsetY, display );

        text.set( CapturedExtInfoString( Resource::GEMS, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 428 - text.width() / 2, offsetY, display );

        text.set( CapturedExtInfoString( Resource::GOLD, myKingdom.GetColor(), income ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 494 - text.width() / 2, offsetY, display );
    }

    void RedrawFundsInfo( const fheroes::Point & pt, const Kingdom & myKingdom )
    {
        const Funds & funds = myKingdom.GetFunds();
        int32_t offsetY = pt.y + 450;

        fheroes::Display & display = fheroes::Display::instance();
        fheroes::Blit( fheroes::AGG::GetICN( ICN::OVERBACK, 0 ), 4, 422, display, pt.x + 4, pt.y + 422, 530, 56 );

        fheroes::Text text( std::to_string( funds.wood ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 56 - text.width() / 2, offsetY, display );

        text.set( std::to_string( funds.mercury ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 146 - text.width() / 2, offsetY, display );

        text.set( std::to_string( funds.ore ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 226 - text.width() / 2, offsetY, display );

        text.set( std::to_string( funds.sulfur ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 294 - text.width() / 2, offsetY, display );

        text.set( std::to_string( funds.crystal ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 362 - text.width() / 2, offsetY, display );

        text.set( std::to_string( funds.gems ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 428 - text.width() / 2, offsetY, display );

        text.set( std::to_string( funds.gold ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 496 - text.width() / 2, offsetY, display );

        offsetY += 14;
        text.set( _( "Gold Per Day:" ) + std::string( " " ) + std::to_string( myKingdom.GetIncome().Get( Resource::GOLD ) ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 180, offsetY, display );

        std::string msg = _( "Day: %{day}" );
        StringReplace( msg, "%{day}", world.GetDay() );
        text.set( msg, fheroes::FontType::smallWhite() );
        ...
        text.set( std::to_string( lighthouseCount ), fheroes::FontType::smallWhite() );
        text.draw( pt.x + 105, offsetY, display );

        const fheroes::Sprite & lighthouse = fheroes::AGG::GetICN( ICN::OVERVIEW, 14 );
        fheroes::Blit( lighthouse, 0, 0, display, pt.x + 100 - lighthouse.width(), pt.y + 459, lighthouse.width(), lighthouse.height() );
    }
}
...
void Kingdom::openOverviewDialog()
{
    ...
    const fheroes::StandardWindow background( display.DEFAULT_WIDTH, display.DEFAULT_HEIGHT, false );
    ...
    const fheroes::Point cur_pt( background.activeArea().x, background.activeArea().y );
    fheroes::Point dst_pt( cur_pt );

    fheroes::Blit( fheroes::AGG::GetICN( ICN::OVERBACK, 0 ), display, dst_pt.x, dst_pt.y );
    ...
    fheroes::Button buttonHeroes( dst_pt.x, dst_pt.y, ICN::BUTTON_KINGDOM_HEROES, 0, 1 );

    dst_pt.y += 45;
    fheroes::Button buttonCastle( dst_pt.x, dst_pt.y, ICN::BUTTON_KINGDOM_TOWNS, 0, 1 );

    dst_pt.y += 48;
    fheroes::Button buttonExit( dst_pt.x, dst_pt.y, ICN::BUTTON_KINGDOM_EXIT, 0, 1 );

    const fheroes::Rect rectIncome( cur_pt.x + 1, cur_pt.y + 360, 535, 60 );
    const fheroes::Rect rectGoldPerDay( cur_pt.x + 124, cur_pt.y + 459, 289, 16 );

    const fheroes::Sprite & lighthouse = fheroes::AGG::GetICN( ICN::OVERVIEW, 14 );
    const fheroes::Rect rectLighthouse( cur_pt.x + 100 - lighthouse.width(), cur_pt.y + 459, lighthouse.width() + 10, lighthouse.height() );
    ...
    fheroes::fadeInDisplay( background.activeArea(), !isDefaultScreenSize );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonHeroes.area() ) ) {
            fheroes::showStandardTextMessage( _( "Heroes" ), _( "View Heroes." ), Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( buttonCastle.area() ) ) {
            fheroes::showStandardTextMessage( _( "Towns/Castles" ), _( "View Towns and Castles." ), Dialog::ZERO );
        }
            ...
            fheroes::showKingdomIncome( *this, Dialog::ZERO );
        }
        else if ( le.isMouseRightButtonPressedInArea( rectLighthouse ) ) {
            fheroes::showLighthouseInfo( *this, Dialog::ZERO );
        }
        ...
        if ( le.MouseClickLeft( buttonExit.area() ) || Game::HotKeyCloseWindow() ) {
            ...
            fheroes::fadeOutDisplay( background.activeArea(), !isDefaultScreenSize );
            if ( isDefaultScreenSize ) {
        ...
        if ( le.MouseClickLeft( rectIncome ) || le.MouseClickLeft( rectGoldPerDay ) ) {
            fheroes::showKingdomIncome( *this, Dialog::OK );
        }

        if ( le.MouseClickLeft( rectLighthouse ) ) {
            fheroes::showLighthouseInfo( *this, Dialog::OK );
        }
