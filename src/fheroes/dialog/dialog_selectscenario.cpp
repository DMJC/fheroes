...
namespace
{
    ...
    {
        fheroes::showStandardTextMessage( header, body, Dialog::ZERO );
    }
    ...
    void mapInfo( const Maps::FileInfo * info )
    {
        ...
        const fheroes::Text header( info->name, fheroes::FontType::normalYellow(), info->getSupportedLanguage() );

        fheroes::MultiFontText body;

        body.add( { _( "Map Type:\n" ), fheroes::FontType::normalYellow() } );
        switch ( info->version ) {
        case GameVersion::SUCCESSION_WARS:
            body.add( { _( "The Succession Wars" ), fheroes::FontType::normalWhite() } );
            break;
        case GameVersion::PRICE_OF_LOYALTY:
            body.add( { _( "The Price of Loyalty" ), fheroes::FontType::normalWhite() } );
            break;
        case GameVersion::RESURRECTION:
            body.add( { _( "Resurrection" ), fheroes::FontType::normalWhite() } );
            break;
        default:
        ...
        if ( info->version == GameVersion::RESURRECTION ) {
            body.add( { _( "\n\nLanguage:\n" ), fheroes::FontType::normalYellow() } );
            body.add( { fheroes::getLanguageName( info->mainLanguage ), fheroes::FontType::normalWhite() } );
        }

        body.add( { _( "\n\nLocation: " ), fheroes::FontType::smallYellow() } );
        body.add( { info->filename, fheroes::FontType::smallWhite() } );

        fheroes::showMessage( header, body, Dialog::ZERO );
    }
    ...
    void renderFileName( const Maps::FileInfo & info, bool selected, const int32_t posX, const int32_t posY, fheroes::Display & display )
    {
        fheroes::Text text( System::GetFileName( info.filename ), selected ? fheroes::FontType::normalYellow() : fheroes::FontType::normalWhite() );
    ...
    void ShowIfFound( ScenarioListBox & listbox, const fheroes::Point & mouseLocation, const ShowFunction & function )
    {
...
void ScenarioListBox::RedrawBackground( const fheroes::Point & dst )
{
    fheroes::Display & display = fheroes::Display::instance();
    fheroes::Blit( fheroes::AGG::GetICN( ICN::REQSBKG, 0 ), display, dst.x, dst.y );
...
void ScenarioListBox::_renderScenarioListItem( const Maps::FileInfo & info, fheroes::Display & display, const int32_t dsty, const bool current ) const
{
    fheroes::Blit( _getPlayersCountIcon( info.kingdomColors ), display, _offsetX + SCENARIO_LIST_COUNT_PLAYERS_OFFSET_X, dsty );
    _renderMapIcon( info.width, display, _offsetX + SCENARIO_LIST_MAP_SIZE_OFFSET_X, dsty );
    fheroes::Blit( _getMapTypeIcon( info.version ), display, _offsetX + SCENARIO_LIST_MAP_TYPE_OFFSET_X, dsty );
    if ( _isForEditor ) {
    ...
    fheroes::Blit( _getWinConditionsIcon( info.victoryConditionType ), display, _offsetX + SCENARIO_LIST_VICTORY_CONDITION_OFFSET_X, dsty );
    fheroes::Blit( _getLossConditionsIcon( info.lossConditionType ), display, _offsetX + SCENARIO_LIST_LOSS_CONDITION_OFFSET_X, dsty );
}

void ScenarioListBox::_renderSelectedScenarioInfo( fheroes::Display & display, const fheroes::Point & dst )
{
    const Maps::FileInfo & info = GetCurrent();

    fheroes::Blit( _getPlayersCountIcon( info.kingdomColors ), display, dst.x + SELECTED_SCENARIO_COUNT_PLAYERS_OFFSET_X, dst.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y );
    _renderMapIcon( info.width, display, dst.x + SELECTED_SCENARIO_MAP_SIZE_OFFSET_X, dst.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y );
    fheroes::Blit( _getMapTypeIcon( info.version ), display, dst.x + SELECTED_SCENARIO_MAP_TYPE_OFFSET_X, dst.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y );

    fheroes::Text mapNameText{ info.name, fheroes::FontType::normalWhite(), info.getSupportedLanguage() };
    ...
    fheroes::Blit( _getWinConditionsIcon( info.victoryConditionType ), display, dst.x + SELECTED_SCENARIO_VICTORY_CONDITION_OFFSET_X,
                    dst.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y );
    fheroes::Blit( _getLossConditionsIcon( info.lossConditionType ), display, dst.x + SELECTED_SCENARIO_LOSS_CONDITION_OFFSET_X,
                    dst.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y );

    fheroes::Text difficultyLabelText( _( "Map difficulty:" ), fheroes::FontType::normalWhite() );
    difficultyLabelText.draw( dst.x + 210 - difficultyLabelText.width(), dst.y + SELECTED_SCENARIO_DIFFICULTY_OFFSET_Y, display );

    fheroes::Text difficultyText( Difficulty::String( info.difficulty ), fheroes::FontType::normalWhite() );
    ...
    fheroes::Text descriptionText( info.description, fheroes::FontType::normalWhite(), info.getSupportedLanguage() );
...
void ScenarioListBox::_renderMapName( const Maps::FileInfo & info, bool selected, const int32_t & baseYOffset, fheroes::Display & display ) const
{
    fheroes::Text mapName{ info.name,
                            { fheroes::FontSize::NORMAL, ( selected ? fheroes::FontColor::YELLOW : fheroes::FontColor::WHITE ) },
...
{
    const fheroes::Sprite & originalSlider = fheroes::AGG::GetICN( ICN::ESCROLL, 3 );
    const fheroes::Image updatedScrollbarSlider = fheroes::generateScrollbarSlider( originalSlider, false, 140, 9, static_cast<int32_t>( mapsList.size() ),
...
void ScenarioListBox::_renderMapIcon( const uint16_t size, fheroes::Display & display, const int32_t coordX, const int32_t coordY )
{
    ...
    if ( mapIconIndex == -1 ) {
        fheroes::Image icon( 17, 17 );
        icon.reset();
        fheroes::Fill( icon, 1, 1, 15, 15, fheroes::GetColorId( 0x8D, 0x73, 0xFF ) );
        fheroes::Text text( _( "N" ), fheroes::FontType::smallWhite() );
        text.draw( ( 17 - text.width() ) / 2, ( 17 - text.height() ) / 2, icon );

        fheroes::Blit( icon, display, coordX, coordY );
    }
    else {
        fheroes::Blit( fheroes::AGG::GetICN( ICN::REQUESTS, mapIconIndex ), display, coordX, coordY );
    }
}

const fheroes::Sprite & ScenarioListBox::_getPlayersCountIcon( const PlayerColorsSet colors )
{
    const uint32_t iconIndex = 19 + Color::Count( colors );
    return fheroes::AGG::GetICN( ICN::REQUESTS, iconIndex );
}

const fheroes::Sprite & ScenarioListBox::_getMapTypeIcon( const GameVersion version )
{
    switch ( version ) {
    case GameVersion::SUCCESSION_WARS:
        return fheroes::AGG::GetICN( ICN::MAP_TYPE_ICON, 0 );
    case GameVersion::PRICE_OF_LOYALTY:
        return fheroes::AGG::GetICN( ICN::MAP_TYPE_ICON, 1 );
    case GameVersion::RESURRECTION:
        return fheroes::AGG::GetICN( ICN::MAP_TYPE_ICON, 2 );
    default:
...
const fheroes::Sprite & ScenarioListBox::_getWinConditionsIcon( const uint8_t condition )
{
    uint32_t iconIndex = 30 + condition;
    return fheroes::AGG::GetICN( ICN::REQUESTS, iconIndex );
}

const fheroes::Sprite & ScenarioListBox::_getLossConditionsIcon( const uint8_t condition )
{
...
const Maps::FileInfo * Dialog::SelectScenario( MapsFileInfoList & all, const bool isForEditor )
{
    ...
    if ( isForEditor ) {
        for ( const auto & mapInfo : all ) {
            ...
            if ( mapInfo.version != GameVersion::RESURRECTION ) {
                ERROR_LOG( "The fheroes engine is corrupted." )
                assert( 0 );
    ...
    fheroes::Display & display = fheroes::Display::instance();
    const fheroes::Sprite & panel = fheroes::AGG::GetICN( ICN::REQSBKG, 0 );
    const fheroes::Rect rt( ( display.width() - panel.width() ) / 2, ( display.height() - panel.height() ) / 2, panel.width(), panel.height() );

    const fheroes::ImageRestorer background( display, rt.x - fheroes::shadowWidthPx, rt.y, rt.width + fheroes::shadowWidthPx, rt.height + fheroes::shadowWidthPx );

    const fheroes::Sprite & shadow = fheroes::AGG::GetICN( ICN::REQSBKG, 1 );
    fheroes::Blit( shadow, display, rt.x - fheroes::shadowWidthPx, rt.y + fheroes::shadowWidthPx );

    const fheroes::Rect countPlayers( rt.x + SCENARIO_LIST_COUNT_PLAYERS_OFFSET_X, rt.y + SCENARIO_LIST_ROW_OFFSET_Y, ICON_SIZE, SCENARIO_LIST_COLUMN_HEIGHT );
    const fheroes::Rect sizeMaps( rt.x + SCENARIO_LIST_MAP_SIZE_OFFSET_X, rt.y + SCENARIO_LIST_ROW_OFFSET_Y, ICON_SIZE, SCENARIO_LIST_COLUMN_HEIGHT );
    const fheroes::Rect mapTypes( rt.x + SCENARIO_LIST_MAP_TYPE_OFFSET_X, rt.y + SCENARIO_LIST_ROW_OFFSET_Y, ICON_SIZE, SCENARIO_LIST_COLUMN_HEIGHT );
    const fheroes::Rect mapNames( rt.x + SCENARIO_LIST_MAP_NAME_OFFSET_X, rt.y + SCENARIO_LIST_ROW_OFFSET_Y, SCENARIO_LIST_MAP_NAME_WIDTH, SCENARIO_LIST_COLUMN_HEIGHT );
    const fheroes::Rect victoryConds( rt.x + SCENARIO_LIST_VICTORY_CONDITION_OFFSET_X, rt.y + SCENARIO_LIST_ROW_OFFSET_Y, ICON_SIZE, SCENARIO_LIST_COLUMN_HEIGHT );
    const fheroes::Rect lossConds( rt.x + SCENARIO_LIST_LOSS_CONDITION_OFFSET_X, rt.y + SCENARIO_LIST_ROW_OFFSET_Y, ICON_SIZE, SCENARIO_LIST_COLUMN_HEIGHT );

    const fheroes::Rect curCountPlayer( rt.x + SELECTED_SCENARIO_COUNT_PLAYERS_OFFSET_X, rt.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y, ICON_SIZE, ICON_SIZE );
    const fheroes::Rect curMapSize( rt.x + SELECTED_SCENARIO_MAP_SIZE_OFFSET_X, rt.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y, ICON_SIZE, ICON_SIZE );
    const fheroes::Rect curMapType( rt.x + SELECTED_SCENARIO_MAP_TYPE_OFFSET_X, rt.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y, ICON_SIZE, ICON_SIZE );
    const fheroes::Rect curMapName( rt.x + SELECTED_SCENARIO_MAP_NAME_OFFSET_X, rt.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y, SELECTED_SCENARIO_MAP_NAME_WIDTH, ICON_SIZE );
    const fheroes::Rect curVictoryCond( rt.x + SELECTED_SCENARIO_VICTORY_CONDITION_OFFSET_X, rt.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y, ICON_SIZE, ICON_SIZE );
    const fheroes::Rect curLossCond( rt.x + SELECTED_SCENARIO_LOSS_CONDITION_OFFSET_X, rt.y + SELECTED_SCENARIO_GENERAL_OFFSET_Y, ICON_SIZE, ICON_SIZE );
    const fheroes::Rect curDifficulty( rt.x + SELECTED_SCENARIO_DIFFICULTY_OFFSET_X, rt.y + SELECTED_SCENARIO_DIFFICULTY_OFFSET_Y, SELECTED_SCENARIO_DIFFICULTY_WIDTH,
                                        SELECTED_SCENARIO_DIFFICULTY_HEIGHT );
    const fheroes::Rect curDescription( rt.x + SELECTED_SCENARIO_DESCRIPTION_OFFSET_X, rt.y + SELECTED_SCENARIO_DESCRIPTION_OFFSET_Y,
                                         SELECTED_SCENARIO_DESCRIPTION_BOX_WIDTH, SELECTED_SCENARIO_DESCRIPTION_HEIGHT );

    fheroes::Button buttonOk( rt.x + 140, rt.y + 410, ICN::BUTTON_SMALL_OKAY_GOOD, 0, 1 );

    fheroes::Button buttonSelectSmall( rt.x + 36, rt.y + MAP_SIZE_BUTTON_OFFSET_Y, ICN::BUTTON_MAPSIZE_SMALL, 0, 1 );
    fheroes::Button buttonSelectMedium( rt.x + 98, rt.y + MAP_SIZE_BUTTON_OFFSET_Y, ICN::BUTTON_MAPSIZE_MEDIUM, 0, 1 );
    fheroes::Button buttonSelectLarge( rt.x + 160, rt.y + MAP_SIZE_BUTTON_OFFSET_Y, ICN::BUTTON_MAPSIZE_LARGE, 0, 1 );
    fheroes::Button buttonSelectXLarge( rt.x + 222, rt.y + MAP_SIZE_BUTTON_OFFSET_Y, ICN::BUTTON_MAPSIZE_XLARGE, 0, 1 );
    fheroes::Button buttonSelectAll( rt.x + 284, rt.y + MAP_SIZE_BUTTON_OFFSET_Y, ICN::BUTTON_MAPSIZE_ALL, 0, 1 );
    ...
    fheroes::ButtonBase * currentPressedButton = nullptr;
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isKeyPressed( fheroes::Key::KEY_DELETE ) && scenarioList.isSelected() ) {
        ...
        if ( le.MouseClickLeft( buttonSelectSmall.area() ) || HotKeyPressEvent( Game::HotKeyEvent::MAIN_MENU_MAP_SIZE_SMALL ) ) {
            if ( small.empty() ) {
                fheroes::showStandardTextMessage( "", _( "No maps exist at that size." ), Dialog::OK );
            }
            else {
