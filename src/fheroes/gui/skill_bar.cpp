...
namespace
{
    fheroes::Image GetBarBackgroundSprite()
    {
        fheroes::Image icon( 34, 34 );
        ...
        icon.reset();
        fheroes::DrawBorder( icon, fheroes::GetColorId( 0xD0, 0xC0, 0x48 ) );
        fheroes::Copy( fheroes::AGG::GetICN( ICN::HSICONS, 0 ), 26, 21, icon, 1, 1, 32, 32 );
        return icon;
    }
}
    ...
    , _allowSkillReset( allowSkillReset )
{
    if ( useSmallSize ) {
        ...
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::PRIMSKIL, 0 );
...
{
    toff = fheroes::Point( ox, oy );
}

void PrimarySkillsBar::RedrawBackground( const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    if ( _useSmallSize ) {
        fheroes::Copy( backsf, 0, 0, dstsf, pos );
    }
}

void PrimarySkillsBar::RedrawItem( int & skill, const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    ...
    if ( _useSmallSize ) {
        ...
        const int ww = 32;
        const fheroes::Point dstpt( pos.x + ( pos.width - ww ) / 2, pos.y + ( pos.height - ww ) / 2 );

        std::string skillText;

        switch ( skill ) {
        case Skill::Primary::ATTACK:
            fheroes::Copy( backSprite, 217, 52, dstsf, dstpt.x, dstpt.y, ww, ww );
            if ( _hero ) {
                skillText = std::to_string( _hero->GetAttack() );
            }
            break;
        case Skill::Primary::DEFENSE:
            fheroes::Copy( backSprite, 217, 85, dstsf, dstpt.x, dstpt.y, ww, ww );
            if ( _hero ) {
                skillText = std::to_string( _hero->GetDefense() );
            }
            break;
        case Skill::Primary::POWER:
            fheroes::Copy( backSprite, 217, 118, dstsf, dstpt.x, dstpt.y, ww, ww );
            if ( _hero ) {
                skillText = std::to_string( _hero->GetPower() );
            }
            break;
        case Skill::Primary::KNOWLEDGE:
            fheroes::Copy( backSprite, 217, 151, dstsf, dstpt.x, dstpt.y, ww, ww );
            if ( _hero ) {
        ...
        if ( _hero != nullptr && !skillText.empty() ) {
            ...
            const fheroes::Text text{ std::move( skillText ), fheroes::FontType::smallWhite() };
            text.draw( pos.x + ( pos.width + toff.x - text.width() ) / 2, pos.y + pos.height + toff.y + 2, dstsf );
        }
    }
    else {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::PRIMSKIL, skill - 1 );
        fheroes::Copy( sprite, 0, 0, dstsf, pos.x + ( pos.width - sprite.width() ) / 2, pos.y + ( pos.height - sprite.height() ) / 2, pos.width, pos.height );

        fheroes::Text text{ Skill::Primary::String( skill ), fheroes::FontType::smallWhite() };
        ...
        auto prepareEditModeText = [this, &dstsf, &pos, &skill]( int baseValue, const int modificatorValue ) {
            if ( _isDefault ) {
                ...
                fheroes::ApplyPalette( dstsf, pos.x, pos.y, dstsf, pos.x, pos.y, pos.width, pos.height, PAL::GetPalette( PAL::PaletteType::DARKENING ) );

                const fheroes::Text defaultText( _( "Default\nvalue" ), fheroes::FontType::normalWhite() );
        ...
        text.drawInRoi( pos.x + ( pos.width - text.width() ) / 2, pos.y + 6, dstsf, pos );
        text.set( std::move( skillValueText ), fheroes::FontType::normalWhite() );
...
bool PrimarySkillsBar::ActionBarLeftMouseSingleClick( int & skill )
{
    ...
    if ( !_isEditMode ) {
        fheroes::showStandardTextMessage( Skill::Primary::String( skill ), Skill::Primary::StringDescription( skill, _hero ), Dialog::OK );
    ...
    auto primarySkillEditDialog = [skill]( int32_t & skillValue ) {
        ...
        const fheroes::PrimarySkillDialogElement skillUI{ skill, {} };
...
bool PrimarySkillsBar::ActionBarRightMouseHold( int & skill )
{
    ...
    if ( !_isEditMode || !_allowSkillReset || _isDefault ) {
        fheroes::showStandardTextMessage( Skill::Primary::String( skill ), Skill::Primary::StringDescription( skill, _hero ), Dialog::ZERO );
    ...
    , _hero( hero )
{
    if ( use_mini_sprite ) {
        ...
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::SECSKILL, 0 );
        setSingleItemSize( { sprite.width(), sprite.height() } );
    }
}

void SecondarySkillsBar::RedrawBackground( const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    if ( use_mini_sprite ) {
        fheroes::Copy( backsf, 0, 0, dstsf, pos );
    }
        ...
        if ( _showDefaultSkillsMessage && std::none_of( items.begin(), items.end(), []( Skill::Secondary const * skill ) { return skill->isValid(); } ) ) {
            ...
            fheroes::ApplyPalette( fheroes::AGG::GetICN( ICN::SECSKILL, 0 ), 0, 0, dstsf, pos.x, pos.y, pos.width, pos.height,
                                    PAL::GetPalette( PAL::PaletteType::DARKENING ) );

            const fheroes::Text text( _( "Default\nskill" ), fheroes::FontType::normalWhite() );
            text.drawInRoi( pos.x, pos.y + ( pos.height - text.height() * text.rows( pos.width ) ) / 2 + 2, pos.width, dstsf, pos );
        }
        else {
            fheroes::Copy( fheroes::AGG::GetICN( ICN::SECSKILL, 0 ), 0, 0, dstsf, pos );
        }
    }
}

void SecondarySkillsBar::RedrawItem( Skill::Secondary & skill, const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    if ( !skill.isValid() ) {
    ...
    const fheroes::Sprite & sprite
        = use_mini_sprite ? fheroes::AGG::GetICN( ICN::MINISS, skill.GetIndexSprite2() ) : fheroes::AGG::GetICN( ICN::SECSKILL, skill.GetIndexSprite1() );
    fheroes::Copy( sprite, 0, 0, dstsf, pos.x + ( pos.width - sprite.width() ) / 2, pos.y + ( pos.height - sprite.height() ) / 2, pos.width, pos.height );
    ...
    if ( use_mini_sprite ) {
        ...
        fheroes::Text text{ Skill::Secondary::String( skill.Skill() ), fheroes::FontType::smallWhite() };
        ...
        text.set( Skill::Level::StringWithBonus( _hero, skill ), fheroes::FontType::smallWhite() );
        if ( text.width() > sprite.width() + 1 ) {
            text.fitToOneRow( pos.width );
        }
...
{
    if ( skill.isValid() ) {
        fheroes::SecondarySkillDialogElement( skill, _hero ).showPopup( Dialog::OK );
        return true;
    }
...
bool SecondarySkillsBar::ActionBarRightMouseHold( Skill::Secondary & skill )
{
    if ( skill.isValid() ) {
        if ( can_change ) {
            ...
            fheroes::SecondarySkillDialogElement( skill, _hero ).showPopup( Dialog::ZERO );
        }
        return true;
    }
