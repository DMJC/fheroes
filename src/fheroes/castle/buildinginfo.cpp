...
namespace
{
    fheroes::Point GetFlagOffset( int race )
    {
        switch ( race ) {
        case Race::KNGT:
            return { 36, 10 };
    ...
    , _status( BuildingStatus::ALLOW_BUILD )
{
    ...
    if ( b == BUILD_CAPTAIN ) {
        const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::getCaptainIcnId( castle.GetRace() ), ( _buildingType & BUILD_CAPTAIN ? 1 : 0 ) );
...
std::string BuildingInfo::getBuildingDescription( const int race, const uint32_t buildingId )
{
    ...
    if ( isDwelling( buildingId ) ) {
        ...
        description = fheroes::getBuildingDescription( race, static_cast<BuildingType>( buildingId ) );
...
void BuildingInfo::RedrawCaptain() const
{
    ...
    if ( _status == BuildingStatus::ALREADY_BUILT ) {
        const fheroes::Sprite & captainSprite = fheroes::AGG::GetICN( ICN::getCaptainIcnId( castle.GetRace() ), 1 );
        const fheroes::Sprite & flag = fheroes::AGG::GetICN( ICN::getFlagIcnId( castle.GetColor() ), 0 );

        fheroes::Blit( captainSprite, display, area.x, area.y );
        const fheroes::Point flagOffset = GetFlagOffset( castle.GetRace() );
        fheroes::Blit( flag, display, area.x + flagOffset.x, area.y + flagOffset.y );
    }
    else {
        fheroes::Blit( fheroes::AGG::GetICN( ICN::getCaptainIcnId( castle.GetRace() ), 0 ), display, area.x, area.y );
    }

    // indicator
    if ( _status == BuildingStatus::ALREADY_BUILT ) {
        const fheroes::Sprite & spriteAllow = fheroes::AGG::GetICN( ICN::TOWNWIND, 11 );
        fheroes::Blit( spriteAllow, display, area.x + 83 - 4 - spriteAllow.width(), area.y + 79 - 2 - spriteAllow.height() );
    }
    else if ( _status != BuildingStatus::ALLOW_BUILD ) {
        if ( BuildingStatus::LACK_RESOURCES == _status ) {
            const fheroes::Sprite & spriteMoney = fheroes::AGG::GetICN( ICN::TOWNWIND, 13 );
            fheroes::Blit( spriteMoney, display, area.x + 83 - 4 + 1 - spriteMoney.width(), area.y + 79 - 3 - spriteMoney.height() );
        }
        else {
            const fheroes::Sprite & spriteDeny = fheroes::AGG::GetICN( ICN::TOWNWIND, 12 );
            fheroes::Blit( spriteDeny, display, area.x + 83 - 4 + 1 - spriteDeny.width(), area.y + 79 - 2 - spriteDeny.height() );
        }
    }
}
...
void BuildingInfo::Redraw() const
{
    ...
    fheroes::Display & display = fheroes::Display::instance();
    const int index = fheroes::getIndexBuildingSprite( static_cast<BuildingType>( _buildingType ) );

    const fheroes::Sprite & buildingFrame = fheroes::AGG::GetICN( ICN::BLDGXTRA, 0 );
    fheroes::Blit( buildingFrame, display, area.x, area.y );
    if ( _status == BuildingStatus::BUILD_DISABLE || _status == BuildingStatus::SHIPYARD_NOT_ALLOWED ) {
        const fheroes::Point offset( 6, 59 );
        fheroes::Sprite grayedOut = fheroes::Crop( buildingFrame, offset.x, offset.y, 125, 12 );
        fheroes::ApplyPalette( grayedOut, PAL::GetPalette( PAL::PaletteType::GRAY ) );
        fheroes::ApplyPalette( grayedOut, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        fheroes::Copy( grayedOut, 0, 0, display, area.x + offset.x, area.y + offset.y, 125, 12 );
    }

    // build image
    if ( BUILD_NOTHING == _buildingType ) {
        const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();
        const fheroes::Sprite & buildBackground = fheroes::AGG::GetICN( isEvilInterface ? ICN::CASLXTRA_EVIL : ICN::CASLXTRA, 0 );
        fheroes::Copy( buildBackground, 0, 0, display, area.x, area.y, buildBackground.width(), buildBackground.height() );
        return;
    }

    fheroes::Blit( fheroes::AGG::GetICN( ICN::getBuildingIcnId( castle.GetRace() ), index ), display, area.x + 1, area.y + 1 );

    // indicator
    if ( _status == BuildingStatus::ALREADY_BUILT ) {
        const fheroes::Sprite & spriteAllow = fheroes::AGG::GetICN( ICN::TOWNWIND, 11 );
        fheroes::Blit( spriteAllow, display, area.x + buildingFrame.width() - 5 - spriteAllow.width(), area.y + 58 - 2 - spriteAllow.height() );
    }
    else if ( _status == BuildingStatus::BUILD_DISABLE || _status == BuildingStatus::SHIPYARD_NOT_ALLOWED ) {
        const fheroes::Sprite & spriteDeny = fheroes::AGG::GetICN( ICN::TOWNWIND, 12 );
        fheroes::Sprite disabledSprite( spriteDeny );
        fheroes::ApplyPalette( disabledSprite, PAL::GetPalette( PAL::PaletteType::GRAY ) );
        fheroes::ApplyPalette( disabledSprite, PAL::GetPalette( PAL::PaletteType::DARKENING ) );
        fheroes::Blit( disabledSprite, display, area.x + buildingFrame.width() - 5 + 1 - spriteDeny.width(), area.y + 58 - 2 - spriteDeny.height() );
    }
    else if ( _status != BuildingStatus::ALLOW_BUILD ) {
        if ( BuildingStatus::LACK_RESOURCES == _status ) {
            const fheroes::Sprite & spriteMoney = fheroes::AGG::GetICN( ICN::TOWNWIND, 13 );
            fheroes::Blit( spriteMoney, display, area.x + buildingFrame.width() - 5 + 1 - spriteMoney.width(), area.y + 58 - 3 - spriteMoney.height() );
        }
        else {
            const fheroes::Sprite & spriteDeny = fheroes::AGG::GetICN( ICN::TOWNWIND, 12 );
            fheroes::Blit( spriteDeny, display, area.x + buildingFrame.width() - 5 + 1 - spriteDeny.width(), area.y + 58 - 2 - spriteDeny.height() );
        }

        const fheroes::Sprite & textBackground = fheroes::AGG::GetICN( ICN::CASLXTRA, 2 );
        fheroes::Copy( textBackground, 0, 0, display, area.x, area.y + 58, textBackground.width(), textBackground.height() );
    }
    else {
    ...
    const fheroes::Text buildingName( Castle::GetStringBuilding( _buildingType, castle.GetRace() ), fheroes::FontType::smallWhite() );
...
bool BuildingInfo::QueueEventProcessing( fheroes::ButtonBase & exitButton ) const
{
    LocalEvent & le = LocalEvent::Get();

    if ( le.MouseClickLeft( area ) ) {
        if ( _status == BuildingStatus::LACK_RESOURCES || _status == BuildingStatus::ALLOW_BUILD ) {
            const fheroes::ButtonRestorer exitRestorer( exitButton );
...
bool BuildingInfo::DialogBuyBuilding( bool buttons ) const
{
    ...
    const fheroes::Text descriptionText( std::move( extendedDescription ), fheroes::FontType::normalWhite() );

    // prepare requirement build string
    std::string requirement;

    if ( _status != BuildingStatus::BUILD_DISABLE ) {
        requirement = fheroes::getBuildingRequirementString( castle.GetRace(), static_cast<BuildingType>( _buildingType ) );
    }

    const bool requirementsPresent = !requirement.empty();

    const fheroes::Text requirementTitle( _( "Requires:" ), fheroes::FontType::normalWhite() );
    const fheroes::Text requirementText( std::move( requirement ), fheroes::FontType::normalWhite() );

    const int elementOffset = 10;

    int32_t requirementHeight = 0;
    if ( requirementsPresent ) {
        requirementHeight = requirementTitle.height() + requirementText.height( fheroes::boxAreaWidthPx ) + elementOffset;
    }

    Resource::BoxSprite rbs( PaymentConditions::BuyBuilding( castle.GetRace(), _buildingType ), fheroes::boxAreaWidthPx );

    const fheroes::Sprite & buildingFrame = fheroes::AGG::GetICN( ICN::BLDGXTRA, 0 );

    const int32_t totalDialogHeight = elementOffset + buildingFrame.height() + elementOffset + descriptionText.height( fheroes::boxAreaWidthPx ) + elementOffset
    ...
    fheroes::Point pos{ dialogRoi.x + ( dialogRoi.width - buildingFrame.width() ) / 2, pos.y = dialogRoi.y + elementOffset };

    fheroes::Display & display = fheroes::Display::instance();
    fheroes::Blit( buildingFrame, display, pos.x, pos.y );

    const fheroes::Sprite & buildingImage
        = fheroes::AGG::GetICN( ICN::getBuildingIcnId( castle.GetRace() ), fheroes::getIndexBuildingSprite( static_cast<BuildingType>( _buildingType ) ) );
    pos.x = dialogRoi.x + ( dialogRoi.width - buildingImage.width() ) / 2;
    pos.y += 1;
    fheroes::Blit( buildingImage, display, pos.x, pos.y );

    const fheroes::Text buildingName( GetName(), fheroes::FontType::smallWhite() );
    ...
    descriptionText.draw( pos.x, pos.y + 2, fheroes::boxAreaWidthPx, display );

    pos.y += descriptionText.height( fheroes::boxAreaWidthPx ) + elementOffset;
    if ( requirementsPresent ) {
        ...
        requirementText.draw( pos.x, pos.y + 2, fheroes::boxAreaWidthPx, display );

        pos.y += requirementText.height( fheroes::boxAreaWidthPx ) + elementOffset;
    }
    ...
    fheroes::ButtonGroup buttonGroup( dialogRoi, Dialog::OK | Dialog::CANCEL );
    fheroes::ButtonBase & buttonOkay = buttonGroup.button( 0 );
    const fheroes::ButtonBase & buttonCancel = buttonGroup.button( 1 );
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonOkay.area() ) ) {
            fheroes::showStandardTextMessage( _( "Okay" ), GetConditionDescription(), Dialog::ZERO );
        }
...
DwellingsBar::DwellingsBar( Castle & cstl, const fheroes::Size & sz )
    : castle( cstl )
...
void DwellingsBar::RedrawBackground( const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    fheroes::Blit( backsf, dstsf, pos.x, pos.y );
}

void DwellingsBar::RedrawItem( DwellingItem & dwl, const fheroes::Rect & pos, fheroes::Image & dstsf )
{
    const uint32_t dwType = castle.GetActualDwelling( dwl.dwType );
    const Monster mons{ castle.GetRace(), dwType };

    const fheroes::Sprite & mons32 = fheroes::AGG::GetICN( ICN::MONS32, mons.GetSpriteIndex() );
    fheroes::Blit( mons32, dstsf, pos.x + ( pos.width - mons32.width() ) / 2, pos.y + ( pos.height - 3 - mons32.height() ) );

    if ( !castle.isBuild( dwType ) ) {
        fheroes::Blit( fheroes::AGG::GetICN( ICN::CSLMARKER, 0 ), dstsf, pos.x + pos.width - 10, pos.y + 4 );

        return;
    }

    // Units available for hire.
    fheroes::Text text( std::to_string( castle.getMonstersInDwelling( dwType ) ), fheroes::FontType::smallWhite() );
    ...
    // Dwelling's growth.
    text.set( "+" + std::to_string( grown ), fheroes::FontType::smallYellow() );
...
bool DwellingsBar::ActionBarLeftMouseSingleClick( DwellingItem & dwl )
{
    ...
    if ( castle.isBuild( dwType ) ) {
        ...
        fheroes::showStandardTextMessage( "", GetBuildConditionDescription( BuildingStatus::NEED_CASTLE ), Dialog::OK );
    else {
