...
StatusBar::StatusBar()
    : MovableText( fheroes::Display::instance() )
{
    // Do nothing.
}
...
void StatusBar::ShowMessage( std::string msg )
{
    ...
    auto text = std::make_unique<fheroes::Text>( std::move( msg ), fheroes::FontType::normalWhite() );

    // Cut off the end of the text if it exceeds the given width
    text->fitToOneRow( _roi.width );

    const int32_t textWidth = text->width();
    const fheroes::Rect messageRoi{ _roi.x + ( _roi.width - textWidth ) / 2, _roi.y + 1, textWidth, text->height() };
    ...
    fheroes::Display::instance().render( fheroes::getBoundaryRect( _prevMessageRoi, messageRoi ) );
