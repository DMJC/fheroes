...
namespace Interface
{
    struct PlayerInfo
    {
        ...
        fheroes::Rect playerTypeRoi;
        fheroes::Rect classRoi;
        fheroes::Rect nameRoi;
        fheroes::Rect handicapRoi;
    };

    struct PlayersInfo : std::vector<PlayerInfo>
    {
        void UpdateInfo( Players &, const fheroes::Point & playerTypeOffset, const fheroes::Point & classOffset );
        bool SwapPlayers( Player & player1, Player & player2 ) const;

        Player * GetFromOpponentClick( const fheroes::Point & pt );
        Player * GetFromOpponentNameClick( const fheroes::Point & pt );
        Player * GetFromClassClick( const fheroes::Point & pt );
        Player * getPlayerFromHandicapRoi( const fheroes::Point & point );
