...
namespace
{
    ...
    {
        return std::string( "fheroes engine, version: " + Settings::GetVersion() );
    }
    ...
    void displayMissingResourceWindow()
    {
        ...
        const fheroes::Image & image = Compression::CreateImageFromZlib( 290, 190, errorMessage, sizeof( errorMessage ), false );

        display.fill( 0 );
        fheroes::Resize( image, display );
    ...
    public:
        DisplayInitializer()
        {
            ...
            fheroes::Display & display = fheroes::Display::instance();
            fheroes::ResolutionInfo bestResolution{ conf.currentResolutionInfo() };
            ...
            if ( conf.isFirstGameRun() && System::isHandheldDevice() ) {
                ...
                const std::vector<fheroes::ResolutionInfo> resolutions = fheroes::engine().getAvailableResolutions();

                for ( const fheroes::ResolutionInfo & info : resolutions ) {
            ...
            fheroes::engine().setTitle( GetCaption() );
            ...
            _systemInfoRenderer = std::make_unique<fheroes::SystemInfoRenderer>();
            ...
            fheroes::cursor().registerUpdater( Cursor::Refresh );
        ...
        ~DisplayInitializer()
        {
            fheroes::RenderProcessor::instance().unregisterRenderers();
    ...
    private:
        ...
        std::unique_ptr<fheroes::SystemInfoRenderer> _systemInfoRenderer;
    };
    ...
    public:
        DataInitializer()
        {
            ...
            try {
                ...
                _h2dInitializer.reset( new fheroes::h2d::H2DInitializer );
    ...
    private:
        ...
        std::unique_ptr<fheroes::h2d::H2DInitializer> _h2dInitializer;
    };
...
// SDL2main.lib converts argv to UTF-8, but this application expects ANSI, use the original argv
    ...
    try {
        const fheroes::HardwareInitializer hardwareInitializer;
        Logging::InitLog();
        ...
        std::set<fheroes::SystemInitializationComponent> coreComponents{ fheroes::SystemInitializationComponent::Audio,
                                                                          fheroes::SystemInitializationComponent::Video };

#if defined( TARGET_PS_VITA ) || defined( TARGET_NINTENDO_SWITCH )
        coreComponents.emplace( fheroes::SystemInitializationComponent::GameController );
#endif

        const fheroes::CoreInitializer coreInitializer( coreComponents );
        ...
        // Load palette.
        fheroes::setGamePalette( AGG::getDataFromAggFile( "KB.PAL", false ) );
        ...
        try {
            ...
            const fheroes::Point pos = conf.getSavedWindowPos();
            Game::mainGameLoop( conf.isFirstGameRun(), isProbablyDemoVersion() );
            const fheroes::Point currentPos = display.getWindowPos();
            if ( pos != currentPos ) {
                conf.setStartWindowPos( currentPos );
                conf.Save( Settings::configFileName );
            }
        }
        catch ( const fheroes::InvalidDataResources & ex ) {
            ERROR_LOG( ex.what() )
            displayMissingResourceWindow();
            return EXIT_FAILURE;
        }
