...
namespace Dialog
{
    ...
    void QuickInfoWithIndicationOnRadar( const Castle & castle, const fheroes::Rect & areaToRestore );
    // Shows the quick info window for the given hero, and also indicates the location of this hero on the radar.
    // 'areaToRestore' defines the area whose contents should be restored when the radar is redrawn.
    void QuickInfoWithIndicationOnRadar( const HeroBase & hero, const fheroes::Rect & areaToRestore );
    ...
    void QuickInfoAtPosition( const Castle & castle, const fheroes::Point & position );
    ...
    void QuickInfoAtPosition( const HeroBase & hero, const fheroes::Point & position );
    ...
    bool SelectCount( std::string header, const int32_t min, const int32_t max, int32_t & selectedValue, const int32_t step = 1,
                      const fheroes::DialogElement * topUiElement = nullptr, const fheroes::DialogElement * bottomUiElement = nullptr );

    // If character limit is set to 0, then no limitation for the resulting string will be applied.
    bool inputString( const fheroes::TextBase & title, const fheroes::TextBase & body, std::string & result, const size_t charLimit, const bool isMultiLine,
    ...
    {
    public:
        explicit FrameBorder( int v = fheroes::borderWidthPx );
        ...
        const fheroes::Rect & GetTop() const
        {
            return top;
        }

        static void RenderRegular( const fheroes::Rect & dstrt );
        static void RenderOther( const fheroes::Image &, const fheroes::Rect & );

    protected:
        fheroes::ImageRestorer restorer;
