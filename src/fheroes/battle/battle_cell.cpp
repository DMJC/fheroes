...
namespace
{
    bool inABC( const fheroes::Point & base, const fheroes::Point & pt1, const fheroes::Point & pt2, const fheroes::Point & pt3 )
    {
...
fheroes::Rect Battle::Position::GetRect() const
{
    if ( first == nullptr ) {
        return {};
    }
...
void Battle::Cell::SetArea( const fheroes::Rect & area )
{
...
Battle::AttackDirection Battle::Cell::GetTriangleDirection( const fheroes::Point & dst ) const
{
    const fheroes::Point pt( infl * dst.x, infl * dst.y );

    const fheroes::Point oneHalf( ( _coord[1].x + _coord[2].x ) / 2, ( _coord[1].y + _coord[2].y ) / 2 );
    const fheroes::Point twoHalf( ( _coord[2].x + _coord[3].x ) / 2, ( _coord[2].y + _coord[3].y ) / 2 );
    const fheroes::Point fourHalf( ( _coord[4].x + _coord[5].x ) / 2, ( _coord[4].y + _coord[5].y ) / 2 );
    const fheroes::Point fiveHalf( ( _coord[5].x + _coord[6].x ) / 2, ( _coord[5].y + _coord[6].y ) / 2 );
...
bool Battle::Cell::isPositionIncludePoint( const fheroes::Point & pt ) const
{
