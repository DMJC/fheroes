...
public:
    ...
    bool isPosition( const fheroes::Point & pt ) const override;
    ...
    void DrawImageCastle( const fheroes::Point & pt ) const;
...
private:
    ...
    void _wellRedrawAvailableMonsters( const uint32_t dwellingType, const bool restoreBackground, fheroes::Image & background ) const;
    void _wellRedrawBackground( fheroes::Image & background ) const;
    void _wellRedrawMonsterAnimation( const fheroes::Rect & roi, std::array<fheroes::RandomMonsterAnimation, maxNumOfDwellings> & monsterAnimInfo ) const;
...
namespace CastleDialog
{
    ...
    {
        BuildingRenderInfo( const BuildingType buildingType, const fheroes::Rect & buildingRect )
            : id( buildingType )
            , coord( buildingRect )
        {
            // Do nothing.
    ...
    {
        BuildingsRenderQueue( const Castle & castle, const fheroes::Point & top );
    };

    void redrawAllBuildings( const Castle & castle, const fheroes::Point & offset, const BuildingsRenderQueue & buildings,
...
public:
    ...
    Castle * Get( const fheroes::Point & position ) const;

    void removeCastle( const fheroes::Point & position );
...
private:
    ...
    std::map<fheroes::Point, size_t> _castleTiles;
};
