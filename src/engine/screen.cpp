...
namespace
{
    ...
    fheroes::ResolutionInfo GetNearestResolution( fheroes::ResolutionInfo resolutionInfo, const std::vector<fheroes::ResolutionInfo> & resolutions )
    {
...
#if !defined( TARGET_PS_VITA )
    bool IsLowerThanDefaultRes( const fheroes::ResolutionInfo & value )
    {
        return value.gameWidth < fheroes::Display::DEFAULT_WIDTH || value.gameHeight < fheroes::Display::DEFAULT_HEIGHT;
    }

    std::set<fheroes::ResolutionInfo> FilterResolutions( const std::set<fheroes::ResolutionInfo> & resolutionSet )
    {
        static_assert( fheroes::Display::DEFAULT_WIDTH == 640 && fheroes::Display::DEFAULT_HEIGHT == 480, "Default resolution must be 640 x 480" );

        if ( resolutionSet.empty() ) {
            return { { fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT } };
        }

        std::set<fheroes::ResolutionInfo> resolutions;

        for ( const fheroes::ResolutionInfo & resolution : resolutionSet ) {
...

        {
            ...
            const std::set<fheroes::ResolutionInfo> possibleResolutions
            ...
            const fheroes::ResolutionInfo lowestResolution = *( resolutions.begin() );

            for ( const fheroes::ResolutionInfo & resolution : possibleResolutions ) {
        ...
        std::set<fheroes::ResolutionInfo> scaledResolutions;

        {
            ...
            assert( !resolutions.empty() );
            const fheroes::ResolutionInfo biggestResolution = *( std::prev( resolutions.end() ) );
            ...
            if ( biggestResolution.gameWidth > fheroes::Display::DEFAULT_WIDTH && biggestResolution.gameWidth >= biggestResolution.gameHeight ) {
                assert( biggestResolution.gameHeight >= fheroes::Display::DEFAULT_HEIGHT );

                scaledResolutions.emplace( biggestResolution.gameWidth * fheroes::Display::DEFAULT_HEIGHT / biggestResolution.gameHeight,
                                           fheroes::Display::DEFAULT_HEIGHT, biggestResolution.gameWidth, biggestResolution.gameHeight );
            }
        }

        // Try to find and add scaled resolutions with integer scale (x2, x3, x4, etc) which correspond to existing "hardware" resolutions
        for ( const fheroes::ResolutionInfo & resolution : resolutions ) {
            assert( resolution.gameWidth == resolution.screenWidth && resolution.gameHeight == resolution.screenHeight );

            const int32_t maxScale = std::min( resolution.gameWidth / fheroes::Display::DEFAULT_WIDTH, resolution.gameHeight / fheroes::Display::DEFAULT_HEIGHT );
    ...
    bool getActiveArea( fheroes::Rect & roi, const int32_t width, const int32_t height )
    {
...
// If SDL library is used
    ...
    protected:
        ...
        void copyImageToSurface( const fheroes::Image & image, SDL_Surface * surface, const fheroes::Rect & roi )
        {
        ...
        SDL_Surface * generateIconSurface( const fheroes::Image & icon )
        {
    ...
    class RenderCursor final : public fheroes::Cursor
    {
    public:
        ...
        {
            if ( _emulation ) {
                return fheroes::Cursor::isVisible();
            }

            return fheroes::Cursor::isVisible() && ( SDL_ShowCursor( SDL_QUERY ) == SDL_ENABLE );
        }

        void update( const fheroes::Image & image, int32_t offsetX, int32_t offsetY ) override
        {
            ...
            if ( _emulation ) {
                fheroes::Cursor::update( image, offsetX, offsetY );
                return;
            }
...
#if defined( TARGET_PS_VITA )
    class RenderEngine final : public fheroes::BaseRenderEngine
    {
    public:
        ...
        fheroes::Rect getActiveWindowROI() const override
        {
            return _destRect;
        }

        fheroes::Size getCurrentScreenResolution() const override
        {
            return { VITA_FULLSCREEN_WIDTH, VITA_FULLSCREEN_HEIGHT };
        }

        std::vector<fheroes::ResolutionInfo> getAvailableResolutions() const override
        {
            static const std::vector<fheroes::ResolutionInfo> filteredResolutions = []() {
                std::set<fheroes::ResolutionInfo> resolutionSet;
                resolutionSet.emplace( fheroes::Display::DEFAULT_WIDTH, fheroes::Display::DEFAULT_HEIGHT );
                resolutionSet.emplace( VITA_ASPECT_CORRECTED_WIDTH, fheroes::Display::DEFAULT_HEIGHT );
                resolutionSet.emplace( VITA_FULLSCREEN_WIDTH, VITA_FULLSCREEN_HEIGHT );

                return std::vector<fheroes::ResolutionInfo>{ resolutionSet.rbegin(), resolutionSet.rend() };
            }();
    ...
    private:
        ...
        bool allocate( fheroes::ResolutionInfo & resolutionInfo, bool isFullScreen ) override
        {
            clear();

            const std::vector<fheroes::ResolutionInfo> resolutions = getAvailableResolutions();
            assert( !resolutions.empty() );
            if ( !resolutions.empty() ) {
        ...
        void render( const fheroes::Display & display, const fheroes::Rect & roi ) override
        {
            (void)roi;
    ...
    {
        return fheroes::cursor().isSoftwareEmulation();
    }

    std::optional<bool> isWindowInAnyDisplay( const fheroes::ResolutionInfo & resolutionInfo, const fheroes::Point & windowPos )
    {
    ...
    class RenderEngine final : public fheroes::BaseRenderEngine, public BaseSDLRenderer
    {
    public:
        RenderEngine( const RenderEngine & ) = delete;
        ...
        void toggleFullScreen() override
        {
            ...
            if ( flags & ( SDL_WINDOW_FULLSCREEN | SDL_WINDOW_FULLSCREEN_DESKTOP ) ) {
                ...
                if ( const fheroes::Display & display = fheroes::Display::instance(); display.width() != 0 && display.height() != 0 ) {
        ...
        std::vector<fheroes::ResolutionInfo> getAvailableResolutions() const override
        {
            ...
            // 848x480 is the smallest resolution supported by fheroes
            ...
            if ( maxDisplayMode.w < fheroes::Display::DEFAULT_WIDTH || maxDisplayMode.h < fheroes::Display::DEFAULT_HEIGHT ) {
                return std::vector<fheroes::ResolutionInfo>{ resolutionSet.rbegin(), resolutionSet.rend() };
            }

            std::vector<fheroes::ResolutionInfo> temp;
        ...
        void setIcon( const fheroes::Image & icon ) override
        {
            if ( _window == nullptr )
                return;
        ...
        fheroes::Point getWindowPos() const override
        {
            if ( isFullScreen() ) {
        ...
        void setWindowPos( const fheroes::Point pos ) override
        {
            if ( pos == fheroes::Point{ -1, -1 } ) {
    ...
    private:
        ...
        fheroes::Point _prevWindowPos{ SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED };
        fheroes::Size _currentScreenResolution;
        fheroes::Rect _activeWindowROI;

        fheroes::Size _windowedSize;
...
namespace fheroes
{
    ...
    {
        const fheroes::Display & display = Display::instance();
