...
HeroesIndicator::HeroesIndicator( const Heroes * hero )
    : _hero( hero )
    , _back( fheroes::Display::instance() )
{
    assert( _hero != nullptr );
}

void HeroesIndicator::SetPos( const fheroes::Point & pt )
{
    _area.x = pt.x;
    _area.y = pt.y;
...
void LuckIndicator::Redraw()
{
    ...
    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::HSICONS, ( 0 > _luck ? 3 : ( 0 < _luck ? 2 : 6 ) ) );
    ...
    while ( count-- ) {
        fheroes::Blit( sprite, display, cx, cy );
        cx += inter;
    }
}
...
void LuckIndicator::QueueEventProcessing( const LuckIndicator & indicator )
{
    ...
    if ( le.MouseClickLeft( indicator._area ) ) {
        fheroes::showStandardTextMessage( fheroes::LuckString( indicator._luck ), indicator._description, Dialog::OK );
    }
    else if ( le.isMouseRightButtonPressedInArea( indicator._area ) ) {
        fheroes::showStandardTextMessage( fheroes::LuckString( indicator._luck ), indicator._description, Dialog::ZERO );
    }
}
...
void MoraleIndicator::Redraw()
{
    ...
    fheroes::Sprite sprite = fheroes::AGG::GetICN( ICN::HSICONS, spriteInx );
...
void MoraleIndicator::QueueEventProcessing( const MoraleIndicator & indicator )
{
    ...
    if ( le.MouseClickLeft( indicator._area ) ) {
        fheroes::showStandardTextMessage( fheroes::MoraleString( indicator._morale ), indicator._description, Dialog::OK );
    }
    else if ( le.isMouseRightButtonPressedInArea( indicator._area ) ) {
        fheroes::showStandardTextMessage( fheroes::MoraleString( indicator._morale ), indicator._description, Dialog::ZERO );
    }
}
...
void ExperienceIndicator::Redraw() const
{
    ...
    const fheroes::Sprite & experienceImage = fheroes::AGG::GetICN( ICN::HSICONS, 1 );
    fheroes::Blit( experienceImage, display, _area.x, _area.y );

    const fheroes::Rect renderRoi{ _area.x + 1, _area.y + 24, 33, 9 };
    ...
    if ( _isDefault ) {
        ...
        const fheroes::Text text{ "40-90", fheroes::FontType::smallWhite() };
        ...
        fheroes::Text text{ std::move( experienceString ), fheroes::FontType::smallWhite() };
        if ( text.width() > renderRoi.width + 1 ) {
            ...
            text.set( std::move( experienceString ), fheroes::FontType::smallWhite() );
            text.drawInRoi( renderRoi.x + ( experienceImage.width() - text.width() ) / 2 - widthReduction, _area.y + 25, display, renderRoi );
        }
...
void ExperienceIndicator::QueueEventProcessing() const
{
    ...
    if ( le.MouseClickLeft( _area ) || le.isMouseRightButtonPressedInArea( _area ) ) {
        ...
        fheroes::showStandardTextMessage( std::move( message ), _description, ( le.isMouseRightButtonPressed() ? Dialog::ZERO : Dialog::OK ) );
    }
}
...
void SpellPointsIndicator::Redraw()
{
    ...
    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( ICN::HSICONS, 8 );

    const fheroes::Rect renderRoi{ _area.x + 1, _area.y + 22, 33, 9 };

    fheroes::Text text;
    if ( _isDefault ) {
        text.set( std::to_string( _hero->GetMaxSpellPoints() ), fheroes::FontType::smallWhite() );
    }
    else {
        text.set( std::to_string( _hero->GetSpellPoints() ) + "/" + std::to_string( _hero->GetMaxSpellPoints() ), fheroes::FontType::smallWhite() );
    }
    ...
    fheroes::Blit( sprite, display, _area.x, _area.y );
    ...
    if ( drawOneLinedText ) {
        ...
        fheroes::Copy( sprite, 0, 21, display, renderRoi.x - 1, renderRoi.y - renderRoi.height - 1, renderRoi.width + 2, renderRoi.height + 1 );

        // Render the current Spell Points value at the top line.
        text.set( std::to_string( _hero->GetSpellPoints() ) + "/", fheroes::FontType::smallWhite() );
...
void SpellPointsIndicator::QueueEventProcessing() const
{
    ...
    if ( le.MouseClickLeft( _area ) || le.isMouseRightButtonPressedInArea( _area ) ) {
        fheroes::showStandardTextMessage( _( "Spell Points" ), _description, ( le.isMouseRightButtonPressed() ? Dialog::ZERO : Dialog::OK ) );
    }
}
