...
namespace
{
    ...
    const std::array<fheroes::Key, playerCountOptions> playerCountHotkeys
        = { fheroes::Key::KEY_2, fheroes::Key::KEY_3, fheroes::Key::KEY_4, fheroes::Key::KEY_5, fheroes::Key::KEY_6 };
    const std::array<fheroes::GameMode, playerCountOptions> playerCountModes
        = { fheroes::GameMode::SELECT_SCENARIO_TWO_HUMAN_PLAYERS, fheroes::GameMode::SELECT_SCENARIO_THREE_HUMAN_PLAYERS,
            fheroes::GameMode::SELECT_SCENARIO_FOUR_HUMAN_PLAYERS, fheroes::GameMode::SELECT_SCENARIO_FIVE_HUMAN_PLAYERS,
            fheroes::GameMode::SELECT_SCENARIO_SIX_HUMAN_PLAYERS };
    ...
    {
        fheroes::showStandardTextMessage( _( "Warning" ),
...
fheroes::GameMode Game::NewStandard()
{
    ...
    return fheroes::GameMode::SELECT_SCENARIO_ONE_HUMAN_PLAYER;
}

fheroes::GameMode Game::NewBattleOnly()
{
    Settings & conf = Settings::Get();
    conf.SetGameType( Game::TYPE_BATTLEONLY );
    // Redraw the empty main menu screen to show it after the battle using screen restorer.
    fheroes::drawMainMenuScreen();
    return fheroes::GameMode::START_BATTLE_ONLY_MODE;
}

fheroes::GameMode Game::NewHotSeat( const size_t playerCountOptionIndex )
{
...
fheroes::GameMode Game::NewSuccessionWarsCampaign()
{
    ...
    const fheroes::Point roiOffset( ( display.width() - display.DEFAULT_WIDTH ) / 2, ( display.height() - display.DEFAULT_HEIGHT ) / 2 );

    // Fade-out screen before playing video.
    fheroes::fadeOutDisplay();

    const fheroes::Text loadingScreen( _( "Loading video. Please wait..." ), fheroes::FontType::normalWhite() );
    ...
    const std::array<fheroes::Rect, 2> campaignRoi{ fheroes::Rect( 382 + roiOffset.x, 58 + roiOffset.y, 222, 298 ),
                                                     fheroes::Rect( 30 + roiOffset.x, 59 + roiOffset.y, 224, 297 ) };
    ...
    const fheroes::ScreenPaletteRestorer screenRestorer;
    ...
    while ( le.HandleEvents( Game::isCustomDelayNeeded( customDelay ) ) ) {
        ...
        if ( Game::validateCustomAnimationDelay( customDelay ) ) {
            fheroes::Rect frameRoi( roiOffset.x, roiOffset.y, 0, 0 );
            ...
            if ( highlightCampaignId < campaignRoi.size() ) {
                const fheroes::Rect & roi = campaignRoi[highlightCampaignId];
                fheroes::DrawRect( display, roi, 51 );
                fheroes::DrawRect( display, { roi.x - 1, roi.y - 1, roi.width + 2, roi.height + 2 }, 51 );
            }
...
fheroes::GameMode Game::NewPriceOfLoyaltyCampaign()
{
    ...
    const fheroes::Sprite & background = fheroes::AGG::GetICN( ICN::X_IVY, 1 );
    fheroes::Blit( background, 0, 0, display, roiOffset.x, roiOffset.y, background.width(), background.height() );

    const fheroes::Sprite & campaignChoice = fheroes::AGG::GetICN( ICN::X_IVY, 0 );
    fheroes::Blit( campaignChoice, 0, 0, display, roiOffset.x + campaignChoice.x(), roiOffset.y + campaignChoice.y(), campaignChoice.width(), campaignChoice.height() );

    display.render();

    const std::array<fheroes::Rect, 4> activeCampaignArea{ fheroes::Rect( roiOffset.x + 192, roiOffset.y + 23, 248, 163 ),
                                                            fheroes::Rect( roiOffset.x + 19, roiOffset.y + 120, 166, 193 ),
                                                            fheroes::Rect( roiOffset.x + 450, roiOffset.y + 120, 166, 193 ),
                                                            fheroes::Rect( roiOffset.x + 192, roiOffset.y + 240, 248, 163 ) };

    const std::array<fheroes::Rect, 4> renderCampaignArea{ fheroes::Rect( roiOffset.x + 214, roiOffset.y + 47, 248, 163 ),
                                                            fheroes::Rect( roiOffset.x + 41, roiOffset.y + 140, 166, 193 ),
                                                            fheroes::Rect( roiOffset.x + 472, roiOffset.y + 131, 166, 193 ),
                                                            fheroes::Rect( roiOffset.x + 214, roiOffset.y + 273, 248, 163 ) };

    size_t highlightCampaignId = videos.size();

    fheroes::GameMode gameChoice = fheroes::GameMode::NEW_GAME;
    ...
    while ( le.HandleEvents( highlightCampaignId < videos.size() ? Game::isCustomDelayNeeded( customDelay ) : true ) ) {
        if ( le.MouseClickLeft( activeCampaignArea[0] ) || HotKeyPressEvent( HotKeyEvent::CAMPAIGN_PRICE_OF_LOYALTY ) ) {
            campaignSaveData.setCurrentScenarioInfo( { Campaign::PRICE_OF_LOYALTY_CAMPAIGN, 0 } );
            gameChoice = fheroes::GameMode::SELECT_CAMPAIGN_SCENARIO;
            break;
        }
        ...
        if ( highlightCampaignId != beforeCampaignId ) {
            fheroes::Blit( background, 0, 0, display, roiOffset.x, roiOffset.y, background.width(), background.height() );
            fheroes::Blit( campaignChoice, 0, 0, display, roiOffset.x + campaignChoice.x(), roiOffset.y + campaignChoice.y(), campaignChoice.width(),
        ...
        if ( highlightCampaignId < videos.size() && Game::validateCustomAnimationDelay( customDelay ) ) {
            fheroes::Rect frameRoi( renderCampaignArea[highlightCampaignId].x, renderCampaignArea[highlightCampaignId].y, 0, 0 );
            videos[highlightCampaignId]->getNextFrame( display, frameRoi.x, frameRoi.y, frameRoi.width, frameRoi.height, palette );

            fheroes::Blit( background, frameRoi.x - roiOffset.x, frameRoi.y - roiOffset.y, display, frameRoi.x, frameRoi.y, frameRoi.width, frameRoi.height );
...
fheroes::GameMode Game::NewGame( const bool isProbablyDemoVersion )
{
    ...
    fheroes::ButtonGroup mainModeButtons;
    ...
    const fheroes::ButtonBase & buttonStandardGame = mainModeButtons.button( 0 );
    fheroes::ButtonBase & buttonCampaignGame = mainModeButtons.button( 1 );
    const fheroes::ButtonBase & buttonMultiGame = mainModeButtons.button( 2 );
    const fheroes::ButtonBase & buttonBattleGame = mainModeButtons.button( 3 );
    const fheroes::ButtonBase & buttonSettings = mainModeButtons.button( 4 );

    // Generate dialog background with extra space added for the cancel button.
    const int32_t spaceBetweenButtons = 10;
    fheroes::StandardWindow background( mainModeButtons, true, buttonStandardGame.area().height + spaceBetweenButtons );
    ...
    fheroes::Display & display = fheroes::Display::instance();
    fheroes::ImageRestorer emptyDialog( display, background.activeArea().x, background.activeArea().y, background.activeArea().width,
    ...
    fheroes::Button buttonCancel( buttonStandardGame.area().x,
    ...
    fheroes::Button buttonHotSeat( buttonStandardGame.area().x, buttonStandardGame.area().y, menuButtonsIcnIndex, 12, 13 );
    buttonHotSeat.disable();

    fheroes::ButtonGroup playerCountButtons;
    ...
    fheroes::Button buttonSuccessionWars;
    fheroes::Button buttonPriceOfLoyalty;
    ...
    fheroes::validateFadeInAndRender();

    LocalEvent & le = LocalEvent::Get();

    if ( isProbablyDemoVersion ) {
        fheroes::showStandardTextMessage( _( "Warning" ),
                                           _( "fheroes needs data files from the original Heroes of Might and Magic II to operate. "
    ...
    while ( le.HandleEvents() ) {
        if ( buttonStandardGame.isEnabled() ) {
            ...
            if ( buttonCampaignGame.isEnabled() && ( HotKeyPressEvent( HotKeyEvent::MAIN_MENU_CAMPAIGN ) || le.MouseClickLeft( buttonCampaignGame.area() ) ) ) {
                if ( !isPriceOfLoyaltyCampaignPresent() ) {
                    return fheroes::GameMode::NEW_SUCCESSION_WARS_CAMPAIGN;
                }
            ...
            if ( HotKeyPressEvent( HotKeyEvent::MAIN_MENU_BATTLEONLY ) || le.MouseClickLeft( buttonBattleGame.area() ) ) {
                return fheroes::GameMode::NEW_BATTLE_ONLY;
            }
            ...
            if ( le.isMouseRightButtonPressedInArea( buttonStandardGame.area() ) ) {
                fheroes::showStandardTextMessage( _( "Standard Game" ), _( "A single player game playing out a single map." ), Dialog::ZERO );
            }
            else if ( buttonCampaignGame.isEnabled() && le.isMouseRightButtonPressedInArea( buttonCampaignGame.area() ) ) {
                fheroes::showStandardTextMessage( _( "Campaign Game" ), _( "A single player game playing through a series of maps." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonMultiGame.area() ) ) {
                fheroes::showStandardTextMessage( _( "Multi-Player Game" ),
                ...
                fheroes::showStandardTextMessage( _( "Battle Only" ), _( "Setup and play a battle without loading any map." ), Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( buttonSettings.area() ) ) {
                fheroes::showStandardTextMessage( _( "Game Settings" ), _( "Change language, resolution and settings of the game." ), Dialog::ZERO );
            }
        }
            ...
            if ( le.isMouseRightButtonPressedInArea( playerCountButtons.button( 0 ).area() ) ) {
                fheroes::showStandardTextMessage( _( "2 Players" ), _( "Play with 2 human players, and optionally, up to 4 additional computer players." ),
                                                   Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( playerCountButtons.button( 1 ).area() ) ) {
                fheroes::showStandardTextMessage( _( "3 Players" ), _( "Play with 3 human players, and optionally, up to 3 additional computer players." ),
                                                   Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( playerCountButtons.button( 2 ).area() ) ) {
                fheroes::showStandardTextMessage( _( "4 Players" ), _( "Play with 4 human players, and optionally, up to 2 additional computer players." ),
                                                   Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( playerCountButtons.button( 3 ).area() ) ) {
                fheroes::showStandardTextMessage( _( "5 Players" ), _( "Play with 5 human players, and optionally, up to 1 additional computer player." ),
                                                   Dialog::ZERO );
            }
            else if ( le.isMouseRightButtonPressedInArea( playerCountButtons.button( 4 ).area() ) ) {
                fheroes::showStandardTextMessage( _( "6 Players" ), _( "Play with 6 human players." ), Dialog::ZERO );
            }
        }
            ...
            if ( le.MouseClickLeft( buttonPriceOfLoyalty.area() ) || HotKeyPressEvent( HotKeyEvent::MAIN_MENU_NEW_EXPANSION_CAMPAIGN ) ) {
                return fheroes::GameMode::NEW_PRICE_OF_LOYALTY_CAMPAIGN;
            }

            if ( le.isMouseRightButtonPressedInArea( buttonSuccessionWars.area() ) ) {
                fheroes::showStandardTextMessage( _( "Original Campaign" ),
                ...
                fheroes::showStandardTextMessage( _( "Expansion Campaign" ), _( "One of the four new campaigns from the Price of Loyalty expansion set." ),
                                                   Dialog::ZERO );
            }
        }
        else {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            fheroes::showStandardTextMessage( _( "Cancel" ), _( "Cancel back to the main menu." ), Dialog::ZERO );
        }
    }
