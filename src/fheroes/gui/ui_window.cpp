...
namespace
{
    const int32_t borderSize{ fheroes::borderWidthPx };
    ...
    fheroes::Rect getSymmetricDialogActiveArea( fheroes::ButtonGroup & buttons, const bool isSingleColumn, const int32_t extraHeight, const fheroes::Image & output )
    {
        if ( buttons.getButtonsCount() == 0 ) {
            return {};
        }

        const fheroes::Rect & buttonArea = buttons.button( 0 ).area();
        ...
        fheroes::Point placement;
        if ( isSingleColumn ) {
            const fheroes::Sprite mainMenuBackground = fheroes::AGG::GetICN( ICN::HEROES, 0 );
            const int32_t panelXPos = output.width() - mainMenuBackground.x() - ( dialogWidth + fheroes::borderWidthPx ) - 8;
            const int32_t panelYPos = mainMenuBackground.y() + fheroes::borderWidthPx + 8;
...
namespace fheroes
{
    ...
    void StandardWindow::render()
    {
        ...
        fheroes::addGradientShadowForArea( _output, { _windowArea.x, _windowArea.y }, _windowArea.width, _windowArea.height, borderSize );
    }
    ...
    void StandardWindow::applyGemDecoratedCorners()
    {
        ...
        const bool isEvilInterface = Settings::Get().isEvilInterfaceEnabled();
        if ( !isEvilInterface ) {
            const fheroes::Sprite & gemDialog = fheroes::AGG::GetICN( ICN::REDBACK, 0 );
            Copy( gemDialog, 20, 2, gem, 0, 0, gemSideLength, gemSideLength );
        }
        else {
            const fheroes::Sprite & corners = fheroes::AGG::GetICN( ICN::EVIL_DIALOG_PLAIN_CORNERS, 0 );
            ...
            const fheroes::Sprite & gemDialog = fheroes::AGG::GetICN( ICN::WINLOSEE, 0 );
