...
namespace
{
    Artifact getImmunityArtifactForSpell( const HeroBase * hero, const Spell & spell )
    {
        ...
        case Spell::MASSCURSE:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::CURSE_SPELL_IMMUNITY );
        case Spell::HYPNOTIZE:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::HYPNOTIZE_SPELL_IMMUNITY );
        case Spell::DEATHRIPPLE:
        case Spell::DEATHWAVE:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::DEATH_SPELL_IMMUNITY );
        case Spell::BERSERKER:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::BERSERK_SPELL_IMMUNITY );
        case Spell::BLIND:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::BLIND_SPELL_IMMUNITY );
        case Spell::PARALYZE:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::PARALYZE_SPELL_IMMUNITY );
        case Spell::HOLYWORD:
        case Spell::HOLYSHOUT:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::HOLY_SPELL_IMMUNITY );
        case Spell::DISPEL:
        case Spell::MASSDISPEL:
            return hero->GetBagArtifacts().getFirstArtifactWithBonus( fheroes::ArtifactBonusType::DISPEL_SPELL_IMMUNITY );
        default:
            break;
        }
...
bool Battle::Unit::UpdateDirection( const fheroes::Rect & pos )
{
...
uint32_t Battle::Unit::EstimateRetaliatoryDamage( const uint32_t damageTaken ) const
{
    ...
    if ( Modes( IS_PARALYZE_MAGIC ) && !isAbilityPresent( fheroes::MonsterAbilityType::UNLIMITED_RETALIATION ) ) {
        return 0;
    }
...
uint32_t Battle::Unit::CalculateDamageUnit( const Unit & enemy, double dmg ) const
{
    if ( isArchers() ) {
        ...
        else if ( !isAbilityPresent( fheroes::MonsterAbilityType::NO_MELEE_PENALTY ) ) {
            dmg /= 2;
        }
    }
    ...
    if ( ( isAbilityPresent( fheroes::MonsterAbilityType::DOUBLE_DAMAGE_TO_UNDEAD ) && enemy.isAbilityPresent( fheroes::MonsterAbilityType::UNDEAD ) )
         || ( isAbilityPresent( fheroes::MonsterAbilityType::EARTH_CREATURE )
              && enemy.isWeaknessPresent( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_EARTH_CREATURES ) )
         || ( isAbilityPresent( fheroes::MonsterAbilityType::AIR_CREATURE )
              && enemy.isWeaknessPresent( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_AIR_CREATURES ) )
         || ( isAbilityPresent( fheroes::MonsterAbilityType::FIRE_CREATURE )
              && enemy.isWeaknessPresent( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_FIRE_CREATURES ) )
         || ( isAbilityPresent( fheroes::MonsterAbilityType::WATER_CREATURE )
              && enemy.isWeaknessPresent( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_WATER_CREATURES ) ) ) {
        dmg *= 2;
    }
    ...
    return std::max( fheroes::checkedCast<uint32_t>( dmg ).value(), 1U );
}
...
uint32_t Battle::Unit::_applyDamage( const uint32_t dmg )
{
    ...
    if ( Modes( IS_PARALYZE_MAGIC ) ) {
        // Units with this ability retaliate even when under the influence of paralyzing spells
        if ( !isAbilityPresent( fheroes::MonsterAbilityType::UNLIMITED_RETALIATION ) ) {
...
uint32_t Battle::Unit::ApplyDamage( Unit & enemy, const uint32_t dmg, uint32_t & killed, uint32_t * ptrResurrected )
{
    ...
    if ( enemy.isAbilityPresent( fheroes::MonsterAbilityType::SOUL_EATER ) ) {
        resurrected = enemy._resurrect( killed * enemy.Monster::GetHitPoints(), true, false );
    }
    else if ( enemy.isAbilityPresent( fheroes::MonsterAbilityType::HP_DRAIN ) ) {
...
bool Battle::Unit::ApplySpell( const Spell & spell, const HeroBase * applyingHero, TargetInfo & target )
{
    ...
    const uint32_t spoint = applyingHero ? applyingHero->GetPower() : fheroes::spellPowerForBuiltinMonsterSpells;
...
void Battle::Unit::PostAttackAction( const Unit & enemy )
{
    if ( isArchers() && !isHandFighting( *this, enemy ) ) {
        ...
        if ( hero == nullptr || !hero->GetBagArtifacts().isArtifactBonusPresent( fheroes::ArtifactBonusType::ENDLESS_AMMUNITION ) ) {
...
double Battle::Unit::evaluateThreatForUnit( const Unit & defender ) const
{
    ...
    if ( attacker.isDoubleAttack() ) {
        ...
        assert( !attacker.isAbilityPresent( fheroes::MonsterAbilityType::TWO_CELL_MELEE_ATTACK )
                && !attacker.isAbilityPresent( fheroes::MonsterAbilityType::ALL_ADJACENT_CELL_MELEE_ATTACK )
    ...
    if ( attacker.isAbilityPresent( fheroes::MonsterAbilityType::ENEMY_HALVING ) ) {
        attackerThreat *= 2;
    }
    ...
    if ( attacker.isAbilityPresent( fheroes::MonsterAbilityType::HP_DRAIN ) ) {
        attackerThreat *= 1.3;
    }

    {
        const std::vector<fheroes::MonsterAbility> & attackerAbilities = fheroes::getMonsterData( id ).battleStats.abilities;

        if ( const auto abilityIter = std::find( attackerAbilities.begin(), attackerAbilities.end(), fheroes::MonsterAbilityType::SPELL_CASTER );
...
{
    if ( applyingHero ) {
        duration += applyingHero->GetBagArtifacts().getTotalArtifactEffectValue( fheroes::ArtifactBonusType::EVERY_COMBAT_SPELL_DURATION );
    }
                                             ...
                                             const bool ignoreDefendingHero ) const
{
    ...
    {
        const std::vector<fheroes::MonsterAbility> & abilities = fheroes::getMonsterData( GetID() ).battleStats.abilities;
        const std::vector<fheroes::MonsterWeakness> & weaknesses = fheroes::getMonsterData( GetID() ).battleStats.weaknesses;

        // Abilities
        //
        if ( const auto certainSpellDmgRedIter
             = std::find( abilities.begin(), abilities.end(),
                          std::make_pair( fheroes::MonsterAbilityType::CERTAIN_SPELL_DAMAGE_REDUCTION, static_cast<uint32_t>( spell.GetID() ) ) );
             certainSpellDmgRedIter != abilities.end() ) {
            dmg = dmg * certainSpellDmgRedIter->percentage / 100;
        }
        else if ( const auto elementalSpellDmgRedIter = std::find( abilities.begin(), abilities.end(), fheroes::MonsterAbilityType::ELEMENTAL_SPELL_DAMAGE_REDUCTION );
        ...
        //
                  ...
                  = std::find( weaknesses.begin(), weaknesses.end(),
                               std::make_pair( fheroes::MonsterWeaknessType::EXTRA_DAMAGE_FROM_CERTAIN_SPELL, static_cast<uint32_t>( spell.GetID() ) ) );
                  certainSpellExtraDmgIter != weaknesses.end() ) {
            dmg = dmg * ( 100 + certainSpellExtraDmgIter->percentage ) / 100;
        }
        else if ( ( isWeaknessPresent( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_FIRE_SPELLS ) && spell.isFire() )
                  || ( isWeaknessPresent( fheroes::MonsterWeaknessType::DOUBLE_DAMAGE_FROM_COLD_SPELLS ) && spell.isCold() ) ) {
            dmg *= 2;
        }
    }
    ...
    if ( applyingHero ) {
        ...
        case Spell::COLDRING: {
            std::vector<int32_t> extraDamagePercent
                = applyingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::COLD_SPELL_EXTRA_EFFECTIVENESS_PERCENT );
            ...
            if ( useDefendingHeroArts ) {
                const std::vector<int32_t> damageReductionPercent
                    = defendingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::COLD_SPELL_DAMAGE_REDUCTION_PERCENT );
                for ( const int32_t value : damageReductionPercent ) {
                    dmg = dmg * ( 100 - value ) / 100;
                }

                extraDamagePercent = defendingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactCurseType::COLD_SPELL_EXTRA_DAMAGE_PERCENT );
                for ( const int32_t value : extraDamagePercent ) {
                    dmg = dmg * ( 100 + value ) / 100;
                }
            }
        ...
        case Spell::FIREBLAST: {
            std::vector<int32_t> extraDamagePercent
                = applyingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::FIRE_SPELL_EXTRA_EFFECTIVENESS_PERCENT );
            for ( const int32_t value : extraDamagePercent ) {
                dmg = dmg * ( 100 + value ) / 100;
            }
            ...
            if ( useDefendingHeroArts ) {
                const std::vector<int32_t> damageReductionPercent
                    = defendingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::FIRE_SPELL_DAMAGE_REDUCTION_PERCENT );
                for ( const int32_t value : damageReductionPercent ) {
                    dmg = dmg * ( 100 - value ) / 100;
                }

                extraDamagePercent = defendingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactCurseType::FIRE_SPELL_EXTRA_DAMAGE_PERCENT );
                for ( const int32_t value : extraDamagePercent ) {
                    dmg = dmg * ( 100 + value ) / 100;
                }
            }
        ...
        case Spell::CHAINLIGHTNING: {
            const std::vector<int32_t> extraDamagePercent
                = applyingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::LIGHTNING_SPELL_EXTRA_EFFECTIVENESS_PERCENT );
            for ( const int32_t value : extraDamagePercent ) {
                dmg = dmg * ( 100 + value ) / 100;
            }
            ...
            if ( useDefendingHeroArts ) {
                const std::vector<int32_t> damageReductionPercent
                    = defendingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::LIGHTNING_SPELL_DAMAGE_REDUCTION_PERCENT );
                for ( const int32_t value : damageReductionPercent ) {
                    dmg = dmg * ( 100 - value ) / 100;
                }
            }
        ...
        case Spell::ARMAGEDDON: {
            if ( useDefendingHeroArts ) {
                const std::vector<int32_t> damageReductionPercent
                    = defendingHero->GetBagArtifacts().getTotalArtifactMultipliedPercent( fheroes::ArtifactBonusType::ELEMENTAL_SPELL_DAMAGE_REDUCTION_PERCENT );
                for ( const int32_t value : damageReductionPercent ) {
                    dmg = dmg * ( 100 - value ) / 100;
                }
            }
...
void Battle::Unit::_spellRestoreAction( const Spell & spell, const uint32_t spellPoints, const HeroBase * applyingHero )
{
    ...
    case Spell::MASSCURE:
        ...
        // restore
        _hitPoints += fheroes::getHPRestorePoints( spell, spellPoints, applyingHero );
    ...
    case Spell::RESURRECTTRUE: {
        ...
        const uint32_t restore = fheroes::getResurrectPoints( spell, spellPoints, applyingHero );
...
bool Battle::Unit::isDoubleAttack() const
{
    if ( !isArchers() || isHandFighting() ) {
        return isAbilityPresent( fheroes::MonsterAbilityType::DOUBLE_MELEE_ATTACK );
    }

    // Archers with double shooting ability can only fire a second shot if they have enough ammo
    if ( isAbilityPresent( fheroes::MonsterAbilityType::DOUBLE_SHOOTING ) ) {
        return GetShots() > 1;
    }
...
uint32_t Battle::Unit::GetMagicResist( const Spell & spell, const HeroBase * applyingHero ) const
{
    ...
    case Spell::HYPNOTIZE:
        ...
        if ( fheroes::getHypnotizeMonsterHPPoints( spell, applyingHero->GetPower(), applyingHero ) < _hitPoints ) {
            return 100;
        }
        break;
    ...
    return fheroes::getSpellResistance( id, spell.GetID() );
}
...
Spell Battle::Unit::GetSpellMagic( Rand::PCG32 & randomGenerator ) const
{
    ...
    const auto abilityIter = std::find( abilities.begin(), abilities.end(), fheroes::MonsterAbilityType::SPELL_CASTER );
...
{
    const fheroes::MonsterSound & sounds = fheroes::getMonsterData( id ).sounds;
...
{
    return fheroes::getMonsterData( id ).sounds.death;
}

int Battle::Unit::M82Move() const
{
    return fheroes::getMonsterData( id ).sounds.movement;
}

int Battle::Unit::M82Wnce() const
{
    return fheroes::getMonsterData( id ).sounds.wince;
}

int Battle::Unit::M82Expl() const
{
    return fheroes::getMonsterData( id ).sounds.explosion;
}

int Battle::Unit::M82Tkof() const
{
    return fheroes::getMonsterData( id ).sounds.takeoff;
}

int Battle::Unit::M82Land() const
{
    return fheroes::getMonsterData( id ).sounds.landing;
}

fheroes::Point Battle::Unit::GetBackPoint() const
{
    const fheroes::Rect & rt = _position.GetRect();
    return _isReflected ? fheroes::Point( rt.x + rt.width, rt.y + rt.height / 2 ) : fheroes::Point( rt.x, rt.y + rt.height / 2 );
}

fheroes::Point Battle::Unit::GetCenterPoint() const
{
    const fheroes::Sprite & sprite = fheroes::AGG::GetICN( GetMonsterSprite(), GetFrame() );

    const fheroes::Rect & pos = _position.GetRect();
    const int32_t centerY = pos.y + pos.height + sprite.y() / 2 - 10;

    return { pos.x + pos.width / 2, centerY };
}

fheroes::Point Battle::Unit::GetStartMissileOffset( const size_t direction ) const
{
